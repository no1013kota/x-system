...
**R39〜R62 を新規追加（2026-08-18 監査・すべて `todo`）。** R1〜R38 は done だが、
その打ち切り（2026-08-13）以降に **287コミット・609ファイル**が未監査で積み上がっていた。
6領域 × 3観点で並列監査し、確定した76件のうち **リファクタ相当は24件**（high 1・medium 11・low 12）。
一覧と根拠は [監査記録 2026-08-18](AUDIT_2026-08-18.md)。**この計画からは番号で参照する**
（1件ずつの再掲はしない。同じ内容を2か所に持つとどちらかが腐る）。

**このパスで分かったこと**: 最も多かったのは重複ではなく、**移行の途中で意味が反転したまま残った
コードとコメント**だった。T-M8-129（投稿パターンをDBへ移す）の前後で
「`prompt_templates` は画像専用」と docs は書き換えたのに実装が追いつかず、
**画像プロンプトの編集画面が投稿パターンを壊す**ところまで行っていた（T-M8-140 で修正）。
移行を段階に分けるときは、**各段で「もう通らない経路」を閉じる**のを同じ作業単位に含める。

**もう一つ**: 「共通部品へ寄せて検査もそこへ移す」は、その部品が使われ続ける限りしか有効でない。
R39 の `campaign-price.tsx` は後続タスクで使われなくなり、検査は**誰も描かないファイルを読んで
「規則は守られている」と報告していた**（T-M8-137 で修正）。部品を捨てるときは検査の向き先も動かす。

### 2026-08-18 監査ぶんの入口（R39〜R62 相当）

| 領域 | 件数 | 主なもの |
|---|---:|---|
| 投稿パターン | 6 | 死んだ属性 `asks_user_opinion` が全レイヤに残る／プレースホルダー正規化が2か所／移行後に意味が反転したコメント |
| jobs・cron | 4 | 子jobの決定的keyが3箇所2形式（`keys.ts` の `childJobKey` は未使用）／到達しない枠返還の分岐 |
| 認証・課金 | 4 | 「md/premium だけ編集できる」の判定が5箇所／AIクレジットの列指定が reserve と settle で不一致 |
| 運営コマンド | 6 | R32で集約した引数・既定URLの読み方を後から来た2本のscriptが使っていない／`config.toml` と `push-auth-templates` の二重持ち |
| 画面・UI | 4 | 認証3画面の外枠が逐語重複／h1のclassが6ファイル7箇所／スケジュールのモード表示が5ファイルで別々（「下書き」の言い方が4種） |

**着手のしかた**: 上の領域を1つ選び、監査記録の該当番号を読んでから WIP=1 で進める。
振る舞いを変えるものは混ぜない（それは BACKLOG 側）。

## 旧・現況（2026-08-13 時点）

**R1〜R38 はすべて done。** 2026-08-13 に R33〜R36・R38 を実施し、この計画の実行可能な項目は
なくなった（R37 は F1 で消化済み）。次の全体監査までは、`/dev-loop` の各タスクで
「変更影響 → 必須の検証」を満たす形で局所的に整えていく。

**このパスで分かったこと**: 空振りしていた機械検査は最終的に **4本**見つかった
（R19 の暗号鍵の境界検査・R32 の鍵の読み方・R33 の委託先・R35 のLP）。いずれも
「検出器が何にも当たらなくなっても緑のまま通る」型で、規約は
[開発とテストの進め方](../docs/operations/development-and-testing.md) §11 に集約した。
**新しい検査を書いたら、意図的に壊して落ちることを1度確認する**のが最も効く。
...
| R30 | USD→円のレート150の二重持ちを1つにし、呼び出し元ゼロの dbSizeLimitBytes と存在しない環境変数を指すコメントを消す。`approxYen()`／`USD_TO_JPY` を R31 の `ops/check.ts` へ置き、doctor と日次サマリの両方が参照する形に。`DiagnosticsOptions.dbSizeLimitBytes`（呼び出し元0件）を削除し、`FREE_DB_SIZE_LIMIT_BYTES` を直に使う。`SUPABASE_DB_SIZE_LIMIT_MB` は repo 全体に1件も無いことを確認のうえコメントから削除 | duplication | XS | low | done |
| R31 | doctor.mjs が写している「まとめ1行」を diagnostics 側と共有し、依存ゼロのモジュールへ切り出す。`src/lib/ops/check.ts`（**import を1つも持たない**）へ `Level`／`Check`／`worstLevel`／`summarize`／`exitCodeFor`／`approxYen` を移し、`diagnostics.ts` は re-export に。doctor.mjs は既存の `await import("../src/lib/ops/release-gate.ts")` と同じ形で読む。**テストが無いので実際に `npm run doctor` を変更前後で実行し、出力が1行も違わず終了コードも同じ（1）であることを確認** | duplication | S | low | done |
| R32 | scripts の argOf / envValue / base既定値の重複を scripts/lib へ集約する。`scripts/lib/cli.mjs`（plain JS）へ `argOf`／`envValue`／`baseUrl` を移し3ファイルから import。探索順（環境変数が最優先）・既定URL・末尾スラッシュ処理は不変で、**抽出前後の関数本体がコメント除去後にバイト等価であることを確認**。smoke-live のコメントが実装と逆（`.env.local → .env → 環境変数`）だったのも正した。`npm run doctor`／`doctor --base staging`／`smoke:live` を変更前後で実行し、`--base staging` は**差分ゼロ**・終了コードも同一（差が出たのは費用の累積とニュース結果の変動のみ）。**この作業で `env-secret-usage.test.ts` の検出条件が空振りになるのを既存ガードが検出**（`argOf("base")` を共通化したため当たらなくなった）。`baseUrl()` も見る形に直し、副産物として `check-turnstile.mjs` が新たに検査対象へ入った | duplication | S | low | done |
| R33 | 法務ページの検査を「ソース文字列の toContain」から PROCESSORS の値そのものへ変える（委託先9社・移転国）。`PROCESSORS` を import して (a) 9社が `provider` に含まれること (b) 全件の `country` が非空 (c) `provider`/`service`/`use`/`data` が全件非空、を検査。**2件を実地に実証**: 委託先を1件足して `country` を空にすると落ちる／`provider` から社名を外すと落ちる（従来はどちらも本文の他8件の「米国」に拾われて緑だった） | testability | XS | low | done |
| R34 | 法務同意プロフィールの列リストを1つの正本にする。`LEGAL_CONSENT_SELECT`（**文字列リテラルが正本**）を `legal-consent.ts` へ置き、配列と pooled 用 select をそこから導出。3箇所（同意画面・Server Action・実行ガード）を置換。**配列から `join` した `string` を Supabase クライアントへ渡すと行の型推論が落ちる**（実際に落ちた）ため、リテラルを正本にする形へ直した。3経路が写経していないことを走査テストで固定 | duplication | S | low | done |
| R35 | landing-page.test.ts の空振りを塞ぐ（検査対象のディレクトリ走査化＋グラデ検査をデータ配列のフラグまで見る＋古いコメントの是正） **実装（2026-08-13）**: (1) `LP_SOURCES` と `CONTENT_SOURCES` を `src/components/lp/` の**ディレクトリ走査**にし、除外だけを名前で列挙する形へ反転（対象を手書きしていたため新規ファイルが漏れていた）(2) グラデは出現数に加え **`gradientTop: true` / `bar: true` の数**を数える。**2件を実地に実証**: lp/ へ `opacity-0` と価格直書きを含むファイルを置くと落ちる（従来は緑）／グラデのフラグを1枚増やすと落ちる（従来は出現数5のままで緑）。個別ファイル参照は `lpFile()` 経由にし、改名時に即座に落ちるようにした | testability | S | low | done |
| R36 | 曜日×時刻ドットの「状態→クラス」対応表と曜日ラベルを1箇所に集約する **実装（2026-08-13）**: `src/components/lp/dots.ts` に `SLOT_DOT_CLASS`（post/draft/none）と `WEEKDAY_LABELS_LP` を置き、3つのマップと2つの曜日配列を置換。寸法（size-2.5 / size-2）とグリッド幅は意図した差なので**共有しない**。**見た目が変わっていないことを2段階で確認**: 変更前の3マップの値がすべて同一へ集約されドットの並びも一致すること／実ブラウザでドットの総数・塗り・枠線の実測値が変更前と完全一致（35/16/0）。クラス集合を固定するテストも追加（UIのドリフトは typecheck・lint・build で捕捉できないため） | duplication | S | low | done |
...
| R38 | スケジュールのセル説明文・パターン名フォールバック・曜日+時刻表記と、保存前の入力検証を lib/schedule の純関数へ切り出す **実装（2026-08-13）**: `lib/schedule/slot-labels.ts`（`patternLabel`／`slotTimeLabel`／`slotScheduleLabel`／`slotDescription`／`WEEKDAY_LABELS`）と `lib/schedule/slot-form.ts`（`validateSlotForm`）へ切り出し。**曜日は 0=日曜**（`extract(dow)` に合わせる。月曜始まりにすると画面と実投稿が1つずれる）——LPの `WEEKDAY_LABELS_LP` とは別物であることをコメントで明示。出力文字列は不変。**E2Eで aria-label と title の一致を見張る**形にし、片方だけ別実装へ戻すと実際に落ちることを確認 | testability | S | low | done |
...
## R39: キャンペーン価格の表示を共通部品へ（done・2026-08-18）

T-M8-118で、バッジ＋価格＋取り消し線を**LPと `/plans` で別々に組み立てていた**。
「通常価格と書かない」という景品表示法の決まりを守る場所が2か所に分かれており、
片方だけ直す事故が起きる形だった。しかも `landing-page.test.ts` はLPしか見ていないので、
`/plans` 側は検査を素通りしていた。

`components/billing/campaign-price.tsx`（`CampaignBadge` / `CampaignAfterPrice`）へ寄せ、
検査対象もその部品へ移した。あわせて `/plans` のキャンペーン表示のE2Eを追加した
（部品を使い忘れても型では落ちないため、画面に出ていることは別で見る）。

**2026-08-18 追記（T-M8-137）**: この共通部品は **T-M8-125（比較表への集約）で使われなくなり、
どこからも import されないまま残っていた**。そして `landing-page.test.ts` は
**その誰も描かないファイルを読んで「規則は守られている」と報告していた**——
設定＞課金は自前で描いており、規則が当たっていなかった。部品は削除し、
検査は実際に `regularPriceJpy` を描く2箇所（比較表・設定＞課金）へ向けた。
設定画面側で「通常価格」と書くと落ちることを実証済み。
**「共通部品へ寄せて検査もそこへ移す」は、その部品が使われ続ける限りしか有効でない。**
部品を捨てるときは検査の向き先も一緒に動かす必要がある。

**E2Eの件数固定をやめた**: `toHaveCount(3)` はプラン数が変わると落ちるだけで、
守りたい意図（全プランに出る）を表していない。`/plans` は比較表にも同じカードを出すため
実際には4件で、意図と実装がずれていた。
