...
## 現在の状況と次の一手（2026-08-04 更新）

M0〜M7は`done`。**M8（UIリデザイン）とその後始末（T-M8-34〜52）も完了**し、stagingへ反映済み。
機能実装は出揃っており、残りは「本番リリースの外部準備（人間側）」と「要決定」。

**stagingの状態**（`npm run doctor -- --base <stg>`）: 対応が必要な問題 **0件**。
注意2件（送信待ちメール4件＝本番前に `db:clean-test-data` が必要／Xトークンの期限切れ＝次の操作で自動更新）。

**M8後始末で分かったこと**（次に同じ落とし穴を踏まないため）:
- **テストが緑でも壊れている型**が繰り返し出た。色の消失・押せない理由の不在・黙って捨てる例外・
  正常な空と失敗による空の混同。いずれも typecheck / lint / 既存E2E を通過する。
- そこで**人の注意力に頼る箇所を機械検査へ移した**（8本）。`badge-tone` / `notice` /
  `notice-banner` / `card-title` / `card-surface` / `icon-source` / `next-action-commands` /
  `tabs`。いずれも「1箇所戻すと落ちる」ことを確認済み。
- **実物を1周させる（`smoke:live`）ことでしか見つからない不具合**が今回も1件あった（T-M8-47）。

| 区分 | 残り | 場所 |
|---|---|---|
| 開発タスク（着手可） | **0件**（M8の後始末19タスクすべて`done`。T-M7-17 Gemini画像のみ`blocked`＝ユーザー判断で一旦不要） | 下記M8セクション |
| 開発タスク（blocked） | 1件（T-M7-17 Gemini画像。ユーザー判断で一旦不要） | 同 |
| 要決定 | **5件**（D-16: `stg` にもブランチ保護を掛けるか。2026-08-04に ruleset `protect-stg` を作成しPR経由へ移行したため、実質は解決済み。正式なcloseはユーザー確認待ち） | 「要決定・外部準備」 |
...
**開発タスクは M7 の39件すべて完了した**（2026-08-01）。残っているのは**運営者の操作が必要なもの**だけ。
...
**リポジトリの状態**: 単体+DB 1,399件緑（191ファイル・skip 6）/ E2E 30件緑（15ファイル）/ `check:providers` 5件緑 / `smoke:live` 3シナリオ緑。CI（GitHub Actions）稼働中。作業ブランチは `stg`、`main` へは**PR経由**で反映する運用（D-8 案A。GitHub側の保護設定は未実施）。**未pushのコミットがあるので `git branch -vv` で確認する**。
...
**D-20: アカウント削除（退会）の手順をどう用意するか（未決・2026-08-08 起票）** — 利用規約第6条とプライバシーポリシー第9条で「お問い合わせ窓口へご連絡いただければ、ご本人確認のうえ削除します」と約束したが、**削除を実行する手順書もスクリプトも無い**（`profiles` の `on delete cascade` は定義済みなので、削除自体は1文で可能）。請求が来てから手順を考えることになる。要決定: (案A)運用スクリプト（`npm run account:delete -- --email <addr>`）を用意し、削除対象テーブルと確認手順を運用メモへ書く（**推奨**。原則3「手順を人間の記憶に依存させない」に沿う。Stripe側の顧客削除・Xトークンの失効も同時に行う必要がある） / (案B)請求時に手作業で対応する（頻度は低いが、削除漏れ＝法令違反のリスクが残る） / (案C)セルフサービスの退会機能を実装する（PRD §3.2で「MVPでやらないこと」と決めたため、スコープ変更の判断が必要）。**時期**: 公開後、最初の請求が来る前。
