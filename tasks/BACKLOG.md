...
### T-M8-294: 設定画面のクライアントJSを必要なタブぶんだけにする `todo`
- 参照: src/app/app/settings/page.tsx:52-66 / 依存: なし / サイズ: M
- 完了条件: 設定画面を開いたときに、表示していないタブの部品のJSを読み込まない
- メモ: 全体監査（2026-08-24）。5タブぶんのclient部品9本（計3,246行）を静的importしており、
  ビルド成果物では `/app/settings` のクライアントJSが **raw 722KB / gzip 196KB**（10ファイル）。
  どのタブを開いても全部届く。`next/dynamic` でタブ単位に分ければ初回の受信量が減る。
  **遷移速度ではなく初回ロードの改善**（監査の検証で「遷移スピードアップとしては計上しない」と補正された）。

### T-M8-293: zodのロケール259KBがクライアントへ届いている `todo`
- 参照: src/lib/validation/**・.next のチャンク解析 / 依存: なし / サイズ: S
- 完了条件: 既定ロケール以外のzodロケールがクライアントバンドルへ入らない
- メモ: 全体監査（2026-08-24）。`/app/settings` のチャンク1本（raw 366KB / gzip 87KB）に
  zodの全52ロケールが含まれていることを実測で確認。日本語しか使っていない。
  **監査の検証で「提案された手順(1)は実測で無効だった」と補正されている**ので、
  着手時は実際に成果物を測りながら手を選ぶこと（推測で入れ替えない）。

### T-M8-292: 一覧の日時整形が呼ばれるたびに Intl を作り直している `todo`
...
### T-M8-291: 日次サマリの配信が利用者数×7往復の直列（上限なし） `todo`
