...
**D-44: トライアル中にプランを往復されるとAIクレジットが何度でも満額に戻る（起票 2026-08-25・`/security-audit` 実行で検出）** — トライアル中に**上位→下位**へ変えると利用枠がリセットされる（2026-08-25の運営者指示・T-M8-299。`billing-return.ts:200-211` が `usage_epoch` を進める）。ところが**上位へ戻すと上限だけが元に戻り、消費済み量は0のまま**なので、Portalを2回開くだけで枠が丸ごと復活する。回数の制限もレート制限も無く、7日間のトライアル中いくらでも繰り返せる。運営キー系プラン（premium/expert）はAI実費が運営者持ちで、支払いは0円。**総額はdoctorで見えるが、`usage_counters` は世代が変わるため「どの利用者が原因か」は辿れない**（原則4に反する）。
