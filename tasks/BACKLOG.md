...
**D-35: 利用規約・特商法の「月間利用枠」→「契約期間ごとの利用枠」の改定文案（起票 2026-08-23・T-M8-259）** — 利用枠のリセットを契約の更新日ごとへ変える（T-M8-258）のに合わせ、法務ページの「月間」表記を改める必要がある。改定文案は T-M8-259 の実装時に添える。要決定: (a) 文案の採否、(b) 利用規約の版を上げて**既存利用者へ再同意を求めるか**（利用枠の数え方の変更は利用者に不利にならない〔期間が短くなることはない〕ため、案A: 版は上げるが再同意は求めず「お知らせ」で周知＝**推奨** / 案B: 再同意を求める）。
