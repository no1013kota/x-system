...
| 外部準備（人間側） | **本番運用開始に残り3件**＝`X_POSTING_MODE=live`・Stripe WebhookのURL修正・Xアカウント連携。ほか法務レビュー・単価確認 | 「要決定・外部準備」P-1／[リリース前チェックリスト §3](../docs/operations/release-checklist.md) |

**2026-08-17: 本番反映を完了した。** migration 5件を適用し、**18日間止まっていた定時実行が動き出した**
...
1. **本番運用の開始に必要な3件（運営者）**
   ...
   (b) **Stripe WebhookのURLを `https://exosai.net/api/stripe/webhook` へ変更**。2026-08-17 時点の設定先は
       `x-system.vercel.app` で**全パス404**。まだ課金イベントが1件も無いため実害は出ていないが、
       **最初の決済で「課金は成功するのにプランが有効にならない」**状態になる。新規作成ではなく
       既存エンドポイントのURL変更にする（署名シークレットが変わらない）
...
### T-M8-143: 自動投稿が下書きを作るだけで投稿されない（連鎖1本が未実装） `todo`
...
- メモ: **外向き副作用（実投稿）に触るので、非productionで実行されないことの確認が必須**
  （CLAUDE.md の変更影響表）。`terminal.ts:104` の `ensurePostPublishJob` が既にあるので、
  成功経路からも同じ関数を呼べる見込み。要件04 §10 手順8の「親を終端へcommitしてから子をdispatch」
  の順序を守ること。詳細と根拠は [監査記録](AUDIT_2026-08-18.md) の該当項。
