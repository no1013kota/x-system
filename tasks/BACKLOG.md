...
### T-M8-325: AIクレジットの粒度を100倍にする（1クレジット=0.01円相当） `todo`
...
- **注意**: 既存利用者の `usage_counters.ai_credits_used` は旧単位のまま。
  移行のmigrationで100倍するか、世代（`usage_epoch`）を進めて0から数え直すかを決める必要がある。
