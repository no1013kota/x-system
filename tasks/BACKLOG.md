...
| 区分 | 残り | 場所 |
|---|---|---|
| 開発タスク（着手可） | 7件（T-M7-27〜32のうち`todo` 4件 ＋ T-M7-37・38・40。T-M7-39は`done`） | 下記M7セクション |
...
4. **発信の質に効く改善（T-M7-37 プロンプトの不利を除去・T-M7-38 分析軸の拡張）と、見つかった不具合（T-M7-40 ニュースの0件。T-M7-39 学習分析は完了）** → 検証カバレッジの残り（T-M7-27・28）→ **運営のしやすさ（T-M7-34 状態確認・T-M7-35 手順の畳み込み・T-M7-29 日次サマリ・T-M7-30 週次メンテ）** → 決定に伴う後始末（T-M7-32 sharp upgrade。T-M7-31 通知の掃除は完了）
