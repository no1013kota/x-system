...
> **最優先: `stg` が push されていない（2026-08-11 時点で origin/stg より24コミット先行・遅れ0。
> 最新の数は `git rev-list --count origin/stg..HEAD` で分かる）。**
...
| 区分 | 残り | 場所 |
|---|---|---|
| 開発タスク（着手可） | **2件**（T-M8-69 providerのmodels.list疎通／T-M8-68 体感速度の残り）。T-M8-84〜86 は 2026-08-12 に done | 下記M8セクション |
| 開発タスク（blocked） | 1件（T-M7-17 Gemini画像。運営者判断で一旦不要） | 同 |
| 要決定 | **5件**（D-16 / D-17 / D-18 / D-19 / D-20）。D-21〜D-26 は 2026-08-11 に解決済み | 「要決定・外部準備」 |
| リファクタ | **R33〜R36・R38 の5件が未着手**（R19〜R32・R37 は done） | [REFACTOR_PLAN](./REFACTOR_PLAN.md) |
...
4. D-18（法28条の根拠づけ）・D-19（ログ保持期間の実設定）を決める。D-16は実質解決済みなのでcloseするだけ
...
**リポジトリの状態**: 単体+DB **1,735件緑**。E2Eは LP・モバイル幅・パスワード再設定を実行して緑
（**全E2Eの通し実行は未実施**）。lint・typecheck・本番ビルドは成功。
作業ブランチ `fix/audit-followups-a`（未push）、`stg` 経由で `main` へPRの運用。
