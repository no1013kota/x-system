...
### T-M8-186: 生成プロンプト編集でプレースホルダーを増減・編集できるようにする `done`
- 参照: 要件06 §7（SC-07 生成に使うプロンプト）・§5（SC-08）・要件02 §3.21 placeholders / 依存: なし / サイズ: M
- 完了条件:
  - 投稿作成・スケジュールの「投稿の型」プロンプト編集で、本文中の {名前}（{{...}}を除く）を増減すると、その場でプレースホルダー入力欄が同じ名前でリアルタイムに増減する
  - 追加した {名前} への入力値が生成へ渡り、本文へ差し込まれる（この生成にだけ／保存して以後もの両経路）
  - 「パターンに保存」した場合は placeholders 定義も本文から導出して保存される
  - 既存の検証（名前の文字制限・件数上限10）と同じ制限をかける
- メモ: 運営者の指示（2026-08-21）。※T-M8-183〜185は並行セッション（LPフッタXアイコン・/blog）が使用中。

### T-M8-188: 最新ニュースを新着順基本・最新500件表示にし、テーマ/インパクトは選択式ソートへ `done`
- 参照: 要件06（SC-06）・要件05 §6・要件04 §14（保持cleanup）・PRD N-2 / 依存: なし / サイズ: M
- 完了条件:
  - 一覧は新着順（取得時刻の新しい順）が基本。新着順のボタンは置かない（運営者の追指示 2026-08-22）
  - テーマ・インパクトは選択式（select）で、選ぶと一致する記事が先頭へ寄る（絞り込みではなく記事は消えない）
  - 表示対象は最新500件のみ（50件×最大10ページ）。「全K件」表示も500を超えない
  - 定時cleanupが新しい順500件を超えるnews_itemsを削除する（下書き・通知payloadから参照される行は残す＝既存の参照ガードと同じ）
  - 40日保持は従来どおり（どちらか早い方で消える）
  - docs（要件04・05・06・PRD N-2）を更新
- メモ: 運営者の指示（2026-08-22）。T-M8-187のソートUI（ピル型リンク）はこの形へ置き換え。
- 実装メモ（2026-08-22）: listNewsItemsを「新着500件の窓→theme/impact一致を先頭へ並べ替え」の
  2段クエリへ変更（whereで絞らないので件数は不変）。新着順キーはfetched_at優先
  （NEWS_ORDER_BY）でcleanupの500件判定と共有。cleanupへtrimNewsItemsOverCap追加
  （row_numberで数え、参照付きは残す。BATCH500/回で収束）。E2Eはselect操作で
  高→先頭・テーマ最優先を実ブラウザ確認。設定画面の説明を「一覧は最新500件」へ。

### T-M8-189: ニュース・発信テーマをAI/Web3/SNS/投資/恋愛/美容の6分野へ `done`
- 参照: PRD N-1・§8.3・要件02 §6・プロンプト設計書 §6.10 / 依存: なし / サイズ: L（取得と表示を分割すると分野がズレて途中状態で壊れるためLのまま）
- 完了条件:
  - news_category に love・beauty が加わり、定時取得が6分野（ai/web3/sns/investment/love/beauty）で回る
  - テーマ選択肢（AI設定・投稿作成・スケジュール・通知設定）が新6分野になる。旧値（ビジネス・業務改善）を保存済みのデータは表示・検証が壊れない（語彙には残し、選択肢から外す）
  - 新規登録の既定 news_config.categories が新6分野になる
  - 取得費用の変化（3→6分野で約2倍）をdocsへ明記し、運営者へ数字で報告する
- メモ: 運営者の指示（2026-08-22）。2026-08-02の「3分野へ絞る」決定（T-M7-55）を運営者指示で更新。
- 実装メモ（2026-08-22）: enumへlove/beauty追加・schedule_slots制約拡張・新規登録既定を6分野へ
  （migration 20260822000001）。語彙は運用6＋旧2（business系は選択肢から外し保存値の表示・検証
  のみ維持。personaフォームは選択中のときだけ旧テーマを出す）。NEWS_FETCH_CATEGORIESを6分野へ
  （実測$0.24〜0.50/分野・回＝月$260〜540。3分野時の約2倍）。check:providers 17件緑・
  smoke:live全緑（$0.19。smokeの旧user_opinion入力をplaceholder_valuesへ修正）・恋愛分野の
  実リサーチ1周（0件=該当なしを正しく説明・$0.075）。

### T-M8-190: プロンプト集・ブログのヘッダーを「ホーム＋もう片方」への遷移ボタンに `done`
- 参照: 要件06（公開ページ）・T-M8-184 / 依存: なし / サイズ: S
- 完了条件:
  - /prompt-templates のヘッダーに「ホーム」と「ブログ」への遷移ボタン、/blog には「ホーム」と「プロンプト集」への遷移ボタンが出る
  - 現在地をタブ風に示す表示（現在地の下線タブ）は無くなる
- メモ: 運営者の指示（2026-08-22）。ページ自体は元から別ページ。ヘッダーの見え方をタブ風から遷移ボタンへ変える。
- 実装メモ（2026-08-22）: PublicPageShellのnavを「ホーム＋現在地でない方」のghostボタン2つへ。aria-current廃止。E2E（blog/prompt-templates 5件）緑。

### T-M8-191: 招待の紐づけを確認コード検証時にもフォールバック実行 `done`
- 参照: docs/cp/invite_cp.md §4 / 依存: なし / サイズ: S
- 完了条件:
  - 登録時の紐づけが失敗してもCookieが残っていれば、確認コード検証の成功時にもう一度紐づけを試みる（冪等・失敗しても検証は止めない）
...
### T-M8-234: 法令まわりの「上乗せルール」を撤廃し、法定の最低限へ寄せる `done`
...
### T-M8-274: ロック判定と操作中アカウントの解決を1波にまとめる `done`
- 参照: 要件03 §5 / 依存: T-M8-273 / サイズ: S
- 完了条件: 対象4画面で「ロック判定」と「操作中アカウントの解決」が直列にならない
- メモ: `/speed-up` の初回実行。**T-M8-269で私が入れたロック判定が直列の1往復を足していた**
  （契約中の利用者は毎回それを待っていた）。両者は user.id しか要らず互いに独立。
- 計測（`/speed-up` の作法）:
  - 段数: 投稿分析・スケジュール・投稿作成 **4段→3段**、最新ニュース **3段→2段**
  - 実測（/app → /app/schedule・5回の中央値・ローカルdev）: **700ms → 647ms**
  - ロック時は解決結果を捨てる（軽い無駄）が、ロックは稀で、待たされるのは毎回の通常利用のほう

### T-M8-273: 支払いが滞っている場合もロックし、「お支払い情報を更新」へ案内する `done`
- 参照: 要件03 §5・要件06 §2 / 依存: T-M8-269 / サイズ: S
- 完了条件:
  - past_due／unpaid／paused でも機能画面がロックされる
  - 文言は「お支払い情報を更新してください」で、導線は課金・プラン（Portal）。/plans へは送らない
  - 友達招待と課金・プランタブは引き続き使える
- メモ: 運営者の指示（2026-08-23）。T-M8-269では「直し方が違う」ことを理由にロック対象外に
  していたが、**運営者の判断でロック対象に含め、文言と導線の方を分ける**形にした。
- 実装メモ（2026-08-23）:
  - `requiresPlanSelection`（真偽値）→ `appLockFor`（`plan_required` / `payment_required` / null）。
    真偽値のままだと、プランはあるのに「プランを登録してください」と直せない案内を出してしまう
  - `AppLockedPage` / `AppLockedNotice` が理由ごとに文言・CTA・行き先を出し分ける
  - 契約バナーの「既存データは閲覧できますが」を削除（実際は閲覧できないため）
  - E2E: past_due で「お支払い情報を更新してください」が出て、リンク先が課金・プランであること

### T-M8-294: 設定画面のクライアントJSを必要なタブぶんだけにする `wontfix`
- 参照: src/app/app/settings/page.tsx / 依存: なし / サイズ: M
- 完了条件（達成できず）: 設定画面を開いたときに、表示していないタブの部品のJSを読み込まない
- メモ: 全体監査（2026-08-24）。5タブぶんのclient部品9本（計3,246行）を静的importしている。
  **遷移速度ではなく初回ロードの改善**（監査の検証で「遷移スピードアップとしては計上しない」と補正された）。
- **調査結果（2026-08-25・実測して効果なしと判断）**: 9本すべてを `next/dynamic` へ置き換え、
  設定ルートのclient reference manifestが参照するチャンクを**変更前後で測った**。
  - 変更前: 10本 / raw **706KB**
  - 変更後: 10本 / raw **707KB**（むしろ1KB増）
  App Router では、Server Component から辿れるclient部品は**`next/dynamic` にしても
  ルートのmanifestに載ったまま**で、Turbopackは元から部品単位に分割している。
  つまり「静的importだから全部届く」という前提が実測と合っていなかった。
- **やらない理由**: 数字が動かないのに、9本ぶんの間接（`dynamic(() => import(...))`）だけが増える。
  受信量を本当に減らすなら、タブごとに**別ルート**へ分ける（`/app/settings/api-keys` 等）形が要り、
  それは画面設計の変更なので運営者の判断が要る。
- **再検討の条件**: 設定画面が体感で重いという報告が出たとき。そのときは
  **実ブラウザで転送量を測ってから**手を選ぶ（manifestの数字だけでは体感と結びつかない）。

### T-M8-293: zodのロケール259KBがクライアントへ届いている `wontfix`
- 参照: src/lib/api-keys.ts・next.config.ts・.next のチャンク解析 / 依存: なし / サイズ: S
- 完了条件（達成できず）: 既定ロケール以外のzodロケールがクライアントバンドルへ入らない
- メモ: 全体監査（2026-08-24）。`/app/settings` のチャンク1本（raw 365KB / gzip 87KB）に
  zodの全52ロケールが含まれていることを実測で確認。日本語しか使っていない。
- **調査結果（2026-08-25・4手すべて実測して不可と判断）**:
  1. **原因**: `zod/v4/classic/external.js` の
     `export * as locales from "../locales/index.js"`（名前空間ごとのバレル再エクスポート）。
     名前空間再輸出は「どの名前が使われるか」を静的に決められないためbundlerが落としきれない。
  2. `experimental.optimizePackageImports: ["zod"]` → **効かなかった**（ビルドして再測定・
     韓国語/フランス語の文言が残る）。監査の注記どおり。
  3. `turbopack.resolveAlias` でバレルを空モジュールへ → **効かなかった**。
     zod内部の import が**相対パス**（`../locales/index.js`）で、resolveAliasは
     モジュール指定子にしか効かない。
  4. `zod/mini`・`zod/v4/core` も**同じバレルを持つ**ので、入口を変えても外れない。
- **やらない理由**: 残る手は「クライアントからzodを外す」ことだが、
  `api-key-settings.tsx` がzodを読むのは**意図的**——保存できるか／押せない理由を
  **サーバー検証と同じスキーマ**で決めている（T-M8-84）。以前は画面が条件を写経して
  長さしか見ておらず、`bad id` のような値でも押せてサーバーに弾かれていた。
  259KBは**その不具合を二度と起こさないために買っている値段**で、外すと同じ穴が戻る。
  gzipで87KBのうちロケールぶんは一部であり、割に合わない。
- **再検討の条件**: zod側がバレルをやめる（upstreamの変更）か、
  ロケールを含まない公式の入口が増えたとき。

### T-M8-292: 一覧の日時整形が呼ばれるたびに Intl を作り直している `done`
- 参照: src/lib/format.ts:14-20 / 依存: なし / サイズ: S
- 完了条件: `Intl.DateTimeFormat` をモジュールスコープで1回だけ作る
- メモ: 全体監査（2026-08-24）。`formatJst` が呼び出しごとに `new Intl.DateTimeFormat` を生成する。
  ロケール・タイムゾーンは固定なので単一インスタンス化して安全（JSTにDSTは無く、`format()` は再入可能）。
  一覧の行数だけ生成されるため、下書き・履歴・通知の描画で効く。小さいが確実。
- 実装メモ（2026-08-25 完了）: `Intl.DateTimeFormat` と `Intl.NumberFormat` を
  モジュールスコープの単一実体にした。**実測: 20,000回で 978ms → 24ms**
  （1,000行の一覧なら約49ms→約1ms）。`format.ts` にテストが無かったので新設し、
  **表記が変わっていないこと**を固定した（共有した実体が壊れると全画面の日時が同時に狂うのに、
  例外は出ないのでテストが無いと気付けない）。

### T-M8-295: 未登録アカウントでのログインに「登録されていません」と案内する `done`
- 参照: 要件03 §1.1 / 依存: なし / サイズ: S
- 完了条件:
  - 登録が無いアドレスでログインすると「このメールアドレスは登録されていません。新規登録してください。」
    と出て、新規登録への導線が付く
  - 登録があってパスワードが違うときは「メールアドレスまたはパスワードが正しくありません。」
  - パスワード再設定は従来どおり有無を明かさない
- メモ: 運営者の指示（2026-08-25）。「ログインできませんでした。入力内容を確認し、**時間をおいて**
  再度お試しください」しか出ず、登録していない人が正しいパスワードを探して何度も試す状態だった。
  **これは列挙対策の方針転換**（旧E2E「原因を推測させる情報も出さない」を書き換えた）。
  判断の材料は要件03 §1.1 に3点とも残した。伏せる方へ戻すときはそこを見ること。
- 実装メモ（2026-08-25）:
  - Supabase はどちらも `invalid_credentials` を返すため、`auth.users` の存在で言い分ける
    （`profiles` は欠損し得るので使わない——登録済みの人へ「新規登録してください」と出てしまう）
  - 存在確認が落ちたら「登録済み」側へ倒す（打ち間違いをDB障害で「予期しないエラー」にしない）
  - 「時間をおいて」は待てば直る失敗にだけ言う形へ整理
  - **ついでに直した**: ログイン画面に新規登録リンクが2本出ていた（並行セッション追加分と
    T-M8-268追加分）。`next` を引き継ぐ1本へ集約

### T-M8-291: 日次サマリの配信が利用者数×7往復の直列（上限なし） `done`
- 参照: src/lib/ops/daily-summary.ts:320-355 / 依存: なし / サイズ: M
- 完了条件:
  - 対象者ごとの重複チェックと作成が1文にまとまる（現在は1人1往復ずつ）
  - 1回の実行で処理しきれない件数になったとき、**黙って途中で終わらない**（残件を次回へ回し、
    運営者が気付ける経路へ載せる）
- メモ: 全体監査（2026-08-24）。1人あたり「重複チェック1＋集計5＋作成1＝7往復」を直列で回し、
  対象者に上限が無い。`scheduler_tick` の maxDuration は200秒なので、**利用者が1,000人規模に
  なると打ち切られ、後半の利用者にサマリが届かない**（しかも失敗として記録されない＝原則1違反）。
  集計5本のバッチ化は形が大きいので、まず重複チェックと作成の1文化＋残件の持ち越しから。
- 実装メモ（2026-08-25 完了）: **重複チェックを対象抽出へ畳んだ**——`not exists` で
  「その日ぶんを既に作った人」を最初から除く。以前は1人ずつ問い合わせており、
  作るのは1日1回きりなのに**5分ごとのtickのたびに人数ぶんの往復**が走っていた。
  **人数に上限（`SUMMARY_BATCH_LIMIT = 200`）を置き、積み残しの人数を返す**。
  残りは次のtick（5分後）が拾う——その日ぶんを作っていない人がそのまま対象に残るので、
  待ち行列を別に持たなくてよい。**積み残しは `onCleanupError` へ載せる**（毎朝のメール・
  doctorが読む経路）——黙って途中で終わると、後半の利用者に届かないのに「成功」に見える（原則1）。
  DBテストで「作成済みの人は対象にも積み残しにも数えない」「配信時刻より前は0で返す」を固定。
- 参考: 同型のニュース配信は T-M8-290 で1文化済み（そちらは1人1往復だった）。

### T-M8-290: ニュースダイジェストの配信を1文にまとめる `done`
- 参照: src/lib/jobs/news-digest.ts / 依存: なし / サイズ: S
- 完了条件: 対象者ごとに1往復ではなく、1文で全員ぶんを作る（退会競合の対策は維持）
- メモ: 全体監査（2026-08-24）。ニュース取得は1日5回走り、その最後にダイジェストを配る。
  対象者が増えるほど所要時間が線形に伸び、ここが遅れると同じ窓のダイジェストがまるごと落ちる。
- 実装メモ（2026-08-24）: `unnest($1::uuid[], …)` と `profiles` の join で1文化。
  **`for key share of p` は維持**——1人の退会で残り全員の配信が止まる競合（T-M8-19）を防ぐ形は
  そのままで、行が0件になるだけになる。往復は **対象者数 → 1**。単体テストも
  「対象者ごとに文を投げない」ことを assert する形へ更新した。

### T-M8-289: 下書き・履歴一覧の並び順に合う索引を足す `done`
- 参照: 要件02 §3.9 / 依存: なし / サイズ: S
- 完了条件: 一覧の `order by coalesce(posted_at, updated_at) desc, created_at desc` が索引に乗る
- メモ: 全体監査（2026-08-24）。既存の2索引はどちらもこの**式**に一致せず、履歴タブは
  limit 50 を付けていても**アカウントの全行を読んでから並べ直して**いた。投稿が増えた利用者ほど
  画面が遅くなる形（利用者数ではなくその人の履歴に比例）。
- 実装メモ（2026-08-24）: 式索引 `(x_account_id, (coalesce(posted_at, updated_at)) desc,
  created_at desc)`（migration 20260824000002）。`status` は `= any($2)` のパラメータ渡しで
  計画時に絞れないため部分索引にはせず、絞り込み条件として残す。
  計測（200,000行のベンチ表・履歴タブ相当）: プランコスト **3641 → 29.7（約1/122）**・Sortが消える。
  下書きタブの上限なし取得は表示件数が変わるため**要決定 D-39** へ回した。

### T-M8-288: App Shellの往復を8→5へ（利用者まわりの読み取りを1行に束ねる） `done`
- 参照: 要件01 §5 / 依存: T-M8-286 / サイズ: M
- 完了条件:
  - App Shell の profile・Xキー状態・未読数・利用枠が1文で読める
  - ホーム・設定が重ねて投げていた利用枠の取得が消える
  - 挙動不変（バナー・通知ベル・利用枠の表示が変わらない）
- メモ: 全体監査（2026-08-24）の遷移速度の本命。App Shellは**全 `/app` 遷移と操作後の
  `router.refresh()` のたび**に丸ごと再実行されるので、ここの往復は全画面に効く。
  T-M8-197（1接続へ束ねる）は `done` だが**本体の配線がコミットされていなかった**
  （ヘルパ2ファイルとBACKLOGだけが入っていた）。今回は接続数ではなく**クエリ数**を
  減らす形にしたので、待ち時間と接続の両方が減る。
- 実装メモ（2026-08-24）:
  - `loadRequestProfile` を「利用者まわりの1行」へ拡張（profile列＋Xキー状態＋未読数＋
    利用枠カウンタ＋リセット日）。行が増えない根拠: `user_api_keys` は unique(user_id,provider)、
    `usage_counters` は PK(user_id,month)
  - port の形（4関数）は変えず、中身を同じ1行から供給（`cache()` 済みなので中核の
    `Promise.all` はそのまま・実往復は1本）
  - 利用枠の**組み立て**を純粋層の `usageSummaryFrom` へ切り出し、SQL経路と束ね経路で
    同じ結果になるようにした（読み方と判定の分離）
  - 計測: App Shell 8往復→5。ホーム・設定はさらに各1本減
  - `request-profile-server.db.test.ts` を追加（行なし／関連なし／関連ありの3ケース）。
    1文へ束ねると壊れたとき**バナーがまとめて黙って消える**ため、境界をDBに対して固定した

### T-M8-287: 当日投稿数の集計が毎描画で `usage_events` を全走査していた `done`
- 参照: 要件02 §3.13 / 依存: なし / サイズ: S
- 完了条件:
  - `countTodaysPostsForXAccount` が索引の効く述語になり、`usage_events` を全走査しない
  - `usage_events` に保持期間が無いことが正本に書かれている
- メモ: 全体監査（2026-08-24）で検出した**スケールの最重要欠陥**。この集計は `/app` の全描画・
  予約枠のenqueue・投稿実行の3経路から呼ばれる。コードには「日次上限と40日の保持期間で
  数千行に収まるので全走査でよい」と書かれていたが、**その保持期間は存在しない**
  （cleanupが消すのは `external_api_usage_events`。`usage_events` は台帳として永続）。
  利用者が増えるほど全描画が遅くなる形だった。
- 実装メモ（2026-08-24）:
  - 部分索引 `(x_account_id, created_at desc) where operation='post_create'`（migration 20260824000001）
  - 述語を JST当日の**範囲比較**へ（日付関数で包むと索引が効かない）
  - 計測（200,000行のベンチ表）: プランコスト **2759 → 130（約1/21）**。件数は一致
  - 誤ったコメントを事実へ差し替え、正本にも「保持期間は無い／索引の効く述語で書く」を明記

### T-M8-286: profiles の重複読み取りを1リクエスト1回へまとめる `done`
- 参照: 要件01 §5 / 依存: なし / サイズ: S
- 完了条件: App Shell・ロック判定・ホームが同じ profile 行を1回だけ読む
- メモ: 全体監査（2026-08-24）。同じ行を1遷移で2〜3回別々のSQLで読んでいた。
- 実装メモ: `loadRequestProfile`（React cache）へ集約。往復は profiles 2〜3回→1回
  （ホームは3→1）。依存境界テストのアンカーを実際に pool を使う側へ移した。

### T-M8-269: プラン未登録・解約中は機能画面をロックする `done`
- 参照: 要件03 §5・要件06 §2 / 依存: T-M8-268 / サイズ: S
- 完了条件:
  - プラン未登録・解約中は ホーム／最新ニュース／投稿作成／スケジュール／投稿分析／
    設定（課金・プラン以外のタブ）が開けず、「先にプランを登録してください」と登録導線が出る
  - 設定はタブが残り、課金・プランへ行ける
  - 友達招待はプラン未登録でも使える
  - 支払い滞り（past_due/unpaid/paused）はロックしない
- メモ: 運営者の指示（2026-08-23。T-M8-268の「閲覧は自由」を撤回する訂正）。
  Xアカウント連携・最新ニュースを含め、プラン登録前は触れない形にする。
- 実装メモ（2026-08-23）:
  - 判定は `requiresPlanSelection`（`SUBSCRIPTION_ACCESS.actionPath === "/plans"` から導出。
    未知statusはロック側へfail closed）。ページ側は `isPlanRequired`（React `cache()` で
    同一リクエスト1回だけ読む）、設定は既に読んでいる profile から判定して往復を増やさない
  - **リダイレクトではなくその場に `LockedState`**（既存コンポーネントを再利用）。設定はタブを
    残して中身だけ差し替え——タブを消すと登録・再開の入口へ辿り着けない
  - 契約バナーの文言も実挙動へ合わせた（「画面の確認はできます」→「友達招待は使えます」）
  - E2E: 機能5画面のロック・設定タブから課金・プランへの到達・招待は使えること

### T-M8-268: 契約前・解約後も通常画面と友達招待を使えるようにする `done`
- 参照: 要件01 §5・要件03 §5・要件06 §2 / 依存: T-M8-267 / サイズ: M
- 完了条件:
  - 登録しただけ／解約済みでもログイン後は `/app` に着き、機能画面を閲覧できる
  - 友達招待（`/app/invite`）は契約なしで使える。LPとログイン画面から到達できる
  - 実行しようとしたときだけプラン選択へ案内される（画面では弾かない）
  - LPに招待キャンペーンの導線（ナビ＋本文セクション）がある
- メモ: 運営者の指示（2026-08-23）。T-M8-266（解約後は機能画面を見せない）を、より一般的な
  「閲覧は自由・操作でアップグレード誘導」へ改めたもの。**実行の抑止は T-M8-267 で
  lease まで固めた**ので、画面を隠す必要が無くなった。
- 実装メモ（2026-08-23）:
  - `routeGuardDestination` を認証だけに単純化。これに伴い **proxy（middleware）の
    profiles SELECT を削除**——`/app`配下の全リクエストで走っていたDB往復が1本消え、
    画面遷移が速くなる（副産物の性能改善）
  - 死んだ概念を削除: `SubscriptionAccess.viewScope`・`canBrowseApp`・`isLimitedSettingsRoute`・
    proxyの profile 読み取り失敗の記録経路（読まなくなったため不要）
  - ログイン後／登録後の着地を `/app`（または `next`）へ統一。確認メッセージはホームで出す
  - LP: ナビ「友達招待」＋料金直後の招待キャンペーンセクション（行き先は `/app/invite` 固定）
  - ログイン画面に「新規登録」リンク（`next` を引き継ぐ）を追加——招待CTAから来た未登録者の行き止まりを解消
  - E2E: `invite-access.spec.ts`（LP→ログイン→招待／プラン未選択でも招待リンク）、plans.spec の解約ケースを新仕様へ
...
- 完了条件:
  - canceled の利用者はログイン直後・/app配下のどのURLでも /plans へ送られる（課金タブと旧supportタブだけ例外）
  - /plans に「ご契約は終了しています。データは保持されています」と設定＞課金（プランを再開）への導線が出る
  - データは削除しない（再開すればそのまま見える）
- メモ: 運営者の指示（2026-08-23「どの機能も見れず操作もできないように」）。変更は共通マッピング
  `SUBSCRIPTION_ACCESS.canceled.viewScope` を app→settings_plans にする1点に集約され、
  route guard・ログイン後遷移・バナーが追随する（要件03 §5「1つの共通マッピングを正とする」の設計どおり）。
- 実装メモ（2026-08-23）:
  - 契約バナーの canceled 文言を「閲覧できます」→「データは保持・再開で継続」へ
  - **関連修正**: 解約予約中バナーが「解約する」を出していた出し分け漏れ（T-M8-253の回帰・
...
- 完了条件:
  - 設定＞設定タブのXアカウント行に「スケジュールをすべて停止」「すべて再開」が表示されない
  - スケジュール画面（SC-08）の同ボタンは従来どおり動く
- メモ: 運営者の指示（2026-08-23）。T-M8-251でSC-08と併設したが、設定タブには不要。
  表示のためだけに引いていたスロット集計（paused_slots等のサブクエリ3本）も削除。

### T-M8-262: follower_snapshotを毎時00分へ・「分析を開始」をレポート欄右上へ `done`
- 参照: 要件04 §6・要件06 SC-09 / 依存: T-M8-257 / サイズ: S
- 完了条件:
  - follower_snapshot のスケジュールが毎時00分（`0 * * * *`）になる（vercel.json・plist・正本の表が一致）
  - 「分析を開始」ボタンが分析レポート欄の右上に表示される（ページヘッダには無い）
- メモ: 運営者の指示（2026-08-23）。毎時10分は分かりにくい。metrics_collectorと同時刻になるが、
  tokenのrefreshは行lease（token_refresh_lock_id）で直列化されるため競合しない（lease取得に
  負けた側はskipして次の毎時に拾われる）。ボタンはページヘッダに置くとフォロワー推移にも
  掛かる操作に見えるため、対象であるレポート欄の中へ。

### T-M8-257: フォロワー数記録だけ毎日自動（毎時cron）へ復帰 `done`
- 参照: PRD K-3・要件04 §6/§13・要件02 §3.11 / 依存: T-M8-255 / サイズ: S
- 完了条件:
  - `/api/cron/follower-snapshot`（毎時10分）が復活し、当日分が無いactiveアカウントを毎日1回記録する
  - **契約が有効（trialing/active）な利用者のアカウントだけ**を読む（解約済みの読取費用を出さない）
  - 「分析を開始」ボタンは従来どおり押した時点の最新値で当日分を上書きする
  - 投稿分析（AI）はボタンの手動実行のまま（自動へ戻さない）
- メモ: 運営者の指示（2026-08-23「フォロワー数取得は安いですよね。毎日でも良いですかね？」）。
  読取$0.010/アカ/日（1アカ月約$0.30≒45円、100人×平均1.5アカでも月約$45）と安価な一方、
  推移グラフ（K-3）は毎日の点が揃って初めて意味を持つため。高コストだった投稿分析AI
  （$0.10〜0.15/回）はT-M8-255の手動のまま。
- 実装メモ（2026-08-23）:
  - `executeFollowerSnapshot` をT-M8-255削除前の形で復元し、選定SQLに
    `profiles.subscription_status = any(EXECUTABLE_SUBSCRIPTION_STATUSES)` を追加（新規ゲート）
  - route・vercel.json（3→4本）・launchd plist・cron-call.sh・関連テスト（バッチ4件＋契約ゲート1件）を復元
  - 文言を「毎日自動で記録されます」へ戻し、ボタン側の記録は「最新値で上書き」の補助と位置づけ
...
- 完了条件:
  ...
  - 使用量（通常投稿・URL付き投稿・AIクレジット）が**契約期間（更新日〜次の更新日）**で数えられ、更新日に0から始まる
  - 上位への即時変更では期間が変わらず、残り期間に新プランの上限が効く。期間末の下位変更では新期間＝新上限で0から
  - 画面の「リセット日」が翌月1日ではなく**次回更新日**になる
  - webhook未達で期間が更新されないときは、期限切れの実行停止（T-M8-235）と同じ向きで**黙って使われ続けない**
...
- 実装メモ（2026-08-23 完了）: migration `20260823000009`（`profiles.current_period_start`・`usage_events/usage_counters.month` の
  ...
  レビュー反映（2026-08-23）: 既存契約者の `current_period_start` は scheduler_tick の日次「契約期間の補完」
  （`period-backfill.ts`・期間の2列だけ）で埋める（運営者の手順不要）。未同期のあいだはリセット日を出さない。
  生成jobの上限メッセージ・Stripe商品説明・投稿枠の実DBテスト・settle/通知dedupe/投影のテストを追加。
  トライアルは最初の有料期間と枠を共有する（運営者の指示 2026-08-23・D-38 解決。`usagePeriodStartExpr` で
  期間の開始をトライアル開始に読み替え。実DBテスト＋ライブテスト「トライアルと最初の有料期間で1つの枠を共有」）。

### T-M8-259: 利用規約・特商法の「月間利用枠」を「契約期間ごとの利用枠」へ改定 `done`
- 参照: 要件06 §11 法務ページ・src/app/terms/page.tsx・legal-pages.test.ts / 依存: T-M8-258 / サイズ: S
- 完了条件:
  - 利用規約・特商法表記・プラン選択の文言が「契約期間（更新日から次の更新日まで）ごと」の利用枠として読める
  - 規約の版と同意の再取得方針が要件06 §11 の規則どおりに扱われる（文言変更が再同意を要するかを判断し記録する）
  - `legal-pages.test.ts` が新しい文言を固定する
- メモ: 運営者の決定（2026-08-23）。**改定文案は公開前に運営者が確認する**（要決定D-35として案を添える）。
- 実装メモ（2026-08-23 完了・暫定案A）: 利用規約第5条と特商法「販売条件」を「契約期間（お支払いの更新日から次の更新日まで）ごと」の
  利用枠へ改定し、繰り越し無し・期間途中の上位変更は同じ期間内で新上限、を明記。**同意version（`CURRENT_TERMS_VERSION`）は据え置き**
  （版を上げると全利用者に再同意を要求する仕組みのため。不利益のない改定）で、見出し横に `updatedLabel`
  「2026年8月23日 一部改定（利用枠の数え方を契約期間ごとへ）」を表示。`legal-pages.test.ts` が新文言を固定。
  文案の確認は要決定D-35（運営者が文案を変えたい場合は `src/app/terms/page.tsx` 第5条と
  `src/app/legal/commercial-transactions/page.tsx` の「販売条件」を直す）。
...
- 完了条件:
  ...
  - 設定＞課金とホームのバナーに「◯月◯日に◯◯プランへ変わります」が出る
  - 「変更を取り消す」でアプリから予約を解除できる（本人の契約だけ。取り消すと表示も消える）
  - `customer.subscription.updated` で `schedule` の有無を見る。購読イベントの不足は doctor が検出する
- メモ: 運営者の決定（2026-08-23・案A＋B）。当初は「予約が付いた契約はPortalで再変更できない」前提だったが、
  **2026-08-23 テストモードの Portal を実ブラウザで操作して確認**: 予約付きでも別プランへ変更でき（予約は置き換わる）、
  無いのは「今のプランのまま続ける（予約だけ取り消す）」だけ。取り消しはその穴を埋める。別Productの Price 間でも
  期間末予約（schedule: phases=[現在の期間, 新Priceが1秒]・release）になることも同時に確認。
- 実装メモ（2026-08-23 完了）: migration `20260823000008`（`profiles.scheduled_plan`/`scheduled_plan_at`・対のCHECK）。
  ...
  レビュー反映（2026-08-23）: schedule を読めなかった回は保存済みの予約を上書きしない（`scheduleUnavailable`）、
  Portal からの戻り（billing-return）も schedule を読む（読まないと予約が null で書かれ後続 webhook が stale になる）、
  予約中も「プランを変更」を残す。

### T-M8-270: プラン変更の説明・日割りの案内（Stripeの確認画面には書けないため、アプリ側で出す） `done`
- 参照: 要件03 §2.2 Portal・要件06 §1.1 課金タブ / 依存: なし / サイズ: S
- 完了条件:
  - プラン選択画面でプラン名の直下に「上位＝すぐ切り替わり差額は日割りで次回請求へ合算／下位＝現在の請求期間の終了日」が出る
  - Stripeの確認画面に「本日が期日の金額」が出ない（`proration_behavior=create_prorations`）
  - 文言とPortal設定の対応を `portal-configuration.test.ts` が固定し、doctor が設定のずれを検出する
- メモ: 運営者の要望（2026-08-23）。当初は `always_invoice`（差額を即時決済）にして Stripe 自身に日割りの内訳を描かせたが、
  同時に出る**「本日が期日の金額」という見出しが分かりにくい**ため取りやめ（運営者の指示）。**Stripe の確認画面・Checkout の
  文言はこちらで差し替えられない**（APIにもダッシュボードにも設定が無い。SDKの型と実画面で確認）。書けるのは
  (1) 商品名・商品説明（選択画面でプラン名の直下。**Checkoutにも同じ文が出る**）、(2) 左パネルの見出し
  （`business_profile.headline`。全画面共通）の2つだけ。今回は (1) を使う。
- **採番の注意**: 実装当初は T-M8-267 で起票したが、並行セッションが同じ番号を使っていたため T-M8-270 へ振り直した。
  コミットメッセージの一部は `T-M8-267` のまま（履歴は書き換えていない）。
- 実装メモ（2026-08-23 完了）: `proration_behavior` は `create_prorations`（差額は次回請求へ合算）のまま。
  商品説明は**運営者がStripeダッシュボード（テストモード）で整えた文面を正本としてコードへ取り込み**、
  `--target local|staging|production` の3環境へ流して**同一内容に統一**した（実測で3環境一致を確認）。
  一時的に商品説明へプラン変更の注記を足したが、運営者の指示で取りやめ（Checkoutにも出てしまうため）。
  **Stripeの確認画面（「更新を確認」）に独自の文章は書けない**——Stripe公式ドキュメントでも、書けるのは
  見出し（`business_profile.headline`）・利用規約リンク・戻り先リンク・ビジネス名・ブランディングのみ。
  **上位変更で日割りの説明が出ない件は案A（確定後に出す）で対応**（運営者の決定 2026-08-23・D-37 解決）:
  Portalから戻った直後（`portal=return`）に、次回請求書の下見の日割り行だけを合計して
  「変更前後の料金を日割りで計算した差額 ¥2,500 は、次回のご請求（2026年9月23日）に加算されます。」を案内へ足す
  （`src/lib/stripe/proration-preview.ts`）。**実物のStripeテスト契約（standard→premium）で ¥2,500 と表示されることを確認**。
  あわせて、予約済みのプラン変更を**プラン名と同じ場所**に「◯月◯日に◯◯プランへ切り替わります」として表示
  （運営者の指示。別行だとプラン名だけ見た人が今のプランが続くと誤解する）。

### T-M8-271: 解約・下位変更の予定を「プラン」の直下へ出し、解約予定の取り消しをその場で行う `done`
- 参照: 要件06 §1.1 課金タブ・要件05 Server Action / 依存: T-M8-270 / サイズ: S
- 完了条件:
  - 解約予定・下位プランへの切り替え予定を「プラン」の下・「現在の期間終了日」の上に出す（予定が無ければ行ごと出さない）
  - 「解約予定を取り消す」を押したらその場で取り消す（Stripeの画面へ遷移しない）
  - 取り消しは本人の契約だけ。解約済みの再開は従来どおり「プランを再開」（T-M8-264）
- メモ: 運営者の指示（2026-08-23）。予定を別行に離すと、プラン名だけ見た人は今のプランがそのまま続くと思ってしまう。
  取り消しも Portal のトップへ飛ばすと「同じ操作をもう1画面挟む」だけに見える。
- 実装メモ（2026-08-23 完了）: `src/lib/stripe/scheduled-cancellation.ts` ＋ Server Action `keepSubscriptionAction`
  ＋ `KeepSubscriptionButton`（課金タブとApp Shellバナーの両方）。**`cancel_at` と `cancel_at_period_end` は片方だけ送る**
  ——両方渡すとStripeが `Received both cancel_at_period_end and cancel_at parameters.` で400を返す（実測して修正）。
  実物のStripeテスト契約で「押す→Stripe側 cancel_at_period_end=false→画面から予定が消える」まで確認。

### T-M8-272: 解約前に引き止めクーポンを提示する `done`
- 参照: 要件03 §2.2 Portal / 依存: なし / サイズ: S
- 完了条件:
  - 「解約する」から入った Stripe の解約画面に、設定したクーポンが「特別オファー」として出る
  - クーポンIDは環境ごとに `STRIPE_RETENTION_COUPON_ID` で持ち、未設定なら提示しない（従来どおり）
  - doctor が「未設定（提示されない）」と「設定はあるが無効」を区別して知らせる
- メモ: 運営者の報告（2026-08-23）「解約する際に、Stripeで設定しているクーポンが提示されずに解約された」。
  原因は **`flow_data` で解約画面へ直接入ると、ダッシュボードの「顧客維持クーポン」設定は効かない**こと
  （`flow_data.subscription_cancel.retention` で明示したものだけが出る）。テストモードで
  「retentionあり＝『特別オファー 3ヶ月間50%割引』が出る／なし＝出ない」を実ブラウザで確認した。
- 実装メモ（2026-08-23 完了）: `portalFlowData` が `retention.coupon_offer` を付ける（未設定なら付けない）。
  `.env.example` に変数名を追加。**staging・本番は Vercel の環境変数に各環境のクーポンIDを設定するまで提示されない**
  （doctor が未設定を warn で知らせる）。

### T-M8-275: 上位プランへの変更は差額をその場で決済する `done`
- 参照: 要件03 §2.2 Portal / 依存: なし / サイズ: S
- 完了条件:
  - Portal設定が3環境とも `always_invoice`（その場で決済）で、doctor がずれを検出する
  - 課金タブの説明が「すぐに切り替わり、差額は日割りでその場でお支払い」になっている
- メモ: 運営者の確認（2026-08-23）「上位プランへ変更した時の請求はその場でされますか？」。調べると
  **本番・stagingはダッシュボードで `always_invoice` に変えられており、テスト環境だけ `create_prorations`**
  という不一致だった（アプリの説明は「次回請求へ合算」で本番の挙動と食い違っていた）。運営者の意図に合わせて
  **その場で決済へ統一**し、コード・doctorの期待値・画面文言をそちらへ揃えた。確認画面に出る
  「本日が期日の金額」はStripe組み込みで差し替えられない（了解のうえ採用）。

### T-M8-276: 招待のXシェア文をプロダクトの価値が伝わる文面へ `done`
- 参照: docs/cp/invite_cp.md §2① / 依存: なし / サイズ: S
- 完了条件:
  - シェア文が「学習・下書き作成・予約と分析」の価値を伝え、7日間無料と招待リンクを含む
  - Xの上限（280。日本語1文字=2・URLは23）に収まることをテストが固定する
  - 誇張・断定（「必ず」「保証」など）を含まないことをテストが固定する
- メモ: 運営者の指示（2026-08-23）。文面の正本は `src/lib/invite/share-text.ts`。
  「やめたこと」から入り、できることは3つだけ、効果の断定はしない（実測していない効果は書かない）。

### T-M8-277: 解約は確認（止まること/残ること）と理由アンケートを挟む `done`
- 参照: 要件03 §2.2・要件06 §1.1・要件05 / 依存: なし / サイズ: M
- 完了条件:
  - 「解約する」→ 確認（止まること／残ること）→ 理由アンケート（選択必須・記述任意）→ Stripeの解約画面
  - 回答が `cancellation_surveys` に残り、運営者が理由ごとに数えられる
  - アンケートの保存に失敗しても解約手続きは進める
- メモ: 運営者の指示（2026-08-23）。理由が分からないと何を直せば解約が減るか判断できない。
  引き止めクーポンの提示はStripe側の解約画面が担う（T-M8-272）ので、アプリ側は事実の提示と理由の収集に絞る。
- 実装メモ（2026-08-23 完了）: migration `20260823000011`（本人insertのみのRLS）、
  `cancellation-reasons.ts`（選択肢と「止まること／残ること」）、`cancellation-survey.ts`（保存と集計）、
  `recordCancellationSurveyAction`、`CancelSubscriptionButton`（2段のダイアログ）。
  実ブラウザで確認→アンケート→保存（`price`/proceeded=true/plan=premium）まで確認。E2E は plans.spec「T-M8-277」。


### T-M8-278: 無料トライアルの解約は即時終了にし、残りの期間で再開できるようにする `done`
- 参照: 要件03 §2.2・要件05・要件06 §1.1 / 依存: T-M8-277 / サイズ: M
- 完了条件:
  - トライアル中の「解約する」は**押した時点で終了**する（Stripeの解約画面へは送らない）
  - 期限内なら課金タブの「無料トライアルを再開」で**残りの日数のまま**トライアルが戻る
  - 期限切れなら従来どおり「プランを再開」（有料・無料期間なし）
- メモ: 運営者の指示（2026-08-23）。払っていない期間を「終了日まで使える」とするのは説明しにくい。
  Portalの解約は`mode=at_period_end`で有料契約向けなので、トライアルだけアプリ内で `subscriptions.cancel` する。
- 実装メモ（2026-08-23 完了）: `cancel-now.ts`（trialing のときだけ・`trial_ends_at` は消さない）＋
  `cancelTrialNowAction`＋解約ダイアログの分岐。再開は `resume.ts` が `trial_end` を引き継いで作り直す
  （Stripeは終了した契約を再開できないため新しい契約になる）。**実物のStripeで確認**:
  解約→`status=canceled`／再開→`trialing` で `trial_end` が元のまま。押す前の説明文も
  「押した時点で終了します（◯月◯日までなら残りの期間で再開できます）」へ変更。

### T-M8-279: 適用中の割引をプラン名の下に表示する `done`
- 参照: 要件02 profiles・要件03 §3 同期・要件06 §1.1 / 依存: T-M8-272 / サイズ: S
- 完了条件:
  - 引き止めクーポンなどの割引が「◯%割引適用中（◯月◯日まで 月額 ¥◯）」としてプラン名の下に出る
  - 金額は `PLANS` から計算し、画面へ書き写さない。割引が無ければ行ごと出さない
- メモ: 運営者の指示（2026-08-23）「半額クーポンを利用した後は、プラン名の下に半額になっている旨を」。
  契約者がいちばん知りたいのは「次にいくら払うのか」。
- 実装メモ（2026-08-23 完了）: migration `20260823000012`（profiles.discount_*）、webhook同期で
  `discounts`（expand）＋クーポンを1回引いて割引率・終了日を保存、`discount-label.ts` で表示文を作る。
  **実物のStripeで確認**（`check:stripe-period` に割引の同期テストを追加）。
  なお**テスト環境の半額クーポンは利用上限2回に達していて `valid=false`**（doctorが検出する）。


### T-M8-300: Stripe webhook の署名検証失敗でリクエスト本文がログへ出ていた `done`
- 参照: 要件01 §8 / 依存: なし / サイズ: S
- 完了条件: 署名検証に失敗しても、記録は残しつつリクエスト本文をログへ出さない
- メモ: `/security-audit` の初回実行で検出（2026-08-25）。Stripe SDK の
  `StripeSignatureVerificationError` は `payload`（本文全文）と `header` を**public プロパティ**に
  持ち、`recordUnexpectedError` の `console.error(error)` でオブジェクトごと出ていた。
  **監査エージェントが実際に実行して stderr に本文が出ることを再現**している。
  `STRIPE_WEBHOOK_SECRET` の不一致が続くと、届くたびに顧客のメール・氏名・請求先住所・
  カード下4桁が平文でログへ溜まる（この不一致状況はコード自身のコメントが想定している）。
- 実装メモ（2026-08-25）: 例外オブジェクトを渡さず**理由の文言だけ**を記録する。記録の目的
  （黙って400にしない）は変わらない。回帰テストは**実SDKの検証**を通す `webhook.test.ts` へ置き、
  修正を戻すと赤くなることを確認した。
  - 既存の `route.db.test.ts` は `constructEvent` を素の `Error` でスタブしていたため、
    `payload` を持つ実クラスが一度も通らず、この経路を守る検出器が無かった。

### T-M8-309: 解約確認の「止まること」からBYOKの但し書きを外す `done`
- 参照: 要件03 §解約は確認とアンケートを挟む / 依存: なし / サイズ: S
- 完了条件: 運営キー系プラン（premium/expert）の解約確認で、AI生成が止まることが1行で伝わる
- メモ: 旧文言「運営が用意しているAIキーでの生成が使えなくなります（ご自身のキーは別途必要です）」は、
  そのプランの利用者にとって**自分のキーの話が要らない**。「AIでの投稿生成や画像生成が使えなくなります」へ。
- 実装メモ: あわせて**テストの検出器が死んでいたのを直した**。既存の
  「BYOKプラン（standard）では運営キーの話を出さない」は `not.toContain("運営が用意しているAIキー")`
  だけを見ており、文言変更でその文字列が実装から消えたため**何にも当たらないまま緑**になっていた
  （T-M8-307 で正本へ書いたばかりの型）。運営キー系で**出ること**を同時に確かめる形にし、
  文言を変えると赤くなることを確認した。
- **記録の注記**: 起票と実装メモは、コミット前に残っていた差分を読んで作成したもの。
...
- 完了条件:
  - アプリが作る値をDBのCHECK制約が弾かないことを、実物の生成側とDBに実在する制約定義で機械的に検査する
  - 正規表現のCHECK制約が増えたら、登録しない限り検査が落ちる（分母が出る）
  - 「実行が全部失敗している利用者」が運営者アラートに **error** として出る
  - 差し替えた依存・スタブの扱いが正本に書かれている
- メモ: T-M8-306 の実装中に、**前日に入った T-M8-299 が本番で機能を止めていた**ことが分かった。
  原因は個別のミスではなく再発している型で、今セッションだけで4件あった——
  T-M8-197（`done` だが配線のコミットが漏れて誰も使っていない）、T-M8-287（存在しない保持期間を
  根拠に「安全」と書かれたコメント）、T-M8-300（スタブが実クラスより単純で検出器が死んでいた）、
  T-M8-299（キーを組み立てる検査と枠を書く検査が別々で、繋ぎ目を誰も通っていなかった）。
  共通点は「**テストが無い**」ではなく「**テストはあるが実物を通っていない**」。件数では防げない。
- 実装メモ（2026-08-25 完了）:
  - `src/lib/db/check-constraints.db.test.ts`（新規9件）: `pg_get_constraintdef` で**いま適用中の**
    制約定義を読み、その定義で一時テーブルを作って書いてみる。値は固定文字列ではなく
    **実物の生成側**から作る（`usagePeriodKeyExpr` / `createScheduleSlotSchema` / `validatePatternInput`）。
    生成側を緩めてDB側を直さなければ落ちる。制約を緩めた場合も「弾くべき値を弾く」側で落ちる。
    制約を T-M8-299 以前へ戻すと**実際に赤くなることを確認**（`"2026-08-25#1"` を名指しで報告する）。
  - `judgeRepeatedFailures` ＋ `collectFailurePatterns`（`diagnostics.ts`）: 直近24時間で
    **同じ理由の失敗が3件以上** → 注意、**成功0・失敗2件以上の利用者がいる** → 対応が必要。
    従来の `judgeJobs` は合計しか見ないため、1人が完全に使えない状態が他人の成功に紛れていた。
    経路は `scheduler-tick`（5分ごと）→ `deliverOperatorAlert` → 運営者メール。**実在を確認済み**。
    個人が特定できる値（メール・ID）は載せない（人数と文言だけ）。
  - テストは純粋関数（`diagnostics.test.ts` 5件）と**実DBを通すもの**（`diagnostics.db.test.ts` 6件）
    の両方を置いた。純粋関数だけにするのが今回の失敗そのものだったため。
  - CLAUDE.md の対応表へ「**DBへ書く値の形式を変えた**」行を追加（今回はここに該当行が無く、
    純粋関数の変更として扱われたことが素通りの直接原因）。
  - `development-and-testing.md` §11 へ3項目: 差し替えた依存は「呼ばれたこと」しか証明しない／
    スタブは実物と同じ形にする／安全性を主張するコメントには検査を隣に書く。
- **やらなかったこと**: 利用者向け日次サマリへの反映。あれは全利用者へ配る通知で、
  運営の内情を混ぜない方針（T-M8-164）のため運営者アラート側だけに載せた。

### T-M8-306: トライアル中のプラン往復で枠が復活する穴を閉じ、世代付きキーの書き込み不能も直す `done`
- 参照: 要件02 §3.13・要決定D-44（案A採用） / 依存: T-M8-299 / サイズ: M
- 完了条件:
  - トライアル中に上位へ戻したらリセットが巻き戻り、往復しても枠が増えない
  - 「下げたら0から数え直し」（T-M8-299の指示）はそのまま残る
  - 世代が付いたキーで利用枠を実際に書ける
- メモ: `/security-audit` が検出した往復の穴（D-44）を運営者が**案A**で決定（2026-08-25）。
  実装のため実DBで往復を再現したところ、**T-M8-299そのものが本番で壊れている**ことが分かった——
  `usage_events`/`usage_counters` の `month` 検査が `^\d{4}-\d{2}(-\d{2})?$` のままで、
  世代が付いた瞬間に check 制約で落ちる。つまり**トライアル中に下位へ切り替えた利用者は、
  以降の生成で利用枠を記録できず何もできなくなる**状態だった。
  **世代を進めた後に利用枠を書くテストが1本も無かった**ため誰にも踏まれずに通っていた
  （キーを作るだけのテストでは足りない）。
- 実装メモ（2026-08-25）:
  - `restoreUsageEpochSql`（`greatest(usage_epoch - 1, 0)`）＋ billing-return の上位変更分岐。
    世代は 0→1→0→1 と**同じ番号を行き来する**ので、戻った先には消費済みの行が残る。
    これが往復を閉じる（2周目は両プランとも使い切り済みに戻る）
  - migration 20260825000003 で `month` の検査へ `(#\d+)?` を追加
  - 実DBで往復6手順を再現し、閉じることを確認。回帰テスト `usage-epoch.db.test.ts` を追加し、
    制約を旧仕様へ戻すと赤くなることも確認した

### T-M8-304: 招待ランクの区切りを 1〜5 / 6〜10 / 11〜25 / 26〜50 / 51〜 にする `done`
- 参照: docs/cp/invite_cp.md §3 / 依存: T-M8-174 / サイズ: S
- 完了条件: 画面・正本・テストの区切りが一致する
- メモ: 運営者の指示（2026-08-25）。以前は 1〜4 / 5〜9 / 10〜24 / 25〜49 / 50〜 で、
  **表の見出しの人数（5人・10人…）と実際に率が上がる人数が1人ずれて**読めた。
- **進捗の分母は「ランクアップが成立する人数」**（5・10・25・50）にする（運営者の指示 2026-08-25 追記
  「5人招待が完了した時点でランクアップという扱いにしたい」）。`minPaidUsers` は
  「その率で報酬が出る最初の人数」（6人目）なので、分母にそのまま使うと「0 / 6人」になり
  数え方と食い違って見える。0人なら「あと5人の有料招待で 35% にアップ」「0 / 5人」。
  **必要人数を招待し終えたら目標は次の段へ進む**（運営者の指示 2026-08-25 再追記
  「5人ちょうどの時は 5 / 10人」）。5人で止めると「5 / 5人・あと0人」という、
  もう達成しているのに残っているような表示になる。判定は**次の紹介に適用される率**
  （`nextReferralRateBps`）を基準に、そこから上がる段を探す。
- 実装メモ（2026-08-25 完了）: `INVITE_TIERS.minPaidUsers` を 1/6/11/26/51 へ。
  `TierProgress.nextAtCount`（帯の終わり）を足し、進捗の分母・残数・バーはこちらを使う。
  **率は作成時にsnapshotされるので過去の報酬は書き換わらない**（変更時点で報酬0件・実データで確認）。
  画面のランク表は `INVITE_TIERS` から帯を組み立てるため自動で追随する（同じ式を単体テストで固定）。
  正本（invite_cp.md §3の表と図）も更新。**帯の両端**（5/6・10/11・25/26・50/51）を
  テストで見る——片側だけだと1人ずれても気付けない。
  あわせて `store.db.test.ts` の題名「5人目から25%」を直した（2026-08-22の率改定にも
  今回の区切り変更にも追随しておらず、二重に古かった）。


### T-M8-306: 特商法表記から所在地を外し、事業者の呼称を統一する `done`
- 参照: 要件06 §法務ページ / src/lib/legal-entity.ts / 依存: T-M8-72 / サイズ: S
- 完了条件:
  - 特商法表記に自宅住所が出ない
  - 省略が法的に成り立つ形になっている
  - 利用規約の事業者の呼称が統一されている
- メモ: 運営者の指示（2026-08-25）。個人事業者のため自宅住所が公開され続けていた。
  あわせて法務3文書の通読で「当社」（3箇所）と「当方」（20箇所）の混在を発見——
  事業者は個人事業主なので「当社」は不正確で、事業主体の認識に疑義が生じうる。
- **省略には条件がある**: 消費者庁「通信販売広告Q&A」Q17（2026-08-25 に消費者庁のページで確認）。
  請求により広告表示事項を記載した書面・電子メール等を**遅滞なく提供する旨を広告に表示し**、
  かつ実際に遅滞なく提供できる措置を講じている場合に限り、住所・電話番号を省略できる。
  **書くだけでは足りず、実際に応じられることが条件。** 問い合わせ窓口を止めるなら
  住所の表示へ戻す必要がある（`legal-entity.ts` のコメントにも残した）。
- 実装メモ（2026-08-25 完了）: `LEGAL_ENTITY.address` を「請求があった場合には遅滞なく
  開示します」へ。特商法ページ冒頭に**請求に応じる旨の一文**を追加（これが無いと省略が成り立たない）。
  利用規約の「当社」3箇所を「当方」へ統一。要件06からも住所を削除し、省略の条件を注記した。
  `legal-pages.test.ts` 65件が緑（法定事項の存在検査は通ったまま）。
- **やらなかったこと**: バーチャルオフィスの住所表示（Q18）。「現に活動している住所」であることと
  運営事業者との合意が要るため、契約が前提になる。必要になったら `legal-entity.ts` を差し替えるだけ。


### T-M8-305: 招待された利用者の契約が動く筋を網羅的に検証する `done`
- 参照: docs/cp/invite_cp.md §5〜§7 / 依存: T-M8-300 / サイズ: M
- 完了条件: プラン変更・解約・復活・大量招待・月の途中のランク変更が検証されている
- メモ: 運営者の指示（2026-08-25）。招待は「1回紹介して終わり」ではなく、
  **紹介した人が払い続けるあいだ報酬が発生し続ける**ので、被招待者の契約が動くたびに
  金額が変わる。どれも**金額が静かに間違う**型で、画面には正しく見えてしまう。
- 実装メモ（2026-08-25 完了）: `src/lib/affiliate/lifecycle.db.test.ts`（新規12件）。
  `scenarios.db.test.ts` が招待者側（停止・締め・振込）を見るのに対し、こちらは
  **被招待者側の契約が動く筋**を担当する。
  - プラン変更: 上位/下位で報酬額だけが動き率は変わらない・**日割りの差額請求も報酬対象**
    （`always_invoice` の請求も `invoice.paid` として届く。「月額しか対象にならない」と
    誤解しやすい）・6ヶ月の期間はプラン変更で動かない
  - 解約: 以後は対象外／**解約前の支払いが遅れて届いたら対象**（日付で判定）
  - 復活: **再契約しても対象外**（運営者の確認どおり）。3か月ぶん試して報酬が増えないこと
  - 未課金の解約: 終了扱いにせず、後から契約したらそこから6ヶ月が始まる
  - 大量招待: 60人で率が 30→35→40→45→50 と段階的に上がる（帯の境目を全部見る）
  - 月の途中のランク変更: 同月に2つの率が混ざったまま締められ、合計が正しい
  - 返金で有料招待が減ると**その後の率も下がる**（返金された分で上がった率が居座らない）
  - **同じ人が何回払っても有料招待は1人**（率が件数で水増しされない）
  - 6ヶ月の終端ちょうどは対象・以降は契約が続いていても止まる
  - **招待者自身が解約しても報酬は受け取れる**（判定に招待者の契約状態は入らない）
- 結果: **不具合は見つからなかった**（12件すべて期待どおり）。3回連続で緑を確認。


### T-M8-303: DB接続の上限を既定で安全側にし、doctorに効いている値を出す `done`
- 参照: 要決定D-43 / 要件01 §9 / 依存: T-M8-197, T-M8-198 / サイズ: S
- 完了条件: 環境変数を設定し忘れても上限が3になり、doctorでいまの値が読める
- メモ: 運営者の質問（2026-08-25）「以前の改修で最大接続数を3にして改善したのでは？」から。
  **コードの既定は10のままで、3にするのはデプロイ先の環境変数（運営者作業）**だった。
  設定を忘れた環境だけが pooler の上限を食い潰すが、症状は「たまに遅い」なので気付けない。
  さらに doctor は待ちの回数しか出しておらず、**「対策が効いていないのか、対策はしたが
  足りないのか」を運営者が区別できなかった**（原則2）。
- 実装メモ（2026-08-25 完了）: `poolMax()` を作り既定を3へ（`DEFAULT_DB_POOL_MAX`）。
  不正な値（0・負・非数）も既定へ倒す。doctorの「DB接続の混み具合」は正常時も警告時も
  「1インスタンスあたり上限 N」を添える。上限が分からないときは数字を作らない。


### T-M8-302: 招待者の停止・報酬の保留を1コマンドでできるようにする `done`
- 参照: 要決定D-40（案A採用） / 要件02 §3.24・§3.26 / 依存: T-M8-174 / サイズ: M
- 完了条件:
  - 運営者がDBを直接触らずに招待者を停止・解除できる
  - 報酬を1件ずつ保留・解除できる
  - 停止と「コードの打ち間違い」が区別される
- メモ: 運営者の判断（2026-08-25・案A）。`held`/`suspended` は定義だけあって
  **書き込む経路が実装に無く**、不正が疑われてもDBを直接触るしか止める手段が無かった。
- 実装メモ（2026-08-25 完了）: `npm run affiliate:moderate`（一覧／`--suspend`／`--activate`／
  `--hold`／`--release`）。**停止は「これ以上増やさない」であって取り上げではない**ので、
  既存の報酬は残す（1件ずつ止めるなら `--hold`）。保留の解除は**確認期間の経過で行き先を決める**
  （`available_at` を過ぎていれば payable・まだなら pending。一律 payable にすると
  確認期間中の報酬を先に払えてしまう）。
  あわせて `attributeSignup` の戻り値に `suspended` を足した——`unknown_code` と混ぜると
  運営者が1人止めるたびに登録側のSentryへ「不明なコード」が上がり、本物の取りこぼしが埋もれる。
  保留中の報酬が振込に束ねられないこと・解除の行き先をDBテストで固定。


### T-M8-301: 振込の記録が金額を突き合わせておらず、台帳が実際とずれ得た `done`
- 参照: docs/operations/affiliate-payouts.md / 要件03 §2.4 / 依存: T-M8-174 / サイズ: S
- 完了条件: `--paid` は、振込予定の額といま束ねている報酬の合計が一致するときだけ記録する
- メモ: T-M8-300 の網羅テスト中に発見（2026-08-25）。**正本には「支払記録の直前にも突き合わせる
  （過払い防止）」と書いてあったが、実装されていなかった。** `scripts/affiliate-payouts.mjs` は
  生SQLで `status='paid'` にするだけで、突き合わせる `markPayoutPaid`（`recalcCreatedPayout` を呼ぶ）は
  **テストからしか呼ばれておらず本番では死んでいた**。②（--show）で額を見て振り込んでから
  ④（--paid）までの間に返金が届くと、束ねた報酬は減るのに振込予定の額は古いまま `paid` になり、
  実際に振り込んだ額と台帳が食い違う。
- 実装メモ（2026-08-25 完了）: スクリプトが記録の直前に
  「振込予定の額 = 束ねている payable の合計」を確認し、**食い違ったら記録せず終了コード1で止める**
  （いくつからいくつへ変わったかを表示する）。**黙って直さない**——直すと運営者が実際に振り込んだ額と
  台帳の食い違いに気付けないため（原則3）。手数料を下回る場合も止める。
  実DBで「振込予定10,000／束ね6,000」を作り、中止されることを確認。


### T-M8-300: 招待機能をアカウント状況の組み合わせで網羅的に検証する `done`
- 参照: docs/cp/invite_cp.md / 要件02 §3.24〜3.28 / 依存: T-M8-174 / サイズ: L
- 完了条件:
  - 招待者・被招待者・契約状態を組み合わせた検証がある
  - 見つかった不具合を直す
- メモ: 運営者の指示（2026-08-25）「複数アカウントを用いて、様々なアカウント状況を仮定して、
  網羅的かつ完全にテストしてください」。
- 実装メモ（2026-08-25 完了）: 既存は1本の道すじ（帰属→報酬→締め）中心だったので、
  **境界と組み合わせ**を足した。
  - `src/lib/affiliate/scenarios.db.test.ts`（新規15件）: 停止された招待者・大文字コード・
    1人が複数招待・多段招待で孫の課金は親に入らない・停止後も既存報酬は残る・
    6ヶ月の終端ちょうど/1秒後・解約後の再契約で再開しない・退会者を率に数えない・
    率のsnapshot不変・最低振込額ちょうど/1円不足・無効な口座・同月2回実行の冪等・
    確認期間中は束ねない・束ねた後の全額返金で振込が取り消される・複数招待者の金額が混ざらない
  - `src/lib/db/rls.db.test.ts`: **招待5表の行分離が未検証だった**ので追加。
    他人の銀行口座（銀行名・名義・末尾4桁）・報酬・振込が1行も見えないことを固定
  - `e2e/invite.spec.ts`: 契約状態5種（未登録/トライアル/契約中/解約済み/支払い滞り）すべてで
    招待画面が開きリンクが出ること、ロック画面へ倒れないこと。自己招待でCookieは付くが帰属しないこと
  - `src/app/actions/actions.db.test.ts`: 振込先口座の入力検証（桁数・文字種・制御文字・空欄・
    長さ・不明な口座種別・登録し直し）。**happy pathしか通っていなかった**
- **見つけた不具合**: T-M8-301（振込記録の突き合わせが未実装）。
- **直していない指摘**（別途要判断・下の「要決定」へ）: `held`/`suspended` はドキュメントにある
  状態だが**書き込む経路が無い**。招待画面の見出しが「お支払い額の◯%が報酬になります」と
  現在の率で書くが、5人目の紹介は1段上の率になるため実際と食い違う。


### T-M8-299: トライアル中の下位プラン変更を即時にし、利用枠もリセットする `done`
- 参照: 要件03 §2.2 / 要件02 §3.1 / 依存: T-M8-260, T-M8-258 / サイズ: M
- 完了条件:
  - トライアル中に下位プランへ切り替えると、その場でプランが変わる
  - そのとき利用枠が0から数え直しになる
  - 有料契約の期間末予約は従来どおり
- メモ: 運営者の指示（2026-08-25）。期間末予約は「払った期間ぶんは使えるべき」が理由なので、
  **1円も払っていないトライアルには当てはまらない**（下げたのに枠が上位のまま待たされる）。
- 実装メモ（2026-08-25 完了）: Portalの `schedule_at_period_end.conditions` は契約の状態で
  分岐できない（金額と請求間隔しか条件に取れない）ため、**戻ってきたときにアプリ側で畳む**。
  `applyTrialDowngradeNow` が予約を `release` してから `proration_behavior=none` で価格を差し替える
  （**解除が先**——予約が生きたまま価格を変えても期間末に戻される）。
  **利用枠のリセットは `profiles.usage_epoch`** で行う（migration `20260825000002`）。
  Stripe はトライアル中の価格変更で `current_period_start` を動かさない（2026-08-25 実測）ので
  日付だけでは同日のリセットを区切れない。世代を1つ進めると期間キーが `YYYY-MM-DD#世代` になり、
  その時点から0で数え直しになる（`usage_events` は消さないので原価の台帳は残る。0のあいだは
  キーの形が従来どおりなので既存利用者の枠は動かない）。
  **リセットの判定は変更前後のプランの比較**にした——予約を畳んだ経路だけを見ると、
  Portalが予約を作らずその場で下げる場合を取りこぼすため。上位への変更では戻さない。
- **実測でわかったこと**: トライアル中の価格変更は即時に効き、日割りも請求も発生しない（¥0）。
  ただし `current_period_start` は動かない。また、`from_subscription` で作った schedule を
  release すると**トライアルが打ち切られる**ことがあるため、検証用の手製scheduleと
  Portalが作る本物のscheduleを同一視しないこと（下の「仮説」参照）。


### T-M8-298: 解約後に残るトライアルを、どのプランでも・カード再入力なしで使えるようにする `done`
- 参照: 要件03 §2.2 / 要件06 §1.1 / src/lib/billing/remaining-trial.ts / 依存: T-M8-278 / サイズ: M
- 完了条件:
  - 期限内なら**どのプランでも**残りの期間が無料で始められる
  - 無料であることが `/plans` と解約バナーの文言から読める
  - カードが保存済みならStripeの画面を挟まない
- メモ: 運営者の報告（2026-08-25）。トライアル中に解約したあと「プランを選択」を押したら
  3プランが出て、選ぶとStripeでカード入力を求められ**即座に ¥3,980 が課金された**
  （実アカウント km.crypto1013 のイベントで確認: 16:01:58 に charge.succeeded、
  作られた契約は `trial_end: なし`）。原因は Checkout が `trial_used_at` だけを見ており、
  **`trial_ends_at` が未来でも無料期間を付けなかった**こと。
- 実装メモ（2026-08-25 完了）: 判定を `lib/billing/remaining-trial.ts` へ集約
  （`/plans`・設定＞課金・Checkout・再開APIが違う答えを出すと「無料と書いてあるのに請求された」
  が起きるため）。Checkout は `trial_ends_at` が未来なら `subscription_data.trial_end` で引き継ぐ。
  `/plans` のCTAは**カードが保存済みならStripeを挟まず** `POST /api/stripe/resume` に `{plan}` を
  渡してその場で作り、**402（カード未保存）のときだけ** Checkout へ倒す
  （他の失敗で倒すと「無料のはず」が有料の申し込みに化ける）。
  文言は上部のピル・解約バナー・各プランのCTAと補足の4か所を「◯月◯日まで、どのプランでも無料」
  「その後は月額 ¥◯」に統一。**実物のStripeで1周確認**——トライアル→解約→
  **別プラン（エキスパート）で再開**して `trialing`・`trial_end` は元のまま・請求額 ¥0。


### T-M8-297: 「無料トライアルを再開」が失敗し、何回か押すと通る `done`
- 参照: src/lib/stripe/resume.ts / 依存: T-M8-264, T-M8-278 / サイズ: S
- 完了条件: 解約直後に押しても1回で再開できる
- メモ: 運営者の報告（2026-08-25・km.crypto1013）。原因は**冪等キーを時間（10分バケット）で
  区切っていた**こと。Stripeの冪等リプレイは24時間有効なので、同じバケット内で
  「再開 → もう一度解約 → また再開」をすると、2回目の `subscriptions.create` が
  **1回目の応答（＝いま解約したばかりの契約）をそのまま返す**。取り直すと `canceled` なので
  `LIVE_SUBSCRIPTION_STATUSES` に入らず「再開を確定できませんでした」になり、
  **次のバケットに入るまで（最大10分）直らなかった**。実物のStripeで再現した。
- 実装メモ（2026-08-25 完了）: 鍵を**置き換える契約のID**（`subscriptions.list` の先頭）と
  プランで作る。同じ意図の二度押しは対象が変わらないのでまとまり、「解約してからもう一度」は
  対象が変わるので別の作成になる。置き換える契約が1本も無いときだけ従来どおり時間で区切る。
  実物で「解約→即再開」が `trialing` になることを確認。


### T-M8-296: 日割り差額の案内が、即時決済にしたことでどこにも出なくなっていた `done`
- 参照: 要件03 §2.2 / src/lib/stripe/proration-preview.ts / 依存: T-M8-270, T-M8-275 / サイズ: S
- 完了条件: 上位プランへ変更してPortalから戻ると、その場で払った差額が画面に出る
- メモ: 運営者の指摘（2026-08-25）「即時で支払いが発生する設定なので、以前のプラン変更の金額は
  次回に含まれないのでは？」から判明。**指摘が正しかった。**
  T-M8-270 の案内は**次回請求の下見（未請求の日割り行）**だけを見ていたが、T-M8-275 で
  `always_invoice` にしたことで差額は**即時に請求・決済される**ようになり、下見には1行も出なくなった。
  つまり「差額はいくらか」の説明が**どの経路でも出ない**状態が続いていた。
  テストクロックで実測（2026-08-25）: プレミアム→エキスパートで `subscription_update` の請求書が
  即時 paid（¥7,330）になり、次回請求プレビューは月額1行（¥14,800）だけ・日割り行なし。
- **運営者の契約に残っていた ¥10,754 について**: プラン変更は 2026-08-23 11:29、Portal設定を
  `always_invoice` にしたのは同日 13:42。**変更のほうが2時間早く**、当時は既定の
  `create_prorations` だったため未請求のまま次回請求へ持ち越されていた。設定後の変更では起きない。
- 実装メモ（2026-08-25 完了）: `loadRecentProrationCharge` を追加し、**直近15分の
  `subscription_update` の請求書**（支払い済み）に限って「ただいまお支払いいただきました」と出す。
  未請求のまま残っている差額は従来の文言で出し分ける（古い差額を「いま払った」と書かない）。
  単体テストで窓の外・月次更新・未払い・取得失敗の4通りを固定。


### T-M8-295: プラン未登録では設定画面ごと「先にプランを登録してください」にする `done`
- 参照: 要件03 §2.1 ロック / 要件06 §SC-11 / 依存: T-M8-269, T-M8-273 / サイズ: S
- 完了条件:
  - プラン未登録では設定画面のタブも出さず、ロック画面を1枚だけ出す
  - 解約済み・支払い滞りは従来どおり課金・プランへ辿り着ける
- メモ: 運営者の指示（2026-08-25）。一度も契約していない人にはタブだけが並び、
  どれを押しても同じ断り文句を読むだけの画面になっていた。
- 実装メモ（2026-08-25 完了）: `lock && !plan` のときに `AppLockedPage` を返す。
  **解約済み・支払い滞りは含めない**——課金・プランのタブに「プランを再開」（T-M8-264）と
  お支払い情報の更新（T-M8-273）があり、塞ぐと戻る手段ごと消えるため、判定は
  「ロックされている」かつ「`plan` が無い」の両方にした。E2Eを新仕様へ更新
  （general/billing/api-keys の3タブすべてでロック画面が出る・タブのリンクが出ない）。
  解約済みのロックを守るE2E（plans.spec.ts）が緑のままであることも確認。


### T-M8-294: 解約理由を複数選べるようにする `done`
- 参照: 要件02 §3.20 / 要件03 §2.2 / 要件05 / docs/operations/monitoring.md §4 / 依存: T-M8-277 / サイズ: M
- 完了条件:
  - 解約アンケートで理由を複数選べる（1つ以上）
  - 既存の回答が消えず、集計が続けて読める
  - 運営者がSQLで集計できることが手順として残っている
- メモ: 運営者の指示（2026-08-25）。理由は1つに絞れないことが多く（「料金が高い」かつ
  「あまり使わなかった」など）、1つしか選べないと集計が**回答者が仕方なく選んだ1つ**に寄り、
  何を直せば解約が減るのかの判断を誤らせる。
- 実装メモ（2026-08-25 完了）: migration `20260825000001` で `reason text` → `reasons text[]`
  （既存5件は1要素の配列へ移し、消していない）。1〜8要素のcheckとGIN索引を付けた。
  保存側は重複を落とし定数の順に正規化、0件は `validation_error`。集計は `unnest(reasons)` で
  数える（**合計は回答数より多くなる**ことを関数コメントと運用手順に明記）。
  画面はラジオ→チェックボックス、「あてはまるものをすべて選んでください」。
  E2Eで2つ選んで両方がDBに残ることを固定。運営者向けのSQLを
  `docs/operations/monitoring.md` §4 に追加（理由ごとの件数・自由記述・留まった人の理由）。


### T-M8-293: 引き止めクーポンを受け取っても「半額適用中」が出ない `done`
- 参照: 要件03 §2.2 同期 / src/lib/stripe/billing-return.ts / 依存: T-M8-279, T-M8-260 / サイズ: S
- 完了条件: Portalの解約導線で半額クーポンを受け取って戻ると、プラン名の下に割引が出る
- メモ: 運営者の報告（2026-08-25）。エキスパートで解約しかけ、半額クーポンで留まったのに
  「設定＞課金・プラン」に半額の表示が出ない。**T-M8-279で実装済みの表示が一度も出ていなかった。**
  原因は `billing-return.ts`——Portalからの戻りは schedule は読むのに **discount を読んでいなかった**。
  Portalの `after_completion` は即リダイレクトなので**戻りが本物のwebhookより先に着き**、
  そこで `discount` を null で書き、`created` を現在時刻にするため後続のwebhookも stale になる。
  つまり**割引が入る瞬間に必ず消える**構造で、webhookを待っても出ない。
  schedule には同じ罠のコメントが書かれていたが、後から足した discount（T-M8-279）が取りこぼされていた。
- **採番のやり直し**: 起票時に T-M8-286 を使ったが、並行セッションの性能改善が同じ番号を先に使っていた（`profiles` の重複読み取り）。**コミットメッセージは `fix(T-M8-286)` のまま**なので、履歴を辿るときはこの起票と対応させる。
- 実装メモ（2026-08-25 完了）: 戻りでも `loadDiscount` を呼び、`subscriptions.retrieve` に
  `expand: ["discounts"]` を付ける。**割引率は `discounts[0].source.coupon`（IDの文字列）**から
  クーポンを1回引いて得る（`discount.coupon` は返らない。2026-08-24 に実データで確認）。
  単体テストで「戻りの同期で割引が消えない」ことと retrieve の呼び方を固定。
  **実物で1周**して運営者の実アカウントに `discount_percent_off=50` が入ることを確認した。
  `resume.ts` は契約を作り直す経路なので null が正しい旨をコメントで明示（同じ取りこぼしの再発防止）。


### T-M8-285: 解約確認の「閲覧できます」が実際の挙動と違う `done`
- 参照: 要件03 §2.2 解約導線 / 要件06 §SC-11 / 依存: T-M8-277, T-M8-269 / サイズ: S
- 完了条件:
  - 解約確認の文言が実際の挙動と一致する
  - トライアル中の解約確認が実ブラウザで検査されている
- メモ: 運営者の指摘（2026-08-24）。「これまでの下書き・投稿履歴・分析結果は残ります（閲覧できます）」の
  **「閲覧できます」が事実と違う**。データは消えないが、解約後は課金・プラン以外のタブが
  ロックされる（T-M8-269）ので開いて見ることはできない。解約してから食い違いに気付くことになる。
- 実装メモ（2026-08-24 完了）: 「これまでの下書き・投稿履歴・分析結果は消えません
  （解約中は開けませんが、再開すればそのまま使えます）」へ変更し、消えないことと見るには
  再開が要ることを1行で言い切る。単体テストに「閲覧できます」と書かないことの検査を追加。
  **トライアル中の解約確認のE2Eが無かった**ので新設（運営者が指摘した画面そのもの）——
  その場で終了する旨・繰り越せない旨・データの扱い・残り期間で再開できる旨・
  ボタンが「今すぐ解約する」であることを実ブラウザで固定。要件03・要件06へ反映。


### T-M8-284: LP・APIキー画面の文言改定に合わせて検査と正本をそろえる `done`
- 参照: 要件06 §1 LP / §SC-11 APIキー / 依存: なし / サイズ: M
- 完了条件:
  - 運営者が書き直した文言のまま、単体・E2E・docsが一致している
  - **LPから外した内容の行き先が正本に書かれている**（どこにも無い状態にしない）
- メモ: 運営者がLPのFAQ・APIキー画面・プロンプト集の文言を書き直した（2026-08-24）。
  その結果、意図的に置かれていた次の5点が画面から消えた。**判断は運営者に確認済み**——
  LPの3点は「戻さない」、APIキー画面の2点は「消したまま」。
  1. 自動投稿の停止で実行待ちもキャンセルされること（LP FAQ）→ 利用規約 第8条に記載あり
  2. APIキーの暗号化・末尾4桁のみ表示・削除可（LP FAQ）→ プライバシーポリシーに記載あり
  3. 生成物を投稿前に確認する旨（LP FAQ）→ 利用規約 第7条に記載あり
  4. Client Secret を空のまま保存すると連携時に拒否される注記（設定＞APIキー・T-M8-63で追加）
     → **代わりの記載は無い**。空で保存でき、失敗するのは連携を試みたとき（401）になる
  5. X APIの費用目安の表（設定＞APIキー）→ X公式の料金ページへのリンクのみ残る
- **検査が黙って通っていた**: `landing-page.test.ts` の「安全性の説明がFAQに残っている」は
  **faq.tsx のファイル全文**に `toMatch` していたため、回答から文が消えても**冒頭コメントに
  同じ語が残っていれば緑**だった（「暗号化」「末尾4桁」「キャンセル」が実際にそうなった）。
  守っているつもりで守れていない状態を、単体テストが緑のまま通していた。
- 実装メモ（2026-08-24 完了）: 検査対象を**回答の文字列リテラルだけ**に変え（`FAQ_ANSWERS`）、
  LPで守る範囲を「既定は下書きまで」「自動投稿には別途同意が要る」の2点へ明示的に縮めた
  （緑にするためではなく、外すと決めたので検査も外した——理由と行き先はこの起票と要件06に記録）。
  E2Eは文言依存をやめ、FAQの回答が折りたたまれず読めることを構造で見る
  （`dt` → 直後の `dd` が可視・非空）。`x-oauth.spec.ts` から注記の検査を外し、
  `api-key-settings.tsx` のコメントアウト済み `<dl>` を削除して、残ったコメントを実態へ合わせた。
  要件06を v1.152 へ更新（LPから外した3点の行き先の表、Secret注記を出さないこと、費用目安の表を出さないこと）。
- **残る懸念（運営者了承済み・戻すならいつでも可能）**: 上の4は、Secretを空で保存できるのに
  理由の説明が画面のどこにも無い経路を作る（T-M8-63 が塞いだもの）。手順ガイドがSecretの
  取得と保存を指示することで補っている。


### T-M8-283: 解約済みでトライアルが残る場合のE2Eが無く、既存specが古い文言を固定していた `done`
- 参照: e2e/billing-resume.spec.ts・T-M8-278 / 依存: T-M8-278 / サイズ: S
- 完了条件:
  - 「解約済み＋トライアル残あり」で「無料トライアルを再開」と残り期限が出ることをE2Eで固定
  - 既存の「プランを再開」specがトライアル切れの状態を明示して通る
- メモ: T-M8-278（トライアルの即時解約と残り期間での再開）の後、`billing-resume.spec.ts` が失敗していた。
  **製品の不具合ではなくspecの取り違え**——fixtureの既定が `trial_ends_at = now() + 7 days` で、
  specは `subscription_status` だけを canceled にしていたため「トライアルが残る解約済み」になり、
  ボタンが正しく「無料トライアルを再開」へ変わっていた。specは変更前の文言を固定していた。
- 実装メモ（2026-08-24 完了）: 既存specに `trial_ends_at = now() - interval '1 day'` を足して
  「トライアルを使い切った解約済み」であることを明示し、**残っている場合のspecを新設**
  （「無料トライアルを再開」が出る・「プランを再開」は出ない・残り期限の説明文が読める）。
  取り違えると無料のつもりで課金される／使える残り期間を捨てることになるため、両方を固定する。


### T-M8-282: CSPの実ブラウザ検査が「起動できなかった」を「違反があった」と報せていた `done`
- 参照: scripts/check-csp-runtime.mjs・原則2（原因が開発知識なしで辿れる） / 依存: T-M8-170 / サイズ: S
- 完了条件: ブラウザを起動できなかった場合、CSP違反ではないと分かる文言と対処が出る
- メモ: `release:check` をsandbox内で実行したところ Chromium が起動できず
  （`bootstrap_check_in ... Permission denied`）、それを「❌ CSP違反が出ました」と報せていた。
  運営者は**存在しないCSPの不具合を探すことになる**。検査の失敗と対象の不具合を混同しない。
- 実装メモ（2026-08-24 完了）: playwrightの出力を流しつつ手元にも残し、`browserType.launch` /
  `Executable doesn't exist` / mach port 系の起動失敗を判定して文言を分ける
  （「検査できませんでした（ブラウザを起動できません）。CSPの違反があったわけではありません。」＋
  `npx playwright install chromium` とsandboxの案内）。両方の経路を実行して確認。
...
- 完了条件:
  - クーポンを受け取り済みの契約者でも「解約する」で解約画面が開く（提示なしで開き直す）
  - それ以外のStripe障害は従来どおり `provider_error` のまま（握り潰さない）
- メモ: 運営者の質問「半額クーポンはユーザーごとに何回まで？」を確かめる過程で発見（2026-08-24）。
  **Portalの提示は顧客ごとに1回だけ**で、2回目は**割引が切れた後でも**セッション作成が400になる
  （`The coupon ... is still usable by the customer.`）。`retention` を必ず付けていたため、
  受け取り済みの人は解約画面へ進めなくなっていた（テストクロックで実測）。
- 実装メモ（2026-08-24 完了）: `portal.ts` で、提示つきの作成が「このクーポンは適用できない」型で
  失敗したときだけ**提示なしで作り直す**。単体テストで「2回呼ばれ、2回目に retention が無い」ことと
  「他の失敗は502のまま」を固定。クーポン自体の上限は無し（API直付けは何回でも可能・実測）。
...
- 完了条件: 内部コメントと要件03が「切り替えは即時・差額は日割りで次回更新日の請求書に合算（その場で決済しない）」と読める
- メモ: 運営者の質問（2026-08-23）への検証で判明。`create_prorations` は日割り行を作るだけで即時請求しない
  （Stripe: "the prorations are created but not automatically invoiced"。テストモードで実測）。
  利用者向け文言は元から正しかったが、コメントと正本が「即時に日割り請求」と読め、運営者が
  「いつ引き落とされるか」を正本から答えられなかった。即時決済にするなら `always_invoice`（運営判断・未変更）。

### T-M8-254: 日付・時刻をすべて日本時間（JST）基準へ揃える `done`
- 参照: 運営者の指示（2026-08-23）・要件02 §13 / 依存: なし / サイズ: M
- 完了条件:
  - 「今月かかった費用」（利用者サマリ・doctor・運営者アラート）の月境界がJSTになる
  - 画面の日時表示に環境TZ依存の箇所が無くなる（`Intl` の直接呼び出しは `Asia/Tokyo` 固定）
  - 招待報酬の履歴の日付がJSTで切られる
  - テストがJSTで走る（手元とCIで結果が変わらない）
- メモ: JST監査（2026-08-23・75エージェント）の確定分。実測の内訳:
  - **費用の月境界がUTC**（`date_trunc('month', now())`）→ **毎月1日のJST 0〜9時は前月分の合計が「今月」として出る**。
    利用枠のリセットは月初なので、費用だけUTCだと説明が食い違う
  - **APIキー設定の「最終確認」だけ `timeZone` 未指定**（`Intl` を使う9箇所で唯一）。サーバー（UTC）と
    ブラウザ（JST）で描画が食い違い、hydration不一致で描き直しが起きていた
  - **ニュース日別集計が `+ interval '9 hours'`**（DBセッションのTZがUTCである前提に暗黙依存）→ `at time zone` へ
  - **vitest がTZ未固定** → `env: { TZ: "Asia/Tokyo" }`。手元だけ通る／CIだけ落ちるテストを書けなくする
...
- 完了条件:
  - 「スケジュールをすべて停止」と「すべて再開」が2つとも出て、「スケジュールを追加」の下に並ぶ
  - 対象が無い側は押せない（動いている枠が0なら停止、止まっている枠が0なら再開）
  - **個別に止めた枠も「すべて再開」で動き、個別に再開した枠も「すべて停止」で止まる**
  - SC-11（設定＞Xアカウント）にも同じ2つが出る
- メモ: 運営者の指示（2026-08-23）。直前（T-M8-233）は「停止操作で止めた枠だけ」を再開する形だったが、
  **「すべて」は文字どおり全部**にする——押した結果が「全部止まる／全部動く」で一致している方が
  状態を言い当てられる。片方だけ出す形（押せるボタンが入れ替わる）もやめた。
- 実装メモ（2026-08-23）: 由来を覚える必要が無くなったので `schedule_slots.paused_by_stop_all_at` を削除
  （migration 20260823000005）。件数は**画面が持っている枠から数える**（サーバから別の数を渡すと、
  行の停止/再開で画面だけ変わってボタンの状態が食い違う）。E2Eは `expect.poll` でServer Actionの反映を待つ。

### T-M8-248: 投稿作成のパターン「追加」が送信中も押せて、同じ型が何本も作られる `done`
- 参照: 要件06 §7 / 依存: T-M8-203 / サイズ: S
- 完了条件: 追加ボタンが送信中は無効化され、失敗表示は押すたびに消えてから出る
- メモ: 監査（2026-08-23）で検出。`void (async () => …)()` で走らせていたため `pending` に乗らなかった
  （予約画面側は既に `startTransition` で揃っていた）。

### T-M8-249: 上限値・実行可否・冪等キーの二重定義を1か所へ寄せる `done`
- 参照: CLAUDE.md（同じ数字を2か所に置かない） / 依存: なし / サイズ: S
- 完了条件:
  - 加重文字数のモジュールが1本になる（`src/lib/text/weighted-length.ts` を廃止し `post/text-metrics.ts` へ）
  - 下書き編集画面が280/25,000をローカル定数で再定義しない
  - 「契約が有効」の判定が `EXECUTABLE_SUBSCRIPTION_STATUSES`（`SUBSCRIPTION_ACCESS` から導出）を通る
  - 朝の提案ジョブの冪等キーがSQLの手組みではなく前置き定数を渡す形になる
- メモ: 監査（2026-08-23）で検出。どれも「片方だけ直して食い違う」形だった。
  加重文字数は**サーバ検証と画面で別モジュールを見ていた**（先頭2行のコメントまで同一の重複）。

### T-M8-250: X APIの空・壊れた応答を成功として通していた（`{}` を捏造） `done`
- 参照: 要件04 §5 / 依存: なし / サイズ: S
- 完了条件: 200でも本文が空／JSONが壊れていれば `empty_body`／`malformed_body` で失敗し、再送しない
- メモ: 監査（2026-08-23）で検出。空本文を `{} as T` で返していたため、呼び出し側は
  「成功したが投稿IDが無い」応答を受け取り、原因（通信断か本文の破損か）も区別できなかった。

### T-M8-246: 署名URLの失敗の握りつぶしと、通知の保持期間の欠落 `done`
- 参照: 要件02 §3.15・要件04 §14・CLAUDE.md 原則1 / 依存: なし / サイズ: S
- 完了条件:
  - 署名URLの生成失敗が記録され、1リクエストにまとまる（`createSignedUrls`）
  - news以外の通知にも40日の保持期間が効く（既読を先に、未読は後に消す）
- メモ: 監査（2026-08-23）で検出。署名URLは失敗すると**画面から画像が黙って消える**のに記録が無く、
  「画像が出ない」の原因を運営者が辿れなかった。cleanup は `type='news'` だけを消しており、
  毎日1通の `summary` が**永久に積もる**（利用者数に比例して無料枠のDB容量を食う）。

### T-M8-247: 毎朝の運営者アラートがStripe検査を1つも実行せず、事実と違う警告を送っていた `done`
- 参照: docs/operations/monitoring.md・CLAUDE.md 原則2 / 依存: T-M8-238 / サイズ: S
- 完了条件:
  - 運営者アラートと doctor が**同じ入口**（`buildStripeProbeDeps`）でStripe検査を行う
  - ローカルの「webhook受け口が無い」は error ではなく warn（`stripe listen` の起動案内つき）
- メモ: 監査（2026-08-23）で検出。アラート側は portal/prices/stripeAccount を渡しておらず、
  実際には設定済みなのに毎日「未設定です」と送っていた。**事実と違う警告は読まれなくなり、
  本当の異常まで埋もれる。** 入口を1つにして、片方だけ足し忘れる形をなくした。
- 実装メモ（2026-08-23）: あわせてローカルのPortal設定も `stripe:portal:setup -- --target local` で
  `continue_trial` へ修正（これが 2026-08-22 の ¥14,800 即時課金の原因だった）。

### T-M8-243: 無料トライアルの終了予告と、トライアル中のプラン変更説明の矛盾 `done`
- 参照: 要件03 §2.1/§4・要件06 §1.1 / 依存: なし / サイズ: M
- 完了条件:
  - トライアル終了3日前に「◯月◯日に無料期間が終了し、¥X の初回お支払いが発生します」の通知が1回だけ作られる
  - トライアル中のプラン変更説明から「差額は日割り」が消え、「終了日まで料金は発生しません」に一本化される
- メモ: 監査（2026-08-23）で検出。`customer.subscription.trial_will_end` を購読していながら実装が無く、
  **いつ・いくら請求されるかを知らせないまま満額を引き落とす**状態だった（LPの「7日間は無料」と合わせて不意打ちになる）。
  プラン変更の説明も「差額は日割りで次回請求に加算」と「終了日まで請求は発生しない」が同時に出ていた
  （Portal設定は `continue_trial` なので後者が正しい）。

### T-M8-244: 2回目の無料トライアルが取れてしまう（trial_used_at が永久にnullになる経路） `done`
- 参照: 要件03 §2.1 / 依存: T-M8-237 / サイズ: S
- 完了条件: Stripeに `trial_start` を持つ契約が1本でもあれば、DBが null でもトライアルを付けない
- メモ: 監査（2026-08-23）で検出。`trial_used_at` は trialing の同期が成功して初めて入るため、
  **webhookが届かなかった利用者は永久に null**。解約→再契約でもう一度7日間の無料が取れた。

### T-M8-245: webhookの恒久エラーに500を返し続け、endpoint無効化＝全利用者の同期停止を招く `done`
- 参照: 要件03 §4.1 / 依存: なし / サイズ: S
- 完了条件: 未知Price・profile対応不能は記録して200を返す。DB障害など再送で直りうるものだけ500
- メモ: 監査（2026-08-23）で検出。Stripeは500のイベントを最大3日リトライしたのち**endpoint自体を無効化**する。
  1件の不整合（別環境のCustomerが混ざる等）で**他の全利用者の契約同期まで巻き添えで止まる**設計だった。
  claimを残さない点は従来どおりなので、設定を直したあとダッシュボードから再送すれば処理できる。

### T-M8-240: 未払いの振込が2件以上あると画面に最新1件しか出ない `done`
- 参照: docs/cp/invite_cp.md §2③・要件06 SC-12 / 依存: T-M8-174 / サイズ: S
- 完了条件: 未払いの振込が期限の近い順に全件出て、2件以上なら合計も出る。最低額・手数料は `affiliate/config.ts` から引く
- メモ: 監査（2026-08-23）で検出。`order by period_start desc limit 1`（最新に作られた1件）だったため、
  口座未登録などで未払いが積み上がると**期限が古い方が画面から消えていた**。金額の直書き（¥5,000/¥980）も解消。

### T-M8-241: 招待報酬の振込期限を運営者へ知らせる（記憶頼みをやめる） `done`
- 参照: docs/operations/affiliate-payouts.md・CLAUDE.md 原則3 / 依存: T-M8-174 / サイズ: S
- 完了条件: doctor に「招待報酬の振込」が出て、期限まで7日で warn・期限超過で error になる（件数・合計額つき）
- メモ: 監査（2026-08-23）で検出。締めは自動でも**払うこと自体が運営者の記憶に依存**しており、
  期限（翌月末）を過ぎても画面にもメールにも何も出なかった。

### T-M8-242: 招待コードの帰属失敗が無記録で、正常な「招待なし」と区別できない `done`
- 参照: docs/cp/invite_cp.md §4・CLAUDE.md 原則1 / 依存: T-M8-174 / サイズ: S
- 完了条件:
  - `attributeSignup` が結果を `attributed / self / already_attributed / unknown_code` で返す
  - 未知コードのときだけ記録に残る（自己招待・紐づけ済みは正常なので記録しない）
  - 大文字混じりのURLで来ても紐づく
- メモ: 監査（2026-08-23）で検出。戻り値が `boolean` で、呼び出し側も捨てていたため
  「そもそも招待なし」と「コードが届いたのに付かなかった」が区別できなかった。
  `/r/[code]` は大文字も受け付ける（`/i`）のに完全一致で照合しており、**大文字URLだと黙って招待が付かなかった**。

### T-M8-239: 実績・フォロワー収集の失敗が運営者に届かない（consoleに流すだけ） `done`
- 参照: CLAUDE.md 原則1/2・要件04 §定時トリガー / 依存: なし / サイズ: S
- 完了条件: 収集の失敗が `recordUnexpectedError` で記録され、結果に `failed` の件数が載る
- メモ: 監査（2026-08-23）で検出。中核が用意した記録口（`onError`）が `console.error` のままで、
  同じファイルの `getAccessToken` の catch だけが「中核のログ経路をこのcatchが潰していた」と
  コメント付きで直されていた（本来の経路が取り残されていた）。結果型にも失敗件数が無く、
  **実績が更新されなくなっても「静かに0件」に見えた**（原則1: 正常な空と失敗による空を区別する）。
...
- メモ: 監査（2026-08-23）で検出。実DBで**別々の2利用者の通知本文が完全に同一**（費用$7.17・DB 38MB）だった。
  当月の実績は `user_id is null`（運営者側のニュース取得等）が5,245件$7.19に対し利用者分は20件$0.39で、
  表示額のほぼ全部が運営者の支出だった。DB使用量は運営者だけが見るべき情報（doctor・運営者アラートが担う）。

### T-M8-235: 契約の期限切れ（webhook未達）でも実行できてしまう `done`
- 参照: 要件03・src/lib/auth/subscription-access.ts / 依存: なし / サイズ: S
- 完了条件: `trialing`/`active` のまま期限＋猶予を過ぎていたら生成・投稿の実行前提で止まる。期限が不明なときは止めない
- メモ: 監査（2026-08-23）で検出。`profiles` を更新する経路は webhook だけなので、届かなくなると解約・期間満了が
  反映されず使い続けられる。実DBでも Stripe が `canceled` なのにDBは `trialing` のままのアカウントがあった。
  **猶予24時間**を置く（0にすると更新の瞬間に支払い済みの利用者を締め出す＝反対向きのもっと重い不具合）。

### T-M8-236: 招待報酬の金銭バグ2件（部分返金の二重差引・解約前の支払いの取りこぼし） `done`
- 参照: docs/cp/invite_cp.md §5/§6 / 依存: T-M8-174 / サイズ: M
- 完了条件:
  - 部分返金が2回以上でも累計で正しく減り、同じイベントの再送で値が動かない（冪等）
  - 解約イベントが先に届いても、解約日より前の支払いは報酬になる。解約後の支払いは対象外のまま
- メモ: 監査（2026-08-23）で検出。(1) `charge.refunded` の `amount_refunded` は**その請求の累計**なのに、
  減額済みの `eligible_amount` から引いていたため二重に差し引かれ、部分返金2回で誤って全額取消になった
  （実測: ¥14,800 → 5,000返金 → さらに1,000返金で reversed）。(2) `commission_terminated_reason` が
  あるだけで支払日を見ずに捨てていたため、Stripeの配送順の入れ替わり（最大3日リトライ）で
  解約前の支払いが恒久に落ちた。正本 invite_cp.md §5 は「解約日より前の支払いは対象」。
- 実装メモ（2026-08-23）: `affiliate_commissions.original_amount`（migration 20260823000004）を足し、
  返金は `original_amount - 累計返金額` で毎回計算し直す。解約判定は `commission_ends_at` の日付だけに一本化
  （一度も報酬期間が始まっていない解約では従来どおり開始しない）。実DBの回帰テスト2件を追加。

### T-M8-237: Checkoutが既存契約を見ておらず、二重契約＝二重請求を作れる `done`
- 参照: 要件03 §Stripe / 依存: なし / サイズ: S
- 完了条件: 契約中（active/trialing/past_due/unpaid/paused）の利用者がCheckoutを押しても2本目を作らず、設定＞課金へ誘導する
- メモ: 監査（2026-08-23）で検出。`/plans` の送り返しは `trialing`/`active` だけを条件にしているため、
  **`past_due` の利用者は /plans に留まりCTAが押せる**。`trial_used_at` は設定済みなのでトライアルも付かず、
  押した瞬間に満額の2本目が走る。Stripeは同じCustomerに複数契約を許すので、押した回数だけ請求ができる。
- 実装メモ（2026-08-23）: `handleCheckoutRequest` に `subscriptions.list` の確認を追加（Customer未作成の初回は問い合わせない）。
  `canceled`・`incomplete_expired`・`incomplete` は契約し直せるので対象外。route実装＋実DBのテストも追加。

### T-M8-231: LPのCTA注記を訴求文へ／利用者の声を一時非表示 `done`
- 参照: SC-01・design_handoff_lp/README / 依存: T-M8-223 / サイズ: S
- 完了条件: CTA直下が「7日間は無料でお試しいただけます。」になり、利用者の声セクションが出ない
- メモ: 運営者が直接編集（2026-08-23）。利用者の声はコメントアウトで残し、行頭の `// ` を
  外せば復活できる形（コメントに手順あり）。**法定開示（カード登録が必要・期間中の解約で無料）は
  プロモ帯 CampaignCallout が引き続き2箇所で描画**しており、実HTMLでも確認済み。
- 実装メモ（2026-08-23）: landing-page.test の該当検査を「page.tsx単体」から
  「page.tsx＋CampaignCalloutの合成」で見る形へ更新（開示3点は必須のまま・両方から
  消えたら落ちる）。CTA注記が2箇所である構造の検査は維持。
- 追補（2026-08-23・別セッション）: 06を隠したままだと番号が 05→07 と飛ぶため、
  **「よくある質問」を07→06へ詰めた**（復活時は07へ戻す旨をコメントに記載）。

### T-M8-232: ブログ一覧のカード全体をタップ可能にする `done`
- 参照: 要件06 §1（/blog） / 依存: T-M8-184 / サイズ: S
- 完了条件: 一覧カードのどこを押しても記事へ遷移する。読み上げ・タブ移動で同じ記事が2回出ない
- メモ: 運営者の指示（2026-08-23）。
- 実装メモ（2026-08-23）: 題名リンクの中に透明な `<span className="absolute inset-0">` を置き、カードを `relative` に。
  リンクはDOM上1本のまま。**`after:content-['']` はこのプロジェクトのTailwindで生成されず無効だった**ので span 方式にした（実ブラウザで当たり判定を確認）。

### T-M8-233: 「スケジュールをすべて停止」で下書き作成も止め、「すべて再開」を追加 `done`
- 参照: 要件06 §3.5・要件05 §7・要件02 §3.10 / 依存: T-M4-02/T-M4-05 / サイズ: M
- 完了条件:
  - 「すべて停止」で自動投稿の枠と下書き作成の枠が両方止まり、その枠起点の未実行ジョブも止まる
  - 停止中は同じ位置に「すべて再開」が出る。押すと停止操作で止めた枠だけが戻る
  - 利用者が個別に停止していた枠は再開しても止まったまま
  - 自動投稿の枠を戻すときは現行版の同意チェックが要る（停止＝同意の撤回のため）
  - 実DBの往復テストとE2E（画面の出し分け・同意チェック必須）が緑
- メモ: 運営者の指示（2026-08-23）。「止めたい人は"いま何も動かないでほしい"のであって、下書きだけ作られ続けるのは意図と違う」。
- 実装メモ（2026-08-23）: `schedule_slots.paused_by_stop_all_at`（migration 20260823000003）で「停止操作が止めた枠」を記録し、
  再開はその枠だけを戻す。`disableAutomationForAccount` に `scope: "auto" | "all"` を追加し、**Xアカウント切断は従来どおり
  自動投稿だけ**を止める（切断で下書き設定まで消さない）。`resumeXAutomation` は auto を含むときだけ同意を取り直す。
  SC-08・SC-11の両方で停止/再開を出し分ける（2つ並べない＝いまどちらの状態かが読める）。
...
- 完了条件:
  - `datetime-local`の素の値がサーバのTZに依らず日本時間として解釈・検証され、UTCへ正規化して保存される
  - draft-schedule.spec がNode=UTC×ブラウザ=JST（CI条件）でも緑
- メモ: stg初デプロイのCIで発見。テスト（Node・CI=UTC）とブラウザ（Playwrightで
  Asia/Tokyoへエミュレート）の時計が9時間ずれ、「+120分」の入力がブラウザには過去に
  なりspecが恒久的に失敗。同じ根で**本番（サーバUTC）ではJST利用者の予約が9時間遅れて
  保存される**潜在バグだった（機能自体が未デプロイのため実害はまだ無い）。
- 実装メモ（2026-08-22）: checkDraftScheduleで素の値へ+09:00を付与して解釈（オフセット
  つきISOは従来どおり）。パネルの表示・初期値もJST化しラベルへ「（日本時間）」明記。
  specの相対日時はブラウザ内で計算へ変更、chromiumへ--lang=ja-JPも設定。
  TZ=UTCでの単体・E2Eローカル再現とも緑。

### T-M8-228: スケジュール枠と今後の予定を1本のリストへマージ `done`
- 参照: 要件06 §3.9 / 依存: T-M8-226 / サイズ: S
- 完了条件: スケジュールページの一覧が1本になり、枠（編集・停止・削除つき）と予約済み下書き（「予約 <日時>」・下書きへのリンク）が時間順で並ぶ。停止中の枠は末尾
- メモ: 運営者の指示（2026-08-22）。2枠に分かれていると認知負荷が大きいため。
- 実装メモ（2026-08-22）: SlotListへscheduledDraftsを渡し、次に起きる時刻（枠=次回実行・
  下書き=予約時刻）でソートして混在表示。末尾の「今後の予定」カードは削除。
  実ブラウザで並び順（+1h予約→明朝の枠→+2日予約→停止中）を確認。

### T-M8-227: 失敗した下書きの専用枠と、スケジュール表示の整理 `done`
- 参照: 要件06 §3.9・§4.3・§7 / 依存: T-M8-226 / サイズ: M
- 完了条件:
  - 下書きタブ先頭に赤い「投稿に失敗した下書き」枠が出て、失敗理由と編集・投稿・日時指定予約の操作ができる
  - 下書きタブのスケジュール枠は誘導文＋「スケジュールを開く」ボタンのみ（中身の一覧なし）
  - スケジュールページのヘッダに「次回の実行」行が出ない。今後の予定のバッジが「スケジュール・自動投稿／下書き作成」
  - 予約パネルが右揃え
- メモ: 運営者の指示（2026-08-22）。失敗枠の置き場所は「履歴の右の新タブ」案もあったが、
  普段は空のタブが常設されて見に行かないと気付けないため、下書きタブ先頭の枠を採用
  （エラー通知からの着地先とも一致。理由は完了報告に記載）。
- 実装メモ（2026-08-22）: DraftsListでstatus=failedとlast_post_error付きdraftを先頭の
  dangerトーン枠へ分離（操作・理由表示は既存カードのまま）。ScheduleSummaryを誘導のみへ
  簡素化しposts/pageのslots取得を削除。ScheduleManagerヘッダの次回の実行行を削除。
  予約パネルはflex justify-endで右揃え。実ブラウザで5点すべて確認。

### T-M8-226: 予約パネルの構造修正とスケジュール「今後の予定」への刷新 `done`
- 参照: 要件06 §3.9（SC-07/08の相互要約）・§4.3 / 依存: T-M8-217 / サイズ: M
- 完了条件:
  - 下書きの「予約を変更」を開いてもタイトル行・ボタン行が崩れない（パネルはヘッダー行の下の独立行・幅は内容なり）
  - 下書きカードのタイトル横に更新日時を出さない
  - スケジュールページ末尾が「今後の予定」になり、予約済み下書き（「予約 <日時>」バッジつき）と有効スロットの次回実行が時間順に1本で並ぶ。未予約の下書きは出ない
- メモ: 運営者の指示（2026-08-22）。
- 実装メモ（2026-08-22）: ScheduleDraftControlをトグル（ScheduleDraftToggle）とパネル
  （ScheduleDraftPanel・w-fit）へ分離し、開閉状態はDraftCardが保持——崩れの原因は
  パネル（w-full）が右側ボタン群コンテナの内側で開きコンテナごと全幅化していたこと。
  スケジュールページは予約済み下書きだけをSQLで取り、nextScheduleRunで各有効枠の
  次回1件を算出して時間順にマージ。種別はバッジで区別し、下書き行だけリンク。
  実ブラウザで崩れ解消・並び・非表示を確認。E2E（draft-schedule・mobile-layout）緑。

### T-M8-225: ホーム重要ニュースの空状態ボタンを「ニュースを見る」へ `done`
- 参照: 要件06 §1.4 / 依存: なし / サイズ: S
- 完了条件: 重要ニュース0件のとき「ニュース設定を変更」が出ず、赤紫背景の「ニュースを見る」（/app/news）が出る
- メモ: 運営者の指示（2026-08-22）。
- 実装メモ（2026-08-22）: 空状態の導線を主操作ボタン（primaryLinkClassName＝bg-brand）の
  「ニュースを見る」1本へ。同文言が重複するため右上の小リンクは空状態では非表示。実ブラウザ確認済み。

### T-M8-221: X Premiumアカウントの280字超投稿を許可 `done`
- 参照: T-M8-219・要件06 §4.3 / 依存: T-M8-219 / サイズ: S
- 完了条件:
  - X Premiumのアカウントでは編集保存・投稿とも加重25,000まで通る（非Premiumは従来どおり280）
  - 編集画面は280超のとき「X Premiumのため投稿できます」と案内し、25,000超だけをエラーにする
- メモ: 運営者の指示（2026-08-22）。
- 実装メモ（2026-08-22）: measurePostText/findOverLengthTextへ上限引数を追加し、編集の再検証
  （revalidateEditedThread）と投稿直前の再検証（post-publish）の両方で `x_accounts.x_premium` から
  上限を切替（PREMIUM_MAX_WEIGHTED_LENGTH=25,000。Xの実仕様は「25,000文字」で加重換算より緩いが
  安全側）。draft-editor・DraftsListへxPremiumを配線し文言を出し分け。生成は280目標のまま変更なし。

### T-M8-222: メール通知の廃止（アプリ内通知のみへ）と設定>通知UIの刷新 `done`
- 参照: 要件01 §3.6・02 §3.15/§4.3・04 §14・05 §10・06 §1.2.2 / 依存: なし / サイズ: L
- 完了条件:
  - 利用者向け通知メールが送られない（コード・DB配送台帳・再送UI・回収処理・掃除スクリプトを削除）
  - 認証メール（Supabase Auth）と運営者向けopsアラートメールは残る
  - 設定>設定の通知UIが「種別ごとの説明つきトグル」、ニュース通知が「チップ型選択」になる
- メモ: 運営者の指示（2026-08-22）。分割すると「送信コードだけ残る」中間状態で壊れるためLのまま。
- 実装メモ（2026-08-22）: migration 20260823000002（notification_configのemailキー剥がし・
  notificationsのemail_7列とemail_delivery_status enumのdrop・trigger既定をin_appのみへ）。
  notification-email(-server)/recover-queued/smtp-error/retryNotificationEmail系を削除し、
  canSendViaSmtpはsmtp-guard.tsへ移設（operator-mailが継続使用）。全insert箇所の条件を
  in_appのみへ、日次サマリのメール滞留行・doctorの「お知らせメール」検査・clean-test-dataの
  メール掃除・EMAIL_REPLY_TO envを削除。docs 8ファイル同期。単体/DB 2,380件緑。

### T-M8-223: UI・文言の細部修正（ボタン色・課金カード・分析見出し・プレースホルダー一覧・LP実名） `done`
- 参照: 要件06 §1.2/§1.4/§5・SC-01 / 依存: なし / サイズ: S
- 完了条件:
  - ホーム「ニュース設定を変更」が「スケジュールを設定」「今すぐ作成」と同じ主操作ボタン
  - 設定>課金・プランの月額がプラン名の真横・「キャンペーン終了後」の併記なし
  - 分析レポート③の見出しが「良かった投稿に近づくプロンプト設定」
  - 設定>プロンプトの右上「メイン＋スレッド最大N・編集はNポストまで」が消え、カラウト内にプレースホルダー一覧
  - LP利用者の声の名前がX APIの実表示名
- メモ: 運営者の指示（2026-08-22）。
- 実装メモ（2026-08-22）: LPの4名はusers/by/usernameの実名を反映（コメント文は引き続き
  本人確認のうえ運営者が差し替えること）。カラウトの一覧は設定＞プロンプトのみ
  （投稿作成・スケジュールは実際の入力欄が下に並ぶため二重に見せない）。

### T-M8-224: 招待報酬率を30%〜50%へ引き上げ `done`
- 参照: docs/cp/invite_cp.md §3・PRD R-2・要件03 §8 / 依存: なし / サイズ: S
- 完了条件: 全ランクの率が30/35/40/45/50%になり、画面・docs・テストが一致する
- メモ: 運営者の指示（2026-08-22）。過去のCommissionはsnapshot済みのため変わらない（仕様どおり）。
- 実装メモ（2026-08-22）: INVITE_TIERSを+10ptへ。invite_cp.mdの表・モック、PRD、要件03、
  関連テスト（config/store/subscription-sync）を同期。

### T-M8-218: プラン変更にエキスパートが出ない・旧価格エラー再発の根治（Portal設定の作り直し） `done`
- 参照: T-M8-215・docs/operations/deployment.md / 依存: なし / サイズ: S
- 完了条件: 新旧どちらの契約でもプラン変更画面に現行3プラン（スタンダード¥1,480／プレミアム¥3,980／エキスパート¥14,800）が並ぶ
- メモ: 運営者の報告（2026-08-22）。kota（新¥1,480契約）で旧価格エラー継続・matsubuzでエキスパート非表示。
- 実装メモ（2026-08-22）: 根本原因は**旧Portal設定にダッシュボード保存の商品リストが残っていた**こと
  （現行APIバージョンではGETに現れず不可視。エキスパート未登録＋プレミアム候補が実質空）。
  APIで新Portal設定 `bpc_1U78flE811f4DP4qPdhugSfn` を現行3商品×現行Price＋全機能同等で作成し、
  旧設定を無効化・`.env.local` の `STRIPE_PORTAL_CONFIGURATION_ID` を差し替え。さらにmatsubuzの契約に
  価格移行時のsubscription scheduleが残りPortal更新をブロックしていたためreleaseで解除。
  両アカウントで3プラン全表示を実ブラウザ確認。本番の運営者手順（liveでPortal設定作成＋env差し替え＋
  schedule解除）はdeployment.mdへ明記。**Vercel環境変数の差し替えは運営者作業**。

### T-M8-219: XアカウントのX Premium表示（verified_type取得と保存） `done`
- 参照: 要件02 x_accounts・要件05 §4.3・要件06 §1.2.1 / 依存: なし / サイズ: S
- 完了条件: X Premium加入アカウントには設定＞設定の行に「X Premium」バッジが出る。加入状態は連携時・「接続を確認」・再有効化で更新される
- メモ: 運営者の指示（2026-08-22・⑦）。X API v2はPremium（verified_type=blue/business）なら25,000字まで投稿可。
- 実装メモ（2026-08-22）: `x_accounts.x_premium`（migration 20260823000001）を追加し、`/2/users/me` を
  `user.fields=profile_image_url,verified_type` で取得（実API確認済み: @ai_newinfo=blue）。
  users/meを呼ぶ3経路（OAuth callback・refreshXAccountStatus・enableXAccount）すべてで更新。
  設定＞設定の行へ「X Premium」バッジ（実ブラウザ確認）。既存行は次の「接続を確認」で追いつく
  （@ai_newinfoは実値で反映済み。@no1013kotaはトークン失効中のため次回確認時）。
  280字超の投稿許可（Premium時の緩和）は未実装——必要になったら別タスク。

### T-M8-220: UI細部修正（予約パネル右余白・AIモデル設定の枠外し・グレー小表記の削除） `done`
- 参照: T-M8-217・要件06 §3.6/§4.2 / 依存: なし / サイズ: S
- 完了条件: 予約の日時入力がパネル右端まで伸びる。AIモデル設定の「利用するAI」「文章生成に使うモデル」が灰色枠の外（画像生成と同じ並び）。投稿作成・スケジュール・設定＞プロンプトにスレッド数・プレースホルダーのグレー小行が出ない
- メモ: 運営者の指示（2026-08-22）。
- 実装メモ（2026-08-22）: 予約入力欄をflex-1で全幅化（モバイル390でも横はみ出しなし）。
  運営キー系プランの文章AI表示を灰色枠から出しラベル＋欄の並びへ。`PlaceholderSummary`と
  スレッド数読み取り行（threadCountFromPromptLabel）を全画面から削除し、説明はカラウトへ一本化。
  上限10件超過の警告だけ`PlaceholderOverflowWarning`として残す（黙って捨てない・原則1）。
  E2E（pattern-manager・generation）の該当assertをDB検証＋カラウト表示へ置き換え。

### T-M8-215: 旧Price契約のプラン変更エラーと決済まわりの残修正 `done`
- 参照: T-M8-213・docs/operations/deployment.md / 依存: なし / サイズ: S
- 完了条件: 旧Priceのままの契約でプラン変更を押したときのエラー原因を解消し、専用の文言を出す
- メモ: 運営者の報告（2026-08-22）。
- 実装メモ（2026-08-22）: 原因はStripeの「契約中Priceが変更候補に無い」400（旧Priceアーカイブの
  副作用）。テストモードの該当契約（¥2,980）を新プレミアムPriceへ移行し（proration none）、
  Portalのプラン変更に現行3プランだけが並ぶことを実ブラウザ確認。該当エラーには
  「旧価格のままのため開けません…」の専用文言を返すようportal.tsへ分岐追加。
  本番手順（①契約の新Price移行→②旧Priceアーカイブ）をdeployment.mdへ明記（運営者作業）。

### T-M8-216: 利用者の声をX投稿風カード＋実アイコンへ／その他UI修正 `done`
- 参照: T-M8-214 / 依存: なし / サイズ: S
- メモ: 運営者の指示（2026-08-22）。
- 実装メモ（2026-08-22）: unavatar経由で4名の実Xアイコンを取得しpublic/lp-avatars/へ同梱
  （外部ホットリンクはCSP・変更耐性の点で不採用。更新手順をコメントに明記）。カードをX投稿風
  （アバター＋名前＋@ハンドル＋右上Xロゴ→本文）へ。ホームの「ニュース設定を変更」を
  brand-subtleへ統一。名前表記は仮置き（本人の表示名に合わせて要確認）。

### T-M8-217: 下書きの日時予約UIの改善と文字数カウントのX準拠表示 `done`
- 参照: T-M8-157・要件06 §4.3 / 依存: なし / サイズ: S
- メモ: 運営者の指示（2026-08-22）。
- 実装メモ（2026-08-22）: 予約入力をボタン下の全幅パネル（order-last w-full）へ——アクション行内
  展開が原因の「カレンダー操作で位置が飛ぶ」を解消。開いたとき現在時刻＋5分を初期値に
  （＋5分は「1分以上先」検証との衝突回避）。エラーは入力下の固定行へ。文字数カウントへ
  「（全角N / 140字換算）」を併記し、超過時は「Xの上限（全角140字・半角280字）を超えています。
  X Premiumに加入していない場合は投稿できません。」を表示。
  既定プロンプトはSYS-GEN共通指示に「加重280以内・日本語なら約140字」が既にあり変更不要。
...
### T-M8-214: LPへ「06 利用者の声」セクション（提携4名・Xリンク付き） `done`
- 参照: 要件06 §1・design_handoff_lp（禁止表現の改定含む） / 依存: なし / サイズ: M
- 完了条件:
  - 「06 よくある質問」の前に「06 利用者の声」が入り、以降の番号が繰り下がる
  - 提携4名（@ai_newinfo・@picaso_youtube・@lin_youtube3・@kimi_marriage）をXアイコン・名前・プロフィールへのリンク付きで表示
  - コメント文はドラフトであることを運営者へ明示し、本人確認済みの実文へ差し替えてもらう（勝手な創作を本番へ出さない）
  - 禁止表現リストの「利用者の声」を「実在の提携者・本人確認済みコメントのみ可」へ改定
- メモ: 運営者の指示（2026-08-22・4名と提携済み）。
- 実装メモ（2026-08-22）: 06利用者の声（2×2カード・頭文字アバター＋XLogo＋@ハンドル→x.comリンク）を挿入しFAQを07へ。URLはapp-configのxProfileUrl経由。禁止表現「利用者の声」を実在提携者のみ可へ改定（landing-page.test/handoff README）。**コメント4件はドラフト——本番リリース前に本人確認済み文へ差し替え必須（運営者）。**

### T-M8-204: LP「結果分析」図版の調整（毎朝レポート削除・グラフ全幅） `done`
- 参照: 要件06 §1（02できること） / 依存: T-M8-201 / サイズ: S
- 完了条件: 「毎朝レポート」チップが消え、フォロワー数グラフが横幅いっぱいに描かれ不自然な空白が無い
- メモ: 運営者の指示（2026-08-22）。
- 実装メモ（2026-08-22）: AnalyticsFigureから毎朝レポートチップを削除し棒をflex-1で全幅へ。

### T-M8-205: スケジュールでのパターン削除と削除確認文の簡潔化 `done`
- 参照: 要件06 SC-07/08・T-M8-133/134 / 依存: なし / サイズ: S
- 完了条件:
  - スケジュールページでも投稿作成と同様に各パターンカードから削除できる
  - 削除確認ダイアログのグレー説明が簡潔な箇条書きになる
- メモ: 運営者の指示（2026-08-22）。
- 実装メモ（2026-08-22）: PatternRadioGroupへonDelete配線＋removePattern（投稿作成と同文言）。削除確認は箇条書き3点へ簡潔化。
  **追記（2026-08-22・T-M8-220の検証で発見）**: 旧仕様のままだったE2E（pattern-card-delete「SC-08には出ない」）と要件06 §該当行の同期漏れを修正（削除が出る仕様へ反転済みを反映）。

### T-M8-206: 通知の既定をメール3種（ニュース・投稿完了・毎日のまとめ）へ `done`
- 参照: 要件02 §4.3・要件06 §3.4 / 依存: なし / サイズ: S
- 完了条件:
  - 新規登録の既定: アプリ内=全てON、メール=ニュース・投稿完了・毎日のまとめのみON（下書き・エラー・課金・利用枠はOFF）
  - コード既定・trigger既定・docs・テストが一致する
- メモ: 運営者の指示（2026-08-22）。既存利用者の保存値は変更しない。
- 実装メモ（2026-08-22）: DEFAULT_NOTIFICATION_CONFIGとtrigger（migration 20260822000004）をメール3種ONへ。docs 02/06更新。既存保存値は不変。

### T-M8-207: 設定＞課金・プラン／AIモデル設定のUI統一 `done`
- 参照: 要件06 SC-11・デザイン規約 / 依存: なし / サイズ: M
- 完了条件:
  - 両タブの文字サイズがデザイントークン（page-title/card-title/body/caption）に揃う
  - 課金・プランのグレー背景の説明から冗長なものを削除・簡潔化
- メモ: 運営者の指示（2026-08-22）。
- 実装メモ（2026-08-22）: 課金: dt=caption/dd=body太字へ統一・プラン変更の説明を1行×4項目リストへ簡潔化。AIモデル: 見出し・値・説明をトークンへ統一し重複説明を削除。premium/expert両方の実ブラウザで確認。

### T-M8-208: 設定＞プロンプトにもプレースホルダー説明カラウトを表示 `done`
- 参照: T-M8-203（PlaceholderCallout） / 依存: T-M8-203 / サイズ: S
- 完了条件: パターン管理の編集・新規作成フォームに投稿作成と同じカラウトが出る（小さな一覧表示も維持）
- メモ: 運営者の指示（2026-08-22。前回の「小さいまま」から変更）。
- 実装メモ（2026-08-22）: PatternManagerの新規・編集フォーム両方へplaceholderHint=prominent。

### T-M8-209: 生成中は下書き一覧に「作成中」の枠を出す `done`
- 参照: 要件06 SC-07（下書きタブ） / 依存: なし / サイズ: S
- 完了条件: 投稿生成が進行中のあいだ、下書きタブの先頭に作成中カード（進行表示つき）が出て、完了で実物に置き換わる
- メモ: 運営者の指示（2026-08-22）。
- 実装メモ（2026-08-22）: posts/pageがqueued/runningのpost_generationを取得しDraftsList先頭へ破線カード（脈動ドット＋進行バー・通常60〜90秒）を表示。完了で次の読込時に実物へ置き換わる。

### T-M8-210: ニュース解説を{ニュース}プレースホルダー化し、「すぐに投稿作成」を投稿作成画面への遷移＋自動入力へ `done`
- 参照: プロンプト設計書 §6（PT-P1）・要件06 SC-06/SC-07・T-M8-186 / 依存: なし / サイズ: M
- 完了条件:
  - PT-P1（ニュース解説）が {ニュース} プレースホルダーを使う（<input>のニュース参照をやめる）
  - 最新ニュースの「すぐに投稿作成」が投稿作成画面へ遷移し、ニュース解説パターン選択＋{ニュース}へ記事内容（見出し・要約・URL）が自動入力される
  - 作成済みバッジ（news_item_id→drafts）が引き続き機能する
  - 直接ジョブを作る旧経路（createDraftFromNews）は削除
  - smoke:live（プロンプト変更のため）緑
- メモ: 運営者の指示（2026-08-22）。<input>の仕組み（分野・参考URL・追加指示）は全パターン共通のため残す。
- 実装メモ（2026-08-22）: PT_P1を{ニュース}方式へ（migration 20260822000005: seed placeholders・既存p1のbackfill・system default行更新）。SC-06はLink遷移（?news=<id>）でp1選択＋{ニュース}/参考URL/テーマ自動入力。createDraftFromNews系を削除。実生成1周で成果物・作成済みバッジ確認。smoke:live全緑。

### T-M8-211: Stripe Productの説明文を現行プランへ更新（決済画面の旧文言） `done`
- 参照: docs/operations/deployment.md（Stripe設定） / 依存: なし / サイズ: S
- 完了条件:
  - テストモードの3 Product（スタンダード/プレミアム/エキスパート）の説明文が現行の文言になる
  - 決済画面の表示内容がStripe側Productに由来する旨と更新手順をdocsへ明記（本番liveモードは運営者作業）
- メモ: 運営者の報告（2026-08-22・決済画面が古い）。価格は正しく、説明文が旧文言（ベースmd等）だった。
- 実装メモ（2026-08-22）: テストモードの3 ProductのdescriptionをAPIで現行文言へ更新。deployment.mdへ「表示文言はProduct・価格はPrice」を明記（本番liveは運営者作業）。

### T-M8-203: パターン編集UIの一本化（「生成に使うプロンプト」の廃止・追加中の同期） `done`
- 参照: 要件06 §4.2（SC-07/08）・T-M8-186/194 / 依存: なし / サイズ: M
- 完了条件:
  - 投稿作成: 折りたたみ「生成に使うプロンプト」（アカウント.md/投稿の型/画像生成の3タブ）を廃止し、選択中パターンのプロンプト編集欄を「パターンを追加」と同じUIでインライン表示する
  - 「パターンを追加」中は既存パターンのアクティブ選択が外れ、下部のプレースホルダー入力欄が追加中プロンプトの {名前} と同期する
  - 追加を確定するとパターン一覧に新しい枠が現れ、それがアクティブになる（既存挙動の維持）
  - 「{名前} と書くと入力欄が出る」説明を目立つ形（カラウト）で表示する
  - スケジュールページも同じ形へ。設定>プロンプトはプレースホルダー表示を小さいまま維持
  - スケジュールの「有効なスケジュールはありません。」行を削除
- メモ: 運営者の指示（2026-08-22）。アカウント.md・画像プロンプトの編集は設定>プロンプトが引き続き担う（投稿作成からは外れる）。
- 実装メモ（2026-08-22）: SC-07/08の折りたたみ3タブを撤去し、選択中パターンのPromptBlockを
  インライン表示（ラベル「生成プロンプト（型名）」・once/saveの2択は維持）。追加フォームを
  開くとアクティブ選択が外れ（キャンセルで復元・確定で新パターンへ）、プレースホルダー入力欄は
  追加中フォームの{名前}から導出。生成/枠保存は追加中は理由つきで不可（slot-formへ
  pattern_id空の検証追加）。PlaceholderCalloutを新設（設定>プロンプトはcompactのまま）。
  スケジュールの「有効なスケジュールはありません。」削除。投稿作成のbase_md/image編集コードと
...
- 完了条件:
  ...
  - 招待の紐づけ（T-M8-191のフォールバック含む）が登録時経路だけで機能し続ける
...
- 完了条件:
  - ヒーロー見出しが「プロンプトドリブンの／使用するほど性能が上がる／SNS運用プラットフォーム（brand色）」になる
  - ヒーローのチェック3行がCTAボタンと同等の文字サイズになる
  - 料金カードの機能リストからグレーの補足文が消える
  - 「リリース記念として期間限定で半額で…お知らせします。」の行が消える
  - ヒーローモック: 「重要度:高」チップ削除・最下段「下書き・予約」→「フォロワー推移」
  - 02できること: 投稿・画像の自動作成カードから「スレッド3投稿」文言を消しスレッドを1つ追加、画像ボタンではなく簡易画像を表示。結果分析カードへプロンプト改善案のイメージを追加
  - 03しくみのグレー説明文を簡潔化
- メモ: 運営者の指示（2026-08-22）。
- 実装メモ（2026-08-22）: ヒーローh1を3行構成へ（3行目brand色）・チェック3行をtext-sm
  （CTAと同サイズ）へ。料金カードのnote表示・CampaignCalloutの注記行を削除（比較表データの
  noteは撤去）。ヒーローモックの重要度チップ削除・下段タブを「フォロワー推移/分析・改善」へ。
  PostComposeFigureをスレッド2連（接続線＋ドット）＋風景風の生成画像サムネへ。
  AnalyticsFigureへ「プロンプト改善案」カードを追加。03の説明を1文へ。
  実ブラウザ3幅（1440/768/390）スクショ・コンソールエラー0・横スクロール0を確認。
  landing-page.test 17件・E2E（landing/plans/mobile 12件）緑。

### T-M8-199: 公開ページヘッダーのブランド削除と遷移アイコン `done`
- 参照: 要件06（PublicPageShell・T-M8-190） / 依存: なし / サイズ: S
- 完了条件:
  - /prompt-templates・/blog のヘッダーから「Exos AI」を削除し、遷移ボタンを左端へ詰める
  - 「ホーム」「プロンプト集」「ブログ」の各ボタンに遷移が分かるアイコンが付く
- メモ: 運営者の指示（2026-08-22）。
...
- 完了条件:
  - 取得がJST 9/12/15/18/21時の1日5回になる（vercel cron・launchd plist・コード定数が一致）
  - 取得窓が新間隔に追随する（通常4時間・初回9時は前日21時からの12時間。隣の回と重なり続ける）
  - 画面・LP・docsの時間帯表記がすべて追随する
  - 費用の変化をdocsへ明記（6分野×5回/日）
- メモ: 運営者の指示（2026-08-22）。指示は「3時間ごと・9:00-21:00・1日4回」だが、9〜21時を3時間ごとに回すと9/12/15/18/21の5回になる（4回は12時間÷3時間=4区間の数え方と解釈）。間隔と時間帯を優先し5回で実装。4回ちょうど（例: 9/13/17/21）にする場合は再指示。
- 実装メモ（2026-08-22）: NEWS_FETCH_JST_HOURS=[9,12,15,18,21]・間隔3h（窓は通常4h・初回12h）。
  vercel cron `0 0-12/3 * * *`（UTC 0/3/6/9/12）・launchd plist・関連テスト・UI/LP/docsの
  時刻表記を全更新。実叩き1回: 6分野全並列成功・7件保存・love/beautyも動作・0件分野は
  「該当なし」・ダイジェスト2通。費用は6分野×5回/日=月900回（$216〜450/月・従来比約17%減）。

### T-M8-196: 複数Xアカウント併用の総点検（分離・切替・上限・ジョブ） `done`
- 参照: 要件02（RLS・x_account_id）・要件03（プラン別アカウント数）・要件04（ジョブ） / 依存: なし / サイズ: M
- 完了条件:
  - 複数アカウントが同時に各機能（生成・予約・分析・ニュース起点生成・パターン・プロンプト）を使ってもデータが混ざらない・壊れないことを検証する
  - 見つかった不具合は修正し、再発防止テストを足す
- メモ: 運営者の指示（2026-08-22）。敵対的レビューのワークフローで探索する。
- 実装メモ（2026-08-22）: 24エージェント（5観点探索→実DB反証検証）で16件が実バグと確定し全修正:
  【critical】usage_countersのcheck制約(200/20)がexpert枠(1000/100)と衝突しX公開後にDB
  エラー→migration 20260822000003で拡張。
  【誤アカウント書き込み】ニュース起点生成: 表示中x_account_idを送りassertActiveAccountで
  検証＋冪等キーへアカウントを含める（2つ目のアカウントで黙って何も作られない穴も同時解消）／
  画像プロンプトの保存・リセットとパターン作成・既定復元にx_account_id必須化（不一致は
  x_account_mismatch）／アカウント切替でエディタstateをkey={accountId}で確実に破棄
  （BaseMd・Persona・PatternManager・PromptTemplates・投稿作成・スケジュール）。
  【ジョブ】post_generationの前提再検証をジョブ対象アカウント基準へ（別アカウント設定中に
  健全なアカウントの自動生成が失敗していた）／dispatchをユーザー間公平化（row_number
  partition by user。1ユーザーの滞留が50枠を独占し他ユーザーが数時間飢餓）／日時予約の
  永久沈黙を解消（終端jobがkeyを占有したら予約解除＋last_post_error＋通知）／60分超の
  遅延予約は投稿せず解除＋通知（失効→再連携で古い予約がまとめて流れるのを防止）／
  assertJobBudgetのTOCTOUをadvisory xact lockで直列化。
  【アカウント状態】disabled行の再連携はプラン上限を数える（連携→切断→別連携→再連携での
  上限突破を閉鎖）／「状態を確認」がdisabledをactiveへ復活させる競合を条件付きUPDATEで封鎖／
  resolveActiveXAccountのフォールバックを比較交換式へ（明示的切替の上書き防止）。
  再発防止テスト: oauth-callback（disabled再連携の上限）・scheduled-drafts（spent解除・
  遅延解除）を追加。全ゲート緑: lint・単体/DB 2,424件・build・CSP・E2E 97件。
  棄却2件（月次枠TOCTOU=advisory lockで既に直列化・自動起票の予算圧迫=再現不成立）。
...
### T-M8-194: パターン編集UIの統一（投稿作成・スケジュール・設定>プロンプト）と新規作成の挙動 `done`
- 参照: 要件06 SC-07/SC-08・AI設定>プロンプト・T-M8-186 / 依存: T-M8-186 / サイズ: M
- 完了条件:
  - 投稿作成ページ: 「パターンを追加」で出るプロンプト記入欄と、既存パターン選択時の「生成に使うプロンプト」が同じUI（追加時の記入欄に一致させる）
  - 新しいパターンを作成したら、アクティブな選択がその新パターンになる
  - プロンプト内に {名前} を入力すると自動でプレースホルダー入力欄が増える（T-M8-186のライブ導出を追加フォームにも適用）
  - スケジュールページにも同じUI・挙動を同期
  - 設定>プロンプトにも同期。ただしプロンプトの下に「プレースホルダー: {名前}…」を小さく表示する形
- メモ: 運営者の指示（2026-08-22）。
- 実装メモ（2026-08-22）: PatternFieldsの手入力プレースホルダー欄を廃止し、本文の{名前}から
  自動導出（toPatternPayloadがextractPlaceholderNamesで送る＝保存時導出T-M8-186と同規則）。
  導出結果はプロンプト直下にPlaceholderSummaryとして小さく表示（3画面共通）。
  PromptBlockをPatternFieldsの記入欄と同形（見えるラベル＋右上字数・h-64・下に分量と
...
- 完了条件:
  - 最新ニュースが保存されている全件を対象に、50件ずつのページで表示される
  - テーマ・インパクト（・日時）でソートできる
  - 設定の「表示件数」入力が消え、保存値も使われなくなる（news_configから廃止。バックエンドの該当ロジックも削除）
  - コストへの影響なし（取得系は変更しない。表示だけの変更であることをdocsに明記）
  - 要件02（news_configの形）・要件06（SC-03）・設定画面のdocsを更新
- メモ: 運営者の指示（2026-08-21）。取得は従来どおり（3分野・2時間おき・保持40日）なので費用は不変。
- 実装メモ（2026-08-21）: listNewsItemsをoffsetページング＋sort（date/category/impact）へ全面書き換え。
  keyset cursor・専用Server Action・一覧の絞り込みUI・表示件数の設定欄と定数群を削除。
  news_configはschemaで旧max_itemsを黙って落とし、保存値はmigration 20260821000002で掃除。
  ダイジェストpayload上限は固定20へ。ホームの重要ニュースは専用クエリ（listTopHighImpactNews）へ
  分離（旧スキーマ入力のまま壊れていたのをE2Eが検出）。E2E: 全件表示・インパクト順（時間窓で
  環境非依存化）・55件seedの50件ページ送りを追加。

### T-M8-181: LP調整（しくみ下の開示行の削除・料金カードの枠と行の揃え） `done`
- 参照: 要件06 §1（LP） / 依存: T-M8-177 / サイズ: S
- 完了条件:
  - 03しくみ下の「反映するかはあなたが選べます…」の行が消える（同趣旨の開示は02の結果分析カードが引き続き担う＝禁止表現の回避は維持）
  - 05料金の3枚のカードが同じ大きさになり、機能リストの各行が3枚の間で同じ高さに揃う
- メモ: 運営者の指示（2026-08-21）。行揃えはCSS subgridで実装（全カードが同じ9行を持つ前提はデータ側で保証済み）。

### T-M8-182: プロンプト集のライブ体験（プレースホルダー入力）・命名整理・CTA位置 `done`
- 参照: 要件06（/prompt-templates） / 依存: T-M8-178 / サイズ: M
- 完了条件:
  - プレースホルダーを持つプロンプトに入力欄が付き、入力するとプロンプト本文の {名前} がその場で置き換わって表示される（体験できる）
  - アカウント.mdタブの項目名がすべて「アカウント.md」になる（主テーマから導出した題名をやめる。「発信定義書（アカウント.md）」はアプリが生成する実ファイルの見出しだが、一覧の題名としては使わない）
...
### T-M8-170: /plansでNextのchunkがnonce無しで注入されCSPに弾かれる（コンソールエラー） `done`
...
- 実装メモ（2026-08-23 完了）: **原因は `/plans` のストリーミング境界**（`src/app/plans/loading.tsx`）。
  本番ビルドの実ブラウザで、本体が届くときに `useMergedRef`（next/link内部）のchunkだけが
  **後から nonce 無しの `<script src>` として注入**され、strict-dynamic に弾かれていた。
  `/app` も `loading.tsx` を持つが、レイアウトの初回スクリプト群に同じchunkが含まれるため起きない。
  Next 16.3.2 のリリースノートに該当の修正は見当たらず（16.2.12 使用中）、**境界を外して解決**した
  （/plans はログイン済みで実測 60〜150ms＝待ち表示が無くても不自然でない。T-M8-67 の意図は
  「無反応に見せない」ことなので、遅くなるようなら押した側にpending表示を足す方が筋が良い）。
  **試して駄目だった案**: 待ち表示側で `next/link` を使ってchunkを先に読ませる——**新しいブラウザ
  セッションでは再現**した（最初は直ったように見えたがキャッシュによる誤判定。検知スクリプトが捕まえた）。
- 再発検知（完了条件2）: `npm run check:csp-runtime`（`scripts/check-csp-runtime.mjs` ＋
  `e2e/csp-runtime.spec.ts`）。**本番ビルドを空きポートで起動し、実ブラウザで公開ページと
  ログイン後の画面（/plans・/app・課金タブ）を開いてCSP違反0を確認**する。`release:check` に含めた。
  `check:csp-nonce`（静的検査）でもE2E（dev）でも原理的に見えない層なので、検査を1つ足した。
...
**P-2（本番の未整備・2026-08-26 `npm run doctor -- --base https://exosai.net` で検出）** — 本番反映の作業中に見つけた、コードではなく**本番の設定側**の問題。どちらも運営者の操作が要る。

1. ~~**【最優先】Stripeの価格が無効で新規登録が失敗する**~~ → **2026-08-26 の本番反映で解消**（doctor: 「一致しています（スタンダードプラン ¥1480 / プレミアムプラン ¥3980 / エキスパートプラン ¥14800）」）。原因は**本番のコードが古く、アーカイブ済みの旧Priceを参照していた**こと。Stripe側の操作は不要だった。以下は発見時の記述 — doctor: 「金額は一致していますが、**無効な価格があります（mdプラン・プレミアムプラン）。新規登録が失敗します**」。該当プランを選んだ人はCheckoutへ進めない。**いま実際に効いている障害**で、今日のデプロイとは無関係（以前から）。→ **Stripeダッシュボード（本番モード）で md プランとプレミアムプランの Price を有効化する。** 価格改定でアーカイブした旧Priceが参照されたままの可能性が高い（deployment.md §0.0 の「①既存契約を新Priceへ移行→②旧Priceをアーカイブ→③Portal設定を更新」の③の取りこぼし）。修正後は `npm run doctor -- --base https://exosai.net` の「請求額と表示額の一致（Stripe）」が緑になることで確認できる。
2. **本番にXアカウントが1つも連携されていない** — doctor: 「連携されたアカウントがありません」。そのため本番の実物スモークは**生成・画像を検証できない**（ニュース取得のみになる）。運営者が本番で1アカウント連携すれば、以後 `npm run release:production -- --account <ハンドル>` で生成まで通しで確認できるようになる。

**D-53: 新LP（/new）の公開判断と3つの付随判断（2026-09-03・T-M8-419のレビューで浮上・**決定 2026-09-05: 運営者「今のままで良い」＝(1) 終了後価格の併記と半額バッジは現状のまま (2) ヒーローのニュースカードは現状可 (3) 実投稿セクションは素材が届くまで非表示のまま (4) 旧LP `/old` は当面残す（削除は運営者が指示するまでしない）**）** — (1) **キャンペーン終了後価格の二重表示**（料金カード共通部品が「キャンペーン終了後 ¥X,XXX」を併記。景表法上は将来価格の根拠として**終了日の明示**が望ましい）。案A（推奨）: キャンペーン終了日を決めて帯に明記する／案B: 終了後価格の併記をやめ「リリース記念価格」だけにする。(2) **ヒーローのニュースカードに第三者記事の要約が写る**（現状はpublickeyのCloudflare公式発表要約）。案A: 現状可／案B: プレスリリース系に限定／案C: 運営者投稿由来のカードで撮り直す。(3) **「運営者の実投稿」セクション**は @ai_newinfo を本システムで運用している事実確認と実投稿3件（本文・表示回数・URL）が届くまで非表示（`lp-new/facts.ts` の `OUTPUT_ENABLED`）。素材が届けば表示に切り替える。**/new → / の差し替えは 2026-09-04 に実施した（T-M8-420）。** 残る判断は **旧LP `/old` と `src/components/lp/` の旧部品をいつ削除するか**（新LPが共有するのは `screenshot.tsx` だけ。暫定案: 新LPで1か月運用して入口ファネル〔admin〕の登録率が旧LPを下回らなければ削除）。

**D-54: LPの見出し「プロンプトを無限に管理」と上限の食い違い（2026-09-04・T-M8-421・**決定 2026-09-04: 運営者「一旦は食い違ってもよい。文章だけ変更」→ 見出しを運営者の語へ戻した〔T-M8-425〕。上限は据え置き**）** — 運営者指定の見出しに対し、実装はアカウント.md・画像プロンプトの本棚が**5件まで**（T-M8-350）、投稿の型が**最大20件**（PATTERN_MAX_COUNT）。上限に達した利用者に「無限」は事実と異なるため、暫定で「プロンプトを、使い分ける」（本文に「5件まで」を明記）にした。同じ理由でカード1「投稿の型を、何個でも」→「自分で増やせる」。案A: 上限を撤廃または大幅に上げて（本棚5→無制限・型20→無制限。DB・画面の変更が要る）指定の見出しに戻す／案B（推奨）: 暫定文言のまま。あわせて、料金の推奨帯から注記を外したため `#pricing` 内で最初のCTAより上にある開示は「はじめての方は7日間無料（カード登録が必要）です。」の1行だけ（帯とツアー直後・最終CTAの注記は残る）。これで足りるかの判断も含む。

**D-57: Web リサーチを「全パターンで必ず行う」形にするか（2026-09-05・運営者の質問から）** → **決定（2026-09-05・運営者の指示）: 既定パターン6種すべて「必要があれば最大3回」（案B の検索必須ではなく、ツールを常に渡して回数上限3。0回の警告は付けない）。T-M8-442 で反映済み。**
現状は P-1/P-3/P-4/P-6/自作パターン=検索ツールを常に渡す（最大3回。使うかはモデル任せ）、P-2=参考URLがあるときだけ、P-5=渡さない。自作パターンは画面から検索の有無を変えられない。
- 案A（現状維持＋docs を実態に合わせる・推奨）: 費用と速度のため。P-2（意見）と P-5（引用）は検索しない方が趣旨に合う。T-M8-441 で docs だけ直す
- 案B（全パターンで検索を必須にする）: `web_search_policy` を全て `always` にし、`web_search_count=0` の生成を「検索していない」として警告する。生成1回あたりの費用・時間が増える（検索1回 約$0.01〜、数秒〜十数秒）
- 案C（パターン編集画面に「Web検索: する／URLがあるときだけ／しない」を出す）: 自作パターンでも選べるようにする。要件06 の記述に近づく


**D-56: 料金の共用部品の文言（2026-09-04・T-M8-424 のレビューで浮上。LPと `/plans` に同時に効く・**決定 2026-09-04: 運営者「迷ったら…の文章は不要。両ページとも見出しは『業界最安価の価格設定』」→ T-M8-425 で反映。2026-09-05 追加決定: (2) 要約の重複・(3) 「AIクレジット100,000」「契約期間ごと」の語は「今のままで良い」＝変更しない。あわせて「業界最安価」は最上級表示（景表法）なので比較の根拠資料を運営者が保持すること**）** — (1) **`/plans` の h1**: 画面名「あなたの運用に合うプランを選択」のまま、直下のサブ文の先頭に「迷ったら、プレミアムプランから。」を置いた（暫定）。案A（暫定・現状）: そのまま／案B: h1 をLPと同じ「迷ったら、プレミアムプランから。」にし、画面名は `<title>` と眉ピルに任せる。(2) **キャップ要約とカード tagline の語の重複**（同じ列で「APIキー不要」が3回、エキスパートは「無制限」「Xアカウント3件」も2回）。案A（暫定・現状）: 据え置き／案B: `capSummary` を「なぜその値段か」の1点に絞る（プレミアム「APIキー不要」のまま、エキスパートは「Xアカウント3件まで」だけ）。(3) **非エンジニアに引っかかる語**（`plan-comparison.ts` 由来）: 「AIクレジット 100,000」は多いのか少ないのか判断できない、「契約期間ごと」は仕様書の語。案: 利用上限セルで `aiCredits×0.01` から「（約1,000円ぶん）」を導出して添え、「契約期間ごと」→「1か月ごと」。LP・`/plans`・設定＞課金に同時に効くので、決めたら別タスクで起票。

**D-55: 運営ダッシュボードの指標定義（2026-09-04 の監査で浮上・T-M8-422 の残り・**決定 2026-09-04: (1) 運営者「割引も加味」→ T-M8-426 で反映。2026-09-05: (2)〜(4) は運営者「適切と思われるものに」→ Claude の判断で T-M8-427（粗利は月末見込み＋これまで・`cancellations`→`cancel_intents`・投稿は posted 件数・最終利用は3源の最大）**）** — (1) MRR が引き止めクーポン（50%×3か月）とキャンペーン価格の改定を無視する。案A: profiles の割引列を掛ける（暫定）／案B: 契約同期で実請求額を写す。(2) 粗利は「月額の走り高 − 月初からの累積原価」で月末に向けて減って見える。案: 「今月これまで」と「月末見込み（日割り）」を並べる。(3) `kpi_daily.cancellations` は「解約手続きへ進んだ」件数（引き止めで残った人を含む）。案: `cancel_intents` に改名し、実解約は status の遷移で数える。(4) 利用者別の「投稿」はツイート単位（スレッド1本＝N件）、「最終利用」は BYOK だと投稿しない限り「—」。案: 投稿は `drafts.status='posted'`、最終利用は generation_jobs も見る。

**D-52: staging で定時処理（cron）をどう動かすか（2026-09-01・運営者の質問「stagingではなぜcronが動かないのか」・決定 2026-09-01: 運営者「stg環境のcronは不要」＝案C 現状維持。必要なときだけ `STAGING_CRON_SECRET` で手動で叩く。起票なし）** — Vercel Cron はプロジェクトの **production デプロイに対してだけ**起動する（Vercelの仕様）。staging は同じプロジェクトの preview 環境（`stg` ブランチ）なので、`vercel.json` の4本は一度も呼ばれず、投稿実績・フォロワー数・ニュース取得・予約投稿がすべて「手で叩かないと動かない」状態になる。いまは `STAGING_CRON_SECRET` で `/api/cron/*` を手動で叩く運用（deployment.md §6）。
- 案A（推奨）: GitHub Actions の `schedule` で staging の4エンドポイントを本番と同じ間隔で叩く（`STAGING_CRON_SECRET` は Secrets に置く。追加費用なし・プロジェクトは1つのまま）。**デメリット**: staging でもニュース取得のAI費用（月$1〜3）とX読取が本番同様に発生する。使わない期間は workflow を止める運用になる。
- 案B: staging を別の Vercel プロジェクトにして `stg` をそのプロジェクトの production ブランチにする（Vercel Cron がそのまま動く）。**デメリット**: 環境変数47件・ドメイン・Stripe webhook を二重管理する。
- 案C: 現状維持（必要なときだけ手動で叩く）。**デメリット**: staging の投稿分析・フォロワー推移は検証できない。
- 決めたら `/add-task` で起票する（案Aなら workflow 1本＋deployment.md §6 の更新）。

**D-51: ai_newinfo（アクティブ）のアカウント.mdが空——旧アカウント行の内容を復元するか（2026-08-31 本番スモークで検出・**決定 2026-09-04: 運営者「はい」＝案A。同日 Claude が本番へ復元済み**: 旧行（39ea75b7…・expired）の本文411文字と5項目の設定（persona／themes／tone／ng）をアクティブ行（41d55b82…）へ写し、本棚に「既定」を1件作って使用中にした〔`base_md_version` 0→1。画像プロンプトは新行に既にあり触っていない〕。Supabase Management API の SQL で、アクティブ行の本文が空のときだけ書く条件付き。復元後の検証: 両行とも本文411文字・設定あり・使用中の版が本文と一致）** — 本番の実物スモークで「週次まとめ」生成だけが2回連続で失敗（AIの返答:「発信定義書の内容が提供されていないため、ペルソナ・発信テーマを特定できず生成できません」）。調べると**アクティブな ai_newinfo のアカウント.mdは0文字**で、**期限切れの旧 ai_newinfo 行には411文字のアカウント.mdが残っている**（再連携で新しい行が作られ、アカウント.mdが引き継がれていない）。題材を自力で決める型（週次まとめ）は、アカウント.mdも入力も無いと分野を特定できず止まる。**本日の変更（T-M8-385〜387）とは無関係**の既存事象。
- 案A（推奨）: 旧行の411文字を確認のうえアクティブ行へ復元する（内容が古ければ設定画面で書き直す）。運営者が「はい」と言えばこちらで復元作業まで行う。
- 案B: 空のまま使う。※2026-09-01のプロンプト刷新後、週次まとめはWeb検索単独で成功するケースを本番canaryで確認（毎回の保証ではなく、ペルソナに根ざした品質も出ない。復元推奨は変わらず）。
- あわせて**再連携でアカウント.mdを引き継ぐべきか**は仕様判断なので、必要なら別タスクで起票する。

**D-50: 下書きの画像アップロードの上限をどうするか（セキュリティレビュー 2026-08-29・決定 2026-08-29: 運営者が案Aを選択→T-M8-367で実施済み）** — 画像アップロード機能（T-M8-353）はコード上「入力20MBまで」を謳うが、**Next.js Server Action の本文サイズ上限は既定1MB**で、`next.config` で上げていない。そのため**1MBを超える画像は、コードの検査に届く前にフレームワーク層で失敗する**（利用者には汎用エラーだけが出る）。ふつうのスマホ写真は数MBあるので、いまの実装は**小さい画像でしか使えない**。
- 案A（推奨・要確認）: `next.config` の `serverActions.bodySizeLimit` を `6mb` にし、`UPLOAD_MAX_INPUT_BYTES` も 5MB へ揃える（Xの上限が5MBのため、それ以上受けても圧縮で落ちる）。**デメリット**: すべての Server Action が受け付ける本文が1MB→6MBに広がり、**DoS（大きな本文を大量に送る攻撃）の余地が増える**。ただし全 Server Action は認証必須で、未認証の入口は増えない。
- 案B: 現状維持（1MB）。**デメリット**: アップロード機能が1MB以下の画像でしか使えず、多くの写真で汎用エラーになる。せめて画面の上限表記を「1MBまで」に直して誤解を防ぐ。
- どちらも利用者の見え方が変わるため、運営者の判断を仰ぐ（案Aを推奨）。

### T-M8-328: プロンプト管理を独立した画面へ移し、ナビを組み直す `done`
- 参照: 要件06 §3 / `src/app/app/prompts/page.tsx` / 依存: なし / サイズ: L
- 完了条件:
  - `/app/prompts` でアカウント.md・投稿の型・画像生成を編集できる
  - ナビが「ホーム／最新ニュース／投稿作成／スケジュール／投稿分析／プロンプト／友達招待」
  - ヘッダーが無くなり、アカウントアイコンから設定タブとログアウトが開く
  - お知らせがナビ内から開ける
  - 狭い画面でも同じことができる
- メモ: 運営者の指示（2026-08-27）。**「設定 > プロンプト」を移設しつつUIも洗練させる。**
  `docs/improve_list.md` の4項目（公開トグル・ブックマーク等）は別タスク。
- 実装メモ（第1段階・2026-08-27）:
  - `/app/prompts` を新設。**編集部品は設定側のものを再利用**（複製すると片方だけ直る）
  - 見出しと説明を追加し、状態別の案内（プラン外／Xアカウント未連携／アカウント.md未保存）へ導線を付けた
  - ナビへ「プロンプト」を追加し「設定」を削除。アイコンは `smart_toy`
    （公開ページの「プロンプト集」が `drafts` を使っているため区別。重複はテストが落とす）
  - 設定からプロンプトタブを削除。**旧slugはエイリアスで転送**（保存済みの通知リンク・ブックマーク対策）
  - **アプリ内の旧リンク5箇所**（persona-settings-form・ai-settings・suggestions-panel ほか）も新URLへ。
    リンク先を検査する `tabs.test.ts` のパターンも追随させた
  - E2E 8件が落ちて検出（6件は移設先URL、2件はクレジット桁区切り）。**全21件緑に戻した**
- 実装メモ（第2段階・2026-08-27 完了）:
  - **ヘッダーを廃止**。画面名は各ページの `h1` が担うので情報は失われない
  - サイドバー下部へ **お知らせ**（未読バッジつき・`NotificationBell` に `sidebar` 表示を追加）と
    **アカウント**（`AccountMenu` 新規）を置いた。メニューは Xアカウント切替／設定の各タブ／ログアウト
  - **設定リンクは `SETTINGS_TABS` から生成**（`ACCOUNT_MENU_SETTINGS_LINKS`）。
    タブを増減したらメニューも自動で追随する（書き写さない）
  - Server Action は props で受け取る（App Shellの依存方向・`dependency-boundaries.test.ts`）
  - モバイルは下部バーを2段に。上段=ロゴ・お知らせ・アカウント、下段=ナビ7項目
    （ナビは7枠が上限なので同じ行に入れない）。本文の下余白を `pb-20` → `pb-36` へ
  - **実ブラウザで1440px・390pxを確認**（コンソールエラーなし）
  - `e2e/app-shell.spec.ts`（新規）で**到達できること**を固定——ヘッダーが無い／ナビに
    プロンプトがあり設定が無い／お知らせが押せる／メニューから課金・プランとログアウトへ行ける
  - **E2E 5件が落ちて検出**（ログアウト・通知ラベル・切替後の表示）。位置を直接知る書き方をやめ、
    `openAccountMenu()` を fixture へ1本化した——次に置き場所を変えても各specを書き換えずに済む
- **積み残し（判断が要る）**: モバイルのナビラベルが窮屈（7項目・390px幅で2行折り返し）。
  以前から同じで悪化はしていないが、気になるなら**プロンプトをモバイルでは隠す**などで緩められる。

### T-M8-372: 「反映は不要です」と言いながら差出人アドレスを直さないのを直す `done`
- 参照: 運用（メール）/ 依存: T-M8-339 / サイズ: S
- 完了条件: doctorの案内どおり実行すれば差出人アドレスが直る／直せないのに「不要」と言わない
- 実装メモ（2026-08-29 完了）:
  - doctorは「確認メールの差出人が `matsubuz.10@gmail.com` です」と ❌ を出し、
    `npm run auth:templates -- --apply` を案内する。ところが**そのコマンドは
    「✅ 反映は不要です」と出して exit 0 する**——判定が「テンプレートの差分」だけで、
    差出人アドレスは ❌ を出すのに `patch` へ入っていなかった。**直す経路がどこにも無く、
    運営者は案内どおり実行しても同じ警告を受け取り続ける**（原則2違反）。
  - 「直すものが1つも無い」ときだけ「不要」と言うようにし、差出人アドレスを直す経路を足した。
  - **前提**: 送信側でそのアドレスを名乗れること。Gmailは**許可の無いFromを拒否せず黙って
    書き換える**（250 OK を返す・T-M8-342）ので、`npm run check:mail-sender -- --send` で
    受信箱のFromを人が見てから切り替える。名乗れないまま切り替えると確認メールが届かなくなる。
  - スクリプトなので単体テストは無い。dry-runで「差分があります」と出ることを実際に確認した。

### T-M8-371: 「契約の同期」が無音を異常として毎日warnを出すのをやめる `done`
- 参照: 監視・アラート（運用） / 依存: T-M8-238 / サイズ: S
- 完了条件: 契約に動きが無いだけの状態でwarnが出ない／本当に反映されていない契約は検出する
- 実装メモ（2026-08-29 完了）:
  - 本番のdoctorが「最後にStripeのイベントを受け取ってから83時間」でwarnを出していた。
    中身は**トライアル中の契約が1件あり、まだ何も起きていない**だけ。Stripeのイベントは
    契約に動きがあったときにしか来ないので、利用者が数人のうちは**毎日かならずwarnになる**。
    運営者の朝のメールに毎日出る警告は**本物の異常を埋もれさせる**（原則1の趣旨）。
  - 判定を「経過時間」から**ずれている証拠**へ変更: 更新日・トライアル終了日を6時間過ぎたのに
    その後のイベントを1件も受け取っていない契約の件数。0件なら無音でも ok。
  - あわせて「イベント0件」もwarn→okへ（契約に動きが無ければ正常）。

### T-M8-411: 「アカウント設定を保存」でアカウント.mdを本棚の一番下に1件追加して使用中にする `done`
- 参照: 要件06 §3.7（アカウント.md） / 要件05（`updatePersonaSettings`） / 依存: T-M8-400 / サイズ: S
- 完了条件:
  - プロンプト＞アカウント.md で「アカウント設定を保存」を押すと、本棚の**一番下**に生成したアカウント.mdが1件追加され、それが**使用中**になる（前の使用中は控えとして残る）
  - 本棚の並びは作成順（古い→新しい）。使用中はバッジで示す
  - 本棚が上限に達しているときは、失敗させずに使用中の1件を書き換え、その旨をトーストで言う
  - 保存後に画面を作り直しても新しい1件が一覧に出ている（実DBテスト＋E2E）
- メモ: 運営者の指示（2026-09-01）。T-M8-332 以来「使用中の1件を書き換える」動きだったものを、「1件追加して使用中にする」へ。名前は `アカウント設定 vN`（N＝base_md_version）。
- 実装メモ（2026-09-01 完了）:
  - `addPresetAndSetInUse`（prompt-preset-sync.ts）: 使用中を外して1件追加（`created_at=clock_timestamp()` で作成順を決定的に）。
    上限5件なら `syncInUsePreset` で使用中を書き換え `added:false` を返し、フォームのトーストがその旨を言う。
  - 本棚の並びを `created_at` 順へ（使用中先頭をやめた）。プロンプト画面の `PromptPresetManager` は `base_md_version` を key に含め、
    保存後の `router.refresh()` で作り直して新しい1件が出る。
  - 学習反映（md_merge）の書き戻しは従来どおり使用中を書き換える（保存とは別経路・削除時の確定に使う）。

### T-M8-412: 最新ニュースのテーマ・インパクトを複数選択式にする `done`
- 参照: 要件06 §2（SC-06） / 依存: なし / サイズ: S
- 完了条件:
  - SC-06 のテーマ・インパクトが**複数選択**できる（例: テーマ2つ＋インパクト2つを同時に選べる）
  - 選択した値の**いずれかに一致**する記事が先頭へ集まる（現行どおり**ソートであって絞り込みではない**——記事は消えない）
  - 選択状態はURLに載り、リロード・共有・ページ送りで保たれる（複数値の表現も含む）
  - 全解除で新着順に戻る。不正なURL値（存在しないテーマ等）は無視して落ちない
  - 選択UIは通知設定と同じ**チップ型**（選択中はチェックアイコン併記・色だけに依存しない）に揃える
- メモ: 運営者の指示（2026-09-02）「選択は複数選択式に。必要があればトグルのUIも変更して良い」。現行は `<select>` の単一選択×2（news-browser.tsx・T-M8-188）。**ソート意味論（選んだ値が先頭・記事は消えない）は維持する暫定判断**——変えるなら要決定。並び替えの実体（`lib/news-items` のクエリ）も複数値対応が要る。UI変更なので `/ui-polish` を通す。
- 実装メモ（2026-09-03 完了）: themes[]/impacts[] のzod配列＋`= any($::text[])` ランク（重複は畳む・空配列=選択なし・単数キーは弾く）。URLは `?theme=…` の繰り返し（カンマ手打ちも受理・未知値は捨てる）。UIは `ChipCheckbox` を components/ui へ共通化（設定＞通知も同部品へ差し替え）。**連続タップで前の選択が黙って消えるレース**（サーバー再描画前に古いselectedからURLを組む）を、ローカル状態の即時合成＋props同期（レンダー中比較）で修正。E2Eはfieldsetのグループ名でチップをスコープし、複数選択・全解除まで固定。

### T-M8-413: 主要画面のスマホUIを最適化する（セルフレビュー3周） `done`
- 実装メモ（2026-09-03 完了・レビュー3周の記録）:
  - **点検方法**: 一時spec（削除済み）で主要13画面を390px・フルページ撮影し、原則（タップ44px・横あふれ・親指圏・入力16px・ラベル可読・全幅CTA）と照合。横あふれと入力16pxは既存の `mobile-layout.spec.ts` が機械で保証済み。
  - **1周目の指摘→対応**: (1) 下部ナビのラベルが「ニュー/ス」等に折れて読めない→ `mobileLabel`（ニュース/作成/予定/分析/招待）＋nowrap・tracking-tight (2) ホームKPI 4枚の縦積みで画面2つぶん→モバイル2×2 (3) プロンプトのタブ見切れ→既存の意図的な横スクロール（T-M8-401）と確認し**変更せず**。
  - **2周目の指摘→対応**: (4) アカウント設定の保存ボタンをスマホ全幅（`w-full sm:w-auto`）へ (5) テーマのチェックボックスのタップ領域が狭く見えた→実装は `min-h-11`（44px）で**問題なしと棄却**。
  - **3周目**: 投稿作成・友達招待・ニュース・設定を新しい目で再点検し追加指摘なし。ボトムバーの safe-area 対応（`pb-[env(safe-area-inset-bottom)]`）は実装済みを確認。
  - 見送り（要望があれば別タスク）: 投稿作成の生成CTAのsticky化（下部ナビと干渉するため設計が要る）・LPの文章量。
- 参照: 要件06 §1（画面共通）・e2e/mobile-layout.spec.ts / 依存: なし / サイズ: M
- 完了条件:
  - 一般的なスマホUIの原則に照らした点検表を作り、主要画面（mobile-layout対象13画面＋LP）を390px幅で点検した記録が残る（原則の例: タップ領域44px以上・横あふれ無し・主要操作が画面下部から届く・入力に適切なキーボード型・文字サイズと行間）
  - 点検で見つかった問題をすべて修正し、**セルフレビュー→改善を最低3周**回した記録（各周の指摘と対応）が残る
  - 修正後も `npm run test:e2e`（mobile-layout含む）が緑
  - 実ブラウザ（390px）で主要フローを通しても横スクロール・操作不能・重なりが無い
- メモ: 運営者の指示（2026-09-02）「一般的なスマホUIの成功法則から逆算。セルフレビューして改善を回す。最低でも3回」。過去のスマホ系不具合: T-M7-36（sr-onlyで横スクロール）・T-M8-365（ニュースのgrid潰れ・break-words）・D-50（スマホ写真がServer Action 1MB上限で弾かれた）。修正は振る舞いを変えるものが出たら個別にテストを付ける。`/ui-polish` を通す。

### T-M8-414: 画像生成のデフォルトプロンプト（PT-IMG）を「インパクト×内容説明」型へ改訂する `done`
- 参照: プロンプト設計書 §6.8（PT-IMG）・§4（GEN-IMG） / 依存: なし / サイズ: S
- 完了条件:
  - PT-IMG が「タイムラインで目を引くインパクト」と「本文の内容が画像だけでも適切に伝わる説明力」を両立する指示になっている（現行は「既定は文字なし・単純さ優先」で説明力が弱い）
  - 生成される画像プロンプトが、本文の主張・数字・構図の焦点を含む形になることを実物で確認した（`smoke:live` の画像1周）
  - プロンプト設計書 §6.8 の全文・意図の記述が新PT-IMGと一致している（公開ページ /prompt-templates は同一定数から描画されるため自動で追随）
  - 実在人物・ロゴ等の禁止、文字の扱い（言語・文字数の上限）の安全則は維持されている
- メモ: 運営者の指示（2026-09-03）「インパクトがありつつ、内容を適切に説明できるような画像を生成するデフォルトプロンプトに」。実体は `src/lib/prompts/gen-prompts.ts` の `PT_IMG`（2段階の1段目・mechanicalモデルで実行）。プロンプト変更なので検証は CLAUDE.md の表どおり `npm run check:providers`＋`npm run smoke:live`（実費 約$0.30・上限$0.50）。
- 実装メモ（2026-09-03 完了）: PT_IMGへ構図の型4種（主役ドン／対比／ミニ図解／数字ドン）＋インパクト操作子（焦点1つ・高コントラスト・主役1/3以上）＋「縮小表示でも何の話か分かるか」の自問を追加。数字のみ最大4文字を許可（文字なし既定・12文字上限・鉤括弧明示の安全則は維持）。設計書§6.8同期・snapshot更新。smoke:live（--account はUUID指定。ai_newinfo同名2件で名前指定は弾かれる）で生成＋画像1周成功（$0.0975）。実費合計約$0.21（1回目のUUIDは途中失敗$0.11込み）＝上限$0.50内。

### T-M8-415: LPをコンバージョン改善へ再設計する（実画面スクショ・訴求文・セルフレビュー5周） `done`
- 実装メモ（2026-09-03 完了・レビュー5周の記録）:
  - **1周目（自己・3幅キャプチャ）**: CSS図版5枚→実画面スクショ4枚＋分析はCSS（→枠だけ統一）。ヒーローをHeroMock→実画面へ。信頼ストリップ新設（下書きから/投稿前確認/Stripe）。本文短縮。指摘: 分析カードだけ浮く→ShotFrameで統一。
  - **2周目**: ヒーロー列比率変更でh1が「上/がる」と途中改行する退行→即時リバート。
  - **3周目（別レビュアー・7/10）**: (1)TBでコンセプト図の文字欠け→中央ラベルを常時3行折返しへ (2)ヒーロー画像がフォロワー減とdevバッジを見せていた→sharpで下端140px切除＋ヒーローは投稿作成の上部クロップへ差し替え (3)5連同型カードの単調→偶数カード反転 (4)料金の走査性→「違いはAPIキーと上限だけ」1行 (5)FAQ 2,400pxの壁→PC2カラム＋手順2へ解約無料を追記。分析カードも実ホーム画面へ（5枚全部実画面）。
  - **4周目（自己）**: 全反映を3幅で確認。ページ全長 約10,000→9,400px。
  - **5周目（別レビュアー・判定「公開可」・マーケ7.5/デザイン8）**: ブロッカー無し。軽微5件中3件を反映（FAQ誤字「初めて→始めて」・循環図の狭幅フォント10px・料金「／月（税込）」のnowrap統一）。見送り2件は下記。
  - **見送り（要望があれば/add-task）**: 虹色グラデ3pxlíne（ブランド演出として意図的・テストで本数固定）／880px未満のセクションナビ（ハンバーガー導入は別設計）／FAQ「失った時間…」の文言柔らげ（運営者の声のため据え置き）／料金カード共通行の淡色化（共有部品につき影響広）。
  - **/plans**: 未ログインは/loginへ送られる仕様のためLPに集中（plans自体はカード型で現状維持。「／月（税込）」nowrapは共有部品経由で/plansにも効く）。
  - 画像: docs/screen_shot の運営者提供5枚を sharp で下端切除・1600px幅・JPEG82へ最適化し public/lp-shots/ に6ファイル（計約750KB）。E2E: landing 4/4・mobile-layout 4/4・全vitest 2,738緑。

### T-M8-416: モバイルのヘッダー（ロゴ・お知らせ・アカウント）を画面上部へ移す `done`（sticky上部ヘッダー化・下部はナビのみ・pb-36→pb-24。390pxで固定/スクロール両状態を実機確認）
- 参照: 要件06 §2（App Shell・T-M8-328） / 依存: なし / サイズ: S
- 完了条件:
  - モバイル幅で、ロゴ・お知らせ・アカウント切替が**画面上部の固定ヘッダー**に出る（現在は下部バーの上段＝「ヘッダーが変な位置」・運営者の指摘 2026-09-03）
  - 下部バーは7タブのナビだけになり、本文の下余白がバー1段ぶんに縮む
  - 390pxで横あふれ・重なりが無く、`mobile-layout` E2Eが緑
- メモ: T-M8-328で「ヘッダー廃止→下部バー上段へ」としたが、運営者が上部へ戻すと判断。デスクトップ（サイドバー）は変えない。

### T-M8-417: LPの微修正（スクショの枠ずれ・信頼ストリップ削除） `done`（「ずれ」の実体＝下端で行・カードが中途半端に切れて見えること〔原本に余白なし・トリム0を確認〕→ShotFrameに下端フェードを追加。ストリップ削除済み）
- 参照: 要件06 §1.5（SC-01） / 依存: なし / サイズ: S
- 完了条件:
  - スクショの枠（ShotFrame）と画像の位置ずれが解消されている（原因を特定して修正）
  - 「まず下書きから」「投稿前に確認・編集」「決済はStripe」の信頼ストリップを削除（運営者の指示 2026-09-03）
  - landing E2E・landing-page.test 緑
- メモ: ストリップ削除に伴い§1.5の記述と変更履歴も更新。開示の正本（帯・FAQ・特商法）は変わらないため法令面の影響なし。

### T-M8-418: LP微修正2（図版上のブラウザ風バー撤去・FAQを1カラムへ） `done`
- 参照: 要件06 §1.5（SC-01） / 依存: T-M8-417 / サイズ: S
- 完了条件:
  - 図版の枠上部のドット3つ（ブラウザ風バー）が無くなり、角丸カード＋影＋下端フェードだけになる
  - FAQが全幅で2カラムにならず、縦1カラム（最大幅840px）で並ぶ
  - landing E2E・landing-page.test 緑
- メモ: 運営者の指示（2026-09-03）「画像の上についている部分も不要」「Q&Aは縦に並べたほうが良い」。FAQ 2カラムはT-M8-415レビュー3周目の変更を運営者判断で差し戻し。

### T-M8-419: 新LPを /new にゼロベースで作る（「AIクローン」コンセプト・参照 noimosai.com 水準のリッチさ） `done`（/ への差し替えは T-M8-420）
- 実装メモ（2026-09-03 完了・ultracodeワークフロー15エージェント）:
  - **Understand**: 参照サイトを実ブラウザ撮影し「リッチさの仕掛け」19件を抽出／再利用部品23件／主張の事実確認17件（「完全自動化」「サイクルを自動で回す」は partly——投稿分析は「分析を開始」ボタン起点・1日1回・表示専用。プロンプト改善は提案表示で反映は利用者）。
  - **Design**: 3軸（クローンの物語／証拠先行／時間が戻る）→2審査→統合。採用は「証拠先行」土台＋「時間が戻る」の一撃。タイトルは運営者文言を一字一句そのまま、サブは事実へ寄せて「集める→作る→記録は自動で回り、投稿は同意後に自動。分析はボタン1つ。反映するかは、あなたが決める。」
  - **Implement**: `src/app/new/page.tsx`（noindex）＋ `src/components/lp-new/`（tokens/facts/hero-diagram/loop-board/tour/control/grow/text-cards/faq/clone-mark/module.css）＋ `public/lp-new/hero-{patterns,schedule,news}.jpg`（既存スクショから sharp で切り出し・`scripts/lp-new-crops.mjs`）。全 server component・出現演出なし・外部リソースなし。
  - **Review→Fix**: 4視点（マーケ6.5／デザイン6／法令7／堅牢性7）でブロッカー10件・改善48件→全ブロッカー反映（分析の「自動」表記を「AI・押すだけ」へ・ホーム画面の弱い実データを tour から外す・背景を fixed 化・和文の折れ対策・768pxの3カラム崩れ・工程帯の突き抜け・スロット時刻を9:00〜22:00に・AAコントラスト）。8幅で横あふれ0・コンソールエラー0・JS無効でも可読。
  - **未実施**: `npm run build`＋`check:csp-nonce`＋E2E全体はCIで（push時）。`/new` は mobile-layout の巡回へ追加。docs は要件06 §1/§1.5 へ追記。
  - **運営者へ**: 素材要望リストと要決定（D-53）は報告に記載。
  - **2周目（2026-09-03・運営者の指示＋セルフレビュー）**: 「勝手には投稿しない」→「複数のプロンプトを管理」（`lp-new/prompts.tsx`: 型を何個でも〔`PATTERN_MAX_COUNT`〕／アカウント.mdは版で残す／画像とAIモデルもここで）・ヘッダーnavへブログ／プロンプト集（遷移マーク付き）・セクション余白 120→96px・ツアー各停止 90vh→72vh（PC全長 12,859→11,867px）。3幅で横あふれ0・エラー0・型スケール検査緑。
  - **3周目（2026-09-03・運営者の指摘「不自然な動き・分かりにくいデザイン・散らかった配置・冗長な文言」）**: ヒーローの板の傾き（±1deg）・上下浮遊・生成バーの動きを撤去して水平に整列／同心円「クローンの印」をヒーロー・図面板・育つ・最終CTAから全撤去（`clone-mark.tsx` 削除）／記法チップ「AI・押すだけ」→「ボタン1つ」／「クローンと呼ぶ理由」「安心して任せるために」「利用している技術」を削除（`text-cards.tsx` 削除）し、鍵の暗号化・書き込み範囲はFAQ1問へ集約／最終CTAを1カラム中央（自動4/AI1/あなた2の集計を撤去）／ヒーロー・図面板・ツアー・料金・育つの各サブ文を短縮、FAQ見出しを「よくある質問」に。PC全長 11,867→10,804px・SP 15,844→13,448px。背景ブロブの緩やかな漂い（9〜13秒）だけは残した（reduced-motionで停止）。
  - **4周目（2026-09-04・運営者の指示）**: (1) 図面板「手を動かす時間が、2h 15m → 5m に」のBefore/Afterタイムラインが分かりにくい→**同じ原点から伸びる2本の比例バー**（手作業3h＝探す45／書く60／画像30／投稿15／数字を見る30の5区切り・Exos AI 5m＝確認の白ピル1つ。長さ＝時間。合計は reduce で `LOOP_TOTALS`＝{3h,5m}、`formatMinutes` の「3h 0m」を修正し `loop-board.test.ts` で固定。760px未満は番号＋凡例）。(2)(3) 画面ツアーの動きの違和感（article ごとのsticky画像: 止まる区間が29%で「引っかかって離れる」・2枚同時に流れる・最後だけ挙動が違う・停止距離がビューポート高依存）→**sticky を画像から見出し側へ移す**（FAQと同型の2カラム。左列＝h2・サブ・索引〔hairline行〕・「料金を見る」文字リンクを `sticky top-[88px]`〔高さ520px未満は静的〕、右列＝4停止を静的縦積み。画像幅 640→739px、帯は「番号＋見出し＋チップ」1行、停止01の分野チップ撤去で4停止が同じ高さ）。実測 1280×900: h2 は停止01〜04の間 y=88 に留まり、停止04の画像下端と一緒に流れ去る。880×720／1024×768 も同様、390 は静的、1280×500 は静的（条件付き sticky）。(4) 料金→**推奨先行**（`lp-new/pricing-recommend-first.tsx`＋module.css＋test。共通部品不変。見出し「迷ったら、プレミアムプランから。」、キャップ行で1日あたりを主役に、推奨帯「まずはこれ・いまだけ半額」〔`RELEASE_CAMPAIGN.active` 連動〕＋`TRIAL_NOTE` を最初のCTAより上に、両隣はガラス＋要約行〔BYOKは「API利用料は別」〕、帯はカードの下、SPは推奨帯にCTAと「含まれる機能を見る」〔`:target` ハロー〕）。3幅で横あふれ0・コンソールエラー0、typecheck／lint／型スケール検査緑。**未実施**: `npm run test:e2e`（push前にまとめて）・WebKit での sticky 実測（Playwright に webkit 未インストール）。
  - **5周目（2026-09-04・セルフレビュー1周目〔novice／designer／robustness〕の反映）**: ブロッカー2件＋改善19件のうち16件を反映。(1) 図面板: After の白ピルが「点・カーソル」に見えたため **同じ尺の薄いトラック（white/10）を敷き左端に白い点**（棒は h-7/h-8・点の最小幅＝高さで正円）、640px未満の行注記は非表示、figcaption を1文に短縮、サブ文「手を動かすのは、下書きを見る5分だけ」（`LOOP_TOTALS.afterJa`）、「画像」→「画像作り」、リング図の注記を平易に（「あなた／飛ばしてもOK」「反応を毎時／1・7・30日後も」「あなたが選ぶ」。note は行配列で、一番右の「確認」は1行だと640〜1024pxで板の右端を最大28px越えたため2行）、「作る」のグラデ輪と gradient defs を撤去、上下2段を板内の罫線で切り、リングの下の空き119px→約70px（負の margin＋pb 24）。(2)(3) ツアー: 左列の「料金を見る」文字リンクを削除し CtaRow を `Tour` の `cta` prop で右列末尾（停止04の直下）へ——sticky が外れる瞬間に同名リンクが2つ並ぶ違和感を消す。sticky の高さ閾値 520→560（左列 実測 354〜371px＋88）。停止03「出す」→「投稿」（リング図の工程名に揃える）、停止02の note を「スタンダードプランは自分のAPIキーで使います」へ、停止04本文を「反映した改善案は…残っていく」（リングの「反映」と結ぶ）。(4) 料金: キャップをカードに**密着**（全幅。カードの上の角丸・両隣の上辺罫線・推奨の「おすすめ」バッジを module CSS で消す。実測 1280／768／390 で caps bottom＝cards top）、**SPは推奨カードを先頭**（`order:-1`・帯の直下に来る。DOM順据え置き）、SP帯のCTAと「含まれる機能を見る」・`:target` ハロー・`planCardAnchor` を削除、キャップの月額は SP のみ・カードの「1日あたり」行（4番目の p）は消す（列ごとの数字 5→3）、eyebrow の「・いまだけ半額」を削除し /new 側はキャンペーン文言を持たない（test で `半額|キャンペーン|RELEASE_CAMPAIGN` 不在を固定・p の並びと SP order のセレクタも固定）、両隣のキャップにプラン名を sr-only で、サブ文1文目に主語「プレミアムプランなら、」。採らなかった提案: 半額チップ＋取り消し線をキャップに足す（D-53(1) 未決・重複が増える）／After の note を「下書きを見る＋改善案を選ぶ」（分析は日次でないため誤り）／`hero-diagram.tsx` の `priority`（対象外ファイル。コンソールの LCP 警告は dev のみ）／スケジュール画面の撮り直し（素材）。検証: typecheck／lint／vitest（ui・landing-page・lp・lp-new 13 files 59 tests）緑、1280／768／390 で横あふれ0・エラー0（警告は sentry DSN と LCP の dev 警告のみ）、JS無効で img 10/10・全長同一、tour 12フレームで左列は y=88 に留まり CtaRow と一緒に外れる、1280×560 sticky／×540 static。
  - **6周目（2026-09-04・セルフレビュー2周目〔novice／designer／robustness〕の反映）**: ブロッカー1件＋改善20件（うち「対応不要」1件）のうち17件を反映。(1) 図面板: After の「薄いトラック」が **同じ長さの棒2本** に見え、白い点が bullet に読めた（ブロッカー）→ トラックを **目盛り線（1px・white/30）** に替え、白い点の直後に和文の「5分」（`formatMinutesJa`）＋「下書きを見るだけ」（線はラベルの後ろも右端まで通し、ラベルが板の色で線を切る。「確認」は左列の注記だけに1回）。見出し「3h → 5m」は aria-hidden にして sr-only「3時間から5分に」（`LOOP_TOTALS.beforeJa` 追加・test 更新）。figcaption「あとの工程は Exos AI が回します（仕組みは下の図）」。注記「同意後・定刻から…」→「自動投稿ONのとき／定刻から概ね5分以内」、「反応を毎時」→「表示回数などを毎時」、本文の「同意」「表示専用」を「あなたがオンにした後」「結果を見るだけで、勝手には反映しません」へ。自動ノード・凡例チップの輪郭を ring-1 white/35 → ring-2 white/60（暗色面で brand 塗りが最も沈んでいた）。960px以上は左列を `items-center`。SPの縦リストで「投／稿」「記／録」が縦に折れたので工程名を `shrink-0 whitespace-nowrap`。(2)(3) ツアー: 停止03本文の「枠ごとに」→「予約した時間ごとに…を選べる。次に動く時刻も表示」、停止04「育てる」→「反映」（リング図の7と同じ語）＋「アカウント.md（あなたの発信方針のメモ）」、索引の行末に chevron・hover で行全体が白60%（-mx-2/px-2）、停止03のスケジュール画面（元画像の下35%が空白）を `shotClassName` で `max-h clamp(240px,30vw,400px)`・object-top（1280で 505→384px）、「画像を拡大して見る」に sr-only「（新しいタブで開く）」。(4) 料金: 推奨だけ 40px/extrabold・両隣 28px/bold、「まずはこれ」を text-sm 白、SP限定の月額行を削除（`order:-1` で推奨カードが直下に来るため二重だった。SPの「／月（税込）」4→3回）、推奨の帯にだけ「月額（税込）÷30日の目安」（換算の根拠。3枚に並べると同じ行が3回並ぶので1回）、帯の sr-only に「（この帯の直下のカード）」（SPは表示順とDOM順が異なる割り切り——共通部品を触らずに並べ替える手段が無い。D-53 へは足さない: 運営者が決める性質の事項ではない）。採らなかった提案: `formatMinutes` を "5min" に（運営者指定の表記「3h → 5m」を優先。5分の読みは点の隣の「5分」とサブ文が担う）／SPの `order:-1` を外して帯に可視のプラン名（帯とカードの密着が崩れ「まずはこれ＝¥1,480」の誤読が戻る）／換算の注記を3枚全部に（重複）／両隣にも月額（カードが直下）。検証: typecheck／lint／vitest（13 files 59 tests）緑、12幅（360〜1440）で横あふれ0・console error 0・失敗リクエスト0、tour 12フレーム（instant scroll）で左列 y=88 に留まり CtaRow と一緒に外れる、1280×700／880×700 sticky・1280×559 static、JS無効で画像10/10・非表示要素0。
- 参照: 要件06 §1.5（SC-01。新ページは別ルート /new として先行公開・noindex） / 依存: なし / サイズ: L（別ページなので既存LPを壊さない）
- 完了条件:
  - `/new` が公開・noindexで見られ、メインタイトル「日々のSNS活動を数時間 → 完全自動化。AIクローン生成プラットフォーム。」とサブタイトル（イメージ「使えば使うほどAIクローンが成長。投稿生成→分析→プロンプト改善→投稿生成のサイクルを自動で回す」を事実に合わせて調整）を掲げる
  - 参照サイトの「リッチさの仕掛け」を取り込みつつ、JS無効でも全文可読・出現演出なし・外部リソースなし（既存LPと同じ安全則）
  - 法令開示（カード登録・7日無料初回のみ・期間中解約無料・BYOK実費・特商法フッタ）と禁止表現の遵守（効果訴求は運営者がリスク受容済み。製品の仕組みの事実は曲げない）
  - 390/768/1280で横あふれゼロ・コンソールエラーゼロ・typecheck/lint/型スケール検査緑
  - 運営者に依頼する画像・アイコンの要望リストを報告に添える
- メモ: 運営者の指示（2026-09-03）。実装は ultracode ワークフロー（参照分析→設計コンペ3案＋審査2→実装→4視点レビュー→修正）。既存LP（/・src/components/lp）は触らない。承認後に / と /plans へ差し替える計画（/plans はログイン必須・Stripe連携のため別タスク）。
- 参照: 要件06 §1（SC-01 LP）・PRD（LP該当）・design_handoff_lp/README（禁止表現） / 依存: なし / サイズ: L（分割すると途中状態でLPが崩れるため一体で）
- 完了条件:
  - LPが「信頼性がありリッチ」な見た目になり、**実アプリのスクリーンショット**（運営者提供 docs/screen_shot 5枚＋必要なら自前撮影）を製品証拠として使っている
  - メッセージは訴求的だが「わざとらしくない」トーンで、**文字数は現行より減る**（シンプル化）
  - 禁止表現（効果の約束・利用者数・「自動で学習し続けて最適化」等）と法定表記（「初回のみ」・BYOK実費・特商法footer）を維持
  - /plans も必要な範囲で同じトーンへ改善
  - マーケター視点＋デザイナー視点の**セルフレビューを5周以上**回し、各周の指摘と対応の記録が残る
  - `npm run test:e2e`（landing/plans含む）緑・390pxと1280pxの実ブラウザ確認
- メモ: 運営者の指示（2026-09-03）。文言変更はE2E（landing.spec.ts・plans.spec.ts）の更新を伴う。スクショは public/lp-shots/ へ最適化して配置（サイズ注意）。ハンドオフの「一字一句変更禁止」は今回の運営者指示が上書きする（新文言も禁止表現リストは通す）。完了後に本番までデプロイ（運営者指示）。

### T-M8-409: 参考アカウントの反映が、アカウント設定が未保存のアカウントで必ず失敗する（PT-MD-MERGEの初回経路） `done`
- 参照: プロンプト設計書 §6.13（PT-MD-MERGE） / 要件04 §12（md_merge） / 要件06 §3.1 / 依存: なし / サイズ: M
- 完了条件:
  - アカウント設定が未保存（`settings={}`・`base_md_version=0`）のアカウントで「アカウント設定を反映する」を押すと、分析だけを材料に5項目の提案（`settings_proposal`）がフォームへ入る
  - 初回経路のプロンプトに**出力の型（全キー）・テーマの許容ID（`THEME_IDS`）・`emoji_policy` 等の列挙値**が明示され、AIが形を当て推量しなくてよい
  - 単体テストで「`<current>`が無いときの user プロンプトに型と許容値が含まれる」ことを固定し、実AIで1周（未保存アカウント→反映→提案が入る）を確認する
- メモ: 運営者の報告（2026-09-01 ローカル・`matsubuz.10@gmail.com`）。分析は6秒で成功したが md_merge が `MdMergeStructureError`（AI出力が `personaSettingsSchema` を2回とも満たさない）で失敗。原因は初回経路で `<current>` を `"none"` にしたまま「<current>と同じ形・キーを増減しない」「themes は <current> の値から選び直すだけ」と指示していること——形も許容値も渡していないので、新規利用者（この機能の主な対象）では**決定的に**失敗する。T-M8-344/349 の「未保存でも作る」はモックのAIでしか通っていなかった（実AI未検証・development-and-testing §9 の型）。
- 実装メモ（2026-09-01 完了）:
  - 未保存なら `<current>` に全キーの空の雛形（`firstSetupCurrent()`）、常に `<allowed>`（運用中テーマの id＋列挙値）、
    初回は `{{mode_note}}` で「persona と primary を分析から埋める」を指示。テーマは id で選ばせる。
  - 単体でプロンプトの中身を固定。**実AIで1周**: 運営者のローカルアカウント（未保存・参考アカウント1件）で md_merge を実行し
    10秒で提案が入った（persona 3項目・primary ["ai"]・トーン）。

### T-M8-420: 新LPを / へ差し替え、旧LPを /old（noindex）へ退避 `done`
- 参照: 要件06 §1（SC-01／SC-01b）・§1.5 / 依存: T-M8-419 / サイズ: S
- 完了条件:
  - `/` で新LP（H1「日々のSNS活動を完全自動化　AIクローン生成プラットフォーム」）が表示され、index許可・`recordPageView("/")` で入口ファネルの分母が数えられる
...
### T-M8-421: 新LPの文言・図の改善（運営者の指示 2026-09-04・9項目＋セルフレビュー3周） `done`
- 参照: 要件06 §1.5「新LP」 / 依存: T-M8-420 / サイズ: M
- 完了条件:
  - ヒーローの帯から「繰り返す」が消え、図面板の説明文（棒の長さ＝…）と見出しの句点が消えている
  - 図面板が白い板で、棒・点・リング図の記法がページの他所のチップと同じ色。640〜1280px でラベルが板の外へ出ない
  - 画面ツアーの見出しが「投稿から改善まで」、停止04が「改善」、サブ文「実際の管理画面です…」が無い。工程名「改善」は帯・リング図・ツアーで一致（テストで固定）
  - プロンプトのカード2・3の見出し・本文・図版が事実（本棚5件・型は合わせて20件・モデル名はカタログ）と一致し、`prompts.test.ts` が固定
  - 「使うほど」の右図版が「押す→貼る→効く」で、存在しない操作（承認・却下）を描かない
  - 料金の推奨帯に法令注記が無く、`#pricing` 内と前後に開示3点が残る（landing-page.test・E2E 緑）
  - 最終CTAの見出しが「7日間の解放をお試しください」
- 実装メモ（2026-09-04 完了）: 運営者の9項目のうち、そのまま反映できなかったのは「プロンプトを無限に管理」だけ——アカウント.md・画像プロンプトの本棚は5件まで（T-M8-350）、投稿の型は最大20件で、レビュー（初見の読者／デザイナー／事実・堅牢性の3視点×3周）で事実の食い違いとして検出。暫定「書き換えても、前の版に戻せる」（要決定 D-54）。同じレビューで「使うほど」の図版に一度描いた「取り入れる／見送る」ピルも、改善案が表示専用＋コピーである事実に反すると分かり撤去した（**図版にも仕組みの事実を描く**）。「7日間の解放を」は運営者指定のまま（デザイナーは「誤字に読める」と指摘。判断は運営者）。白い板への変更で暗色板→紫へ溶かす帯も削除。図面板の注記は初心者の語へ（予約した時刻に／自動投稿ONなら省略OK／1時間ごと／分析した日に）。テスト: `landing-page.test.ts` に運営者指定の文言5つと工程名の一致、`prompts.test.ts` に上限とモデル id。3周目で本文に `BODY`（文節折り）を導入、プロンプトのカードは3カラムを 1180px 起点・横並びを 760px 起点にし（1040px 起点だと型名が省略された）、図版のモデル名は「（バランス）」を外して出す。検証: typecheck／lint／vitest（lp-new・landing-page）緑、1280／390 で横あふれ0・console error 0、640／700／760／1024 でリングのラベルはみ出し0、390〜1280 の9幅でカードの図版に省略なし・1180px 以上で3カードの見出し高が一致。

### T-M8-422: 運営ダッシュボードの計測を正す（生成が常に0・BYOK費用の混入・料金画面の段・自動アクセスの除外） `done`
- 参照: PRD §6.1（原価＝運営キーの費用）・要件02 §3.31/3.32（kpi_daily・page_views）・要件04「事業KPIの日次スナップショット」・docs/operations/monitoring.md / 依存: なし / サイズ: M
- 完了条件:
  - 利用者一覧の「生成」が、プレミアム／エキスパートの精算行（delta はクレジット量）でも BYOK でも実際の生成回数を数える（`generation_jobs` 成功件数）。ファネル「初回生成」も同じ定義になり、注記に「投稿」ではないことが分かる
  - 「今月の原価」「粗利」「原価内訳」「原価/日」が運営キー分（`payer='operator'`）だけを合計し、利用者負担（BYOK）は「参考」として別に出る。台帳に `payer` 列（CHECK）を足し、書き込み側（AI各job・X呼び出し）が決める。既存行は plan／auth_type から近似バックフィル
  - 入口ファネルの「料金画面」行（未ログインでは到達不能）を「登録完了」に置き換え、通過率が意味を持つ
  - 閲覧記録が `sec-fetch-dest: document` の GET だけを数え、運営者自身（SUPPORT_EMAIL）と運用スクリプト（release／doctor の fetch）を除外する
  - 「登録者（累計）」の表示名を「登録者（現在）」に直し、直近30日の窓を JST に統一する
  - doctor（production）に「直近24時間の `/` の閲覧が0件」の警告が1つある（書き込みが壊れたとき「来訪が減った」に見えない）
...
- 完了条件:
  ...
  - `/`・`/signup`・`/plans` の閲覧記録が `src` を持ち（`page_views.source`・既定 ''）、LPのCTA（ヒーロー・ツアー直後・料金カード）が `?src=` を `/signup` へ引き継ぐ
...
### T-M8-424: /plans の料金表示をLPの推奨先行に揃える `done`
- 参照: 要件06 §1.1（SC-04）・§1.5（LP `#pricing`）・PRD §プラン / 依存: T-M8-419（推奨先行の組み立て）・T-M8-420（新LPの昇格） / サイズ: S
- 完了条件:
  - `/plans` の料金がLP `#pricing` と同じ見せ方で出る: 上に「約N円／日」のキャップ行（推奨だけ brand 帯「まずはこれ」・両隣は要約行・カードに密着）、両隣のカードはガラス、プロモ帯はカードの下、SPは推奨カードを先頭。CTA は既存の `CheckoutButton` のまま（Checkout・残りトライアルの再開・消化済みの文言・awaitingCheckout／canceled／confirmed の Notice を変えない）
  - 組み立て（キャップ行＋`PlanPricingCards`＋`CampaignCallout`＋CSS module・`perDayYen`・`capSummary`・`RECOMMENDED`）が `src/components/billing/plan-picker-recommend-first.tsx` の1か所にあり、LP側 `lp-new/pricing-recommend-first.tsx` は `/signup` への CTA を渡す薄い層（既存の export 名は再exportで維持）。価格・上限・プラン名は `plans.ts`／`plan-comparison.ts` から（直書きなし・禁止表現なし）
  - トライアル消化済みの利用者には帯の「7日間無料」を出さない（`trialAvailable` を共用部品経由で帯へ渡す）
  - h1 の直下にLPと同じ選び方の1文（推奨・スタンダード・エキスパートの使い分け。プラン名は `PLANS` から）
  - `e2e/plans.spec.ts` が帯「まずはこれ」・「約133円／日」・「おすすめ」バッジ非表示・選び方の1文を見て、半額バッジ・終了後価格・「通常価格」なし・機能リスト・開示・「7日間無料で利用」・390px の横あふれ0 は従来どおり通る
- 実装メモ（2026-09-04 完了）: `lp-new/pricing-recommend-first.tsx`＋`.module.css` の中身を `billing/plan-picker-recommend-first.tsx`＋`.module.css` へ移し、props を `{ cta, trialAvailable?, className? }` に（`PlanPricingCards` と同じ `cta` の形）。LP側は `PlanPickerRecommendFirst` に Link の CTA を渡すだけ。`/plans` は `CampaignCallout`＋`<section aria-label="料金プラン"><PlanPricingCards/></section>` を `PlanPickerRecommendFirst`（cta＝`CheckoutButton`・`trialAvailable`）へ置換し、h1 の下に選び方の1文。テスト: `lp-new/pricing-recommend-first.test.ts` は移動先の CSS／TSX を読み、LP側が組み立て（`PlanPricingCards`・`CampaignCallout`・module.css）を持たないことと帯へ `trialAvailable` を渡すことを追加で固定。`landing-page.test.ts` は帯・カードの使用を共用側で見る。E2E: `plans.spec.ts` の「おすすめ」バッジ→帯「まずはこれ」＋「約133円／日」＋バッジ hidden＋選び方の1文。要件06 §1.1／§1.5 を現状に更新（v1.205）。検証: typecheck／lint／vitest（src/components・landing-page）／check:doc-dates／check:doc-refs／E2E（plans・landing）緑、1280×900・390×844 で `/plans` と LP `#pricing` を撮影し比較。
- メモ: 運営者の依頼（2026-09-04）「ホームページの料金に /plans も合わせる」。`/plans` のコンテナは max-w-5xl（1024px）でLPの 1240px より狭いが、列幅は同じ変数（`--cols: 1fr 1.14fr 1fr`）で比率が保たれる。
- レビュー反映（2026-09-04・初見の読者／デザイナー／堅牢性の3視点）: **blocker**「帯をカードの下へ移した結果、最初のCTAより上に『カード登録が必要』が無くなった（SPではCTAから約2,000px下・Stripeの1クリック手前）」→ 選び方の1文を共用部品 `PlanChoiceLead`（LP・`/plans` が同じ部品を呼ぶ。書き写し禁止を `billing/plan-picker-recommend-first.test.ts` が固定）にし、末尾にLPと同じ「はじめての方は7日間無料（カード登録が必要）です。」を置く（消化済み・残りトライアル中には出さない）。`plans.spec.ts` に「最初のCTAよりDOM順・位置とも前」と「消化済みには7日間無料が一切出ない」を追加。**improve**: サブ文の先頭に「迷ったら、プレミアムプランから。」（h1 は画面名のまま。LPと同じ見出しにするかは D-56）／CheckoutButton を `font-bold`／サブ文 15→17px／SPの `space-y-6`／ロゴ行を全幅へ／背景に radial glow／帯の括弧を文節で折る／docs（要件06 §1.1・§1.5、要件03 §2）の「帯に『初回のみ』を必ず残す」を実装（2026-08-26 撤去）へ訂正し、`landing-page.test.ts` の `/初回のみ/` 検査（コメントにしか一致していなかった）を「帯は画面文字列で2点・初回限定はFAQ」へ／テストを `billing/plan-picker-recommend-first.test.ts` へ移動（LP側は薄い層の検査だけ）。**据え置き**: SPの表示順（`order:-1`）と DOM／Tab順の不一致——直すなら `PlanPricingCards`／`comparisonColumns` に表示順の引数を足して DOM ごと推奨を先頭にし、md 以上は `md:order-*` で左→右へ戻す（sr-only の「この帯の直下のカード」も不要になる）。`capSummary` と tagline の語の重複・「AIクレジット」「契約期間ごと」の語は D-56。docs の `plan-picker-recommend-first` は新規ファイルが untracked のあいだ拡張子無しで書いてある（`check:doc-refs` は `git ls-files` で判定）——**`git add` 後にフルパス表記へ戻す**。

### T-M8-425: 料金と LP の文言を運営者の決定に合わせる（業界最安価・選び方の1文の削除・カードの括弧書き削除・プロンプトのカードの見出し） `done`
- 参照: 要件06 §1.1（SC-04）・§1.5、要件03 §2 / 依存: T-M8-424 / サイズ: S
- 完了条件:
  - LP `#pricing` と `/plans` の見出しが「業界最安価の価格設定」で、見出し下の選び方の1文と「はじめての方は7日間無料（カード登録が必要）です。」が無い（開示はカード下の帯と特商法ページ）
  - スタンダードのカードの行が「自分で用意」「なし」（括弧書き無し）。BYOKのAPI実費はキャップ要約「API利用料は別」だけが言う
  - プロンプトのカードの見出しが「投稿の型を、何個でも」「プロンプトを無限に管理」（D-54 の決定）
  - landing-page.test／plan-picker の test／E2E（landing・plans）が新しい文言で緑
- 実装メモ（2026-09-04 完了・運営者の決定 D-54／D-56 と指示「（利用料はご自身のAPI課金）」「（ご自身のAPI課金の範囲）」は不要）: 共用部品から `PlanChoiceLead` を削除し、両ページのサブ文と `/plans` の h1（旧「あなたの運用に合うプランを選択」）を置換。`plan-comparison.ts` の行の括弧書きを削除したため、BYOKのAPI実費の常時表示はキャップ要約（md以上）だけになり、SPの `/plans` ではカードにAPI実費の注記が無い（帯とFAQ・特商法ページが担う）。「業界最安価」は最上級表示なので、根拠資料（他社との比較・時点）の保持を運営者に依頼（D-56 に記載）。

### T-M8-426: MRRに引き止め割引を反映する（D-55(1)） `done`
- 参照: 要件04「事業KPIの日次スナップショット」・要件03 §2（profiles.discount_*）・PRD O-6・docs/operations/monitoring.md / 依存: T-M8-422 / サイズ: S
- 完了条件:
  - /admin の MRR（サマリ）と kpi_daily の `mrr_jpy` が、契約者ごとの割引後の月額（`discount_percent_off`／`discount_amount_off_jpy`・`discount_ends_at` が null または未来のときだけ）の合計になる
  - MRRカードの注記に「引き止め割引中の人数と定価比の減額」が出る
  - 実DBのテストが、割引中・期限切れ・定額引き・past_due・trialing の各契約でサマリとスナップショットの一致を固定する
- 実装メモ（2026-09-05 完了・ワークフロー〔実装→反証2視点→修正〕）: 割引の式は契約者画面の `discount-label.ts` と同じ `applyDiscount`（率→四捨五入、額は引き算、下限0）を `src/lib/billing/discounted-price.ts` に共通化。`readSubscriptionTotals` が profiles を1行ずつ読み、trialing は除外・active/past_due を合計。**本番へ出す前の `mrr_jpy` は割引前の値のまま残るので、出した日の推移グラフに割引合計ぶんの段差が1回出る（解約ではない）。** 写しは Stripe の `discounts` 先頭1件・`duration` 未保存のため、`once` のクーポンは次の請求書確定までMRRを下げる（引き止めは repeating 3か月なので現行運用では起きない）。見送り: `percent_off` が小数のクーポンだと profiles（integer）への同期が落ちる件（T-M8-279 由来・現行の50%では起きない。起票候補）。

### T-M8-427: 運営ダッシュボードの指標定義を直す（粗利の月末見込み・解約手続き数の改名と実解約・投稿（済み）と最終操作） `done`
- 参照: 要件02 §3.31・要件04「事業KPIの日次スナップショット」・PRD O-6・docs/operations/monitoring.md / 依存: T-M8-426 / サイズ: M
- 完了条件:
  - /admin の粗利カードが「月末見込み（MRR − 原価の日割り延長）」を値に、これまでの粗利と根拠を注記に出す。月初3日は前月実績を仮置き、負値は −¥ 表記
  - kpi_daily の `cancellations` を `cancel_intents`（解約手続きへ進んだ数）に改名し、migration が既存行を冪等に移す。実解約は状態指標 `users_canceled`／`users_cancel_scheduled` と /admin の時系列・解約アンケート節の現況1行で SQL なしに読める
  - 利用者一覧の「投稿（済み）」が posted 下書きの件数、「最終操作」が利用者自身の操作（手動の生成ジョブ・手動投稿）の最新。「生成（成功・90日）」は窓を明記
  - 実DBテストが上の定義と migration の冪等性を固定し、admin E2E が注記と列見出しを見る
- 実装メモ（2026-09-05 完了・運営者「適切と思われるものに」→ Claude の判断。ワークフロー〔実装→反証2視点→修正〕）: 反証で「最終利用」の初案（drafts.updated_at・trigger無条件の generation_jobs の最大）が、投稿指標の自動収集が updated_at を進める・予約枠の自動生成が入るため「1回投稿して離脱した人が最大30日・枠を持つ人が常に今日」と出る問題を検出し、**最終操作＝手動操作のみ**（trigger='manual' のジョブ・posted_mode='manual' の posted_at）へ変更。実解約の日付列は無い（profiles に遷移日が無く、stripe_events は90日・本番のみ）ため、状態指標の日次差分で読む形に。migration は insert…on conflict→delete の形で、コードが migration より先に動いて新名の行がある日でも落ちない。D-55(4) の「最終利用」は上の定義（最終操作）で確定。要決定候補（未起票）: 「生成」の累計が要るなら usage_events の精算行を併用（BYOK は累計にならない）。

### T-M8-428: LPの文章を簡潔・明快・魅力的に書き直す（運営者の依頼 2026-09-05） `done`
- 参照: 要件06 §1.5「新LP」 / 依存: T-M8-425 / サイズ: M
- 完了条件:
  - 運営者指定の見出し・CTA・工程名・記法チップ・価格・法令の共通部品は変えずに、本文（ヒーローLEAD・図面板・ツアー・プロンプトのカード・使うほど・FAQ・招待カード）が短く読者の得が先に来る文になっている
  - 仕様書の語（表示専用・定刻・契約期間・精算など）が本文に無い。新しい数字・実績を足していない
  - landing-page.test／lp-new のテスト／E2E（landing・mobile-layout）が緑で、1280・390 で横あふれ0・語の途中の折り返しなし
- 実装メモ（2026-09-05 完了・ワークフロー〔2案の執筆（削る／惹く）→審査2名→実装→レビュー2視点→修正〕）: 可変部分の文字数を概算で2〜3割削減。惹き語は1画面に1つ（ヒーロー「あなたの分身」・使うほど「あなたに似てくる」など）。運営者は「誤認表示や法律は一旦忘れてよい」と言ったが、テストが開示語として固定している「同意」「下書きまで」（FAQ Q1）「はじめてのお申し込みに限り」（Q6）は残して短縮した（外す判断は運営者へ）。読者向けの語を「発信方針（アカウント.md）」に統一、重複していた但し書きを1回ずつに。FAQ回答と招待本文を文節折り（BODY）にし、BudouX の誤分割は inline-block と言い換えで回避。要件06 v1.208。

### T-M8-429: 追跡URL（?src=）の閲覧が運営者自身だと数えられない `done`
- 参照: 要件02 §3.32・docs/operations/monitoring.md / 依存: T-M8-423 / サイズ: S
- 完了条件:
  - ログイン中の運営者が `https://exosai.net/?src=<登録済みslug>` を開くと、/admin の流入元の「ホーム表示」が増える
  - URL無しのホーム閲覧は従来どおり運営者自身の分を除く。bot・画面遷移でないアクセスは追跡URL付きでも数えない（単体テストで固定）
  - /admin の流入元カードに「追跡URL付きはあなた自身の閲覧も数える」旨の注記がある
- 実装メモ（2026-09-05 完了・運営者の指摘「?src=x_bio を叩いても数値が増えない」）: 本番で切り分け——ログアウト状態の疑似ブラウザでは 1→2 に増えたので仕組みは動いており、T-M8-422 の「運営者自身は数えない」規則が確認クリックにも効いていた。`isCountableRequest` に source を渡し、形式に合う src が付いた閲覧は運営者除外の対象外に。

### T-M8-430: LPの「よくある質問」の回答をブラッシュアップする（運営者の依頼 2026-09-05） `done`
- 参照: 要件06 §1.5「新LP」FAQ / 依存: T-M8-428 / サイズ: S
- 完了条件:
  - 9問の問いと答えが「結論→条件」の順で1〜3文、仕様書の語なし、人称は「あなた」に統一。並びは仕組み→始め方→作る→場所→お金→安全
  - 固定語（第1問「全部自動ですか」「下書きまで」「同意」、無料トライアルの「はじめてのお申し込みに限り」）が残り、landing-page.test と E2E（landing）が緑
- 実装メモ（2026-09-05 完了・ワークフロー〔2案→審査→実装→レビュー→修正〕）: レビューのブロッカー「Claude のキーだけで画像が作れると読める」を、画像は OpenAI／Gemini のどちらかのキーが要ると明示して解消。解約の答えから根拠の無い「日割りの返金はありません」を削除。要件06 v1.209。

### T-M8-431: 図面板と料金帯の微修正（÷30日の注記・「全部、手で」「確認だけ」の削除・リング図の注記とラベル位置） `done`
- 参照: 要件06 §1.5 / 依存: T-M8-424 / サイズ: S
- 完了条件:
  - LP と /plans の推奨帯に「月額（税込）÷30日の目安」が無い
  - 図面板の合計の直下に「全部、手で」「確認だけ」が無く、リング図の「記録」「改善」の注記が「表示回数・いいねを／自動で記録」「AIの改善案を／あなたが反映」
...
- 実装メモ（2026-09-05 完了・運営者の指摘「配布ファイルの中身が違う。.claude/skills の実物を同封して」）: 以前の zip は各スキルを2〜4KBに簡略化した雛形だった。雛形（CLAUDE.md・docs・tasks・README）は zip から `blog/kit/` へ取り出して正本にし、`scripts/blog-kit.mjs` が `blog/kit/`＋`.claude/skills/` を zip 化する（約72KB）。記事本文の「約31KB」と中身の説明は、進行中のブログ改善（T-M8-433）の後に合わせる。 → zip・`blog/kit/`・`scripts/blog-kit.mjs`・`blog:kit` は T-M8-435 で廃止（配布はプラグイン `docdd` のみ）。
...
- 実装メモ（2026-09-05 完了・運営者の依頼「ブログ系を除いたスキル一式を新リポジトリへ入れる手順とフォルダ。プラグイン化した方が配布しやすいか」→ プラグイン形式で決定、配布先は運営者作成のリポジトリ）: ワークフロー〔実装→導入テスト＋読者視点の反証→修正〕。反証で見つかった罠: 空リポでは `check:doc-refs` が stage 前に空振り・`check:doc-dates` がコミット0件でスタックトレース・`audit-check` が lock 無しで落ちる → それぞれ日本語の理由で止まるガードを正本のスクリプトへ追加し、init の手順を「add → refs → 承知でコミット → dates」に並べ替え。二重管理を避けるため kit/ には雛形の正本（CLAUDE.template.md・docs・tasks・init スキル・README 3種）だけを置き、設定と検査スクリプトはルートの実物を生成時に取り込む。`kit/BUILD.json`（版＋中身ハッシュ）で「中身が変わったのに版が同じ」を停止（`--same-version` で明示的に許す）、`src/lib/blog/dev-kit-zip.test.ts` が zip の鮮度を守る。 → zip 出力と `dev-kit-zip.test.ts` は T-M8-435 で廃止。鮮度検査は `src/lib/ops/dev-kit.test.ts`。プラグイン名 `docdd`・マーケットプレイス名 `claude-docdd-dev-kit`・ライセンス Apache-2.0（リポジトリの LICENSE に合わせ、生成側では LICENSE を作らない）。

### T-M8-435: 開発キットの配布をプラグインに一本化（zip 配布と `.agents/skills` を廃止、記事はプラグイン導入を主手順に） `done`
- 参照: kit/README.md・kit/PUBLISHING.md・blog/published/claude-code-non-engineer-workflow.md・docs/operations/local-development.md / 依存: T-M8-434 / サイズ: S
- 完了条件:
  - 記事「非エンジニアがClaude Codeで…」の「キットの使い方」が `/plugin marketplace add` → `/plugin install` → `/docdd:init` の3コマンドで書かれ、zip への言及とリンクが無い（`blog:check` 緑）
  - `npm run dev-kit` が zip を作らず、`public/blog-files/claude-code-dev-kit.zip` が無い。`kit/BUILD.json` の鮮度は `src/lib/ops/dev-kit.test.ts` が守る
  - プラグイン同梱の README・公開リポジトリの README・init・雛形 CLAUDE.md から「zip 版」の案内が消え（残っていれば `dev-kit` が止まる）、手順書を直したい人向けに `plugins/docdd/skills/` を写す手順がある。公開リポジトリのライセンス表記が実物（Apache-2.0）と一致する
  - `.agents/skills/`（Codex 向けの旧版4本）が無く、AGENTS.md がそれを指していない
- 実装メモ（2026-09-05 完了・運営者の指示「古いやつは削除」「zip の案内は不要、プラグイン導入をメインに」）: zip は記事からしか辿れず、記事から外すと誰も届かない生成物になるため生成ごと廃止した（`dev-kit.mjs` の zip 出力・記事の「約NNKB」書き換え・`dev-kit-zip.test.ts` を削除）。手順書を自分で直したい利用者の受け皿は公開リポジトリの `plugins/docdd/skills/` のコピー（README「手順書を自分で直したいとき」）。公開リポジトリ README の「MIT」は実物の LICENSE（Apache-2.0）と食い違っていたので直した。CI の typecheck 失敗（記事ページの og:image 静的 import）は `src/types/next-image.d.ts` で解消済み（T-M8-433 の補足参照）。

### T-M8-436: 配布物（プラグイン）と Claude Code 記事から本リポジトリ固有の文言を除く（配布用スキル `kit/skills/` を `.claude/skills/` の汎用化した派生として持つ） `done`
- 参照: kit/PUBLISHING.md「`.claude/skills/` と `kit/skills/` の二重管理について」・kit/README.md・docs/operations/local-development.md / 依存: T-M8-435 / サイズ: M
- 完了条件:
  - `dist/docdd/` の .md／.json に「Exos」「x-system」「T-M…」「REQUIRE_DB」「smoke:live 等の固有 npm script」「要件定義書.md／プロンプト設計書.md」が無い（`npm run dev-kit` が機械検査し、あれば止まる。記事へのリンクだけ許す）
  - `kit/skills/<名前>/SKILL.md`（11本）が `.claude/skills/<名前>/SKILL.md` と同じ手順・表・停止条件を持ち、固有名・作業ID・固有コマンド・`src/` のパス・実測値を含まない（「型検査」「単体テスト」「E2E」は雛形 CLAUDE.md「検証コマンド」表を指す）
  - `.claude/skills/<名前>/` を変えると `src/lib/ops/dev-kit.test.ts` が「派生へ反映するか判断して `npm run dev-kit`」で赤になる（`kit/BUILD.json` に元スキルのハッシュを記録）。`.claude/skills/` にスキルが増えて派生が無いときも `dev-kit` が止まる
  - 雛形 CLAUDE.md・init・README・スキル冒頭の前提ブロックに「元のアプリ（Exos AI）」の記述が無い。配布する検査スクリプト3本のコメントに作業ID・事故の日付・固有ファイル名が無い
  - 記事本文に「Exos AI」「私のアプリ専用のコマンド」の記述が無く、著者の経験は固有名なしで書かれている
- 実装メモ（2026-09-05 完了・運営者の指摘「プラグインやブログは x-system に限らず汎用的なものなので、x-system に関する文言は不要」）: 以前の指示（T-M8-432「簡略化した雛形ではなく実物のスキルを同封」）と両立させるため、手順・表・停止条件はそのまま、固有文言だけを言い換えた派生を `kit/skills/` に置く方式にした（`.claude/skills/` 自体を汎用化すると本リポジトリの手順から固有の教訓が抜ける）。派生の作成はワークフロー〔書く→原文と突き合わせて反証（残存文言・失われた規則・捏造・可読性）→直す〕×11本。二重管理の drift は BUILD.json の元スキルハッシュで検知する（反映不要なら `npm run dev-kit` で記録だけ更新）。配布する検査スクリプトはルートの実物を写すため、コメントの作業ID・日付・固有ファイル名を落とした（挙動は不変）。


### T-M8-437: AIニュース記事の時系列図を具体化、Claude Code 記事を「結論＝要約」で簡潔化、記事作成スキルを読者目線で整理 `done`
- 参照: blog/README.md・.claude/skills/blog-write/SKILL.md・blog/published/ai-news-august-2026.md・blog/published/claude-code-non-engineer-workflow.md / 依存: T-M8-436 / サイズ: S
- 完了条件:
  - 「2026年8月の主な出来事」の図に「規制」「配る」等の分類札が無く、各日付の下が具体的な出来事（数字・固有名入り、1行20字以内・2行）になっている。スマホ幅プレビューで読める。alt も同じ内容
  - Claude Code 記事の `## 結論` だけで、主張・4つのファイルの役割・効く理由・次にやることが分かる。本文は具体例（5原則・検証の表・1日の流れ・使い方の手順）を残したまま短くなっている（`blog:check` 緑）
...
- 完了条件: PT-IMG の冒頭が「入力は以下の二つ。足りない情報は本文から推測して補う」「このポストに添える画像1枚の生成プロンプトを作る。画像に描く文字は日本語」になり、公開ページ「プロンプト集」と設計書 §6.8 が同じ文面。`check:providers` と `smoke:live`（生成＋画像）が通る
- 実装メモ（2026-09-05 完了・運営者の指示）: 「指示は英語」の縛りと「狙いは2つ」の行は運営者の文面に無いので外し、出力の説明も「画像の生成指示（画像に描く文字は日本語）」に揃えた。文面の typo（「入力はこ以下二つ」）は「入力は以下の二つ」と読んだ。`smoke:live` は生成＋画像が成功（$0.036）。「週次まとめ」は smoke 用ローカルアカウントに発信定義（base_md）が無いという前提不足で失敗し、プロンプト変更とは無関係（本番アカウントには発信定義がある）。
...
- 完了条件:
  - 設計書 :252「本文へ付けるURLは検証済み引用元（citations）からコードで選ぶ」を実装（モデルの `sources` を SSRF 検証して付ける。citations は usage に保存するだけ）に合わせるか、実装を仕様に合わせる（D-57 で決める）
  - 要件06 :307「Web検索の有無はプロンプトの中に利用者が書く」を実態（自作パターンは常に `always`・最大3回で、画面から変えられない）に合わせる
  - （済・T-M8-442 で ADR-0008 に注記済み）
  - `pattern-spec.ts:171-172` のコメント（予約では参考URLが無い）を、予約スロットが `source_url` を持てる実態に直し、`scheduledPostSlots` の見積りが URL 付き P-2 で過少になる点を確認する
  - パターン別の Web 検索方針（P-1/P-3/P-4/P-6/自作=常に最大3回、P-2=参考URLがあるときだけ、P-5=しない）の一覧表を設計書に置く
- メモ（2026-09-05・運営者の質問「どのパターンも Web リサーチしてから作る形か」への調査で判明）: 「常に」は検索ツールを渡す意味で、実行は保証されない（`usage.calls[].web_search_count` が 0 のこともある）。

### T-M8-442: 既定パターン6種すべてを「必要があれば Web検索・最大3回」に統一（P-2・P-5 を含む） `done`
- 参照: プロンプト設計書 §6.3・§6.6・変更履歴 v1.53 / 要件06 §3.8 / 要件02 §post_patterns / ADR-0008 / 依存: D-57 / サイズ: S
- 完了条件:
  - migration `20260905000002_web_search_all_patterns.sql`: 既存の P-2／P-5（seed 由来）が `always`／3 になり、seed 関数も同じ既定で新規アカウントへ入る（`REQUIRE_DB=1` のテストが緑）
...
- 参照: プロンプト設計書 §6.4・§4（封筒の説明） / 依存: なし / サイズ: S
- 完了条件: PT-P3 の品質基準が「手順は分量（ポスト数の上限）に収まる数で完結しているか」になり、利用者が編集するパターン本文（PT-P1〜P6）に `<pattern_rules>` が無い。封筒自体は SYS-GEN と `gen-context.ts` が従来どおり組み立てて渡す
- 実装メモ（2026-09-05 完了・運営者の質問「<pattern_rules> とは何か。ユーザーには分からないので入れたくない」）: `<pattern_rules>` はアプリが毎回パターンの設定（分量・Web検索の回数・参考URLの方針）を文にして本文に添える「封筒」で、利用者が書くものではない。利用者に見える本文で名指ししていたのは P-3 の1行だけだったので言い換えた。他のパターンにも `<user_input>`・`<quote_post>`・`<news_digest>` の名指しが残っている（P-1・P-4・P-5・P-6）。同じ理由で言い換えるかは運営者の判断待ち（要決定に載せず、報告で提示）。

### T-M8-445: 友達招待の導線を一時非表示にする（`FEATURE_INVITE_ENABLED`。復活はフラグだけ） `done`
- 参照: PRD §5.9・要件01 §環境変数・要件06 SC-12・docs/cp/invite_cp.md §1 / 依存: なし / サイズ: S
- 完了条件:
  - `FEATURE_INVITE_ENABLED` 未設定のとき、新旧 LP の「友達招待」nav とキャンペーンカード、アプリのサイドバー・モバイル下部ナビの「友達招待」、ロック画面と契約バナーの「友達招待は使える」の文が出ない。モバイル下部ナビは6列で右端が空かない
  - `/app/invite`・`/r/{code}`・報酬の確定・月次振込・利用規約第19条は変えない（既存の報酬が続く）。`true` にすると全部戻る（コード変更なし）
...
### T-M8-447: アカウント.md に「書き方のチェックポイント」（AIっぽさを消す10条・伸びる投稿の型10条）をチェックボックスで取り込む `done`
- 参照: PRD §5.7 M-4・要件02 §3.2 `writing_checkpoints`・要件05 §8 `saveWritingCheckpoints`・要件06 §3.8・プロンプト設計書 §4.1／§6.17 / 依存: なし / サイズ: M
- 完了条件:
  - プロンプト＞アカウント.md の本棚の下に「書き方のチェックポイント」（AIっぽさを消す／伸びる投稿の型）が出て、チェックの切り替えごとに即保存される（失敗したら元へ戻してトースト）
  - 選んだ条項が生成のたびに `<base_md>` の末尾へ「## 書き方のチェックポイント」＋「素材に無い体験・数字は作らない」の1行付きで渡る（`gen-context.test.ts`）。本文（本棚）は変わらず、字数上限にも数えない
  - migration `20260906000001_writing_checkpoints.sql`（`x_accounts.writing_checkpoints` jsonb 配列）。本人以外は書けず、未知の ID は拒否（`writing-checkpoints.db.test.ts`）。E2E `writing-checkpoints.spec.ts` 緑
  - 文面の正本はコード（`src/lib/prompts/writing-checkpoints.ts`）で、設計書 §6.17 の表とコードのずれは `writing-checkpoints.docs.test.ts` が赤にする
- 実装メモ（2026-09-06 完了・運営者の指示「AIっぽくない投稿・バズる投稿の汎用チェックポイントを複数考えて入れる。プロンプト＞アカウント.md からチェックマークで各条項を入れるか決められるように」）: 本文へ物理的に書き込まず ID だけ持つ方式にした（学習の反映や本棚の切替で消えない・文面の改善が全利用者へ届く・5,000字の上限を消費しない）。条項の文面はワークフロー〔執筆→X運用者と編集者のレビュー→改稿〕で20件に確定。レビューで「体験・数字を求める条項が捏造を誘う」と出たため、節の先頭に素材限定の1行を必ず置く。同時に選ぶ目安は群ごとに3〜4件（画面の説明に記載）。既定は未選択。

### T-M8-410: 参考アカウントの反映の失敗が画面に出ない（成功扱い・古い「開始が遅れています」・通知文言が削除用） `done`
- 参照: 要件06 §3.1（反映の進行表示） / 要件04 §14（通知の文言） / 依存: なし / サイズ: S
- 完了条件:
  - 反映のjob（md_merge・提案モード）が失敗したら、画面に「反映できませんでした」と理由・次の一手（もう一度押す／自由入力で作る）が出る。成功トーストは**提案が入ったときだけ**出す
  - 反映が終わったら（成功・失敗とも）登録済み一覧を取り直し、分析済みのソースに「開始が遅れています（最大5分）」が残らない
  - md_merge 失敗の通知は「反映」と「削除」で文言を分ける（反映の失敗に「学習ソースの削除に失敗しました」と出さない）
- メモ: 運営者の報告（2026-09-01）。`learningApplyStatusAction` が running の真偽しか返さないため、jobが失敗しても `applying` が解けた時点で成功トーストを出していた。`sources` のクライアント状態は追加直後にしか取り直さず、`router.refresh()` では `useState` の初期値が変わらないので「分析待ち」のまま60秒で「開始が遅れています」になる。通知の文言は `notifications.ts` の `md_merge` が削除経路の前提のまま。
- 実装メモ（2026-09-01 完了）:
  - `learningApplyStatusAction` が直近の反映job（`learning_source_id` 無しの md_merge）の成否・理由と `settings_proposal` の有無を返し、
    画面は提案が入ったときだけ成功トースト、失敗は理由と次の一手を Notice で残す。完了時に一覧を取り直す。
  - `MdMergeStructureError` に利用者向け文言（反映／削除）を持たせ、`fallbackJobError` と失敗通知（`FAILED_NOTICE.md_merge`）が
    それを使う——反映の失敗に「削除に失敗しました」を出さない。
...
- 完了条件:
  ...
  - 確認済みの利用者のログインはパスワードだけ（コード入力なし）。未確認のまま放置した登録は、ログイン時にコード画面（再送あり）へ回し、ログインで勝手に確認済みにしない（T-M8-377の自動確認を廃止）
...
- 完了条件:
  - 区分タブの並びが「AIモデル設定／アカウント.md／投稿作成プロンプト／画像生成プロンプト」
  - `/app/prompts`（区分指定なし・ナビから）で開くのは先頭のAIモデル設定
  - 既存の `?sec=` リンクはそのまま動く
- メモ: 運営者の指示（2026-09-01）。
- 実装メモ（2026-09-01 完了）: `PROMPT_SECTIONS` の先頭を ai-models にし、`normalizePromptSection` の既定を先頭要素へ（配列の順が正本）。

### T-M8-406: プロンプト＞アカウント.md の5項目フォームに「自由入力でアカウント設定を作る」の見出しを置く `done`
- 参照: 要件06 §3.1・§3.6 / 依存: T-M8-400 / サイズ: S
- 完了条件:
  - 「参考アカウントからアカウント設定を作る」の枠の下、ペルソナの上に見出し「自由入力でアカウント設定を作る」（設定が保存済みなら「自由入力で設定を更新する」）と1行の説明が出る
  - E2E `learning-setup` の見出し順（参考アカウント → 自由入力 → ペルソナ）が守られる
...
- 完了条件:
  ...
  - ONの利用者には、時間単位ダイジェストの作成時に同じ内容（件数・上位5件・一覧リンク）がメールで届く。アプリ内通知OFF・メールONの組み合わせでもメールは届く
...
- 完了条件:
  - 設定＞通知・ニュース一覧・LPの「毎日12時と19時」が消え、「10分おきに新着を確認し、該当する新着があった時間帯に1時間1回までまとめて通知」の意味の文言になる
  - PRD N-3・要件04 §14・要件06 §1.2.2・プロンプト設計書のNEWS行も同じ実態に揃う
- メモ: 運営者の指摘（2026-09-01「毎日12時と19時（日本時間）に届きますは間違い」）。T-M8-380/383でRSS10分巡回へ変えたときに文言が残った。通知は `news-digest` のUTC1時間窓＋dedupeなので「1時間に1回まで」。
- 実装メモ（2026-09-01 完了）: 設定＞通知・ニュース一覧・LP（本文と図版の時刻）を修正。PRD N-3・要件04 §14・要件06 §1.2.2・プロンプト設計書 §1 のNEWS行も実態へ。**同じ1時間窓で後から入った新着は通知されない**（dedupeの仕様）——問題なら別タスクで「窓内の未通知分を次窓へ持ち越す」を検討。
...
- 完了条件:
  ...
  - 取得できないURL（非公開・削除・X未連携・読取失敗）は「どのURLがなぜ読めなかったか」を画面に出し、黙って空のまま生成しない
  - 生成に使うモデルは Claude Sonnet 5 に固定（運営Claude／BYOKのAnthropic）。テストで守る。BYOKでOpenAI/Googleしか無い利用者はClaudeを呼べないので同格のanalysis層モデル（現状どおり）
...
- 完了条件:
  ...
  - プロンプト＞アカウント.md の並びが「参考アカウントから作る（記入欄＋登録済み一覧＋反映ボタン）→ 5項目フォーム（ペルソナ…）→ アカウント.mdの本棚」
  - 参考投稿の記入欄と「参考投稿の欄を増やす」は出ない（参考アカウントのみ・最大3件）。登録済みの参考投稿が残っていれば一覧に出て削除できる
...
- 完了条件:
  ...
  - Xアカウント未連携・プロンプト編集不可プランでもこの区分だけは編集できる（profile単位・従来どおり）
  - 見出し・ラベルが「文章生成」「文章生成に使うAI」「文章生成に使うモデル」（「・リサーチ」を外す）
  - 画像生成は何も保存していない状態で「OpenAI」「GPT Image 1.5（バランス）」が選ばれた形で表示される（premium: 運営キーがOpenAIにあるとき／BYOK: OpenAIキーがvalidのとき）。モデルの「おまかせ」は既定モデル名を明示する（例「おまかせ（GPT Image 1.5・約1,120クレジット/回）」）
...
### T-M8-402: 設定ページのタブを「Xアカウント／APIキー／通知／課金・プラン」へ分ける `done`
- 参照: 要件06 §1（SC-11）・§1.2.1・§1.2.2 / 依存: T-M8-400, T-M8-401 / サイズ: S
- 完了条件:
  - 設定のタブが「Xアカウント」「APIキー」「通知」「課金・プラン」の4つ（旧 `general` はXアカウントへ、`x-accounts`/`api-keys`/`notifications` は各タブへ）
  - ログイン中のメールは最初のタブ（Xアカウント）の先頭に残る
  - premium/expert の「APIキー」タブは従来の「APIキー不要」案内＋利用枠を出す（タブを隠さない）
  - アカウントメニューの導線が4タブに追随する（`SETTINGS_TABS` から生成）
  - 各タブで読むデータはそのタブぶんだけ（1タブに全部読ませない）
- メモ: 運営者の指示（2026-09-01「設定と課金・プランしかタブがなく質素」）。T-M8-104 で1タブへ統合したものを、アカウント設定・AIモデル設定が抜けた後の構成で再分割する。旧エイリアスは残っているのでリンク互換は保てる。
- 実装メモ（2026-09-01 完了）:
  - `SETTINGS_TABS` を x-accounts／api-keys／notifications／billing の4つへ。旧 `general` はエイリアス、
    別画面へ移った旧slug（account/persona/learning/purposes/prompts/base-md）は `SETTINGS_TAB_REDIRECTS` で転送。
  - 各タブは自分の分だけ読む（Xアカウントタブは前提判定のためAPIキーも読む。利用枠はAPIキー・課金の2タブ）。
  - アカウントメニューの導線は `SETTINGS_TABS` から生成されるので4件に自動追随。
  - 検証: 単体全件緑、全E2E＋`npm run build`＋`check:csp-nonce`（push前のまとめ実行）、実ブラウザのキャプチャ。

### T-M8-398: 下書きの画像をポスト別へ（各ポストで生成・アップロード・外す） `done`
- 参照: 要件06 §6（v1.177） / 要件04 §9（v1.67） / 要件05（v1.74） / 依存: T-M8-353 / サイズ: L
- 完了条件: スレッドの各ポストに「画像を生成する」「画像をアップロードする」がある／画像はそのポストの本文から生成され、投稿時に対応するポストへ添付される／差し替え・削除は対象ポストの画像だけに効く
- メモ: 運営者の指示（2026-09-01）。旧データ（post_local_id無し）は1ポスト目扱い（互換規則は draft-images.ts に集約）。
- 実装メモ（2026-09-01 完了）:
  - 画像job・アップロード・削除・投稿添付の4経路すべてへ `post_local_id` を通した。
    差し替え・失敗印は**対象ポストの分だけ**（他ポストの画像を巻き込まない）。
  - 投稿時はready画像を post_local_id で対応ポストへ添付（1ポスト1枚）。1枚でも上げられなければ
    本文を1件も投稿しない（画像だけ欠けた投稿を黙って出さない・原則1）。
  - 実機で2ポスト目への生成→DBで post:"p2" ready を確認。E2E（draft-image）・単体（helpers 4件）緑。
  - ついでに直した別件: scheduler-tickテストが2098年固定時計でkpi_dailyへ書き、
    kpiテストのmax()検査を壊す並行干渉（両側を修正・T-M8-161と同型の記録）。

### T-M8-397: パターン追加に「参考投稿からAIで作る」（最大3件・プロンプト自動生成） `done`
- 参照: プロンプト設計書 §6.16（v1.47） / 要件05（v1.73） / 要件06（v1.176） / 依存: なし / サイズ: M
- 完了条件: プロンプト・投稿作成・スケジュールの3画面のパターン追加で参考投稿を最大3件入れられる／AI生成の結果（名前・説明・本文・プレースホルダー込み）が記入欄へ自動で入る／保存は利用者が行う
- メモ: 運営者の指示（2026-09-01）。
- 実装メモ（2026-09-01 完了）:
  - 新プロンプト PT-PATTERN-GEN（analysis層固定・Web検索なし・素材のみ分析）。共通部品
    `PatternFields` に生成ブロックを足したので3画面同時に対応。
  - **同期Server Action**（jobを作らない）。費用は原価台帳＋AIクレジットへ記録——このために
    `settleUsage` を「jobId null＋明示の冪等キー」対応にした（`usage_events.job_id` のFKを
    実UUIDで踏んで500になるのを実機で検出→修正・回帰テスト2件）。
  - 実AI・実ブラウザで1周確認: 運営者実例を貼って生成→「悲報型ニュース速報」＋{題材}入り本文が
    記入欄へ入った。

### T-M8-396: 設定＞アカウント設定を参考アカウント専用へ（30投稿分析・フォームはプロンプトへ移設） `done`
- 参照: 要件06 §3（v1.175） / 依存: T-M8-395 / サイズ: M
- 完了条件: 設定タブに残るのは参考ソースのみ／5項目フォームはプロンプト＞アカウント.mdで編集・提案反映できる／参考アカウントは直近30投稿で分析される
- メモ: 運営者の指示（2026-09-01「アカウント設定では参考アカウントの参照のみに。過去の30投稿くらいを確認して入力項目に反映」）。
- 実装メモ（2026-09-01 完了）:
  - 反映（settings_proposal）の流れは既存機構（PT-L1分析→PT-MD-MERGE）をそのまま使い、
    取得件数を20→30へ。反映完了トーストと設定タブの案内が移設先（プロンプト＞アカウント.md）を指す。
  - 提案でフォームを作り直すkey・取り消し・保存確定の挙動は移設後も同一（E2Eで確認）。

### T-M8-395: アカウント.mdを5項目へ再編（文末自由入力・手書き章廃止） `done`
- 参照: 要件06 §3（v1.174） / プロンプト設計書 §3（v1.46） / 要件05（v1.72） / 依存: なし / サイズ: M
- 完了条件: 項目がペルソナ・テーマ・トーン・スレッド量や文章量・NG設定の5つ／文末が自由入力／全セクションが設定から生成される
- メモ: 運営者の指示（2026-09-01）。旧「参考にする型」の役割は参考アカウント分析（T-M8-396）とパターン別参考投稿（T-M8-397）が継ぐ。
- 実装メモ（2026-09-01 完了）:
  - schema: `tone.sentence_style` を自由入力へ（旧enum値はmigrationで「です・ます調」「断定調」へ置換）。
    `volume.free_text`（500字・任意）を追加（旧データはdefaultで互換）。
  - アカウント.mdは**全5セクションを設定から生成**（手書き章の温存機構を撤去）。旧形式の保存済みmdは
    次の保存で新形式へ置き換わる（本番の該当データは期限切れアカウント1件のみ・内容確認済み）。
  - PT-MD-MERGEへ volume の扱いを追加。フォームは文末をテキスト入力・「スレッド量や文章量」欄を新設。

### T-M8-394: scheduler-tickの実DBテストが蓄積テストユーザー数で落ちるのを修正 `done`
- 参照: docs/operations/development-and-testing.md §flaky / 依存: なし / サイズ: S
- 完了条件: ローカルDBの利用者数に依存せずテストが緑／日次サマリの積み残し検出そのものは殺さない
- 実装メモ（2026-09-01 完了）: 日次サマリの対象（X連携済みで当日未送信）が過去のテスト利用者の蓄積で
  バッチ上限を超え、「積み残し8人」警告でcleanupエラー検査が赤くなった（件数依存の決定的失敗・
  T-M8-161と同型）。テスト冒頭で既存利用者を「当日送信済み」にし、自分のfixtureだけを見る形へ。

### T-M8-393: 刷新プロンプトのセルフレビュー（実生成3周・品質改善） `done`
- 参照: docs/prompt/README.md（横断の学び） / 依存: T-M8-392 / サイズ: M
- 完了条件: 全パターンを実AIで生成し評価した／見つけた問題がプロンプトへ反映されている／記録がdocs/promptに残る
- メモ: 運営者の指示（2026-09-01「セルフレビューを繰り返して品質を最高まで改善」）。実費 約$0.5（9生成）。
- 実装メモ（2026-09-01 完了）:
  - P1: 3周で合格（1周目=一言欠落・2周目=箇条書き欠落 → **必須4要素のチェックリスト化**で安定）。
  - P3/P6: 締めポストが本文に食われる → **枠の予約**（ステップ数=分量−2・件数=分量−1）で解消。
  - P6: digest無視に見えた事象はテスト環境のdigest空が主因と特定（テーマ設定後はseed件が目次に載った）。
  - 横断: 「入れる」では揺れる→「この順で全部・欠けたら書き直す」／文体はSYSの読み直し指示でも
    零にはならない（下書き確認と学習ループが最後の砦）／捏造は「確認できない具体名を書かない」で減らす。

### T-M8-392: 投稿パターンのプロンプトを高インプレ実例ベースで刷新（docs/prompt新設） `done`
- 参照: docs/prompt/README.md / プロンプト設計書 §6（v1.45） / 依存: T-M8-391 / サイズ: L
- 完了条件: docs/promptに各パターンの設計書（実例・構造分解・判断）がある／SYS-GEN・PT-P1〜P4/P6が設計に基づき刷新されている／設計書§6がコードと一致
- メモ: 運営者の指示（2026-09-01。ニュース解説の実例2本を提供・「設計書を作ってからプロンプトを作り直す。品質重視でプロンプトは長くて良い」）。
- 実装メモ（2026-09-01 完了）:
  - **docs/prompt/** を新設: README（正本関係・共通原則・文字数方針）＋パターン別6ファイル
    （運営者実例の全文掲載＋構造分解＋設計判断）。docs/README.mdのマップへ登録。
  - **P-1は運営者実例の型をそのまま落とし込んだ**: 角括弧ラベル＋数字の1行目・実質の言い切り・
    人の声の一言・「・」箇条書き3〜5点（1行1事実）・「↓詳細」・(2/4)通し番号・■補足、
    「基準100なら」の数字変換、品質基準の自己チェック。
  - P-3/P-6は**目次型メイン**（メインだけで保存価値が完結）へ。P-2/P-4は**長文可でも短く**の
    逆指定（意見と速報は凝縮が価値）。P-5はflag OFFのため骨格維持。
  - SYS-GENへ「読まれる投稿の原則」（保存＝具体・1行目・人の声・返信の余白）を追加し、
    字数の固定文を撤去（<pattern_rules>へ委譲・T-M8-391）。

### T-M8-391: 文字数をX Premiumで分岐（長文プロンプトの成果物を壊さない） `done`
- 参照: プロンプト設計書 §4.1/§7-5（v1.45） / 依存: T-M8-221 / サイズ: M
- 完了条件: Premiumアカウントの生成は280短縮・240目標が適用されない／非Premiumは従来どおり／AIにも文字数の前提が伝わる
- メモ: 運営者の指示（2026-09-01「プレミアム前提で文字数制限は忘れて良い。制限あり/なしで分けても良い」）。パターン複製ではなく`x_accounts.x_premium`での自動分岐を採用（選択肢を倍にしない・投稿可否はアカウント側の事実）。
- 実装メモ（2026-09-01 完了）:
  - `buildPatternRules`へ「文字数:」行を追加（Premium=長文可／非Premium=加重280以内）。
  - `finalizeThread`へ`premium`を追加: 上限=`maxWeightedLengthFor`（25,000/280）、Premiumは
    240目標の読みやすさ短縮も適用しない。post-generationが`xa.x_premium`を読んで渡す。
  - 回帰テスト: 長文500字が無傷で通る／25,000超だけ短縮／未指定は従来どおり、の3点＋rules行2点。

### T-M8-390: 入力グループの見出しを「プレースホルダー」「共通の入力（user_input）」へ `done`
- 参照: 要件06 §SC-07/SC-08（v1.173） / 依存: T-M8-388 / サイズ: S
- 完了条件: 両画面の見出しが指示どおり／並び順E2Eが新見出しで緑
- メモ: 運営者の指示（2026-09-01）。タグ名user_inputを画面に出すのはT-M8-314の方針より新しい判断として要件06に明記。
- 実装メモ（2026-09-01 完了）: 両画面のlegend・E2E・要件06を更新。

### T-M8-389: リリースゲートのCI判定がPRのskipped runに隠されるのを修正 `done`
- 参照: docs/operations/deployment.md（リリースの流れ）/ 依存: T-M8-372 / サイズ: S
- 完了条件: PRマージ後に release:staging を再実行しても「結果が skipped です」で止まらない／CIが本当に赤・実行中のときは従来どおり止まる
- メモ: T-M8-388のstaging再検証で実際に停止（2026-08-31）。CI重複排除（T-M8-372）でPRトリガーのrunが同一SHAに `skipped` で積まれ、`gh run list` の新しい順先頭一致がpush時の `success` を隠した。
- 実装メモ（2026-08-31 完了）:
  - 選び方を純粋関数 `pickCiConclusion`（release-gate.ts）へ移し、**実行された結論をskippedより優先**。
    実行中runがあれば結論を先取りせず、全部skippedなら従来どおり止める（緑をでっち上げない）。
  - 回帰テスト5件（隠され・failure優先・実行中・全skipped・run無し）。

### T-M8-388: 投稿作成・スケジュールの入力を「型の入力項目→共通の入力」のグループへ再編 `done`
- 参照: 要件06 §SC-07/SC-08（v1.172） / 依存: T-M8-387 / サイズ: M
- 完了条件: プレースホルダーが共通入力より先に並ぶ／どちらのグループか画面上で見分けられる／既存の保存・生成の挙動は変わらない
- メモ: 運営者の指示（2026-08-31「プレースホルダーとuser_inputがどれなのか分かりにくい。プレースホルダー→user_inputの順にしてUI上分かりやすく」）。
- 実装メモ（2026-08-31 完了）:
  - 両画面とも **パターン → 生成プロンプト → 「この型の入力項目」（プレースホルダー）→
    「共通の入力」（テーマ・参考URL・追加指示）** の順へ。各グループは枠線＋見出し付きの
    `fieldset`/`legend`（支援技術にもグループとして伝わる）に1行の説明を添えた。
    プレースホルダーの無い型では「この型の入力項目」ごと出さない。
  - **テーマ先頭（T-M8-101・2026-08-15）と予約の並び順（2026-08-18）は本指示で上書き**。
  - UIに生タグ名（user_input）は出さない（T-M8-314の方針）——「共通の入力」という画面の言葉にした。
  - 実ブラウザで1440/768/390の3幅・型切替時のグループ表示切替・コンソールエラー無しを確認。
    並び順のE2E（schedule-generation-inputs）を新順序＋グループ見出しの検証へ更新。

### T-M8-387: 生成プロンプトの構造タグ `<input>` を `<user_input>` へ改名 `done`
- 参照: プロンプト設計書 §4.1 / 要件05 §（生成） / 依存: なし / サイズ: S
- 完了条件: アプリが組む構造タグが `<user_input>` になる／SYS-GEN・PT・pattern_rules内の参照文言も一致／保存済みプロンプトへの影響が無いことを確認済み
- メモ: 運営者の質問（2026-08-31「<input>とは何でしたっけ？もう少し分かりやすい名前にできませんか」）。
- 実装メモ（2026-08-31 完了）:
  - `<input>` は「利用者が投稿作成画面で入力した内容」（分野・参考URL・追加指示・プレースホルダー値）を
    包む封筒タグだったが、名前から中身が読めなかった。`<user_input>` へ改名。
  - **改名の前に本番・stgのDBを確認**: post_patterns / prompt_presets / prompt_templates /
    schedule_slots / improvement_suggestions のどこにも旧タグへの言及ゼロ（利用者のプロンプトは壊れない）。
  - 発信側（gen-context.ts）と参照側（SYS-GEN・PT-P1/P4/P5・buildPatternRules）を同一コミットで更新。
    歴史の記述（変更履歴・過去の経緯コメント）は当時の名のまま残した。

### T-M8-386: ニュースからの遷移でテーマへ記事の分野を自動入力 `done`
- 参照: 要件06 §（SC-06/SC-07・T-M8-210の引き継ぎ） / 依存: なし / サイズ: S
- 完了条件: 「すぐに投稿作成」で開いた投稿作成画面のテーマに記事の分野が選択されている／画面で選べない分野（運用外）は従来どおり「その他」
- メモ: 運営者の指示（2026-08-31「テーマも入力されているようにしてください」）。テーマIDとニュース分野は同じマスタ（6テーマ↔6分野が1対1）なのでそのまま渡せる。
- 実装メモ（2026-08-31 完了）:
  - 引き継ぎクエリへ `category` を追加し、選択肢に実在する分野ならテーマ初期値へ、無ければ「その他」。
  - E2E（news.spec.ts の引き継ぎテスト）をテーマ=分野の検証へ更新。
...
- 実装メモ（2026-08-31 完了）:
  ...
  - **本番検証（2026-08-31 反映後）**: canary（ai_newinfo指定）で**生成＋画像が成功、画像ready 435KB**
    ＝sharpが本番Linuxで実際に動くところまで確認。ニュース取得も9/9本緑。
    （週次まとめの失敗は別件・D-51＝アカウント.md未設定によるもので本修正とは無関係）
...
- メモ: 運営者の指示（2026-08-31「AIにはclaudeも入れて。全部取得できるかテスト。要約はsonnet固定になっていますか」）。
- 実装メモ（2026-08-31 完了）:
  - **Anthropic公式のRSSは存在しない**（/news/rss.xml 等すべて404を確認）。TechCrunchの
    Anthropic専門タグフィード（20件・最新30h前）で補い、AI総合フィードとの重複は
    source_url のcanonical重複排除が吸収する。
  - **要約はHaiku固定だった**（mechanical層）のを **analysis層＝Sonnet級固定**へ。要約と
    impact判定は「観察して要約する」処理で分類上もanalysisが正しい。費用差は月+$10前後
    （新着数に比例）。台帳で claude-sonnet-5 の使用を確認。
  - 全20フィードの実HTTP取得テスト: **20本OK/0本NG**。スモークでSonnet要約の実物1周も緑
    （要約例「Claude Code週次上限、9月改定で実質減少」）。

### T-M8-383: ニュース巡回を20分間隔→10分間隔へ `done`
- 参照: 要件04 §6 / PRD N-1 / 依存: T-M8-380 / サイズ: S
- 完了条件: 新着が最短10分で画面に載る／費用がほぼ変わらないことを設計で説明できる
- メモ: 運営者の指示（2026-08-31「10分置きにできますか？費用はどの程度変わる？」）。
- 実装メモ（2026-08-31 完了）:
  - **費用は新着記事の数にしか比例しない設計**なので、巡回を倍にしても増えるのは
    要約callの固定プロンプト分だけ（月+$1未満）。フィード取得（HTTP）は無料。
  - vercel.json `*/10`・窓key（tenMinWindowKey）・launchd plist・各同期テスト・docsを一括更新。

### T-M8-382: news_itemsの保存上限を全体500件から分野別150件へ `done`
- 参照: 要件04 §14 / 依存: T-M8-380 / サイズ: S
- 完了条件: 多産な分野が他分野の記事を押し出さない／参照付き行の保全は維持
- メモ: 運営者の指示（2026-08-31「改善できる箇所の確認」）で発見。
- 実装メモ（2026-08-31 完了）:
  - RSS化で流量が分野間で大きく違う（実測: 投資系≈70件/日・美容≈39件/日 vs AI≈20件/日）。
    全体500件の新着順上限だと**約3日で1周し、多産な分野が低頻度・高価値のAI記事を押し出す**。
  - 分野ごとに新着150件（6分野×150=最大900行。1行は短いtextでDB肥大は軽微）。
    trimのrow_numberへ `partition by category` を足しただけで、参照ガード・バッチ枠は従来どおり。

### T-M8-381: 監視フィードの見直し（実測評価）とフィード健全性の常時チェック `done`
- 参照: ADR-0009 / 依存: T-M8-380 / サイズ: S
- 完了条件: 全フィードの新しさ・更新頻度を実測して不良を差し替える／死んだフィードに気付ける経路がある
- メモ: 運営者の指示（2026-08-31「取得元サイトの適切性を確認。英語サイト含め最新情報が取れるか、X投稿を踏まえてインパクトがあるか」）。
- 実装メモ（2026-08-31 完了）:
  - **全フィードを実測評価**（最新記事の古さ・件数/日）: snsの2本が事実上死んでいた
    （ITmediaマーケ 最新111h前・0.1件/日、Gaiax 448h前・0.1件/日）。**Gaiaxを除外し
    MarkeZine（7.5件/日）へ差し替え**。
  - **AI分野へ英語速報を追加**: TechCrunch AI（最新1.1h前・5.7件/日）・The Verge AI
    （0.5h前・3.2件/日）。国内メディアは週末に止まる（実測で全滅）ため速報の穴を英語圏で
    埋める。英語記事は要約時に日本語のtitle・summaryへ直る（スモークで確認済み）。
  - 試して不採用: Web担(403)・Anthropic News(404)・VentureBeat AI(0.04件/日)・
    トウシル(404)・MONEY PLUS(404)・愛カツ(403)。investmentは既存3本を維持
    （東洋経済は総合寄り・ZUU/幻冬舎は生活寄りだが、動く代替が見つからず。今後の見直し候補）。
  - **スモークへ全フィードの健全性チェックを追加**: 読めない・0件のフィードを名指しで警告に出す
    （フィードは相手側の都合で死ぬ。分野が静かに痩せる前に気付く経路）。
  - 死骸掃除: RSS化で呼ばれなくなった `newsOutcome`（スモークの旧判定）を削除。
...
- メモ: 運営者の指示（2026-08-30）。
- 実装メモ（2026-08-30 完了）:
  - 系列は「昨日まで＝kpi_daily（400日）・**今日＝生のpage_views**」の合成。スナップショットは
    前日までしか書かないため、kpi_dailyだけだと今日が常に0に見える。
  - チャート部品は既存のKpiChart（ホバー値・期間切替つき）をそのまま使用。

### T-M8-378: 入口ファネル（公開ページの閲覧数とユニーク）を記録して/adminへ出す `done`
- 参照: PRD O-6 / 要件02 §3.32 / 要件04 §14 / 依存: T-M8-373 / サイズ: M
- 完了条件: ホーム・新規登録・料金の閲覧数と日次ユニークが記録される／/adminの入口ファネルに出る／個人を追わない
- メモ: 運営者の指示（2026-08-30「ホーム・新規登録・plan画面の閲覧数とユニークユーザー数も記録」）。
- 実装メモ（2026-08-30 完了）:
  - **個人を追わない設計**: 訪問者の識別は「日替わりの塩＋IP＋UA」のHMACハッシュ32桁だけを保存。
    生のIP・UAは保存せず、**Cookieも使わない**（Plausible等のCookieレス解析と同じ考え方）。
    塩が日替わりなので日をまたいだ突合は不可能＝**ユニークは日次ユニークの合計**として表示。
    プライバシーポリシーは通信情報（IP・ブラウザ）の取得を開示済み・外部解析ツール不使用の
    記載とも整合するため、法務文書の変更は不要と判断。
  - **数えないもの**: bot（UA判定・最小限）と**Next.jsの先読み**（`next-router-prefetch`——
    リンクが見えただけで飛ぶため数えるとホームが数倍に膨らむ）。HeadlessChromeはわざと数える
    （E2Eがこの記録自体を検証する経路。実利用者はheadlessで来ない）。
  - 書き込みは `after()`（応答後）で、**画面を1msも待たせない**。失敗は握り潰してSentryへ。
  - 生記録（page_views）は40日保持、日次集計は kpi_snapshot が kpi_daily へ書いて400日。
    /admin に「入口ファネル（直近30日・未ログイン含む）」を既存ファネルの上へ追加。

### T-M8-377: ログインで未確認アカウントに6桁コードを求めない `done`
- 参照: 要件03 §1 / 依存: なし / サイズ: S
- 完了条件: 未確認アカウントがコード画面なしでログインできる／DB上も確認済みへ揃う
- メモ: 運営者の指摘（2026-08-30「新規登録時もメールアドレス認証は不要だったのでログインも不要に」）。
- 実装メモ（2026-08-30 完了）:
  - 新規登録は確認なしで完了する設定（mailer_autoconfirm）なのに、**設定変更より前に登録された
    未確認アカウント**は Supabase がログインを `email_not_confirmed` で拒否し、6桁コード画面へ
    回されていた。登録時に求めない確認をログインでだけ求めるのは一貫しない。
  - `signInWithPassword` の前に `confirmLegacyUnconfirmedEmail` が未確認をその場で確認済みへ
    揃える。**確認を付けてもセッションは発行されない**（パスワードが正しいときだけ）ので、
    この更新で新しくできるようになることは無い（新規登録が既に確認なしを許している）。
  - 揃え書き込みが失敗したときの受け皿として、6桁コード画面＋自動再送（T-M8-153）は残す。
  - E2E「未確認アカウントのログインは6桁画面へ」を**新しい仕様（コード画面なしで入れる）へ
    書き換え**。DBテスト2件（冪等・大文字小文字無視）。

### T-M8-376: リリースのCI待ちを3回分→1回分へ縮める `done`
- 参照: docs/operations/ci.md / 依存: なし / サイズ: S
- 完了条件: stg→本番の1リリースでフルCIが1回だけ走る／内容が変わったpushは従来どおりフルで走る
- メモ: 運営者の指摘（2026-08-30「pushが完了する時間は長すぎませんか」）。
- 実装メモ（2026-08-30 完了）:
  - 実測: 1リリースで**同一内容のフルCIを3回**（stg push 14分 → stg→main PR 12分 →
    main push 14分＝計約40分）。必須チェックはコミット単位なのでPR分は元々不要、
    mainのマージコミットは**中身（tree）がstg headと同一**なので再検証しても結果は変わらない。
  - (1) stg→mainのPRではCIを走らせない（push時のrunが同じコミットを検査済み。skippedは
    必須チェック上成功扱い） (2) mainへのpushは、マージ元コミットが同一treeで
    「release:check」成功済みならスキップ（check runsをGitHub APIで照合）。
  - **treeが変わったpushは従来どおりフルで走る**（検査を弱めるのではなく、同じものを
    3回検査するのをやめる）。効果: リリースのCI待ち 約40分 → 約14分。

### T-M8-375: 既定の投稿パターンをインプレッション設計で改定する `done`
- 参照: プロンプト設計書 §6.1〜6.7 / PRD P / 依存: なし / サイズ: M
- 完了条件: 代表パターンが「伸びやすい構成」の指示を持つ／ユーザー入力すべき箇所がプレースホルダーになっている／文字数・スレッド数が明記されている
- メモ: 運営者の指示（2026-08-30）。
- 実装メモ（2026-08-30 完了）:
  - **根拠（2026年のXアルゴリズム解説の一致点）**: 返信は「いいね」の約8倍の重み（会話の発生が
    最重要）／滞在時間が直接のランキング信号／冒頭1〜3行で読む・離脱が決まる／本文中の外部リンクは
    リーチ減／ハッシュタグはほぼ無意味。**既存のSYS-GENは大半を先取り済み**（URL本文禁止・
    タグ禁止・フックの型・60〜120字）だったので、足りない2点だけ足した:
    (1) **フックは1行目**（前置き禁止） (2) **会話を生む締め**——具体的な問いか言い切り。
    「どう思いますか？」等の汎用の問いは禁止（答える気が起きない）。
  - PT-P1〜P4を「フック→具体→問い」の3部構成へ改定し、スレッド数を本文へ明記
    （P1=2〜4・P2=単発・P3=4〜6・P4=1〜2。コード側の上限 GENERATION_MAX_POSTS と一致）。
    P3へ保存されやすさ（後で見返す価値の予告）、P6の最終ポストへ問いを追加。
  - **プレースホルダー**: P3へ `{題材}`・`{読者の悩み}`、P4へ `{話題}` を追加（UIの入力欄は
    本文から自動導出・実ブラウザで表示確認済み）。P1 `{ニュース}`・P2 `{自分の考え}` は
    UIが名前で参照しているため変更しない。
  - 既定プロンプトはコードが正本（上書きしていない利用者には自動で効く）。プロンプト設計書
    §6.1〜6.7 をコードから機械的に再生成して同期。
  - 検証: check:providers 緑・smoke:live 全シナリオ成功（$0.23。生成物が新構成どおり:
    1行目言い切り→理由→具体→「高機能ツール派？それとも続けやすさ優先派？」の具体的な問い）。

### T-M8-374: 運営ダッシュボードに環境切替・グラフのホバー値・利用者一覧を足す `done`
- 参照: PRD O-6 / monitoring.md / 依存: T-M8-373 / サイズ: S
- 完了条件: 表示中の環境が分かり他環境へ移動できる／グラフにカーソルで値が出る／利用者一覧に代表データが出る
- メモ: 運営者の指示（2026-08-30）。
- 実装メモ（2026-08-30 完了）:
  - **環境別表示は「切替リンク」にした**: DBは環境ごとに完全分離（stgの検証データを本番へ
    混ぜない構成・deployment.md）なので、1画面から3環境を横断して読むには本番へstgのDB鍵を
    置くことになる。各環境の /admin へ移動する形にし、表示中の環境をAPP_ENVでバッジ表示。
  - グラフ: カーソル位置から**最も近い点**を選んで日付と値を表示（点そのものへのホバーを
    要求すると半径2.5pxを狙うことになる）。viewBox縮尺をgetBoundingClientRectで換算。
    タッチ（スマホ）はタップで同じ経路。circleに`<title>`も併設。
  - 利用者一覧: メール・登録日・プラン/状態・X連携・生成/投稿数（利用枠の消費記録）・
    今月の原価（USD）・最終利用。登録の新しい順・最大200人。運営者しか見ない画面なので
    メールをそのまま出す。

### T-M8-373: データ分析基盤と運営ダッシュボード（/admin） `done`
- 参照: PRD O-6 / 要件02 §3.31 / 要件04（kpi_snapshot） / 依存: なし / サイズ: L
- 完了条件: /admin で事業KPI（MRR・原価・粗利）とファネルが見える／時系列が400日残る／運営者以外には404
- メモ: 運営者の指示（2026-08-29「データ分析ができる基盤と管理者用サイト。データは400日。事業KPIとプロダクト改善用の両方。最小限の機能・分析はしっかり」）。
- 実装メモ（2026-08-30 完了）:
  - **仮定（運営者不在のため置いたもの）**: (1) 管理画面の入口は `/admin`・ゲートは
    「ログイン済み かつ メール一致（`SUPPORT_EMAIL`）」・他者には**404**（存在を教えない）
    (2) MRR＝active×プラン月額（トライアル除外・past_dueは含む） (3) 円換算1ドル=160円（PRD §6.1と同じ）
    (4) 新しいcronは増やさず scheduler_tick 相乗り（原則3）
  - **なぜスナップショットか**: 元データは消える（原価400日・jobs90日）か現在値しか無い
    （profiles.planは変わると前の値が消える）。`kpi_daily`（日付×指標×内訳）へ日次で書き出す。
    状態指標は「前日の終わり」、出来事指標は直近3日を毎回再計算（遅延データを拾う）。
    **初回は残っている元データから最大400日をバックフィル**——手作業のコマンドを作らない（原則3）。
  - 原価台帳の保持を40→400日へ（運営者の決定「明細も400日残す」）。kpi_daily も400日で削除。
  - **実DBテストが2つの実バグを捕まえた**: (1) `profiles.plan` は null があり得る
    （ローカルで558行・not-null制約違反になっていた） (2) `cancellation_surveys` の列は
    `reason` ではなく `reasons`（配列・後のmigrationで置換）——/admin が500になっていた。
    列名の思い込みは実DBを通さないと出ない。
  - E2E: 非運営者404／未ログインはlogin／運営者にKPI表示＋モバイル幅で横スクロール無し。
    実ブラウザ確認済み（1440/390・コンソールエラー0）。
  - **本番デプロイで設計の穴を1つ踏んで直した**: コードのデプロイがmigration適用より数分早く、
    その間のtickがclaim後に「relation kpi_daily does not exist」で落ち、**その日のclaimだけが
    消費されて翌日までスナップショットが書かれない**状態になった。claimと本処理を
    同一トランザクションへ（affiliate_batch の claimTx と同じ形）。失敗すればclaimごと
    ロールバックされ、次のtick（5分後）が再試行する。回帰テスト付き。

### T-M8-370: 画像モデルのenvが必須のままで、stg/prdへ反映できないのを直す `done`
- 参照: 要件01 §3.5 / プロンプト設計書 §3 / 依存: T-M8-334 / サイズ: S
- 完了条件: 画像モデルのenvが無くてもbuildが通る／画面の画像provider一覧が消えない
- 実装メモ（2026-08-29 完了）:
  - **反映が止まった実害**: `git push` 後のVercel buildが3回とも
    「環境変数の検証に失敗しました - OPENAI_IMAGE_MODEL は preview 環境で必須です」で失敗し、
    **stagingが古いまま**だった。`release:staging` はCIとmigrationは見るが**Vercelのbuild結果は
    見ない**ので「✅ 反映と検証が完了しました」と出ていた（画面のLP文言が変わらないことで気付いた）。
  - **原因**: T-M8-334で「画像モデルの既定は**コード**（`DEFAULT_IMAGE_MODELS`）が持ち、envは
    上書き用」と決めたのに、`ALWAYS_REQUIRED` から外し忘れていた。Vercelの preview/production
    どちらにも `OPENAI_IMAGE_MODEL` が無い（決定どおり不要なので消されたと思われる）。
    **稼働中のデプロイは取り込み済みの値で動く**ため、画面はずっと正常に見えていた（原則1に反する形）。
  - `ALWAYS_REQUIRED` から `OPENAI_IMAGE_MODEL`/`GEMINI_IMAGE_MODEL` を外し、空文字も未設定として
    扱うようにした。`imageProvidersFor` の判定も**運営キーの有無だけ**へ（モデルenvを条件にすると
    「実行はできるのに画面では選べない」——この関数が無くそうとしていた食い違いそのものが起きる）。
  - 回帰テスト: envに画像モデルが無い状態で `buildServerEnv` が通ること／provider一覧に残ること。
    どちらも修正を戻すと赤くなることを確認。`APP_ENV=preview OPENAI_IMAGE_MODEL="" npm run build`
    が通ることも実際に確認した。

### T-M8-369: AIへの指示が「## 6.」のままで、提案の反映が必ず弾かれるのを直す `done`
- 参照: プロンプト設計書 §6.12（PT-SUGGEST）/ 依存: T-M8-356 / サイズ: S
- 完了条件: SUGGESTの出力が保存検証を通る見出し数になる／画面の案内文も実際の上限と一致する
- メモ: セキュリティレビュー後のCI失敗を追う過程で発見（2026-08-29）。
- 実装メモ（2026-08-29 完了）:
  - **正本（プロンプト設計書 §6.12）は `## 1.〜## 5.` に直っていたのに、実際に送るプロンプト
    （`gen-prompts.ts`）が `## 1.〜## 6.` のままだった**（T-M8-356 の取りこぼし）。
    AIは6見出しのアカウント.md改訂案を返し、それを貼ると `validateManualBaseMd`（5見出し）に
    **必ず弾かれる**。プロンプト自身が「貼った先の保存検証で弾かれる」と書いていたとおりの状態。
  - あわせて画面の案内文（`prompt-preset-manager.tsx`）も「## 1.」から「## 6.」までと
    案内していたので5へ。`base-md.ts` のコメントと `suggestion.db.test.ts` の6見出しの
    テストデータも実際の形式へ揃えた。
  - **snapshotテストは守れなかった**。`gen-prompts.test.ts` の「design doc §6 snapshot
    (drift detection)」は**保存済みsnapshotとの比較**なので、コードとdocsがずれても
    コードが変わらない限り緑のまま。docsを直してコードを直し忘れた形は検出できない。

### T-M8-368: 保存E2Eが「何が起きたか」を出せるようにし、入力が消える競合を閉じる `done`
- 参照: 要件06 §2.1 / 依存: なし / サイズ: S
- 完了条件: 保存が失敗したら理由が出る／冷えたサーバでも入力が消えない
- 実装メモ（2026-08-29 完了）:
  - CIで「アカウント.mdを保存」が落ちたが、**保存の失敗はトーストではなくインライン通知**に出る
    仕様なのに成功トーストだけを待っていたため、結果が「要素が見つからない」で理由が出なかった。
    両方を待ち、失敗ならその文言を添えて落とす形にしたところ、**1回目のCIで原因が判明**した
    （「見出しの形が合っていません」＝T-M8-369）。
  - さらに **hydration前に入れた文字は直後の再描画でサーバー側の値へ戻される**。そのまま押すと
    「元の本文を保存した」ことになる。`toHaveValue` で**残ったことを確かめてから押す**形にした
    （残らない間は入れ直す）。devサーバが冷えているCIでだけ再現し、手元では常に通っていた。
  - `Notice` に `data-slot`/`data-tone` を追加（表示は変わらない）。重大度で待てるようにする。

### T-M8-367: 画像アップロードの本文サイズ上限を5MBへ（D-50・案A） `done`
- 参照: D-50 / 要件06 §6 / 依存: T-M8-353 / サイズ: S
- 完了条件: 1MBを超える画像がアップロードできる／入口の上限表記が実際の上限と一致する
- メモ: 運営者の決定（2026-08-29「案A」）。
- 実装メモ（2026-08-29 完了）:
  - `next.config` の `experimental.serverActions.bodySizeLimit` を既定1MB→**5MB**へ
    （Next 16 では experimental の下）。全 Server Action の受付本文が広がる（DoS surface）が、
    全 Action は認証必須で未認証の入口は増えない。
  - `UPLOAD_MAX_INPUT_BYTES` を 20MB→**5MB**（Xの画像上限）へ揃えた。フレームワークの本文上限より
    大きい値をコードに書くと、この検査に届く前にフレームワーク層で汎用エラーになり
    「何MBまでか」を画面で言えなくなる（D-50の根本）。
  - **E2Eが小さい画像だけで確かめていたのがD-50を素通しにした原因**。乱数ピクセルで約2MBのPNGを
    作ってアップロードする検査を足した（1MB超が通ることを固定。この検査は変更前なら赤くなる）。

### T-M8-366: ニュースのsource_urlをhttp(s)必須にする（多層防御） `done`
- 参照: 要件04 §2 / プロンプト設計書 §6.10 / 依存: なし / サイズ: S
- 完了条件: 非http(s)のsource_urlを持つニュースitemを落とす／正規のニュースは影響なし
- メモ: セキュリティレビュー（2026-08-29）で発見。
- 実装メモ（2026-08-29 完了）:
  - `source_url` はAIが書き、画面で `<a href>` として描かれる。`z.url()` は
    `data:`・`ftp:`・`javascript:` も通すので、**プロンプトインジェクションで非http(s)の
    URLを混ぜられると**リンクに載る（AIはWeb検索で任意のページを読むため到達しうる）。
  - **実害は限定的**（反証で確認）: React 19 が `javascript:` を実行前に止め、CSPが
    inline script を止め、ブラウザがリンクからの `data:` トップ遷移を止める。だが
    **正規のニュースURLは必ず http(s)** なので入口で落とすのが素直（原則: 多層防御）。
  - `newsItemSchema.source_url` に http(s) の refine を足した。無効なitemは
    `pickValidItems` が捨てる（応答全体は捨てない）。回帰テストを追加し、
    修正を戻すと赤くなることを確認した。

### T-M8-365: ニュースが長い文字列で横スクロールする（スマホ幅） `done`
- 参照: 要件06 §2（画面幅）/ 依存: なし / サイズ: S
- 完了条件: 折り返せない長い文字列を含むニュースがあっても390pxで横に伸びない
- メモ: 全体テストの通し実行で `mobile-layout` が落ちて発見（2026-08-29）。単独実行では通っていた。
- 実装メモ（2026-08-29 完了）:
  - **落ちる条件はデータだった。** ニュースの見出し・本文はAIが書いた文章がそのまま入るので、
    長いURLや区切りの無い語が混ざる。**そのときDBに入っていた内容次第**でしか落ちず、
    通し実行でだけ落ちて単独では素通しだった。長い文字列を入れるE2Eで条件を固定した。
  - 原因は3つ重なっていた。(1) 見出し・本文に `break-words` が無い（`line-clamp` は行数を
    切るだけで1行の幅は切らない）。(2) gridの子に `min-w-0` が無く、**箱の方が中身に合わせて
    広がる**（`break-words` を付けても効かない）。(3) 一覧が
    `repeat(auto-fill, minmax(0,1fr))` で、最小幅0だと入る限りトラックを作るため
    **狭い画面でカードが数十pxまで潰れ**、中の固定幅ボタンがはみ出す。件数が増えるほど起きる。
  - ホーム（重要ニュース）にも同じ (1) があったので直した。
  - **入れたニュースは必ず片付ける。** `news_items` はアカウントに紐づかないのでfixtureの
    後片付けでは消えず、残すと**他のテストが見るニュースの中身が変わる**
    （この作業中に `gen-context.db.test.ts` を実際に落とした）。新しいE2Eに削除を入れた。

### T-M8-364: 分析用データに400日の保持期間を付ける `done`
- 参照: 要件02 §3.11・§3.22 / 依存: T-M8-363 / サイズ: S
- 完了条件: 400日を過ぎた取得済み投稿とフォロワー数の記録が消える／1年以内は残る
- メモ: 運営者の決定（2026-08-29・確認のうえ「400日で消す」）。
- 実装メモ（2026-08-29 完了）:
  - `x_timeline_posts`（分析のために読んだ自分の投稿）と `follower_snapshots`（1日1行）に
    **削除経路が無く、アカウントが生きている限り増え続けていた**。
  - **利用者への影響を先に確認した**（原則5）: 1年より前の分析レポートで引用投稿が空欄になり、
    1年より前のフォロワー推移が描けなくなる。運営者の判断で400日を採用。
  - 投稿は**投稿日時で切る**（取得日ではない）。取り込み直しで `fetched_at` が新しくなっても、
    古い投稿は古い投稿のまま扱う。
  - **ニュース取得のAI費用（月$137〜156・最大の費用）は今のまま**という判断も同時に得た
    （1日2回×6分野・Web検索3回を維持）。

### T-M8-363: 増え続けるだけの表に保持期間を付ける（サーバー費用） `done`
- 参照: 要件02 §3.8・§3.16 / 要件04 §14 / 依存: なし / サイズ: S
- 完了条件: 終わったジョブとStripeイベントが90日で消える／実行中のジョブは消えない
- メモ: 運営者の指示（2026-08-29）「サーバーコストが高くなっている原因があれば改善」。
- 実装メモ（2026-08-29 完了）:
  - 保持期間を持つ表を数え上げたところ、`generation_jobs` と `stripe_events` に
    **削除経路が1つも無かった**。前者は1実行1行で `input` に上書きプロンプト（最大8,000字）まで
    入るため、**使うほど無限に増える**（原則4）。
  - **終端した行だけ**を消す。queued/running を消すと実行側が「jobが無い」で黙って終わる。
    子（`parent_job_id`）が残っている親も残す。
  - **40日ではなく90日。** `request_key` が全statusにまたがる恒久uniqueで、行を消すと
    同じキーで作り直せるようになる。予約の期限（60分）や日次キーの寿命を大きく超えるところまで待つ。
  - 成果物（下書き・レポート）は別の表なので、**画面から何かが消えることはない**。
  - 検査: `schedule-cleanup.db.test.ts`（90日超は消え、直近と実行中は残る）。

### T-M8-362: アカウント.mdの変更履歴とロールバックを廃止する `done`
- 参照: 要件02 §3（`base_md_versions` 削除）/ 要件05 §8 / 要件06 §3.7 / PRD M-1 / 依存: T-M8-332 / サイズ: M
- 完了条件: 履歴の表・画面・Action・記録処理が残っていない／「履歴から戻せる」と案内する文言が無い／
  `base_md_version`（楽観ロック）は残っている
- メモ: 運営者の指示（2026-08-29）「アカウント.mdの変更履歴は不要」。
- 実装メモ（2026-08-29 完了）:
  - **戻す道は本棚（`prompt_presets`）に移っていた。** アカウント.mdは複数持てて使う1本を
    選べる（T-M8-332）ので、版を自動で積んで戻す仕組みは**同じことの2通り目**だった。
    しかも版ごとに本文全文を持ち、`md_merge` が学習のたびに積むので、
    **利用者が何もしなくてもストレージが増え続けていた**（原則4）。
  - 削除したもの: 表 `base_md_versions`（migration）・RLSポリシー・`base-md-history.ts`
    （保持上限と刈り込み）・`applyRollbackBaseMd`・`listBaseMdVersions`・`rollbackBaseMdAction`
    ・履歴パネル・各所の版記録。seed / clean-test-data / 各テストの掃除リストからも外した。
  - **`x_accounts.base_md_version` は残す。** これは履歴ではなく**楽観ロックの番号**で、
    「保存しようとした間に誰かが後ろで書き換えた」を検出する。消すと同時編集で上書き消失が起きる。
  - **無くなった機能を案内し続けない**（原則2）。保存前の警告を「履歴から戻せる」から
    「残したいときは先に控えを作る」へ、プライバシーポリシーの保有項目からも「変更履歴」を外した。

### T-M8-361: 設定画面のprofilesの二重読みを解消し、スキルを整理する `done`
- 参照: CLAUDE.md「変更影響 → 必須の検証」 / .claude/skills / 依存: T-M8-355 / サイズ: S
- 完了条件: 同じprofiles行を1リクエストで2回読まない／スキルが短くなっても判断材料が減っていない
- メモ: 運営者の指示（2026-08-29）「画面遷移スピードの向上」「スキルも簡潔（かつ低コスト）に」。
- 実装メモ（2026-08-29 完了）:
  - 設定画面が PostgREST 経由で profiles をもう一度読んでいた（App Shell が同じ行を
    pooled接続で読んでいる）。必要な列を共有の1行へ足して往復を1つ消した
    ——**実測でいちばん遅い画面**がこの設定タブだった。
  - **速度は「改善した」と書けるだけの数字が取れていない。** `next dev` の遷移時間は
    ±150msぶれ、往復1本ぶんは埋もれる。**本番ビルドで測る**手順を `/speed-up` へ書いた。
  - スキルを 1,275行 → 899行（-30%）。**削ったのは重複と一般論だけ**で、
    実際に踏んだ落とし穴は残した。最大の削減は `playwright-cli`（420→58行）——
    ベンダーのコマンド表を丸写ししていたが、詳細は同じディレクトリの `references/` にある。
  - 今日の学びを足した: `/verify-e2e`（外部依存・通り過ぎる状態・前提を作る順番）、
    `/speed-up`（本番ビルドで測る・PostgRESTの二重読み）、`/refactor`（参照0件の洗い方・
    型だけ使われている罠・検出器が死ぬ）、CLAUDE.md（定時処理へ外向き呼び出しを足すとき）。

### T-M8-360: 全体点検で見つけた不具合の修正と、死んだコードの掃除 `done`
- 参照: 要件04 §「Xトークンの先回り更新」 / 要件06 §3.1 / 依存: T-M8-355〜359 / サイズ: M
- 完了条件: 反映を取り消せる／連携が多くてもメールが読める／先回り更新が開発環境で外向きに出ない／
  参照0件のexportが残っていない
- メモ: 運営者の指示（2026-08-29）「全体的に不具合がないかのテスト」「リファクタリング・疎結合化」。
- 実装メモ（2026-08-29 完了）:
  - 実物1周（`smoke:live`）成功（$0.19）。`doctor` を実際に回して**2つの実害**を見つけた。
  - **反映を取り消せなかった**（行き止まり）: 提案は保存するまで残るので、気に入らないと
    開くたびに「まだ保存されていません」が出るのに消す方法が無く、**保存済みへ戻すには
    全欄を手で打ち直すしかなかった**（原則2）。取り消しの導線とActionを足した。
  - **連携が多いとメールが読めなかった**: 67件のhandleが1行に並び、**問題のある1件がその中に
    埋もれていた**。5件を超えたら件数でまとめ、問題のあるものだけ名前で出す。
  - **先回り更新が開発環境でも外向きに出ていた**: Xのtoken endpointへ実際にリクエストする処理で、
    ローカル・E2Eのアカウントは偽tokenを持つ。**毎時、確実に失敗するリクエストを相手のAPIへ
    投げ続ける**形だった（実DBのroute testが20秒で時間切れになって気付いた）。
    `APP_ENV=development` では動かさない（CLAUDE.md「外向き副作用の環境ガード」）。
  - **参照0件のexportを掃除**（26件検出→14件削除）。使われないまま残っていた3つのcomponent
    （`x-account-switcher` / `current-screen-title` / `upgrade-plan-button`）と、
    素通しのwrapper（`textReserveEstimate` 等）、学習の作り替えで役目を終えた関数
    （`replaceProfileSections` 等）。**型だけ使われていた `SwitchAccountAction` は
    `lib/app-shell/types.ts` へ移した**——中身が消えた後もファイルが残っていた原因がこれ。
  - **検出器の相手を実在するファイルへ寄せた**（`dependency-boundaries.test.ts`）。
    消えたファイルを見張り続けると、検査そのものが落ちて何も守らなくなる。

### T-M8-359: 毎朝の運営者メールから「直す必要のない注意」を無くす `done`
- 参照: 要件04 §「Xトークンの先回り更新」 / 要件02 §3.19 / 運用メモ 監視 / 依存: T-M8-96 / サイズ: M
- 完了条件: Xの連携がアクセス期限切れで【注意】にならない／使わない日が続いても要再連携にならない／
  短いDB待ちが数回で【注意】にならない／人間確認が「確認できません」にならない
- メモ: 運営者の報告（2026-08-28・本番8:00のメール）。「特にXアカウント連携のトークンは
  無制限時間で切断されないようにしたい。ユーザー体験が悪い」。
- 実装メモ（2026-08-28 完了）:
  - **Xの連携を切らさない**: `scheduler_tick` に相乗りで**1時間に1回**、期限が90分以内の
    activeなアカウント（最大25件）を先回りで更新する（`x/token-keepalive.ts`）。
    Xのrefresh tokenは**使うたびに入れ替わる**ので、投稿も分析も走らない日が続くと古いまま
    置き去りになり、久しぶりに使ったときに `invalid_request` で要再連携になる（T-M8-96で実際に発生）。
    利用者から見れば「何もしていないのに切れた」で、最も体験が悪い壊れ方。
    **定時トリガーは増やさない**（原則3）。1件の失敗で他を巻き添えにしない。
  - **期限切れそのものを警告しない**: access tokenは2時間で切れる設計で、refresh tokenがあれば戻る。
    毎朝【注意】が出ていたため、**直す必要のない警告が毎日届き**、本当の異常まで読み飛ばされる
    状態だった（原則2）。警告するのは「要再連携」と「自動更新の許可が無い」＝人が操作しないと
    直らない場合だけにした。
  - **DB接続**: 「0.2秒の待ちが5回」で毎朝【注意】になっていた。回数（24時間で20回）と
    待ち時間（1回2秒）の**どちらかを超えたときだけ**知らせる形へ。本物の枯渇は必ずどちらかに出る。
  - **人間確認**: 毎朝のメールだけ接続情報を渡しておらず「確認できません」と出続けていた
    （T-M8-247でStripeに入れた同じ抜け）。doctorのrouteと同じ値を渡すようにした。

### T-M8-358: E2Eの人間確認を手元で完結させる（通しの実行を安定させる） `done`
- 参照: e2e/fixtures/turnstile.ts / 開発とテストの進め方 / 依存: T-M8-357 / サイズ: M
- 完了条件: 通しの実行を繰り返してもログインで落ちない／実行時間が短くなる／
  本物のウィジェットの確認は残る
- メモ: 運営者の指示（2026-08-28）「E2Eを安定になるようにできますか」。
- 実装メモ（2026-08-28 完了）:
  - **落ちる条件は「毎回外部サービスに依存していること」そのものだった。** 125件の
    ほぼ全部がログインから始まり、通し1回で `challenges.cloudflare.com` へ100回以上
    チャレンジを要求していた。繰り返すほど失敗が増える（1→8→21件）。
    待ち時間を伸ばす対処では、失敗が「トークンが来ない」から「ログインだけで時間を使い切る」へ
    ずれるだけだった（T-M8-357）。
  - **スクリプトの配信だけをスタブへ差し替える**（`page.route`）。`window.turnstile` の契約は
    本物と同じで、呼ばれたらすぐダミートークンを返す。**サーバー側の検証は本物のまま通る**
    ——ローカルのsecretはCloudflare公式の「常に成功する」テストキーなので、
    アプリのServer ActionもSupabaseも実際に検証し、トークンが空なら今までどおり失敗する。
  - **本物のウィジェットは1件だけ残す**（`password-reset.spec.ts` の T-M7-26 回帰）。
    許可ドメイン・sitekey・CSPの確認は `npm run check:turnstile` の担当（CLAUDE.mdの必須検証）。
  - 結果: 通しが **25.3分・21件失敗 → 7.1分・0〜1件**へ。**アプリ側のCAPTCHAは緩めていない。**
  - あわせて `ai-settings.spec.ts` の削除テストの順番を直した。**先にjobを畳むと、
    反映の待ちループがその瞬間に「終わった」と判断して `md_merge` を起票する**——
    畳んだ側から新しいjobが生えて削除だけが弾かれていた。画面を作り直してから状態を作る。
  - **走り出す前にrouteを温める**（`globalSetup`）。`next dev` は最初のリクエストで初めて
    コンパイルするため、**1件目のspecだけがその時間を丸ごと被る**（`src/**` を触った直後は
    20〜30秒。CLAUDE.mdの落とし穴にある事象）。落ちるのがいつも先頭という分かりにくい形なので、
    主要routeをまとめて叩いておく。失敗しても止めない（速さのためで、正しさの前提ではない）。

### T-M8-357: E2Eのログインが連続実行でTurnstileに絞られる `done`
- 参照: e2e/fixtures/test.ts / 開発とテストの進め方 / 依存: なし / サイズ: S
- 完了条件: フルスイートを短時間に繰り返しても、ログイン待ちで落ちない
- メモ: 2026-08-28に観測。**落ちた回数ではなく落ちる条件**（CLAUDE.md）。
- 実装メモ（2026-08-28 完了）:
  - **条件**: フルE2Eは125件がそれぞれログインするため、`challenges.cloudflare.com` へ
    短時間に100回以上チャレンジを要求する。**回すほど失敗が増えた**（1回目1件→2回目8件→
    3回目21件）。落ちたのは全て `signIn` のトークン待ちで、**同じspecを単独で回すと必ず通る**。
    テストキー（常に通過）でもウィジェット本体はCloudflareから読むので、絞られると取れない。
  - 15秒で来なければ**1度だけ読み込み直して**25秒待つ形にした。失敗文言にも
    「連続実行で絞られている可能性」を入れる——次に見た人が原因を探し直さずに済むように。
  - **アプリ側は変えない**（テストのためにCAPTCHAを緩めない）。
  - 同じ全体実行で見つけたもう1つの条件: `ai-settings.spec.ts` の削除テストが
    `learning_analysis` だけを畳んでいたため、**反映のボタンが起票する `md_merge` が残って
    削除だけが弾かれて**いた（単独実行では反映まで進む前に削除が走るので通っていた）。
    前提の畳み込みに `md_merge` を足した。
  - 反映を**保存せずに2回続けた**ときにフォームが作り直されない穴も塞いだ（提案の中身から
    keyを作る）。「提案があるか」だけを見ていると、2回目で「押しても入らない」に戻る。
  - 読み込み直しを入れたあと、失敗の形が「トークンが来ない」から
    **「ログインだけで60秒を使い切る」**へ移った（落ちるのは常にログイン直後の遷移待ちで、
    アプリ側は数秒で応答している）。トークン待ちを8秒に縮め、手元のテスト上限を60→90秒にした。

### T-M8-356: 反映した内容が欄に入らない不具合の修正と、画面・md構成の整理 `done`
- 参照: 要件06 §3.1 / プロンプト設計書 §3 / 依存: T-M8-349, T-M8-355 / サイズ: L
- 完了条件: 反映した内容がアカウント設定の欄に入る／「文体・自分らしさ」が画面とmdから消える／
  参考ソースの枠がペルソナの上にある／「アカウント設定を反映する」が枠の右下にある
- メモ: 運営者の報告・指示（2026-08-28）。
- 実装メモ（2026-08-28 完了）:
  - **反映が欄に入らなかった原因**: フォームの初期値は `useState` なので、`router.refresh()` で
    新しい提案を渡しても**すでにmountされた画面は古い値のまま**だった。押しても欄に何も入らず、
    しかもエラーも出ない——**静かに壊れる**型（原則1）。提案の有無を `key` に混ぜて
    作り直すようにし、E2Eで「提案がフォームへ入る→保存で確定して提案が消える」を固定した。
  - **アカウント.mdの見出しを6→5へ**（「## 5. 文体・自分らしさ」を廃止）。既存の本文は
    migrationで「参考にする型」の先頭へ畳んだ——**不要と言われたのは見出しの区分であって、
    利用者が書いた文章ではない**（原則1）。実物・履歴（`base_md_versions`）・本棚
    （`prompt_presets`）の3か所とも変換する。片方だけ直すと、**版から戻した瞬間に
    6見出しのmdが復活して保存できない**状態になる。
  - 画面: 参考ソースの枠をペルソナの上（同じ白いカードの中）へ移し、実行ボタンを枠の右下へ。
    登録済みの一覧もその枠の中（実行ボタンの上）に入れ、記入 → 材料の確認 → 実行の順に並べた。
  - **E2Eのアカウント.mdが見出しの無い1行だった**ので本物と同じ構造にした。この状態から
    「アカウント設定を保存」する経路は必ず構造エラーで落ちるのに、**どのE2Eもそこを踏んでいなかった**
    （保存の経路がテストの網に入っていなかった）。
  - 使われていない `replaceLearningSections` と関連定数を削除（T-M8-336で学習の反映先が
    1〜4へ移ったときから死んでいた）。

### T-M8-355: 内部品質の整理（重複クエリ・巨大ファイル・md5/6章の記入欄） `done`
- 参照: 要件06 §3.1 / 要件05 §5 / 依存: T-M8-349〜354 / サイズ: M
- 完了条件: 振る舞いを変えずに往復とファイルを減らす／アカウント.mdの5・6章を
  アカウント設定の画面から書ける／全テストが緑
- メモ: 運営者の指示（2026-08-28）「改めてリファクタリング・疎結合化・画面遷移スピードの改善・
  最終入念テスト」。あわせて「他にアカウント.mdへの入力項目があればUIにも記入欄を」。
- 実装メモ（2026-08-28 完了）:
  - **profiles の二重読みを1回にした。** プロンプト画面が `active_x_account_id` 欲しさに
    PostgREST経由でprofilesをもう一度読んでいた。共有の1行（`loadRequestProfile`）へ
    その列を足して往復を1つ消した——「呼び出しごとに列を絞った別クエリを書かない」という
    この関数の方針そのもの（T-M8-286）。
  - アカウント設定タブの2本のクエリ（参考ソース一覧・進行中jobの有無）を `Promise.all` へ。
    互いに依存しないので直列だと往復2回ぶんが待ち時間に乗っていた。
  - `drafts-list.tsx`（861行）から画像パネルを `draft-image-panel.tsx` へ切り出した。
    下書きカードは「どの画像を渡すか」だけ知っていればよく、jobのpollやファイル選択の
    作法を知らずに済む（664行＋215行）。
  - **アカウント.mdの5・6章（文体・自分らしさ／参考にする型）に記入欄を足した。**
    1〜4はフォームから機械生成されるが、5〜6は人が書く場所で、これまでは
    プロンプト画面のmdエディタからしか触れなかった。**渡されなければ1バイトも変えない**
    （学習・mdエディタ・ロールバックが書いた内容を別経路の保存で消さない）。
  - 画面遷移の実測（`next dev`・3回の中央値）: /app 336ms・設定 350ms・投稿作成 342ms・
    プロンプト 340ms・スケジュール 264ms・料金 279ms。**どれも突出していない**ので、
    往復を1つ減らした以上の手は入れていない（数字が動かない変更を「速くなった」とは書かない）。
  - **穴を1つ塞いだ**: アカウント.mdを設定の保存前から作れるようにした（T-M8-350）結果、
    「先に控えを1件作ってからアカウント設定を保存する」と**生成が読む内容を表す行がどこにも無い**
    状態が作れた（画面には控えしか並ばず、いま何が効いているのか説明できない・原則1）。
    一覧表示時の補完条件を「本棚が空のとき」から「**使用中が1件も無いとき**」へ変えた。
  - 検査: `persona-settings-store.db.test.ts` に5・6章の往復（渡す／渡さない／空にする）、
    `prompt-presets.db.test.ts` に「控えだけがある状態で開いても使用中が並ぶ」。

### T-M8-354: 料金カードの機能一覧を運営者指定の内容へ差し替える `done`
- 参照: 要件06 §1（LP）・§2 / src/lib/plan-comparison.ts / 依存: T-M8-171 / サイズ: S
- 完了条件: ホーム（LP）と `/plans` の機能一覧が指定の共通8行＋プラン別2行になる／
  3枚のカードの高さが揃ったまま
- メモ: 運営者の指示（2026-08-28）。
- 実装メモ（2026-08-28 完了）:
  - 正本は `plan-comparison.ts` の1本のまま（LPと `/plans` は同じ行定義を描く。
    画面ごとに書き写すと食い違う）。共通8行＋「AI生成・X連携用の専用鍵（APIキー）」
    「利用上限（契約期間ごと）」の2行にした。
  - **BYOKのAPI実費の開示**（「利用料はご自身のAPI課金」）は落とさずAPIキーの行へ残した
    ——T-M8-171でこの行が唯一の常時表示になっているため（景表法）。
  - 機能の行が9→10になったので、カードのCSS subgridの `span` を18→19へ揃えた
    （揃っていないと3枚の高さがずれる）。
  - 文言は `e2e/plans.spec.ts` が固定する（**画面の文言はE2Eでしか守られない**）。
  - あわせて `rls.db.test.ts` が public の全表へ `grant` していたのを、**実際に読む表だけ**へ絞った。
    `GRANT` は表ごとにACCESS EXCLUSIVEロックを取るため、並行して走る他のdbテストの書き込みと
    競合し、**フルスイートでだけ稀に落ちていた**（2026-08-28に観測）。

### T-M8-353: 下書きに自分の画像を添えられるようにする `done`
- 参照: 要件06 §6 / 要件05 §5 / 依存: なし / サイズ: M
- 完了条件: 下書きから画像をアップロードできる／差し替え・取り外しができる／
  投稿するとその画像が添付される／投稿作成・スケジュールの画像チェックの下に案内が出る
- メモ: 運営者の指示（2026-08-28）。生成した絵が意図と違うとき、画像なしで出すか
  作り直して当たりを待つしかなかった。
- 実装メモ（2026-08-28 完了）:
  - **足すのではなく置き換える。** 投稿に使われるのは最初の `ready` 画像1枚なので、
    2枚目を足せる形にすると「画面には2枚あるのに投稿されるのは片方」という
    説明できない状態になる（原則1）。差し替えた実体はStorageから消す（原則4）。
  - 中身は生成画像と同じ `normalizeForX` に通す。**申告MIMEは信用できず**、
    Xの5MB上限をここで守らないと**投稿の瞬間に初めて失敗する**（そのときには本文だけが
    Xに残り得る）。断る理由（形式・大きさ）は画面にそのまま出す。
  - 形式→拡張子の対応（`EXT_BY_FORMAT`）を `image-normalize.ts` へ移した——
    Storageのpathを作る側が生成とアップロードの2か所になったため。
  - カードで**生成物か自分のアップロードか**を区別して出す（どちらが投稿されるか分かるように）。
  - 検査: `draft-image-upload.test.ts`（受け取り条件・置き換え）、
    E2E `draft-image.spec.ts`（**ファイル選択からStorage・署名URL・描画まで通す**。
    `naturalWidth > 0` まで見る——要素があるだけでは読み込めたことにならない・T-M7-22）。

### T-M8-351: 報酬率の人数にTrial中も数え、Trial解約は解約済みにする `done`
- 参照: docs/cp/invite_cp.md §3・§5 / 依存: T-M8-345 / サイズ: M
- 完了条件: Trial中の招待も率の人数に入る／Trial中に解約した人は数から外れ「解約済み」と表示される／
  画面に出る人数と実際に付く率が同じ条件で決まる
- メモ: 運営者の指示（2026-08-28）「Trial中の人も1カウント。ただしTrial中に解約した人は解約済み扱い」。
- 実装メモ（2026-08-28 完了）:
  - 数える条件を `COUNTED_REFERRAL_SQL`（`config.ts`）に1本化した。**率を決める場所
    （`store.ts`）と画面に出す場所（`summary-server.ts`）が同じ条件を読む**——片方だけ直すと
    「35%と出ているのに30%で報酬が付く」という、利用者からは説明できない状態になる（原則1）。
  - 条件は3つ: 報酬期間が終了していない／契約が続いている（`trialing` を含む）／
    **全額返金だけの利用者ではない**（T-M8-236の意図を保つ。ここを落とすと返金で戻ったお金の分だけ率が居座る）。
  - 画面のKPI「有料招待」を「**報酬率の人数**」へ改名した（Trial中が入るので「有料」だと数が合わない）。
  - **テストの前提が現実と違っていた**: 招待テストは `profiles.subscription_status` を
    既定の `incomplete`（申込の途中）のまま支払わせていた。契約状態を明示するよう直した。
  - 検査: `lifecycle.db.test.ts` に2件（Trialも数える／Trial解約は外れる）、
    `summary.db.test.ts`（新規。画面へ渡す人数と状態を本番実装で確認）。

### T-M8-350: プロンプトの持てる件数に上限を置き、アカウント.mdを自由に足せるようにする `done`
- 参照: 要件06 §3.8 / 要件02 §3.29 / 依存: T-M8-332 / サイズ: M
- 完了条件: アカウント.md5件・投稿作成プロンプト20件・画像生成プロンプト5件で「追加」が止まり理由が出る／
  サーバー側でも断る／アカウント設定が未保存でもアカウント.mdを作れる
- メモ: 運営者の指示（2026-08-28）。
- 実装メモ（2026-08-28 完了）:
  - 上限は**Xアカウント1件あたり**で数える。プロンプトはXアカウントに紐づくので、
    ここ以外に数えられる単位が無い（1アカウントの利用者なら「1ユーザーにつき」と同じ）。
  - **残数を作る前に見せる**（「n / 上限件」）。書き終えてから弾かれると、書いた内容の行き先が無い。
    画面で「追加」を無効化したうえで、`applyCreatePromptPreset` / `applyCreatePattern` でも断る
    （画面だけの制限は迂回できる）。
  - アカウント.mdの区分を**アカウント設定が未保存でも開ける**ようにした。以前は
    「アカウント設定を保存すると作られます」の空状態で画面ごと止めており、
    自由入力で足せる形と矛盾していた。新規作成の本文には**6見出しだけの雛形**を入れる
    （白紙だと「保存できない理由が見出しの形」という、書いてみるまで分からない失敗になる）。
  - 検査: `prompt-presets.db.test.ts`（6件目を断る）、`post-patterns-crud.db.test.ts`（21件目を断る）。

### T-M8-349: 反映を「保存前の提案」にし、アカウント設定画面の並びを直す `done`
- 参照: 要件06 §3.1 / 要件02 §3.3 / 要件04 §12 / 依存: T-M8-346 / サイズ: L
- 完了条件: 参考アカウント1件で「アカウント設定を反映する」を押しても `job_conflict` にならない／
  反映した内容がフォームへ入り、保存するまで確定しない／対象アカウントがタブ直下に出る／
  各欄に残数が出る／ペルソナ〜NG設定が1枚の白いカードで、保存ボタンが左下にある
- メモ: 運営者の報告（2026-08-28）「参考アカウントだけ入れて押すと『ほかの操作と重なりました』になる」。
- 実装メモ（2026-08-28 完了）:
  - **原因**: 画面は分析の完了を待っていたが、**待ち時間を使い切ったあともそのまま反映を起票**していた。
    分析jobが動いている間はサーバーが `job_conflict` で拒むため、必ずこの失敗になる。
    しかも文言が「ほかの操作と重なりました。画面を再読み込みしてから…」で、
    **待つべき場面で再読み込みという無意味な行動**を指示していた（原則2）。
  - 待ち切れなかったら**起票せずに理由を出す**（「分析がまだ終わっていません。一覧が『反映済み』に
    なったら、もう一度押してください」）。サーバー側の文言もこの状況のものへ変えた。
  - **反映＝保存ではなくした。** `md_merge` は `settings_proposal`（新設）へ書き、
    `settings`・アカウント.md・版は触らない。フォームがその提案を読み込み、
    「アカウント設定を保存」で確定する（保存で提案は null へ戻る）。
    以前は押した瞬間に本番の設定が変わり、**中身を見る前に書き換えられていた**。
  - 参考ソースの一覧を**設定が未保存でも読む**ようにした（`base_md_version >= 1` の条件を外した）。
    登録しても一覧が空のままで「登録できたのか」が分からなかった。
  - 画面: 対象アカウントをタブ直下へ、ペルソナ〜NG設定を1枚の白いカードへ（中は区切り線）、
    保存ボタンをカードの左下へ、参考ソースを設定の下へ。各欄に残数（n/上限件）。
  - 検査: `md-merge.db.test.ts`（提案だけ書き版を積まない）、`persona-settings-store.db.test.ts`
    （保存で提案が消える）、E2E `learning-setup.spec.ts`。実ブラウザで1440px・390pxを確認。

### T-M8-348: ローカルDBを毎回作り直さない（migrationは差分適用を既定に） `done`
- 参照: ローカル開発環境 §4 / supabase/README.md / 依存: なし / サイズ: S
- 完了条件: migrationを足したときの手順が `supabase migration up` になっている／
  `db reset` が要る条件（適用済みファイルを編集・削除したとき）が明記されている
- メモ: 運営者の指摘（2026-08-28）「修正依頼のたびにローカルDBがリセットされているが意図的か」。
- 実装メモ（2026-08-28 完了）:
  - **意図ではなく、手順書がそう書いていた。** `supabase migration new` の説明が
    「→SQL記述→`db reset`→`npm test`」で、migrationを足すたびに全消しする流れだった。
    加えて 2026-08-27（T-M8-344）に**適用済みのmigrationファイルを1つ削除**したため、
    そのときは差分適用では辻褄が合わず reset が要った。
  - 既定を `supabase migration up`（差分適用・データは残る）へ変えた。**`db reset` は
    適用済みファイルを編集・削除したときだけ**と条件を書いた。あわせて
    **一度適用したファイルは編集せず新しいmigrationを足す**という原則を明記した。
  - ローカルの適用状況は `supabase migration list --local` で確認できる（現在ズレなし）。

### T-M8-347: 運営者向けアラートの宛先を `SUPPORT_EMAIL` に戻す（環境変数を増やさない） `done`
- 参照: 要件01 §8 / デプロイ手順 / 依存: T-M8-343 / サイズ: S
- 完了条件: 新しい環境変数を追加せずにアラートが届く／`check:mail-sender` の判定が同じ変数を見る
- メモ: 運営者の指示（2026-08-28）「宛先は `SUPPORT_EMAIL` でよい。`OPERATOR_ALERT_EMAIL` が
  新しい環境変数なら不要」。
- 実装メモ（2026-08-28 完了）:
  - T-M8-343で足した `OPERATOR_ALERT_EMAIL` を撤去し、`SUPPORT_EMAIL` 一本に戻した
    （`env-schema.ts` / `operator-alert-server.ts` / `scripts/check-mail-sender.mjs`）。
    **環境変数は増やすほど「どれを設定し忘れているか」が分からなくなる**（原則3）。
  - 「差出人と宛先が同じだと Gmail の受信トレイに入らない」という**注意そのものは残した**
    （`check:mail-sender` が同値のとき警告する）。変数を1つにしても罠は消えないため。
  - `legal-entity.ts` のコメントが消えた変数名を指していたので直した。

### T-M8-346: 参考ソースの入口を1枠にまとめる（記入欄＋1ボタン・見出しの重複を削る） `done`
- 参照: 要件06 §3.1・SC-11 / 依存: T-M8-344 / サイズ: M
- 完了条件: 1つの枠に参考アカウント／参考投稿の記入欄がある（増減できる）／欄ごとの追加ボタンが無い／
  下の「アカウント設定を作る」1つで登録から反映まで進む／「アカウント設定」の見出しと説明が無い／強調色を使わない
- メモ: 運営者の指示（2026-08-28）。T-M8-344の形が「登録する枠」と「作る枠」に分かれていて手数が多かった。
- 実装メモ（2026-08-28 完了）:
  - **「登録」と「反映」を1つの操作に畳んだ。** 記入欄に入れて「アカウント設定を作る」を押すと、
    記入分の登録 → 分析の完了待ち → 設定への反映まで進む。以前は欄ごとに「追加」を押して
    登録し、そのあと別の枠のボタンを押す必要があり、**途中で止まったまま気付けない形**だった（原則1）。
  - 欄は「＋ 参考アカウントの欄を増やす」「＋ 参考投稿の欄を増やす」で増やし、✕で減らせる
    （増やすだけだと書き間違いを消せない）。上限に達した種別はボタンを無効化して残り件数を出す。
  - **画面内の見出し「アカウント設定」と説明を削除**した。タブ名と同じ言葉を繰り返していたため。
    対象アカウントの表示だけ残す（切り替えて使う人が、どのアカウントを直しているか分かるように）。
  - 枠の強調色（ピンク）をやめ、他のカードと同じ地にした。
  - 検査: `npm run test:e2e -- learning-setup.spec.ts`。実ブラウザで1440px・390pxを確認
    （横スクロールなし・コンソールエラーなし）。

### T-M8-345: 招待した人が解約したら報酬率の人数から外す（一覧では解約済みと分かる） `done`
- 参照: docs/cp/invite_cp.md §3・§5 / 要件02 §3.26 / 依存: T-M8-236 / サイズ: S
- 完了条件: 招待したユーザーが解約すると、次に作られる報酬の率が下の段へ戻る／
  「招待ユーザー」欄で解約済みだと分かる／確定済みの報酬の率は変わらない
- メモ: 運営者の指示（2026-08-28）。
- 実装メモ（2026-08-28 完了）:
  - 率を決める人数を**「いま続いている招待」だけ**にした（`affiliate_attributions.commission_terminated_reason`
    が null のものに限る）。**率は報酬を作った時点のsnapshot**なので、過去に確定した報酬は動かない。
  - 「招待ユーザー」の状態表示を、報酬期間の終了だけでなく**購読の状態**（`canceled` /
    `unpaid` / `incomplete_expired`）からも「解約済み」にした。報酬期間の終了は
    初回課金後にしか起きないため、それだけを見ると**Trial中に解約した人が「Trial」のまま**残り、
    人数が減った理由が画面から辿れなかった（原則2）。
  - 検査: `lifecycle.db.test.ts` に「招待した人が解約すると、次の報酬から率の人数が減る」を追加
    （解約前は35%・解約後は30%で報酬が作られることまで見る）。

### T-M8-344: 参考ソースを入口にしてアカウント設定を作る（反映はボタン・通知は廃止） `done`
- 参照: 要件06 §9 / 要件04 §12 / プロンプト設計書 §6.14 / 依存: T-M8-341 / サイズ: L
- 完了条件: アカウント設定が未保存でも参考ソースを登録できる／そこから設定を作れる／
  押した後に「アカウント設定を書き換え中です」が出る／お知らせは出ない
- メモ: 運営者の指示（2026-08-27）。「学習ソースによってアカウント設定をする」形にする。
- 実装メモ（2026-08-27 完了）:
  - **順序を逆にした。** 参考ソースをアカウント設定タブの**先頭**へ置き、保存を待たずに使える。
    「誰に何を発信するか」を最初から言葉にできる人ばかりではないので、
    **真似したいアカウントを挙げるところから始められる**方が入口として易しい。
  - **分析と反映を分けた。** 登録時は分析だけ（`learning_analysis`）。設定への反映は
    「参考ソースからアカウント設定を作る／更新する」を押したときに単独 `md_merge` が走る——
    自分の設定がいつ書き換わるかを利用者が決められるようにするため。
  - MD-MERGEが**アカウント設定が未保存でも走る**ようにした（`<current>` を "none" にして
    分析だけから初版を作る）。**材料が1件も無ければ押せない**（根拠の無い設定を作らない）。
  - **通知を廃止**（T-M8-341で入れたものを撤去）。押していないのに変わることが無くなったので、
    進行と完了は**その画面**で示せば足りる（「書き換え中です」→ 完了で画面を作り直す）。
    使わなくなった通知種別（`settings_updated`）はmigrationごと消した——
    使わない値を残すと「これは何のためにあるのか」が分からなくなる。
  - アカウント設定画面のUIを整えた: 画面の役割を1文で言う導入、見出し「アカウント設定」、
    用語を「参考ソース」へ統一（「学習ソース」と混在していた）、
    保存前の警告から「学習で磨いた分が戻る」を削除（**もう戻らない**ので嘘になった）。
  - 検査: E2E `learning-setup.spec.ts`（順序・押せない理由・未保存でも欄が使えること）。
    実ブラウザでPC/モバイルの描画とコンソールエラーまで確認した。

### T-M8-342: 差出人アドレスで実際に送れるかを確かめるコマンド `done`
- 参照: デプロイ手順 / 依存: T-M8-339 / サイズ: S
- 完了条件: `npm run check:mail-sender` で接続と名乗れるかが分かる／`--send` で実際に1通送れる
- メモ: 運営者の依頼（2026-08-27）「『他のアドレスとしてメールを送信』をやったと思うので確認して」。
- **確認結果（2026-08-27）**: `support@exosai.net` を名乗って送信**できました**。設定は有効です。
- 実装メモ: **拒否は送信時にしか返らない**（設定画面を見ても分からず、
  「利用者に確認メールが届かない」形でしか表面化しない）ので、実際に送る経路を用意した。
  宛先は `SUPPORT_EMAIL`（運営者自身）に**固定**する——2026-07-27の98通誤送信があるため、
  宛先を引数で受けられるようにはしない。秘密値は出さず、SMTPユーザーはドメインだけ表示する。

### T-M8-341: 学習がアカウント設定そのものを書き換える形へ（通知つき） `done`
- 参照: プロンプト設計書 §6.14 / 要件02 §3.4 / 依存: T-M8-336 / サイズ: M
- 完了条件: 学習の結果がフォームの各項目へ入る／アカウント.mdはそこから作り直される／
  設定フォームを保存しても学習の成果が消えない／変わったことが通知で分かる
- メモ: 運営者の指示（2026-08-27）「全項目で良い。勝手に変わっていて良い。変わる旨は伝えたい」。
- 実装メモ（2026-08-27 完了）:
  - T-M8-336ではアカウント.mdの**本文**を書き直していたが、それだと
    **設定フォームを保存した瞬間に消える**（1〜4はフォーム値から機械生成される領域のため）。
    フォームの値そのものを更新する形にして、画面の表示・本文・学習の成果を常に一致させた。
  - PT-MD-MERGEの出力を`personaSettingsSchema`と同じJSONにし、検証を通してから保存する。
    **読めない出力は1回直させ、それでも駄目なら構造エラー**——壊れた設定を書き込むと
    設定画面が開けなくなる。
  - 全項目を対象にした（運営者の判断）。ただし `ng.words`（利用者が個別管理するNGワード）と
    themesの新規作成だけは禁じている——前者は別管理の欄、後者は画面の選択肢に無い値を
    保存できないため。
  - **変わったことを通知する**（`settings_updated` を新設）。利用者が何もしていないのに
    設定が変わるので、黙って変えると「設定した覚えのない値になっている」状態になる（原則1）。
    通知は購読設定に関係なく出し、**変わった項目名**を並べる（値そのものは載せない）。
  - 検査: 設定が保存されること・通知が出ること・壊れた出力を拒むことを単体と実DBで固定した。

### T-M8-340: コード側に既定を持つ環境変数を「必須」の一覧から外す `done`
- 参照: 要件01 §3 / 依存: T-M8-337・T-M8-339 / サイズ: S
- 完了条件: `NEWS_TEXT_PROVIDER`・`EMAIL_FROM` を消しても本番が起動する／一覧に嘘が無い
- メモ: 運営者が本番から `NEWS_TEXT_MODEL`・`NEWS_TEXT_PROVIDER` を削除したのを受けて確認した。
  `NEWS_TEXT_PROVIDER` はzodの既定で埋まるため**起動は落ちない**（確認済み）が、
  必須一覧に残っていると「消したら落ちる」と読める。`EMAIL_FROM` も同様にコード側の既定を持った。
- **ローカルの環境変数を整理した**（運営者の依頼）: 用途ごとに並べ替え、1行ずつ何のための変数かを
  コメントで書いた。値は1つも変えていない（60件を保持し、コード側に既定がある4件を削除）。
  `.env` は `.env.local` と重複していただけなので、説明だけにして値を持たせない形にした。

### T-M8-339: Exos AIから出るメールの差出人を support@exosai.net に統一 `done`
- 参照: 要件01 §3 環境変数 / デプロイ手順 / 依存: なし / サイズ: S
- 完了条件: 認証メールもアプリのメールも `support@exosai.net` から出る／doctorで確認できる
- メモ: 運営者の指示（2026-08-27）。以前は送信に使うSMTPのユーザー名（運営者個人のアドレス）が
  そのまま差出人で、利用者には見知らぬアドレスから6桁コードが届いていた。
- 実装メモ（2026-08-27 完了）: `EXPECTED_SENDER_EMAIL` を正本にし、
  `auth:templates` が `smtp_admin_email` へ流し込む／`operator-mail-server` の from の既定にする／
  `doctor` が実際の設定値と突き合わせる。**envは差し替え用に残す**（未設定でも正しい差出人で出る・原則3）。
- **運営者の操作が要る**: 送信側で `support@exosai.net` を名乗る許可（Gmail/Workspaceの
  「他のアドレスとしてメールを送信」）を先に済ませること。許可が無いと送信自体が拒否される。

### T-M8-338: ニュース取得をBatch APIへ（投げる＋20分おきに取り込む） `done`
- 参照: 要件02 §3.30 / 要件04 §6 / 依存: T-M8-337 / サイズ: L
- 完了条件: 定時cronがバッチを投げて終わる／20分おきの取り込みcronが保存する／
  失効・失敗が行として残る／同期実行と同じ選別・同じ記録を通る
- メモ: 運営者の指示（2026-08-27）「ニュース取得だけで良い。取り込みは20分ごと」。
  **トークン単価が半額**になる（検索料は割引対象と明記されていないため、そこは変わらない）。
- 実装メモ（2026-08-27 完了）:
  - `news_batches` を新設。**「投げた／取り込んだ／失効した」を行で持つ**——非同期になった分
    「AI側で処理中」という状態が増え、行が無いと「なぜニュースが来ないか」を追えない（原則2）。
  - 取り込みは**同期実行と同じ関数**（`selectNewsItems`／`saveItems`／`recordOutcome`）を通す。
    別々に書くと「同期では捨てる記事がBatchでは残る」ズレが生まれる。
  - 新しさの判定の基準時刻は**投げたとき**（取り込みは任意の時刻に走るため）。
    取得窓とモデルも投げたときの値を行に持つ（取り込みで推測しない）。
  - 原価台帳はトークン分だけ半額にして記録する（そのまま入れると実費の2倍で残り、原則4が崩れる）。
  - 24時間で失効したバッチは `expired` として畳む。`pending` のまま残すと
    「動いているのか止まっているのか」が分からない。
  - Anthropic以外のprovider構成では従来どおり同期実行へ落とす（Batchを持つのがAnthropicだけ）。
  - 検査: `news-batch.test.ts`（8件）で「該当なし」と「取りに行けなかった」が混ざらないこと、
    JSONLの1行破損で他分野を巻き添えにしないことを固定。`news-fetch.db.test.ts` に取り込みの実DB検証を追加。
    cron一覧・外向きチャネル一覧・要件02のスキーマ一致の3つの検査が新規追加を検出したので、すべて追随させた。

### T-M8-337: ニュース取得の時刻設定を実態へ合わせる（文言＋取得窓の不具合） `done`
- 参照: 要件04 §6 / 依存: なし / サイズ: S
- 完了条件: 画面の文言が実際の取得時刻と一致する／取得窓が起動時刻から導かれる／
  アカウント設定が未保存でも投稿を作れる／ニュースのモデル既定がコード側にある
- メモ: 運営者の指示（2026-08-27）。文言の修正を頼まれて調べたところ、**取得窓の不具合**が見つかった。
- **見つかった不具合**: T-M8-326で cron を1日2回へ減らしたとき `NEWS_FETCH_JST_HOURS` が
  9/12/15/18/21 のまま残り、12時の起動が「前回から4時間」で走っていた（実際の空きは前日19時からの17時間）。
  つまり **19時〜翌8時のニュースを一度も取りに行っていなかった**。画面には「記事が無い」ようにしか
  見えないので、利用者からも運営者からも気付けない（原則1）。
- 実装メモ（2026-08-27 完了）:
  - 窓を**起動時刻の表から導く**形にした（回数を変えたときに直し忘れない）。12時→18h／19時→8h。
  - `vercel.json` の cron と時刻表が一致することを検査で固定した。launchdのplistも追随（既存の検査が検出）。
  - 画面の文言（最新ニュース・LP・設定）を「毎日12時と19時」へ。
  - **アカウント設定が未保存でも生成できる**ようにした（前提から `persona` を外し、
    アカウント.mdが空なら `<base_md>` ごと渡さない）。プロンプト画面もアカウント.mdの区分だけ設定を要求する。
    **ただし初期設定ガイドからは消さない**——前提の判定結果をそのままガイドに使っていたため、
    最初の実装では未保存なのに「完了」と表示され、設定への導線ごと消えていた（E2Eが検出）。
    ガイド側で `baseMdVersion < 1` を自分で判定する形に直し、回帰を単体テストで固定した。
  - ニュースのモデル既定を**コード側**に持たせた（envは上書き用）。実測はSonnet $0.15〜0.17/回・
    Haiku $0.09/回で、1日12回なら月約9,200円と約5,200円の差。品質を採ってSonnetを既定にした。
  - 「通常投稿クレジット／URL付き投稿クレジット」を「〜回数」へ（運営者の指示）。

### T-M8-336: 学習の反映先をアカウント.mdのセクション1〜4へ・分析系をSonnetで固定 `done`
- 参照: プロンプト設計書 §6.14/§5.1 / 要件04 §12 / 要件06 §9 / 依存: T-M8-334 / サイズ: M
- 完了条件: 参考ソースの学習がセクション1〜4を洗練する／5〜6と前文は不変／設定由来の値は変わらない／
  学習・md洗練・投稿分析レポートがSonnet級で固定される／設定保存で失われることが画面に出る
- メモ: 運営者の指示（2026-08-27）。「5・6へ追記」ではなく「1〜4を洗練・ブラッシュアップ」へ。
  モデル選択の反映対象は**文章生成と画像生成だけ**という整理も同時に確定した。
- 実装メモ（2026-08-27 完了）:
  - MD-MERGEの反映先を 5|6 の出し分けから**セクション1〜4を1回で書き直す**形へ変えた。
    種別（ref_account/ref_post）でセクションを分ける理由が無くなったので、
    **全active sourceの分析をまとめて渡す**。own_posts（→旧セクション5）はT-M8-103で廃止済み。
  - 出力の規約が**反転**した（見出し禁止→見出し1〜4が必須）。ここを緩めると差し替え後に
    6見出し構造が壊れ、原因の分からない失敗になるため、`md-merge.ts` 側でも同じ規約で検証する。
  - プロンプトに「**設定由来の行の値を変えない**」を明記した——画面で決めた事実が分析で
    書き換わると、設定画面の表示と実際の指示が食い違う（原則1）。
  - **アカウント設定の保存で1〜4は作り直される**ので、学習で磨いた分もそこで戻る。
    黙って消えないよう保存前の警告へ明記した（履歴からは戻せる）。
  - モデル: MD-MERGE を mechanical → analysis（Sonnet級）へ。投稿分析レポートも analysis で固定。
    利用者の選択が効くのは投稿生成（と画像生成）だけになった。配線の検査も更新した。
  - **クレジットは以前から消費している**（`settleIfPremium` が実費で精算・T-M8-109）。
    学習job内のmergeは親が分析分と併せて精算し、削除起因の単独md_mergeは自分で精算する。

### T-M8-335: JSON修復を2段化・キャッシュ無効化・ニュース検索3回・レポート入力の削減 `done`
- 参照: プロンプト設計書 §5.2/§7.1/§6.10/§6.15 / 依存: T-M8-334 / サイズ: M
- 完了条件: 形の崩れは安いモデルの整形1回で直る／直らなければ従来どおり本モデルで作り直す／
  キャッシュを付けない／ニュースの検索上限が3／レポートの入力が50件＋前回md全文なし
- メモ: 運営者の指示（2026-08-27）。**品質を下げずに**という条件は全項目で維持している。
- 実装メモ（2026-08-27 完了）:
  - **JSON修復を2段にした。** 従来の修復は「元の依頼をまるごと生成し直す」もので、
    Web検索も再実行され費用は初回とほぼ同じだった。実際の失敗の多くは前後の説明文・
    末尾カンマといった**形の問題**なので、1段目に「失敗した応答そのものを渡して形だけ直す」
    安いモデルの整形を置いた（検索なし）。直らなければ2段目で従来どおり作り直す。
    **直せる範囲は狭まらない**（2段目が従来と同じ）。整形には「足りないキーを作らない」と
    明記した——埋めさせると検証は通るのに中身が嘘になる。
  - **プロンプトキャッシュを切った。** 当たるのは同じ入力を5分以内に再送したときだけで、
    先頭ブロックにアカウント.mdが入るため利用者をまたいで共有されない。
    書き込み料は入力の1.25倍なので、当たらない分は純損だった。
    **systemに可変値を入れない作りは残した**ので、戻すのは1行。
  - ニュースの検索上限 5→3。プロンプトの「検索は3〜5回」も「最大3回」へ揃えた——
    指示と実際の上限が食い違うと、モデルが4回目を試して弾かれる無駄が出る。
  - 投稿分析レポートの入力を 100→50件、前回レポートからアカウント.md改訂案の全文を落とした
    （現行mdは別に渡しており、前回の「提案」の本文は要らない。投稿作成プロンプトの全文は
    「前回を土台に磨く」指示があるので残す）。
  - 検証: 整形で救われる経路のテストを新設（3回目が呼ばれないことまで見る）。
    既存の「修復しても直らない」テストは3回に更新。プロンプト差分の検出（snapshot）が
    ニュースの文言変更を捕まえたので同時に更新した。

### T-M8-334: 品質を落とさずAIの費用を下げる（用途別モデル・検索回数・画像の既定） `done`
- 参照: プロンプト設計書 §5.1（モデル選択） / `src/lib/ai/model-catalog.ts` / 依存: なし / サイズ: M
- 完了条件: 裏方の実行が安いモデルで動く／学習分析がSonnet級で動く／Web検索の既定が3回／
  画像の既定が gpt-image-1.5（設定から変更可）／配線が外れたら検査で落ちる
- メモ: 運営者の指示（2026-08-27）。**成果物そのものを作る実行（投稿生成・投稿分析レポート）は
  利用者の選択のまま**。裏方だけ固定する。
- 実装メモ（2026-08-27 完了）:
  - `PURPOSE_TEXT_MODELS`（mechanical/analysis）を追加し、`resolveTextProvider` に
    `purpose` を足した。**モデルだけ差し替え、providerとキーは変えない**——providerを変えると
    キーの有無で失敗経路が増え、利用者が選んだ会社と違う先へ本文が渡る。
  - mechanical: GEN-FIX短縮・PT-IMG・MD-MERGE。analysis: 学習分析（L1/L2）。
    学習jobの中で連鎖するMD-MERGEは mechanical（同じjobでも用途が違う）。
  - **JSON修復は本モデルのまま残した。** 修復callは形を直すのではなく**同じ依頼を生成し直す**
    実装なので、安いモデルにすると成果物の品質が落ちる（運営者への報告で指摘済み）。
  - 短縮用のproviderは**実際に短縮が要るときだけ**解決する（毎回キーを2回引かない）。
    原価台帳も短縮に使ったモデルで記録する（親のモデルで記録すると実費とずれる）。
  - Web検索の既定回数 4→3（p1/p4）。seed関数は**直前の定義を写して回数だけ差し替えた**——
    古い版を写して列の増減を巻き戻し、新規Xアカウントの連携が落ちるのを実際に踏んだ。
  - 画像の既定を `gpt-image-1.5` へ。既定は**コード**に持ち env は上書き用（原則3）。
  - 検査: `model-purpose.test.ts`（10件）。表の健全性（カタログ・単価表・安い順）に加え、
    **配線が外れていないかをソースで見る**——`purpose` は省略可能な引数なので、
    消えても型検査は通り動きも変わらず、**費用だけが静かに元へ戻る**。
    実際に1つ外して赤くなることを確認した。

### T-M8-333: 予約で「枠だけの上書き」が増やした入力欄の値が保存で消える `done`
- 参照: 要件06 §SC-08 / `src/lib/schedule-slots.ts` / 依存: なし / サイズ: S
- 完了条件: 上書きプロンプトで足した `{名前}` の値が保存後も残る／上書きを消したらその値は残さない
- メモ: 入力の行き先（プレースホルダー→`<pattern>`本文／参考URL・追加指示→`<input>`）を
  確認していて見つけた。**画面は入力欄を出し、値を書いて保存でき、エラーも出ないのに、
  保存の瞬間に値だけが消えていた**（次に開くと空欄。生成では「（未指定）」が差し込まれる）。
- **原因**: 保存時の `keptPlaceholderValues` が**パターンが宣言した名前だけ**を残していた。
  画面と生成側は「いま見えている本文に `{名前}` があるか」で判定しており、
  枠だけの上書き（`prompt_override`）で足した項目は保存側の判定だけが知らなかった。
- 実装メモ（2026-08-27 完了）: 判定を生成側（`placeholdersForFill`）と同じ
  「本文に `{名前}` があるか」へ揃えた。`schedule-slots.db.test.ts` に1件追加し、
  **元の実装へ戻すと赤くなることを確認**した。

### T-M8-332: アカウント.md・画像生成プロンプトを複数持てるようにし、3区分のUIを統一 `done`
- 参照: 要件02 §3.29 / 要件05 §8 / 要件06 §9 / 依存: なし / サイズ: L
- 完了条件: アカウント.mdと画像生成プロンプトを複数作れる／「使用中」を選べる／
  使用中の内容が生成に使われる／3区分（アカウント.md・投稿作成・画像生成）の画面が同じ形
- メモ: 運営者の指示（2026-08-27）。投稿作成プロンプト（パターン）は複数持てたのに、
  他の2つは1つしか持てず、別の書き方を試すには上書きするしかなかった。
- 実装メモ（2026-08-27 完了）:
  - **生成が読む場所は変えていない。** `prompt_presets`（新設）は人が育てる本棚で、
    使用中の1件を既存の置き場（`x_accounts.base_md` / `prompt_templates`）へ**同じtxで写す**。
    読む側（生成・学習・画像）を1行も変えないので、新しい失敗の種を作らない。
    逆に置き場が別経路（学習反映・アカウント設定の保存）で変わったときは
    `syncInUsePreset` で本棚へ書き戻す——**本棚と実物が食い違うと、画面の文字と
    生成に使われる文字が違う**ことになる（原則1）。
  - アカウント.mdの保存・切り替えは既存の `applyUpdateBaseMdManual` を通すので、
    **版と履歴が残り、いつでも切り替える前へ戻せる**。学習running中は同じ理由で拒否される。
  - UIは共通部品（`components/prompt/prompt-list-parts.tsx`）に寄せ、3区分とも
    「説明＋件数＋再読み込み → カード一覧（2カラム）→ 一覧末尾の追加パネル」に揃えた。
    パターン管理も同じ部品へ移し、単独の「パターンを追加」ボタンは廃止（T-M8-331 と同じ形）。
  - 旧UI（`base-md-editor.tsx`・`prompt-templates-editor.tsx`）と、到達しなくなった
    Server Action 5本を削除した。**削除は到達性テストが検出した**（消し忘れは残らない）。
  - 検証: `prompt-presets.db.test.ts`（7件・本番実装を通す）、E2E `prompt-presets.spec.ts`（2件）、
    既存E2E（アカウント.md編集・画像プロンプト）を新UIへ更新。
    RLS/GRANT（`authenticated` へ grant しない）とCHECK制約の登録も既存の検査が要求してきた。
...
- メモ: 運営者が報告（2026-08-27）「ニュース解説で投稿生成すると『入力内容に誤りがあります』。
  赤くなっている項目はありません」。**ニュース解説に限らず全パターンで生成できない状態だった。**
- **原因**: フォームが `pattern` を送っていたが、受け側のスキーマは `pattern_id` を要求していた
  （T-M8-129 U5 で内部IDからパターンIDへ改名したとき、**スケジュール画面だけが追随**した）。
  `pattern_id` が undefined のまま検証にかかり毎回 `validation_error`。
  **画面のどの入力欄も間違っていない**ので、赤くなる項目も出しようがなかった。
- **なぜ気付けなかったか（3つ重なった）**:
  1. Server Action は `unknown` を受けて zod で検証するので、**画面側のキー名が違っても typecheck は通る**
  2. スケジュール側だけ改名に追随していたため、片方だけ動いていた
  3. **生成のE2Eは常にスキップ**だった（`E2E_LIVE_AI=1` のときだけ動く。実AI費用がかかるため）。
     **唯一この経路を通るテストが走っていなかった**
- 実装メモ（2026-08-27 完了）: `pattern_id: pattern` へ修正。
  `create-post-payload.test.ts`（新規3件）を追加し、**フォームのソースから送信キーを拾って
  スキーマの必須キーと突き合わせる**。必須キーの欠落も、スキーマに無いキー（綴り違い）も落とす。
  **AIを叩かないので毎回の実行で回る**——スキップされる live E2E に頼らない。
  元のバグへ戻すと赤くなることを確認した。

### T-M8-329: 利用者向けエラー文言に「次に何をすればよいか」を書く `done`
- 参照: `src/lib/observability/errors.ts` / 要件06 §8 / 依存: なし / サイズ: S
- 完了条件: 全エラーコードの文言に次の一手がある／内部の値（上限・ID・providerの生文言）が出ない／検査で固定
- メモ: 運営者の指摘（2026-08-27）「投稿作成を押してエラーが出た時はエラー内容がユーザー目線で
  わかりやすいように」。調べたところ**11件が「〜できません。」で終わっており、打つ手が書かれていなかった**
  （例「予期しないエラーが発生しました。」「入力内容を確認してください。」）。
  T-M8-134 が `legal_consent_required` だけ「画面上部の案内から同意してください」と直していたので、
  **同じ形を全コードへ広げた**。
- 実装メモ（2026-08-27 完了）:
  - 11件を書き直し（行き先・待つべき理由・問い合わせ先のいずれかを必ず含める）
  - `ALL_ERROR_CODES` を公開し、**全コードを漏れなく回る検査**を追加（コードを足したら文言も要る）
  - 「〜できません。」だけの文言を落とす検査で**5件の見落としを自動検出**した
  - 内部の値（3桁以上の数値・内部語）が混ざらないことも固定
- **注意（未解決）**: 運営者がローカルで踏んだ生成エラーそのものは**再現できていない**。
  `smoke:live` でローカルの生成は3シナリオとも成功した（$0.1994）。スモークはServer Actionを
  経由せずjobを直接作るため、**押したときの経路は別**。実際のエラー文言が分かれば原因を特定できる。

### T-M8-323: DB接続の判定が「新しい接続を張る時間」を混雑と誤報する `done`
- 参照: `src/lib/ops/diagnostics.ts` の `judgePoolWaits` / `src/lib/db/pool.ts` / 依存: なし / サイズ: S
- 完了条件:
  - 5分ごとのcronだけで毎日❌になる状態が解消している
  - **本物の待ち行列**（`waiting_count > 0`）は従来どおり検出する
  - 判定を消して緑にしていない
- メモ: 2026-08-27、本番の実データで判明。302回のうち **292回が `total_count=1・waiting_count=0`**
  ＝**プールが空で、接続を新しく張るのに約600msかかっていた**だけで、誰も並んでいない。
  発生時刻は5分ごとちょうどで `scheduler-tick`（12回/時×24h＝288回）と一致する。
  `judgePoolWaits` は**件数しか見ておらず** `waiting_count` を見ていないため、
  閾値20回に対しcronだけで毎日必ず超える。**T-M8-310（ニュースの誤報）と同じ設計の欠陥**で、
  直せない正常な状態を赤くすると本物の異常が埋もれる。
- 実装メモ（2026-08-27 完了）: 収集側で `count(*) filter (where waiting_count > 0)` を別に数え、
  `judgePoolWaits` の判定を**そちらへ移した**。接続確立の回数は
  「ほかに接続を新しく張った待ちが N 回（混雑ではありません）」として数字で残す（黙って捨てない）。
  `queuedWaits24h` 省略時は従来どおり全件を空き待ちとみなす（後方互換）。
  **件数だけで判定する旧仕様へ戻すと赤くなることを確認**（71件緑）。

### T-M8-324: AIクレジットを実費のみの後払いにし、マイナスを許容する `done`
- 参照: 要件03 §7 / `src/lib/usage/generation-reserve.ts`・`reserve-if-premium.ts` / 依存: なし / サイズ: L
- 完了条件:
  - 生成前の予約（reserve）が無くなり、**実費が確定したときだけ**記録される
  - 利用枠を超えても**生成が途中で止まらない**（残量がマイナスになりうる）
  - 次の期間の頭で、**マイナス分を引いた状態**から始まる
  - 画面の残量表示がマイナスを正しく出せる
- メモ: 運営者の指示（2026-08-27）「先に多めに予約する形は不要。マイナス数値を許容。
  翌月初にはマイナス分は引かれた状態でリセット」。現状は「開始時に見積もりを押さえ→成功時に実費で精算→
  失敗時は全額返還」で、**完了のたびに数字が下がる**ため利用者には分かりにくい（実際に問い合わせが来た）。
- **要決定 D-48**（残量0のときに新しい生成を始めさせるか）に依存する。
- **注意**: 運営キー系プラン（premium/expert）はAI実費が運営者持ち。予約を外すと
  **上限を超えた分の実費は運営者が被る**。超過は次期間から引かれるが、解約されれば回収できない。
- **D-48 決定（2026-08-27・運営者）: 案A**（残量0以下なら始めない／走り出した分は通す）。
- 実装メモ（2026-08-27 完了）:
  - `reserveUsage` を**残量確認だけ**に（eventもcounterも書かない）。判定は `used >= limit`
  - `settleUsage` を**実費の1回書き**へ（冪等key `job:{id}:{type}:charge`）。上限チェックはしない
  - `carryOverUsage` を新設。**新しい期間の最初の1行を作るときに前期の超過を初期値として入れる**。
    無かったことにすると期間の変わり目に毎回チャラになり、上限が実質的に効かなくなる
  - 残量表示は**AIクレジットだけマイナスを許す**（投稿枠は0で丸めたまま）。0で丸めると
    超過した事実も繰り越される事実も画面から消える
  - `refundUsage` の呼び出しは**残した**——デプロイ時に走行中だったjobが持つ旧予約を返すため。
    通常は no-op で、旧予約が一巡したら削除してよい旨を `terminal.ts` に明記
  - モデル別の見積もりは**画面表示専用**になった（AIモデル設定の「約N クレジット」）
  - テスト17件が旧契約を固定していた。返還の検査は撤去し、新契約のケース（実費は1回だけ／
    尽きたら止める／走り出した分は超過を残す／繰り越し3件）へ置き換えた。E2E 118件緑

### T-M8-325: AIクレジットの粒度を100倍にする（1クレジット=0.01円相当） `done`
- 参照: `src/lib/usage/ai-credits.ts`・`src/lib/ai/model-catalog.ts`・`src/lib/plans.ts` / 依存: なし / サイズ: M
- 完了条件:
  - プレミアムが **100,000クレジット**、エキスパートが 500,000クレジット
  - `creditsFromUsd` が `ceil(usd × 160 × 100)`
  - モデル別の見積もり（16→1600 等）も100倍
  - **全画面**（プラン表・LP・設定・投稿作成・スケジュール）に反映されている
- メモ: 運営者の指示（2026-08-27）「1000円分だけれどクレジットとしては100倍の100,000クレジット」。
  **実質価値は変わらない**（1000円分のまま）が、粒度が100倍細かくなる。
  副次的な利点として、`Math.max(1, ...)` の丸めで**極小の実行が過大請求されていた**のが解消する
  （$0.001の実行＝0.16円が1クレジット＝1円として引かれていた）。
- **移行の決定（2026-08-27・運営者）: 案2＝世代を進めて0から数え直す。**
  migration `20260827000001_credits_x100_reset.sql`。単位の混在を残さず、検証（本番スモーク）で
  消費した分もここで清算できる。`usage_events` の台帳は残るので原価の追跡はできる。
- 実装メモ（2026-08-27 完了）:
  - `plans.ts` を premium 100,000 / expert 500,000 へ。**画面はすべて `PLANS` から引いている**ので
    定数の変更で伝播する（LP・プラン表・設定・利用枠カード・法務ページ）
  - `creditsFromUsd` を `ceil(usd × 160 × 100)` へ（`CREDITS_PER_JPY = 100`）
  - モデル別の見積もりを100倍。ただし**画像は単価表から計算し直した**——旧値は
    `ceil(usd×160)` の丸めが入っており、100倍すると誤差が拡大していた
    （例 gpt-image-2: 31×100=3,100 ではなく正しくは 3,040）。これが今回の丸め改善そのもの
  - **桁区切りを入れた**（`yen()` を流用）。`100000` のままだと読めない
  - **Stripeの商品説明に数字が直書きされていた**のを `plans.ts` から引く形へ。
    100倍にしたときここだけ旧値で残り、テストが検出した（二重管理の解消）
  - テスト22件が旧単位を固定していた。**導出できるものは定数からの導出へ変え**（見積もり境界・
    単価表からの換算）、直値は100倍にした

### T-M8-326: ニュース自動収集を1日2回にし、stg・ローカルでは止める `done`
- 参照: `vercel.json` / 要件04 §6 / `src/lib/ops/diagnostics.ts`（`NEWS_FETCH_UTC_HOURS`） / 依存: なし / サイズ: S
- 完了条件:
  - 本番のニュース取得が**1日2回**になっている
  - **stg・ローカルでは動かない**（運営者が意図的に戻すまで）
  - doctorの停止判定（T-M8-310）が新しい回数に追随している
- メモ: 運営者の指示（2026-08-27）「一旦試験的なリリースとして1日2回（日本の活動時間において
  最もニュースが出されやすい時間の直後2回がベター）」。
  **費用の97.6%がここ**（本番実績: Anthropic $23.31 のうち $23.14 がニュース取得196回）。
  1日5回→2回で**約6割減**の見込み。
- **D-49 決定（2026-08-27・運営者）: JST 12時・19時**（UTC `0 3,10 * * *`）。
- 実装メモ（2026-08-27 完了）:
  - `vercel.json` を `0 3,10 * * *` へ。要件04 §6 の表と `vercel-crons.test.ts` も同時に更新
  - doctorの停止判定が使う `NEWS_FETCH_UTC_HOURS` を `[3, 10]` へ。T-M8-310 のテスト5件は
    新しい時刻表で期待値を計算し直した（UTC10時→翌3時は**予定として17時間空く**）
  - `handleCronRoute` に `productionOnly` を追加し、news-fetch へ付けた。
    production 以外は `ran: false` と `skipped`（理由つき）を返す——**黙って何もしない形にしない**（原則1）
  - **env は認証を通したあとに遅延ロード**する。先頭でimportすると module読込で検証が走り
    `route-auth.test.ts` が落ちる（このリポジトリの既定の作法）
  - `cron-route.test.ts`（新規3件）で `productionOnly` の実在と遅延ロードを固定

### T-M8-327: 新規登録・ログイン画面を2カラム（右側にプロダクト説明）にする `todo`
- 参照: 要件06 §SC-02/SC-03 / `src/components/auth/auth-page-shell.tsx` / 依存: なし / サイズ: M
- 完了条件:
  - 新規登録・ログインが2カラムになり、右側にプロダクト説明が出る
  - 狭い画面（390px）では縦積みになり、フォームが先に来る
  - 主要状態（エラー・送信中）とキーボード操作が従来どおり
- メモ: 運営者の指示（2026-08-27）。参考として `https://app.noimosai.com/signup` を提示された。
  **参考は構成（2カラム・右に説明）まで**とし、文言・配色・図版は自社のデザイントークンで作る。
  `/ui-polish` の手順（画面幅・状態・a11y・実ブラウザ）で行う。

### T-M8-322: 当日投稿数のDBテストが毎日1分間だけ必ず落ちる `done`
- 参照: `src/lib/usage/daily-post-limit-server.db.test.ts` / 依存: なし / サイズ: S
- 完了条件: どの時刻に走っても緑／日付境界を跨がないことが確認できている
- メモ: 2026-08-27 **JST 00:00 ちょうど**に落ちた（`expected 1 to be 2`）。
  種まきが `offset: "-1 minute"` で、**JST 00:00台に走ると前日の23:59になり**
  「今日の件数」が1件しか数えられない。**「たまに落ちる」ではなく、毎日1分間だけ必ず落ちる**
  決定的な失敗（CLAUDE.md「落ちた回数ではなく落ちる条件を探す」・T-M8-161と同じ型）。
- 実装メモ（2026-08-27 完了）: `earlierToday()` を追加し、**日の先頭では戻す量を縮める**
  （`min(60, secondsIntoDay - 1)` 秒）。offsetは `$6::interval` のパラメータ値なので
  SQL式では渡せず、JS側で計算する。JST 00:00:00／00:00:30／00:01:00／12:00:00 の4点で
  当日内に収まることを確認した。
- **製品の不具合ではない**（`countTodaysPostsForXAccount` は正しくJSTで数えている）。テスト側の欠陥。

### T-M8-321: LPの帯から「初回のみ」を外す（運営者の最終レビュー） `done`
- 参照: `src/components/billing/campaign-callout.tsx` / `src/components/lp/pricing.tsx` / 依存: なし / サイズ: S
- 完了条件: キャンペーン帯に「初回のみ」が戻っている／E2E（landing・plans）が緑
- メモ: 2026-08-26、LP文言の整理で `CampaignCallout` から「初回のみ」が外れた。
  **この帯が「トライアルは初回限り」の法令上の開示を担っている**——`lp/pricing.tsx` に
  「法令上の開示は次で担う: 『初回のみ』『カード登録が必要』＝CampaignCallout」と明記され、
  `plans/page.tsx` にも同じ記述がある。外すと**2回目以降の利用者が再度無料と受け取れる＝有利誤認**。
  FAQ（「はじめてのお申し込みに限り7日間無料」）とも食い違っていた。
- **E2Eが検出した**: `landing.spec.ts`（2件）と `plans.spec.ts`（1件）が `/初回のみ7日間/` を
  見ており、CIで落ちた。**単体テストは緑のまま**で、画面の文言はE2Eでしか守れていない。
- **手順の反省**: LPの文言を触ったのに**E2Eを回さずpushした**。CLAUDE.md「いつ回すか」は
  E2Eをpush前の段としているが、`src/**` を触ればLPも対象になる。文言だけの変更でも
  **画面の文字列を検査しているのはE2E**なので、UIを触ったらpush前に必ず回す。
- **決着（2026-08-27・運営者）: 帯から「初回のみ」を外したままにする。** 運営者が最終レビュー済みの文面。
  いったん復元したが指示により戻した。あわせて次を揃えた:
  - E2E 3件の期待値を新しい文面へ（`landing.spec.ts` 2件・`plans.spec.ts` 1件）
  - **事実と食い違うコメントを3ファイルで修正**（`lp/pricing.tsx`・`plans/page.tsx`・
    `campaign-callout.tsx`）。「開示は帯の中に残る」と書かれたままだと
    「安全と書いてあるが実際は違う」型の嘘になる（T-M8-287 と同じ）
- **記録として残す事実**（運営者が把握したうえでの判断）: `/plans` は T-M8-171 で
  申込前確認事項カードを帯へ畳んでいるため、**この画面の本文に「初回限り」は出ない**。
  開示はフッタの特定商取引法ページ・利用規約と、LPのFAQが担う。
  なお `trialAvailable` による出し分け（消化済みの人へ「7日間無料」を見せない）は残してある。

### T-M8-320: 実行リージョンを東京へ固定して全画面の遅延を消す `done`
- 参照: 要件01 §構成 / `vercel.json` / 依存: なし / サイズ: S
- 完了条件: `vercel.json` の `regions` が `["hnd1"]`／設定が消えたら検査が落ちる／前後の実測がある
- メモ: 運営者の質問「画面遷移が遅いのはレンダリングか」から調査（2026-08-26）。**レンダリングではなかった。**
  - ローカル（本番ビルド）の内訳: TTFB 13〜20ms／HTML解析 39〜96ms／JS実行と描画 22〜60ms＝**合計100〜190ms**
  - 本番: 静的ファイル **0.156s** に対しページ **0.356s**。staging はページ **0.50s**
  - **決め手**: `/`（大きい）と `/login`（小さい）が同じ0.50sで、**描画量では変わらなかった**。
    つまりページごとの処理ではなく**固定のオーバーヘッド**
  - `x-vercel-id: hnd1::iad1::…` の2つ目＝実行リージョンが **`iad1`（ワシントンD.C.）**。
    `vercel.json` に `regions` が無く、Vercelの既定になっていた。
    **利用者もSupabase（`ap-northeast-1`）も東京なのにアプリだけアメリカ東海岸**で、
    1回の遷移で太平洋を最大4回横断していた
- 実装メモ（2026-08-26）: `"regions": ["hnd1"]` を追加。`vercel-crons.test.ts` へ検査を足し、
  **消すと落ちることを確認**（「既定の iad1 になり全画面が0.2〜0.35秒遅くなる」と理由つきで出る）。
  Supabaseと同じ場所に揃えるのが要点で、片方だけ動かすと往復が増えることも検査のコメントに残した。
- **T-M8-316 の訂正**: 前日「速度改善の余地なし」と結論したが、あれは**アプリのコード内**だけを見ていた。
  往復回数もJS量も絞られているのは事実で判断自体は正しいが、**コードの外（インフラ設定）に
  コード内のどの改善よりも大きい要因**が残っていた。次に速度を見るときは
  **実行環境の配置（リージョン・DBとの距離）を最初に確認する**。

### T-M8-319: LPの招待キャンペーンを料金の直下・白い面へ移す `done`
- 参照: `src/app/page.tsx` / design_handoff_lp/README.md / 依存: なし / サイズ: S
- 完了条件: 招待が料金セクションの中にある／背景が白い面のまま切り替わらない／前後の余白が二重にならない
- メモ: 運営者の指示（2026-08-26）「料金のすぐ下に置いて、白色の背景の部分に置いて、
  前後の不自然な空白はなくして」。以前は独立した `<section>` で、料金（`bg-surface`＝白）から
  グレー（`bg-page`）へ背景が切り替わり、さらに `SECTION_PAD` が上下に入って間延びしていた。
- 実装メモ（2026-08-26 完了）: 料金セクションの `<PricingCards />` 直後へカードを移し、
  独立セクションを削除。余白は `mt-12` の1つだけにした。
  **実ブラウザ（1440幅）で確認**: 招待が `#pricing` の内側にあること、
  料金セクション下端→FAQ が113pxで二重の余白が無いことを測定した。
  招待の上にあった `{/* 07 よくある質問 */}` の迷子コメントもFAQへ戻した。
- **判断が要るかもしれない点**: カード自体の色は `bg-brand-subtle`（淡い紫）のまま残した。
  「白色の背景の部分に置く」を**セクションの面のこと**と解釈したため。
  カードも白くするなら `bg-surface` へ変える（枠線だけの落ち着いた見た目になる）。

### T-M8-317: プロンプト集でニュース解説のプレースホルダーが出ない `done`
- 参照: `src/lib/prompt-template-gallery.ts` / 要件06 §SC-07 / 依存: T-M8-210 / サイズ: S
- 完了条件: プロンプト集のニュース解説に「ニュース」が差し込み欄として出る／同じズレが再発しない形になっている
- メモ: 運営者が発見（2026-08-26）。原因は `POST_PATTERN_INFO` が
  **プレースホルダーを手で並べて二重管理していた**こと。T-M8-210 で PT-P1 へ `{ニュース}` を足したとき
  この表だけ追随せず、`placeholders: []` のままだった。表には「変えるときは両方揃える」と
  注記があったが、**注記は忘れられる**（CLAUDE.md 原則3）。
- 実装メモ（2026-08-26 完了）:
  - 表から `placeholders` を削り、**本文から導出**する（`extractPlaceholderNames(SYSTEM_DEFAULT_TEMPLATES[kind])`）。
    投稿作成画面の入力欄と同じ関数なので、揃える先が1つになった
  - **既存の検査がバグの側を固定していた**——「差し込み欄はseedと同じ定義を持つ（p2だけが`{自分の考え}`）」が
    手で並べた値の側を見ていたため、PT-P1 が変わっても緑のままだった。本文基準へ直した
  - `{ニュース}` を本文から消すと赤くなることを確認（8件緑）
- **アプリ画面は正常だった**: 投稿作成の入力欄は本文から導出する作り（要件06 §SC-07）なので、
  ズレていたのは公開プロンプト集だけ。

### T-M8-318: 画像生成プロンプトを無人運転前提へ書き直す `done`
- 参照: docs/プロンプト設計書.md §6.8 / 依存: T-M8-315 / サイズ: S
- 完了条件: 利用者の入力なしで成立するプロンプトになっている／本文が抽象的でも絵になる
- メモ: 運営者の指摘（2026-08-26）「画像作成のデフォルトプロンプトは、ユーザーが特にプロンプトを
  いじることなく利用するもの（UI上は投稿作成画面からチェックマークを押すだけ）」。
  確認したところ `PT_IMG` に渡るのは `{{post_text}}` と `{{tone_section}}` の2つだけで、
  **元から利用者の入力はゼロ**だった。問題は入力ではなく**出来を誰も確認しないまま投稿へ添えられる**こと。
- 実装メモ（2026-08-26 完了）:
  - 冒頭に「入力はこの2つだけ、追加入力は無い」を明示し、足りない情報は本文から推測させる
  - **文字なしを既定**にした（T-M8-315 では「伝わるなら入れない」だったのを一段強め、
    「本文に短い言い切りがあり、それが絵の主題そのものになるときだけ」に限定）。
    崩れた日本語が無人で世に出るのを避けるため
  - **本文が抽象的で具体物を選べないときの逃げ道**（汎用の静物・人物は描かない）を追加。
    無人運転では「絵にならない」が失敗として表に出ない
  - トンマナから色と形を導く指示を追加（硬い文体＝寒色と直線／柔らかい＝暖色と丸み）。
    毎回同じプロンプトを使うので、アカウント内で見た目が揃う
  - 設計書 §6.8 を同じ本文へ同期（v1.35）。**コードと1文字まで一致を確認**
- **未検証**: T-M8-315 と同じく `smoke:live` は実行していない（運営者の指示）。

### T-M8-311: LPヒーローのチェック3点を差し替える `done`
- 参照: docs/improve_list.md / design_handoff_lp/README.md §禁止表現 / 依存: D-47 / サイズ: S
- 完了条件:
  - 1点目が「高品質なプロンプトによる高品質な投稿」になっている
  - 3点目は「月額1,480円から」のまま（`startingPrice` から生成し、数値を画面へ書き写さない）
  - 2点目は D-47 の決定に従う
- メモ: 運営者の指示（`docs/improve_list.md`）。**2点目の指定「多くのX運用者が利用」は禁止表現に当たる**
  ——`design_handoff_lp/README.md` §禁止表現が「導入実績数」を明記しており、本番の利用者は現状ほぼ0。
  景品表示法（優良誤認）の観点でも実証できない。D-47で代案を決めてから入れる。
  該当箇所は `src/app/page.tsx:283`。
- 実装メモ（2026-08-26 完了）: 「高品質なプロンプトによる高品質な投稿」「**投稿結果を分析してプロンプトを改善**」
  「月額1,480円から」。2点目は運営者の再指定「プロンプトと投稿結果を**自動で**分析・改善」から
  **「自動で」を外した**——投稿分析の起点は「分析を開始」ボタンだけ（T-M8-255）で、禁止表現リストにも
  「AIが自動で学習し続けて最適化（正: 提案は押したときだけ・1日1回・表示専用）」とある。
  **`landing-page.test.ts` は自分が書いたコメントにも反応する**（ソース全文を走査するため）。
  経緯を説明するコメントに禁止語そのものを書いて1度落ちた。言い換えて緑。17件緑。

### T-M8-312: LPの文体（ですます調／体言止め）を揃える `done`
- 参照: docs/improve_list.md / docs/requirements/06_screens_onboarding_posting.md / 依存: なし / サイズ: M
- 完了条件:
  - 長い文（おおむね1文20字超）はですます調、短い見出し・ラベルは体言止め、で揃っている
  - 不自然になる箇所は揃えず、理由を実装メモに残している
  - 禁止表現を新たに増やしていない（`landing-page.test.ts` が緑）
- メモ: 運営者の指示（`docs/improve_list.md`）「不自然にならない程度に」。**機械的な一括置換をしない**。
- 実装メモ（2026-08-26 完了）: **直したのは4か所だけ**。段落の中で体言止めとですますが同居していた箇所に絞った。
  - ヒーロー説明文: 長文の体言止め。**あわせて事実誤りも直した**——「分析・プロンプト改善までを自動で実施」は
    実装と食い違う（分析の起点は「分析を開始」ボタンだけ・T-M8-255、改善案は表示専用）。
    禁止表現「AIが自動で学習し続けて最適化」に触れる書き方でもあった。T-M8-311 と同じ型の食い違いが本文にも残っていた
  - 03の見出し: 長文なので断定形→ですます（**運営者が直前に書いた文**なので、揃えた旨を報告した）
  - 02カード2枚の本文: 1文目だけ体言止めだったのをですますへ
- **あえて直さなかったもの**: 短い体言止めの見出し（04・05・招待）はルールどおり。
  カードの見出し5つは短い言い切りで**群として揃っている**ため、体言止めへ寄せると不自然になる。
  01のタグライン「使うほど、プロンプトが磨かれる。」はリズムが立つので残した。
- **ついでに見つけた統一漏れ**: 「当社」が2箇所残っていた（`lp/faq.tsx`・`legal-entity.ts`）。
  T-M8-306 は法務3文書だけを直しており、LPとプライバシー記述が漏れていた。個人事業主なので「当方」へ。
- ルール自体を `design_handoff_lp/README.md` §文体 へ書き足した（次に触る人が同じ判断をできるように）。

### T-M8-313: ニュース解説の「{ニュース}」をプレースホルダー扱いにする `done`（T-M8-210 で実装済み）
- 参照: docs/プロンプト設計書.md / 要件02 §3.10（`placeholder_values`）/ 依存: なし / サイズ: M
- 完了条件:
  - ニュース解説パターンの本文中の `{ニュース}` が、他のプレースホルダーと同じ仕組みで差し込まれる
  - 画面のプレースホルダー入力欄に現れ、未入力時の挙動が他と揃っている
  - 既存のニュース解説パターンを使っている利用者の生成が壊れない
- メモ: 運営者の指示（`docs/improve_list.md`）。**調べたところ既に実装済みだった**（T-M8-210・2026-08-22）。
- 確認（2026-08-26）: `PT_P1` の本文は `ニュース: {ニュース}` で、`extractPlaceholderNames(PT_P1)` が
  `["ニュース"]` を返すことを**実際に実行して確認した**（一時テストで検証し削除）。
  ニュース画面の「すぐに投稿作成」から `{ニュース}`・参考URL・テーマが自動入力される導線も
  要件06 §SC-06/SC-07 に記載済み。**新規の作業は不要。**

### T-M8-314: プロンプト内の `<input>` 等のタグを整理する `done`
- 参照: docs/プロンプト設計書.md / 依存: なし / サイズ: S（調査）
- 完了条件:
  - `<input>`・`<news_digest>`・`<pattern_rules>` が何で、なぜタグ形式なのかが正本に書かれている
  - プレースホルダー化すべきか否かの結論と理由が残っている（変えないなら「変えない理由」）
- メモ: 運営者の質問（`docs/improve_list.md`）。**まず調べて答える**。
  タグ形式はモデルへの構造提示（XMLタグ）で、利用者が編集するプレースホルダーとは役割が違う可能性が高い。
  その場合は「変えない」が答えになるが、**画面での見え方**（利用者が触れる場所に生タグが出ていないか）は別途確認する。
- 実装メモ（2026-08-26 完了）: **結論はプレースホルダーにしない。** タグはアプリが組み立てる「封筒」で、
  モデルに情報の境界を示す構造（`lib/ai/gen-context.ts`）。利用者が値を入れる `{名前}` とは役割が違い、
  外すとモデルが境界を読めなくなる。プロンプト設計書 §4.1 へ対比表を追加（v1.33）。
- **ただし1か所、実際の問題があった**: 新規パターン作成時に画面へ出る初期テキスト
  （`NEW_PATTERN_PROMPT_TEMPLATE`）に生の `<input>` が含まれ、非エンジニアには意味が分からなかった。
  画面の言葉（「追加指示」）へ直した。他の露出は全て**AIへ送る文**かコード内コメントで、画面には出ない
  （`src/app src/components` の tsx を走査して確認）。190件緑。

### T-M8-315: 画像生成プロンプトを改善する（画像内は日本語） `done`
- 参照: docs/プロンプト設計書.md §画像生成 / 依存: なし / サイズ: M
- 完了条件:
  - 画像内の文字が日本語で出る
  - 生成物を `npm run smoke:live` で実際に確認している（要件どおり実物で見る）
- メモ: 運営者の指示（`docs/improve_list.md`）。**AI providerのプロンプトを触るので
  `npm run check:providers` ＋ `npm run smoke:live` が必須**（CLAUDE.md 変更影響表）。実費が出る。
- **実行しなかった検証と理由（省略ではなく判断）**: `smoke:live` を回していない。
  **運営者の指示（2026-08-26）「実際に生成する必要はない。現状のプロンプトを読んで判断してほしい」**による。
  したがって**日本語が読める形で描けるかは未確認**。崩れるようなら「文字なしを既定」の比重を上げるか
  上限文字数を下げる。次に画像を触るときは実物で1周させること。
- 実装メモ（2026-08-26 完了）:
  - **禁止していた日本語を解禁**。旧「文字は短い英単語のみ（日本語文字は入れない）」は
    T-M3-02 の初期seed以来の**当初の仮定**で、崩れを観測して入れた制約ではなかった（git log で確認）。
    読者は日本語話者なので英単語だけの画像は本文から浮く
  - **崩れ対策を3点明示**: (1) 図と構図で伝わるなら**文字なし**（読めない字が写るより良い）
    (2) 入れるなら**1〜2語・最大12文字** (3) **描く文字列を鉤括弧で明示**（目標が曖昧だと字形が崩れる）
  - **組み立ての枠を追加**: 被写体→構図→画風→色→光と背景→除外。
    「美しい」「高品質」のような曖昧な語は画像モデルにとって情報量がほぼ無いので禁じた
  - 既定モデルは `gpt-image-1-mini` / `gemini-3.1-flash-lite-image`（**小型なので文字は崩れやすい側**）
  - `PT_IMG` の全文は**公開ページ「プロンプト集」に出る**（T-M8-173）ので内部用語を書いていない
  - 設計書 §6.8 を同じ本文へ更新（v1.34）。**コードと設計書が1文字まで一致することを確認**した。
    drift検出のスナップショットは意図した変更として更新（35件緑）

### T-M8-316: 新規登録・ログイン・Stripe遷移の待ち時間を短くする `done`（計測の結果、変更なし）
- 参照: docs/improve_list.md / 要件01 §5 / 依存: なし / サイズ: M
- 完了条件: 前後の数字（往復回数・段数・実測ms）がある
- メモ: 運営者の指示（`docs/improve_list.md`）。`/speed-up` の手順で行う
  （計測 → 利用者への害の判定 → 1単位ずつ）。T-M8-151/152/154 で一度触れている領域なので、
  **まず現在値を測り、残っている無駄があるかを確かめてから**着手する。
- **結論（2026-08-26）: 実装は変更していない。** 計測した結果、候補がすべて関門か「効果が確認できない」で
  落ちた。`/speed-up` の原則「数字が取れなければその候補は捨てる／効果が確認できない変更はリスクだけが残る」に従う。
- **測った値（staging・本番ビルド。次に触る人のベースライン）**:
  | 対象 | 値 |
  |---|---|
  | TTFB `/` | 0.724s（中央値・5回） |
  | TTFB `/login` | 0.585s |
  | TTFB `/signup` | 0.550s |
  | `/login` のJS | 14本・生 927KB／**転送 約270KB**（最大チャンク 転送140KB） |
  | `/plans`（未ログイン） | 307で `/login?next=/plans` へ。0.394s は転送のみで比較対象外 |
- **落とした候補と理由**:
  - **proxyの `getUser()` を減らす** → **既に対応済み**。`getCurrentUser` は proxy がHMAC署名して渡す
    検証済みヘッダを読むだけで往復ゼロ（T-M8-154）。契約のためのDB読み取りも T-M8-268 で撤去済み
  - **Stripe checkout の直列4段を並列化** → **できない**。`getCurrentUser`→`getProfile`→
    `subscriptions.list`→`sessions.create` は前段の結果に依存する。支配的なのは Stripe API 2回の
    往復で、アプリ側では縮められない
  - **`subscriptions.list` を省いて1往復減らす** → **関門「認可・契約・前提の検査を削る＝やらない」**。
    二重契約（T-M8-237）と2回目のトライアル（T-M8-244）を防ぐ検査で、消すと課金事故になる
  - **クライアントJSを減らす** → 最大チャンク（生436KB）の中身は **react-dom＝Next.jsの実行時本体**で、
    App Router では全ページが必ず積む。LP・login・signup に `"use client"` の部品は**1つも無く**
    （設計方針どおり）、削れる自前コードが無い。zodのlocale（旧T-M8-293）も既に消えている
  - **上限の無い取得** → 走査した結果**該当なし**。`order by` があって `limit` の無い4本はいずれも
    別の形で有界（`follower_snapshots`と`drafts`は期間窓、`schedule_slots`は時間割の枠数、
    `user_api_keys`はprovider数）
- **次に効きそうな所（未計測なので着手していない）**: ログイン後の `/app` 配下の遷移。
  ここは**セッションが要るため今回は測れていない**。測るなら `/speed-up` の一時E2E（`page.goto`→
  主要要素が見えるまで）を使い、3回の中央値で前後を比べること。

### T-M8-310: ニュース取得の停止判定が毎晩かならず赤くなる `done`
- 参照: `src/lib/ops/diagnostics.ts:409`（`judgeNews`）・`vercel.json`（`news_fetch`） / 依存: なし / サイズ: S
- 完了条件:
  - 定時実行が予定どおり動いている夜間に、doctorの「ニュースの取得」が赤にならない
  - 本当に止まったときは従来どおり赤くなる（判定を消して緑にしない）
  - 判定の根拠が「最後の実行からの経過時間」ではなく「**次に走るはずの時刻を過ぎたか**」になっている
- メモ: 2026-08-26 の本番反映後の doctor で検出。`news_fetch` の cron は `0 0-12/3 * * *`＝
  **UTC 0/3/6/9/12時にしか走らない**（JST 9/12/15/18/21時）。一方 `judgeNews` は
  `hoursSinceLastRun > 6` で赤にするため、**UTC18時〜24時（JST 3時〜9時）は毎晩かならず赤**になる。
  実際 2026-08-26 02:xx JST の実行で「6時間以上実行されていません。定時実行が止まっている
  可能性があります」と出たが、直前の実行（UTC12時）は予定どおりで**何も壊れていない**。
- **なぜ急ぐか**: 運営者アラートは `error` で送られる（`operator-alert.ts`）。このままだと
  **毎朝「対応が必要」のメールが届き続ける**。CLAUDE.md と要件が繰り返し警告している
  「直せない理由で赤くすると本物の異常が隠れる」（T-M7-44・T-M8-51）の状態そのもので、
  T-M8-307 で足したばかりの「繰り返している失敗」の error もこれに埋もれる。
- 暫定案: 判定を「cronの次回予定時刻＋猶予」で行う。スケジュールは `vercel.json` が正本なので
  そこから読むか、`news_fetch` の窓（12時間）と同じ定数を使う。**閾値を単に伸ばすだけにしない**
  ——夜間に本当に止まっても朝まで気付けなくなる。
- 実装メモ（2026-08-26 完了）:
  - `hoursSinceDueNewsRun(now)`: `now` から猶予1時間を引いた時点より前の予定のうち**最も新しいもの**を
    「もう終わっているはずの回」とみなし、そこからの経過時間を返す。判定は
    `hoursSinceLastRun > hoursSinceDueNewsRun(now)` の1行。猶予を先に引くので、
    cronの起動が数分遅れているだけの状態を異常にしない
  - **閾値を伸ばす案は採らなかった**——夜間に本当に止まっても朝まで気付けなくなるため。
    実測でも UTC23:57 時点の基準は11.95時間で、UTC12時の回が飛べば実測15時間となり必ず赤くなる
  - `now` を注入可能にしてテストで固定（本番の再現時刻 UTC23:57 をそのまま使った）
  - **スケジュールの二重管理を検査で塞いだ**: `vercel-crons.test.ts` が `vercel.json` の cron式を
    展開し `NEWS_FETCH_UTC_HOURS` と突き合わせる。`vercel.json` だけ変えると落ちることを確認
  - 旧仕様（`> 6`）へ戻すと「予定どおりなのに赤い（毎晩の誤報が再発している）」で落ちることを確認
  - 単体67件緑・全体2,651件緑


**D-48【未決】: 残量0のとき新しい生成を始めさせるか（起票 2026-08-27・T-M8-324の前提）** — 予約（reserve）を外すと「実行してみないと実費が分からない」ので、上限の当たり方が変わる。運営キー系プラン（premium/expert）はAI実費が運営者持ちなので、無制限に走らせると回収できない支出になる。
- (案A) **開始時に残量0以下なら新しい生成を始めさせない。走り出した生成は最後まで通す**（結果として残量はマイナスになりうる）。**推奨** — 「途中で止まらない」という指示を満たしつつ、超過は1回ぶんに限定できる
- (案B) 完全に無制限にし、超過はすべて次期間から引く。利用者にとって最も分かりやすいが、**1人が短時間に大量実行すると運営者の持ち出しが青天井**
- (案C) マイナスの下限を決める（例: −10,000クレジットまで）。案Aより緩く案Bより安全だが、下限に当たったときの説明が増える

**D-49【未決】: ニュース自動収集の実行時刻（起票 2026-08-27・T-M8-326の前提）** — 「日本の活動時間で最もニュースが出されやすい時間の直後2回」を具体的な時刻に落とす必要がある。国内の企業・官公庁の発表は**午前（10〜11時台）と夕方（15〜17時台）**に集中する傾向がある。
- (案A) **JST 11時・18時**（UTC `0 2,9 * * *`）。午前の発表が出そろった直後と、夕方の発表後。**推奨**
- (案B) JST 10時・17時（UTC `0 1,8 * * *`）。より早く拾えるが、午前の発表を取りこぼす可能性
- (案C) JST 9時・21時。現行の窓（9〜21時）の両端で、いまの運用に近い
- 決めるのは運営者。**取得窓は12時間**なので、2回の間隔が空きすぎると窓の外の記事が増える点に注意。

**D-47【未決】: LPヒーローの2点目に何を書くか（起票 2026-08-26・運営者の指示 `docs/improve_list.md` から）** — 指定は「多くのX運用者が利用」。**しかし `design_handoff_lp/README.md` §禁止表現は「導入実績数」を明記して禁じている**（架空の利用者の声も同様。実在の提携者による本人確認済みコメントのみ可・T-M8-214）。本番の利用者は現状ほぼ0で、景品表示法（優良誤認）の観点でも実証できない。実績が積み上がる前に書くと、**表示を裏づける資料の提出を求められたときに出せない**。
- (案A) **機能で言い換える**（例「プロンプトは何度でも改善」「Xアカウント複数対応」）。実証不要で、1点目・3点目と並びも揃う。**推奨**
- (案B) **無料トライアルを出す**（例「7日間無料でお試し」）。事実で、CTA直下の注記とも一致する
- (案C) 指定どおり「多くのX運用者が利用」にする。**利用者数が実際に「多く」と言える水準に達し、その根拠（実数）を保存してから**にすること。いま入れると禁止表現リストと矛盾する
- 決めるのは運営者。**決まるまで2点目は現行文言のまま**にしてある（T-M8-311）。

**D-46: メール確認の廃止（T-M8-202）がリモートに反映されていない（起票 2026-08-26・本番反映の作業中に発見・**決定 2026-08-26: 運営者が案A〔決定どおり true にする〕を選択。staging・本番とも適用済み**）** — T-M8-202（`done`・2026-08-22）と D-36 は「6桁確認を一時的に廃止し、登録→即ログインにする」だった。リポジトリ側は決定どおり（`supabase/config.toml` の `enable_confirmations = false`、`scripts/auth-settings.mjs` の `mailer_autoconfirm: true`、整合は `auth-settings-sync.test` が固定）。**ところが staging・本番の実設定はどちらも `mailer_autoconfirm = false`** で、決定から4日間、**実際には6桁確認が要るまま**だった（2026-08-26 に Management API で実測）。反映コマンド `npm run auth:templates -- --target <env> --apply` が決定後に一度も実行されていない。
...
- **適用状況（2026-08-26）**: staging は `npm run auth:templates -- --target staging --apply` で
  `mailer_autoconfirm: true` を確認済み（差出人名「Exos AI」も維持）。
  **本番も適用済み**（2026-08-26・運営者が手元で `npm run auth:templates -- --target production --apply` を実行。
  Claude からの実行は自動承認の判定でブロックされたため）。両環境を読み直して
  `差出人名: Exos AI` / `mailer_autoconfirm: true` / `反映は不要です` を確認した。
  **これで「登録→即ログイン」が実際に効いている状態になった**（決定から4日遅れで追いついた）。
...
**D-45: Claude Code に `git push` を許すか（起票 2026-08-25・**決定 2026-08-25: 当面は許す〔案A〕。本番リリース後に案Bへ戻す → 再決定 2026-09-04: 運営者「今のまま」＝案Aを継続（Claude が push・PR・マージまで進める）**）** — `.claude/settings.json` の `permissions.deny` に `Bash(git push:*)` があり、これが他のすべての設定に優先するため Claude は push できない（`settings.local.json` の `defaultMode: bypassPermissions` より deny が強い）。ネットワーク側は `github.com` が許可済みなので、障害はこの1行だけ。
...
- **決定（2026-08-25・運営者）: いったん案A。** 本番リリースまでの反復を止めないため。
  **後々の改修（切り替えの合図）**: **本番リリースが済んで運用フェーズに入ったら案Bへ戻す。**
  push は取り消しが効きにくい唯一の操作で、運用に入ると1回の事故の影響が開発中とは比べものにならない。
  実際、2026-08-25 に push 前で止まったことで `plans/page.tsx` の不具合（解約中の利用者が
  無料トライアルへ戻る導線を失う）を出荷前に見つけている。案Bへ戻すときは
  `.claude/settings.json` の `deny` へ `"Bash(git push:*)"` を書き戻すだけ。

**D-44【保留】: 複数アカウントで似た内容を投稿することへの注意喚起（起票 2026-08-25・**暫定決定: 案a〔何もしない〕。将来 案b へ切り替えることを推奨**）** — Xの開発者ポリシーは「複数アカウントに同一・実質同一の内容を投稿しない」を明確に禁じている（2026-08-25にX公式ドキュメントで確認）。本サービスは1人が複数のXアカウントを運用でき、同じプロンプト・同じ設定なら似た内容が生成されうるが、**アカウント間の重複を防ぐ仕組みも警告も無い**。利用規約第10条の免責は「**利用者が**Xの規約に違反した結果」を条件にしているため、ツールの設計が原因で違反が起きた場合はこの条文に素直に当てはまらない（「禁止と知りながら防止も警告もしなかった」＝重過失と評価される余地が残る）。第18条の賠償上限（直近12か月の支払額・通常損害のみ）は効く。(案a) 何もしない＝**2026-08-25 運営者の判断で暫定採用** / (案b) 同意ダイアログか設定画面に「複数アカウントで似た内容を投稿するとXの規約違反になります」の1行を出す（実装はほぼゼロ費用で、「知らせた」という事実が残り第10条に当てはまりやすくなる） / (案c) アカウント間で似た本文を検出して警告する（実装が重い）。**リリース後の様子を見て案bへ切り替えることを推奨**。


**D-44: トライアル中にプランを往復されるとAIクレジットが何度でも満額に戻る（起票 2026-08-25・`/security-audit` 実行で検出・**解決 2026-08-25: 運営者が案A〔上げ直したら消費を引き継ぐ〕を選択・T-M8-306で実装済み**）** — トライアル中に**上位→下位**へ変えると利用枠がリセットされる（2026-08-25の運営者指示・T-M8-299。`billing-return.ts:200-211` が `usage_epoch` を進める）。ところが**上位へ戻すと上限だけが元に戻り、消費済み量は0のまま**なので、Portalを2回開くだけで枠が丸ごと復活する。回数の制限もレート制限も無く、7日間のトライアル中いくらでも繰り返せる。運営キー系プラン（premium/expert）はAI実費が運営者持ちで、支払いは0円。**総額はdoctorで見えるが、`usage_counters` は世代が変わるため「どの利用者が原因か」は辿れない**（原則4に反する）。

- (案A) **上位へ戻すときは消費済み量を引き継ぐ**（世代を戻す）。指示された「下げたら0から数え直し」はそのまま残り、往復だけが閉じる。**推奨**
- (案B) 1トライアルにつきリセットは1回まで
- (案C) リセットを0でなく「新プランの上限まで切り詰め」にする

いずれも利用者から見える挙動が変わり、利用規約・特商法ページの記載（`terms/page.tsx:95`・`legal/commercial-transactions/page.tsx:81`）にも触れるため実装前に判断が要る。**未確認の前提**: Portalが作る schedule を release するとトライアルが打ち切られる可能性があり（T-M8-299のメモ）、そうなら1周で止まる。ただしその場合は T-M8-299 の完了条件自体が成立していないことになるので、どちらでも確認が要る。


**D-40: 招待の `held` / `suspended` をどう扱うか（起票 2026-08-25・**解決 2026-08-25: 運営者の判断で案A。T-M8-302 で `npm run affiliate:moderate` を実装**）** — ドキュメント（要件02 §3.24・invite_cp.md §8）は報酬の `held`（保留）と招待アカウントの `suspended`（停止）を状態として定義し、招待画面にも `held` の表示（「保留」バッジ）があるが、**どちらも書き込む経路が実装に無い**（画面・スクリプト・cronのどこからも設定されない）。不正な自己招待や規約違反を見つけたときに止める手段が、今はDBを直接触るしかない。(案A) 運営者向けスクリプト `npm run affiliate:hold -- --account <id>` / `--suspend` を足す（1コマンドで止められる。原則3に沿う） / (案B) 状態をドキュメントから外し「起きたらDBを直接触る」と明記する（実装を増やさないが、忘れると触れない） / (案C) 現状維持（定義だけ残す）。**推奨は案A**——不正が見つかった瞬間に止められないと、その間の報酬が積み上がる。ただし停止した招待者のコードは現在 `unknown_code` として扱われ、登録側のSentryに「取りこぼしの疑い」として記録されるため、**意図的な停止と区別できる戻り値**も同時に要る。

**D-41: 招待画面の見出しの報酬率が実際と食い違う（起票 2026-08-25・**解決 2026-08-25: 運営者の判断で案B。進捗の隣に「次の紹介から◯%」を出す**）** — `/app/invite` の見出しは「お支払い額の**{現在の率}**が報酬になります」と書くが、報酬の率は**その紹介を含めた累計有料招待人数**で決まる。有料招待が4人の人の画面は「30%」と出るのに、5人目の紹介は実際には35%になる（1段上がる）。逆方向の食い違い（実際より高く見える）は起きないので**不利な誤解**だが、「増えるはずが増えていない」と読まれ得る。(案A) 見出しを「現在の率」ではなく「いま紹介すると適用される率」にする（`tierProgress` は既に次段を持っているので計算は足りている） / (案B) 見出しはそのままで、次段まであと何人かの表示（既にある）に「次の紹介から◯%」と添える / (案C) 現状維持。**推奨は案B**——見出しは現在の実績を表す方が素直で、変化の予告は進捗の隣が読みやすい。


**D-39: 下書きタブの表示件数に上限を設けるか（起票 2026-08-24・**解決 2026-08-25: 運営者の判断で案A。履歴と同じ50件＋「直近50件を表示しています」**）** — 投稿作成＞下書きタブは件数上限なしで全件（`status in ('draft','failed')`）を取得している。履歴タブは50件＋「ほかN件は…」の明示があるのに、下書きタブだけ無制限。`thread`・`images` のJSONを含むため、溜めている利用者ほど画面が重くなる（利用者数ではなく**その人の下書き数**に比例）。**表示件数は利用者に見える変更なので勝手に決めない**（/speed-up の関門）。(案A) 履歴と同じく50件＋「ほかN件は…」を出す（一貫していて説明が要らない。ただし51件目以降は画面から消えるので、下書きを溜めて使っている場合に見えなくなる） / (案B) 100件など緩めの上限＋明示 / (案C) 現状維持（件数が増えたときの遅さを受け入れる）。**推奨は案A**——履歴タブと同じ形で、既に文言と実装がある。


**D-37: 上位プランへの変更時、Stripeの確認画面に日割りの説明が出ないことへの対応（起票 2026-08-23・**解決 2026-08-23: 案A〔確定後にアプリ側で実際の日割り金額を出す〕を実装**・T-M8-270）** — 下位プランへの変更では Stripe が「お客様のサブスクリプションは、現在の請求期間の最終日 (◯月◯日) に更新されます。…」と自動で説明するが、**上位プランへの変更では何も出ない**（「次回からのお支払い (毎月) ◯月◯日 以降 ¥3,980」だけ）。運営者の要望は「今月の請求が日割りになる旨をそこに出したい」。**Stripeの確認画面に独自の文章は書けない**（公式ドキュメント: ポータルで設定できるテキストは見出し・利用規約リンク・戻り先リンク・ビジネス名・ブランディングのみ。SDKの型と実画面でも確認）。選べるのは次の3つ。(案A) **見出し**（`business_profile.headline`）に1文足す——確認画面にも出る唯一の自前テキストだが、左パネルに出て解約画面や請求書履歴でも同じ文が出る / (案B) `proration_behavior=always_invoice` に戻す——Stripe自身が日割りの内訳を書くが、同時に**「本日が期日の金額」**という分かりにくい見出しが出る（2026-08-23 に運営者が却下） / (案C) **アプリ側に自前の確認ステップ**を作る——「プランを変更」を押したら Stripe へ行く前に、`invoices.createPreview` で計算した**実際の日割り金額**（例「本日〜9月23日の差額 ¥2,500 が次回のご請求に加算されます」）を出して確認させる。Stripeの画面ではないが金額まで具体的に示せる。

**D-38: トライアル期間の利用枠を「1つの契約期間」として満額与えるか（起票 2026-08-23・T-M8-258 レビュー指摘・**再決定 2026-08-23: 運営者の指示で「トライアル終了後にクレジットが復活する」＝案A〔トライアルは独立した期間・有料化で満額へ戻る〕へ戻した。一度は共有案を実装したが、運営者が『その方が良い』と判断。テストクロックで実物確認済み**）（旧記録: **解決 2026-08-23: 運営者の指示で「有料化で満額へ戻さない」＝案C'〔トライアルと最初の有料期間で枠を共有。期間の開始をトライアル開始に読み替える〕を実装。テストクロックで実物確認済み**）** — 利用枠を契約期間ごとにした結果、Stripe がトライアルを独立した期間（`current_period_end = trial_end`）として扱うため、**7日間のトライアルに満額（AIクレジット1000・通常投稿200・URL付き20）の枠があり、有料化の日にもう一度満額へ戻る**。以前（暦月）は同じ月ならトライアルと有料初月で枠を共有していた。運営者の費用は最大で「トライアル1周ぶん（AIクレジット1000≒約1,000円相当）」増えうる。(案A) 許容する＝**暫定でこの形を実装済み**（トライアルで十分に試せる方が契約率に効く。要件03 §7.2 に明記） / (案B) トライアル中の枠を小さくする（例: 各枠の1/4。`plans.ts` に trial 用の上限を持ち、`usageLimitsForPlan` が trialing のとき切り替える） / (案C) トライアルと有料初期間で枠を共有する（期間キーを `trial_start` 起点に寄せる。Stripe の期間と食い違うので説明が難しい）。

**D-35: 利用規約・特商法の「月間利用枠」→「契約期間ごとの利用枠」の改定文案（起票 2026-08-23・T-M8-259・**解決 2026-08-23: 運営者が (a) 文案の採用と (b) 案A〔同意versionは据え置き・再同意を求めず、見出し横に改定日を表示〕の両方を承認**）** — 利用枠のリセットを契約の更新日ごとへ変える（T-M8-258）のに合わせ、法務ページの「月間」表記を改めた。**実装済みの文案（運営者の確認待ち）**:
  - 利用規約 第5条: 「プレミアムプランの利用枠は、**契約期間（お支払いの更新日から次の更新日まで）ごと**に、通常投稿200件、URL付き投稿クレジット20、AIクレジット1000（…）です。利用枠は更新日に新しい契約期間の上限へ戻り、使い残した分は次の契約期間へ繰り越されません。契約期間の途中で上位のプランへ変更した場合は、その時点から同じ契約期間内で変更後のプランの上限が適用されます。エキスパートプランに利用枠はありません。ただし…（従来どおり）」
  - 特商法「販売条件」: 「プレミアムプランには契約期間（お支払いの更新日から次の更新日まで）ごとの利用枠（通常投稿クレジット200・URL付き投稿クレジット20・AIクレジット1000〔…〕）があります。使い残した分は次の契約期間へ繰り越されません。エキスパートプランに利用枠はありません（…）。」
  - 追記（D-36 の決定後）: 利用規約 第5条と特商法に「無料トライアル期間中のご利用分は、トライアル終了後の最初の契約期間の利用枠に含まれます（トライアル終了時に利用枠は回復しません）。」
  - 見出し横の表示: 「2026年8月20日版　2026年8月23日 一部改定（利用枠の数え方を契約期間ごとへ）」
  要決定: (a) 上の文案の採否（直すなら `src/app/terms/page.tsx` 第5条・`src/app/legal/commercial-transactions/page.tsx`）、(b) 再同意の要否。**この仕組みでは同意versionを上げる＝全利用者に再同意を要求**なので、案A（**推奨・実装済み**）: versionは据え置き、改定の事実を見出し横に表示して周知 / 案B: `CURRENT_TERMS_VERSION` を `2026-08-23` へ上げて再同意を求める（1行の変更）。
...
**D-32: プロンプトテンプレートページに「実ユーザーの作成物」を載せるか（起票 2026-08-21・T-M8-173・**解決済み 2026-08-21: 案B〔自動掲載・匿名〕を T-M8-175 で実装。利用規約第7条とプライバシーポリシーに匿名掲載の開示も追加済み。2026-08-25 に運営者が改めて「載せてください」と指示したが、既に載っていることを実データで確認した**）** — 運営者の要望は「ユーザー達が作成しているアカウント.md・投稿プロンプト・画像生成プロンプトの一覧」。現状DBに公開許諾の仕組みが無く、実ユーザーのプロンプト公開はプライバシーポリシー（利用目的）の範囲外。暫定実装は**アプリが実際に使うシステム既定テンプレートを正本から表示**。要決定: (案A)現状のシステム既定のみで運用（追加開発なし・**推奨**） / (案B)利用者が明示的に「公開する」を選んだテンプレートを載せる共有機能を作る（公開フラグ・審査・削除導線・規約改定が必要）。

**D-36: メールの6桁確認の一時廃止（起票 2026-08-22・**解決済み: 運営者の意向どおり廃止し T-M8-202 で実装済み。戻すときは T-M8-191 のフォールバックとE2Eを復元する**）** — 運営者の意向は「一旦なくす・有名になったら戻す」。デメリット: (1) **アドレスの打ち間違いに誰も気付けない**——パスワード再設定も通知も届かないアカウントができ、問い合わせでしか救えない (2) **他人のアドレスで登録できる**——そのアドレスの持ち主に請求・通知メールが届き、迷惑報告→Gmail送信元の評価低下→**確認済みユーザー宛のメールまで迷惑判定**される連鎖が最大のリスク（D-35のResend移行後は影響を隔離しやすい） (3) bot登録が通りやすくなる（ただしTurnstileは残り、AI利用にはカード登録必須なので実害は限定的） (4) 戻すときに「未確認のまま使っている既存ユーザー」の扱いが要設計。**登録の離脱を減らす効果は大きく、現フェーズでは合理的**。進める場合は /dev-loop T-M8-202。

**D-42: メール送信を独自ドメインの配信サービスへ移すか（起票 2026-08-22・旧D-35＋旧D-27を統合。同じ話題が2つの番号で起票されていたため2026-08-25に1本化・**決定 2026-08-25: 運営者が案C〔当面Gmailのまま〕を選択**）** — 現在は認証メール（Supabase Auth）と通知メールの両方をGmail SMTP（App Password）で送っている。制約: (1) 送信上限 約500通/日（通知メールはユーザー数×最大5回/日のダイジェスト等で、利用者100人で上限に届く）(2) Gmail個人アドレス発の一斉送信は迷惑判定されやすく、2024年以降のGmail/Yahoo送信者要件（独自ドメイン＋SPF/DKIM/DMARC）を満たせない (3) 送信元と同じアドレス宛は受信トレイに入らない既知の罠（T-M8-147）。移行はコード変更不要（`SMTP_*` 環境変数の差し替え＋`npm run auth:templates -- --apply` の再実行のみ）。要決定:
- (案A・**推奨**) **Resend**（$20/月・50,000通/月）へ移行。SMTP互換で差し替えのみ・独自ドメイン認証（SPF/DKIM）を画面の案内どおりに設定でき、非エンジニアの運用に合う。無料枠（100通/日）はGmailより狭いため、移行するなら有料前提
- (案B) **Amazon SES**（$0.10/1,000通＝ほぼ無料）。最安だがsandbox解除申請・DNS設定など初期設定がエンジニア向きで、原則2（開発知識なしで辿れる）に反しやすい
- (案C) 当面Gmailのまま（現ユーザー数なら十分）。**利用者50人を超える前に案Aへ**。doctorに日次送信数の観測は無いため、移行判断はStripe契約数を目安にする
移行時の作業: Resend契約→ドメイン認証（DNSレコード2件）→Vercelの `SMTP_HOST/PORT/USER/APP_PASSWORD` を差し替え→`npm run auth:templates -- --target production --apply`→テスト登録1回。
  **後々の改修（2026-08-25 決定時のメモ）**: いまの利用者数ならGmailで足りるが、上の制約(1)〜(3)は人数が増えると必ず効く。
  **利用者が50人を超えたら案A（Resend・$20/月）へ移行する。** 移行はコード変更不要（`SMTP_*` の差し替え＋
  `npm run auth:templates -- --apply` の再実行）なので、着手は半日で済む。**doctorに日次送信数の観測は無い**ため、
  判断の目安はStripeの契約数。「登録したのにメールが届かない」は利用者から報告が来にくい失敗なので、
  上限に当たってから動くのでは遅い。

**D-43: 同時接続100人対応（Supabase Pro移行の判断・起票 2026-08-22・旧D-34から2026-08-25に採番し直し。同じ番号が別件で先に使われていたため・**決定 2026-08-25: Proへは移らない〔案A〕。無料の対策で解消した**）** — 現構成（Vercel Pro＋Supabase Free）の実用上限は同時アクティブ約30〜50人。律速はSupabase FreeのSupavisor pooler（クライアント上限200を「Vercelインスタンス数×Pool max=10」が食い潰す）。無料でできる対策（DB_POOL_MAXを3へ・App Shellクエリ統合・pool監視＝T-M8-197/198）で同時100人は概ね到達可能だが、余裕を持たせるには**Supabase Pro（$25/月〜）への移行**が確実。要件01 §9の移行条件（pooler枯渇の観測）に該当し始めたら移行を推奨。要決定: (案A)無料対策のみで様子見（監視T-M8-198で枯渇を観測したら移行・**推奨**） / (案B)いまProへ移行（$25〜35/月）。※**2026-08-25 追記（T-M8-303）**: 「`DB_POOL_MAX=3` の設定は運営者作業」という前提そのものが弱点だったので、**コードの既定値を10→3へ下げた**（設定を忘れた環境だけが上限を食い潰し、症状が「たまに遅い」なので気付けない）。あわせて **doctor が「いま効いている上限」を必ず表示する**ようにした——回数だけでは「対策が効いていないのか、対策はしたが足りないのか」を区別できなかった。
  **判断の前にまず doctor で上限が3になっているか確認すること。** 10のままなら、Pro移行の前に既定値の反映（再デプロイ）で改善する可能性がある。
  **決定（2026-08-25・運営者）: Proへは移らない。** 無料でできる対策が出揃って接続待ちの主因が消えたため。
  内訳は (a) `DB_POOL_MAX` の既定を10→3（T-M8-303）、(b) profiles の重複読み取りを1リクエスト1回へ（T-M8-286）、
  (c) **ジョブ処理をまるごと囲っていたトランザクションを外した**（T-M8-308）——AIへの問い合わせ（数十秒）の
  あいだ接続を1本占有し続けていたのが、上限3本の環境では最も重かった。
  **後々の改修（再検討の合図）**: 本番の `npm run doctor` で「DB接続の混み具合」が再び **error** になったら、
  要件01 §9 の移行条件に該当したものとして Pro（$25/月〜）を再検討する。回数だけでなく
  doctorが表示する「いま効いている上限」も併せて見る（上限が3でないなら、移行の前に設定の反映を疑う）。

**D-33: 招待報酬の振込先口座の保持方法（暫定決定 2026-08-21・T-M8-174・**決定 2026-08-25: 現行方式を継続〔自社DBで暗号化して保持〕**）** — invite_cp.md は「機密情報は可能な限り外部Payout Providerで管理」とするが、Provider未契約。振込は運営者の手動振込のため全桁が必要。**暫定: 口座番号はAES-256-GCM（既存のAPIキーと同じ鍵運用）で暗号化してDBに保存し、画面は末尾4桁のみ表示（provider='internal'）**。要決定: このまま運用するか、Payout Provider（例: Stripe Connect等）を契約して移行するか。移行するまで運営者は復号コマンドで全桁を確認して振り込む。
  **決定（2026-08-25・運営者）: 当面このまま。** 口座はAES-256-GCMで暗号化してDBに保持し、
  振込のときだけ復号して全桁を見る。画面には末尾4桁しか出さない。
  **後々の改修（再検討の合図）**: 次のどちらかが起きたら Payout Provider（Stripe Connect等）への移行を再検討する。
  (a) **実際に振り込む対象者が数人になり**、「復号→確認→振込」の手作業が毎月積み上がるようになったとき、
  (b) **口座を登録する利用者が増えたとき**——他人の銀行口座を自社で預かる範囲が広がり、
  漏えい時の影響と個人情報保護法の安全管理措置の負担が上がる。
  移行は実装が小さくない（契約・本人確認の導線が要る）ので、上の合図が出たら独立したタスクとして起票する。
