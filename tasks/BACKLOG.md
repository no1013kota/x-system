...
| 要決定 | **1件**（D-16: `stg` にもブランチ保護を掛けるか。2026-08-04に ruleset `protect-stg` を作成しPR経由へ移行したため、実質は解決済み。正式なcloseはユーザー確認待ち） | 「要決定・外部準備」 |
...
### T-M8-54: 解除したXアカウントを一覧から畳み、課金の行き止まりを解消する `done`
- 参照: 要件02 §3.3、要件06 §1.2.1、要件03 §2.2、CLAUDE.md 原則1・2 / 依存: T-M8-53 / サイズ: M
- 利用者からの指摘2件（2026-08-04）。**うち1件は T-M8-53 の私の修正が作った不具合**。
- (1) **「連携を解除」しても一覧に「停止中」として残り続けていた。** 片付けたつもりの行が
  いつまでも見えて紛らわしい。
  - **行は消せない**（下書き・履歴・実績が参照する・要件06 §14）ので、`<details>`
    「解除したアカウント N件（投稿履歴と実績は残っています）」へ畳む。畳んだ中から
    再有効化・再連携で一覧へ戻せる（行き止まりにしない）。
  - **`disabled` は2つの出来事に使われていた**ので区別が必要だった——(a) 利用者の解除、
    (b) プラン変更による自動停止（standardは active 1件だけ・要件02 §4.1）。
    (b) を隠すと「なぜ止まったのか分からない」状態になる（原則1）。
    migration `20260804000001` で `disconnected_at` を足し、**(a) だけを畳む**。
  - 再連携（upsert）と再有効化で `disconnected_at` を null へ戻す。
  - 副産物: 行のマークアップを `renderAccount()` へ切り出した（連携中と畳んだ側で同じものを
    描くため。2回書くと片方だけ直して見た目と操作が食い違う）。
- (2) **T-M8-53 で「跳ね返り」を「行き止まり」に変えてしまっていた。** 顧客未紐づけのときに
  「プランを選ぶ」を消して説明文だけにしたため、**押せるものが何も無い**状態になった
  （同期が来なければ永久に「再読み込みしてください」のまま）。
  - 説明文は残したまま**「プランを選ぶ」を必ず出す**。
  - `/plans` 側も **Stripeの顧客が無い契約者は送り返さない**ようにした（申し込みをやり直せる
    場所へ入れる）。顧客が紐づいていれば従来どおり `/app` へ送る（決済直後の行き止まり防止）。
  - 教訓: **行き先を消して説明に置き換えるのは解決ではない。** 状況を伝えても、
    進める場所は必ず残す。
- 検証: 単体1,633（DB込みで同数・skip 6）／E2E 9件（x-oauth）。migrationはローカルへ適用済み。
  **T-M8-33 の `schema-doc-sync.db.test.ts` が要件02への書き漏れを捕まえた**（自動検査が効いた）。
  実ブラウザで解除→畳み→展開と、課金タブの説明文＋ボタン共存を確認した。
- **stagingへの反映には migration の適用が必要**（`npm run release:staging -- --apply`）。
