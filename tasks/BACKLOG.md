...
| 区分 | 残り | 場所 |
|---|---|---|
| 開発タスク（着手可） | **0件**（M7の39タスクすべて`done`。残るは T-M7-17 Gemini画像＝`blocked`・ユーザー判断で一旦不要） | 下記M7セクション |
...
**リポジトリの状態**: 単体+DB 1,392件緑（190ファイル・skip 6）/ E2E 29件緑（14ファイル）/ `check:providers` 5件緑 / `smoke:live` 3シナリオ緑。CI（GitHub Actions）稼働中。作業ブランチは `stg`、`main` へは**PR経由**で反映する運用（D-8 案A。GitHub側の保護設定は未実施）。**未pushのコミットがあるので `git branch -vv` で確認する**。
