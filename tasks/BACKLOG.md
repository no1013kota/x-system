...
| 区分 | 残り | 場所 |
|---|---|---|
| 開発タスク（着手可） | **2件**（T-M8-69 providerのmodels.list疎通／T-M8-68 体感速度の残り）。T-M8-84〜87 は done | 下記M8セクション |
...
1. **T-M8-87 を本番へ反映する**（`/signup` が直るまで、運営者自身が本番で登録できない）。
   反映後に実ブラウザで会員登録を1周し、そのアカウントで **`npm run release:production`** の実物スモークを回す
   （本番DBには利用者もXアカウントも1件も無く、スモークは `--account` を要求するため登録が前提になる）
2. **D-17（法務文書の弁護士レビュー）**。T-M8-75で条項を19条へ増やし、T-M8-81で屋号を足したため
   レビュー対象が広がっている。専属管轄・免責上限・賠償条項の有効性が論点
3. **D-27（新規環境へのseed投入がコマンドに畳まれていない）**。いま本番は投入済みだが、
   次に環境を作るとき同じ手順を思い出す必要がある。本番のmigration履歴に残った `20260813999999` の扱いも含む
4. D-18（法28条の根拠づけ）・D-19（ログ保持期間の実設定）・D-28（`stg`/`main` の履歴分岐）を決める。
