...
**D-50: 下書きの画像アップロードの上限をどうするか（セキュリティレビュー 2026-08-29）** — 画像アップロード機能（T-M8-353）はコード上「入力20MBまで」を謳うが、**Next.js Server Action の本文サイズ上限は既定1MB**で、`next.config` で上げていない。そのため**1MBを超える画像は、コードの検査に届く前にフレームワーク層で失敗する**（利用者には汎用エラーだけが出る）。ふつうのスマホ写真は数MBあるので、いまの実装は**小さい画像でしか使えない**。
