...
### T-M8-188: 最新ニュースを新着順固定・最新500件のみ表示にし、500件超をDBから削除 `todo`
- 参照: 要件06（SC-03）・要件02 §6・要件04 §14（保持cleanup） / 依存: なし / サイズ: M
- 完了条件:
  - 一覧は常に新着順（取得時刻の新しい順）で表示され、ソートUIは無い
  ...
  - docs（要件02・04・06・PRD N）を更新
- メモ: 運営者の指示（2026-08-22）。T-M8-187で足したテーマ・インパクトのソートはこの指示で廃止。
