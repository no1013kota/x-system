...
| 外部準備（人間側） | **本番の未整備3件（P-1）**＝Supabase CAPTCHA・Stripeポータル設定・定時実行。ほかアカウント・実キー・法務・単価確認 | 「要決定・外部準備」P-1／[リリース前チェックリスト §3](../docs/operations/release-checklist.md) |
...
4. **D-27（新規環境へのseed投入がコマンドに畳まれていない）**。いま本番は投入済みだが、
   次に環境を作るとき同じ手順を思い出す必要がある。本番のmigration履歴に残った `20260813999999` の扱いも含む
5. D-18（法28条の根拠づけ）・D-19（ログ保持期間の実設定）・D-28（`stg`/`main` の履歴分岐）を決める。
   D-16は実質解決済みなのでcloseするだけ。D-20（退会手順）は運営者判断で**削除依頼が来たときに考える**（2026-08-14）
6. 以降は `/maintenance` を週1回・`/add-task` で要望を起票 → `/dev-loop` の運用へ
...
## M8: UIリデザイン（design_handoff_spaceai_ui）
...
- 参照: 要件06 §1.5、`design_handoff_space_ai_lp/README.md`（v2.1へ改訂） / 依存: T-M8-74 / サイズ: M
...
- 参照: 要件06 §1.5、`design_handoff_space_ai_lp/README.md`（v2.2） / 依存: T-M8-76 / サイズ: S
...
- 参照: 要件06 §1.5、`design_handoff_space_ai_lp/README.md`（03 しくみ） / 依存: T-M8-77 / サイズ: S
...
- 参照: 要件06 §1.5（v1.64）、`design_handoff_space_ai_lp/README.md` / 依存: T-M8-78 / サイズ: M
...
- 参照: 要件06 §1.5（v1.65）、`design_handoff_space_ai_lp/README.md` / 依存: T-M8-79 / サイズ: S
...
### T-M8-97: 投稿分析の取得を「初回は直近30日」から「最新100件」にする `todo`
- 参照: PRD §5.6 K-2・要件04 §12・要件02 §3.20・プロンプト設計書 §6.15（SUGGEST）・要件06 §8（SC-09） / 依存: なし / サイズ: M
- 完了条件:
  - 初回取得が`start_time`なしで最新100件（`TIMELINE_FETCH_MAX`）を取得する（直近30日に投稿が無いアカウントでも、過去の投稿があれば分析される）
  - 2回目以降の増分取得（保存済み最新の48時間前から・重なりはupsertでメトリクス更新）は従来どおり
  - 30日超の投稿を含む取得で`non_public_metrics`要求がリクエスト全体を失敗させない（X公式docsに挙動の明記が無いため実アカウントで確認し、必要ならフォールバックを実装する）
  - impressionsが取れない投稿（30日超）はnullのまま分析へ渡る（0と区別・既存挙動の維持）
  - プロンプト・UI文言・docsの「直近30日」前提を更新する
- メモ: 運営者の指示（2026-08-15）。発端は@ai_newinfoが直近30日に投稿0件で、実行しても仕様どおり「レポートなし」になったこと。X読取費用の上限は従来どおり100件=$0.50/回で変わらない。
