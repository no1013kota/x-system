...
| 区分 | 残り | 場所 |
|---|---|---|
| 開発タスク（着手可） | 5件（T-M7-27〜32のうち`todo`。T-M7-26・34・36は`done`） | 下記M7セクション |
| 開発タスク（blocked） | 1件（T-M7-17 Gemini画像。ユーザー判断で一旦不要） | 同 |
| 要決定 | **1件**（D-14: `main` 保護の実現方法。private × GitHub Free ではブランチ保護が使えないと判明） | 「要決定・外部準備」 |
...
4. 検証カバレッジの残り（T-M7-27・28）→ **運営のしやすさ（T-M7-34 状態確認・T-M7-35 手順の畳み込み・T-M7-29 日次サマリ・T-M7-30 週次メンテ）** → 決定に伴う後始末（T-M7-31 通知の掃除・T-M7-32 sharp upgrade）
