...
| 要決定 | **1件**（D-16: `stg` にもブランチ保護を掛けるか。CIが赤でもstagingが更新される穴が2026-08-03に実際に起きた） | 「要決定・外部準備」 |
...
| B（保守性） | T-M8-41 | 失敗下書きの可否boolean 8個を純関数へ抽出 |
| B | T-M8-42 | `upcoming-schedule` のローカル定数（focusリング欠落） |
| B | T-M8-43 | `CategoryChip` のラベル導出2重化 |
| C（統一） | T-M8-44 | インラインバナー45箇所 → `Notice` 新設 |
| C | T-M8-45 | 手書きチップ13箇所 → `Badge` |
| C | T-M8-46 | カード見出し3規格 → `CardTitle` |
| C | T-M8-47 | `lucide-react` 撤去（6ファイル） |
