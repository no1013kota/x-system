...
### T-M8-59: BYOKキー画面の監査で未検証の19所見を再検証する `todo`
- 参照: T-M8-58 の監査結果 / 依存: なし / サイズ: M
- 経緯: T-M8-58 の監査で32所見中、**19件の検証エージェントがセッション上限で失敗した**。
  これらは**棄却ではなく未検証**（見かけ上「棄却」に分類されたが、検証が走っていない）。
  結果ファイル: `/private/tmp/claude-501/.../w686yf5y8.output`（消えている場合は再監査）。
- 未検証の主な所見（真偽不明のまま）: 「形式を確認」がOAuth確認済みキーを画面上「未確認」に
  戻す／Client種別が保存済みの種別を無視して常にPublic初期表示／zodの具体的な文言が画面へ
  届かない／削除ダイアログが波及（X連携全失効）を告げない／Server Action呼び出しに
  try/catchが無い／Xキー保存後の次ステップ（X連携）への導線が無い、など。
- やること: 各所見を否定側から検証し、実害のあるものだけ直す（T-M8-51と同じ進め方）。
