...
| 要決定 | **1件**（D-15: 投稿の日次上限を事前に知らせるか。D-14は2026-08-02に案B＝リポジトリ公開で解決済み） | 「要決定・外部準備」 |
...
- 検証: typecheck / lint 緑。E2E 3件緑（プラン画面のテストへ「行き止まりにならない」を追加）。
  1440px の実ブラウザで確認。
- 後続への注意: **デザインのT-4「上限到達バナー（画面上部固定）」は入れていない。** 2種類のうち
  「今月の生成を使い切った」側はこのカードと課金タブで読めるが、「本日の投稿が安全上限（50件/日）に
  達した」側は**投稿しようとして初めて分かる**（`X_DAILY_POST_LIMIT` はjob内で判定）。事前に出すには
  「今日すでに何件投稿したか」を画面表示時に数える必要があり、**新しいデータ取得＝「見た目だけ」の
  範囲を超える**ため保留。要決定D-15。
