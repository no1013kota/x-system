...
| 区分 | 残り | 場所 |
|---|---|---|
| 開発タスク（着手可） | 6件（T-M7-27〜32のうち`todo` 4件 ＋ T-M7-37・38。T-M7-39・40は`done`） | 下記M7セクション |
...
4. **発信の質に効く改善（T-M7-37 プロンプトの不利を除去・T-M7-38 分析軸の拡張）と、見つかった不具合は完了（T-M7-39 学習分析・T-M7-40 ニュースの0件）** → 検証カバレッジの残り（T-M7-27・28）→ **運営のしやすさ（T-M7-34 状態確認・T-M7-35 手順の畳み込み・T-M7-29 日次サマリ・T-M7-30 週次メンテ）** → 決定に伴う後始末（T-M7-32 sharp upgrade。T-M7-31 通知の掃除は完了）
...
**リポジトリの状態**: 単体+DB 1,320件緑（184ファイル・skip 6）/ E2E 28件緑（13ファイル）/ `check:providers` 5件緑 / `smoke:live` 3シナリオ緑。CI（GitHub Actions）稼働中。作業ブランチは `stg`、`main` へは**PR経由**で反映する運用（D-8 案A。GitHub側の保護設定は未実施）。**未pushのコミットがあるので `git branch -vv` で確認する**。
