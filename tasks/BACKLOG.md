...
**D-40: 招待の `held` / `suspended` をどう扱うか（起票 2026-08-25・T-M8-300 の網羅テストで発見）** — ドキュメント（要件02 §3.24・invite_cp.md §8）は報酬の `held`（保留）と招待アカウントの `suspended`（停止）を状態として定義し、招待画面にも `held` の表示（「保留」バッジ）があるが、**どちらも書き込む経路が実装に無い**（画面・スクリプト・cronのどこからも設定されない）。不正な自己招待や規約違反を見つけたときに止める手段が、今はDBを直接触るしかない。(案A) 運営者向けスクリプト `npm run affiliate:hold -- --account <id>` / `--suspend` を足す（1コマンドで止められる。原則3に沿う） / (案B) 状態をドキュメントから外し「起きたらDBを直接触る」と明記する（実装を増やさないが、忘れると触れない） / (案C) 現状維持（定義だけ残す）。**推奨は案A**——不正が見つかった瞬間に止められないと、その間の報酬が積み上がる。ただし停止した招待者のコードは現在 `unknown_code` として扱われ、登録側のSentryに「取りこぼしの疑い」として記録されるため、**意図的な停止と区別できる戻り値**も同時に要る。

**D-41: 招待画面の見出しの報酬率が実際と食い違う（起票 2026-08-25・T-M8-300 の網羅テストで発見）** — `/app/invite` の見出しは「お支払い額の**{現在の率}**が報酬になります」と書くが、報酬の率は**その紹介を含めた累計有料招待人数**で決まる。有料招待が4人の人の画面は「30%」と出るのに、5人目の紹介は実際には35%になる（1段上がる）。逆方向の食い違い（実際より高く見える）は起きないので**不利な誤解**だが、「増えるはずが増えていない」と読まれ得る。(案A) 見出しを「現在の率」ではなく「いま紹介すると適用される率」にする（`tierProgress` は既に次段を持っているので計算は足りている） / (案B) 見出しはそのままで、次段まであと何人かの表示（既にある）に「次の紹介から◯%」と添える / (案C) 現状維持。**推奨は案B**——見出しは現在の実績を表す方が素直で、変化の予告は進捗の隣が読みやすい。


**D-39: 下書きタブの表示件数に上限を設けるか（起票 2026-08-24・全体監査 T-M8-289 関連）** — 投稿作成＞下書きタブは件数上限なしで全件（`status in ('draft','failed')`）を取得している。履歴タブは50件＋「ほかN件は…」の明示があるのに、下書きタブだけ無制限。`thread`・`images` のJSONを含むため、溜めている利用者ほど画面が重くなる（利用者数ではなく**その人の下書き数**に比例）。**表示件数は利用者に見える変更なので勝手に決めない**（/speed-up の関門）。(案A) 履歴と同じく50件＋「ほかN件は…」を出す（一貫していて説明が要らない。ただし51件目以降は画面から消えるので、下書きを溜めて使っている場合に見えなくなる） / (案B) 100件など緩めの上限＋明示 / (案C) 現状維持（件数が増えたときの遅さを受け入れる）。**推奨は案A**——履歴タブと同じ形で、既に文言と実装がある。
