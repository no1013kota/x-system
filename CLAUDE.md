...
**該当行を「いつ」回すかは3段に分ける。** `.github/workflows/ci.yml` が push時に `release:check`
（build・E2E込み）を回すので、**コミットごとにローカルでE2Eまで回すのはCIとの二重実行**になる。
実測（2026-08-20・T-M8-155/158）: 実DB全テスト2,296件は**18秒**、typecheck+lintは40秒。
一方E2E全件は**約8分**、buildは約1.5分。遅いのは後者だけなので、前者は毎回・後者はpush前にまとめる。
...
- **`git add -A` を使わない。** 利用者が並行して編集中のファイルを巻き込む
  （2026-08-20、無関係な2ファイルをstageしかけた）。パスを明示して `git add` する。
