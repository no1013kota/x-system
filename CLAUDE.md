...
| `kit/` | 配布用のClaude Code開発キットの正本（README・VERSION・雛形・配布用スキル `kit/skills/`＝`.claude/skills/` を汎用化した派生・プラグイン専用スキル・`BUILD.json`）。`npm run dev-kit` がプラグイン（`dist/docdd/`）を生成し、公開リポジトリ no1013kota/claude-docdd-dev-kit で配る。配布物に本リポジトリ固有の文言（アプリ名・作業ID・固有コマンド）を入れない。運営者向けの手順は`kit/PUBLISHING.md` |
...
**CI の要否は `/release` が差分の対応表で決める**（`src/**`・`supabase/**`・`e2e/**`・依存・設定・CI 定義に触れば必要、docs・BACKLOG・ブログ・`.claude/**`・`kit/**` だけなら不要）。不要なら空コミットで `[light ci]` を HEAD に付ける。CI は走るが本体（build・E2E）を飛ばし、型検査・lint だけで約2分で緑になる（main へのマージでも CI はマージ元の印を見て本体を飛ばす）。**GitHub 公式の `[skip ci]` は使わない**——workflow ごと止まり、必須チェックが報告されず PR がマージできない（2026-09-05 に実際に起きた。`release:staging` もこの印を検出して止める）。運営者の「CI なしで／ありで」は表より優先するが、動作に影響する変更で「なし」と言われたら理由を添えて CI を回す。
...
- **画面の文言はE2Eでしか守られていない。** 文言だけの変更でも単体テストは緑のまま通る。
  2026-08-26、LPから「初回のみ」（トライアルが初回限りである法令上の開示）が外れたのを
  検出したのは `landing.spec.ts` と `plans.spec.ts` だけだった（T-M8-321）。
  **UI・文言を触ったら push 前に必ず `npm run test:e2e` を回す。**
- **`check:doc-dates` はコミット後にしか落ちない。** 判定は「その文書を最後に**変えたコミット**の日付」
  との比較なので、**コミット前のローカル確認は必ず通り、CIで初めて赤くなる**
  （2026-08-26 に同じ理由で2回踏んだ）。**docsを編集したら、同じ編集の中で冒頭の「更新日」も直す。**
...
- **コミットメッセージに角括弧付きの「skip ci」を書くな（本文で言及するときも角括弧を外す）。** GitHub は本文も見て workflow ごと止め、必須チェックが報告されず PR がマージできない（2026-09-05、説明として本文に書いただけで必要な CI が走らなかった）。軽量化は `[light ci]` を空コミットの件名にだけ付ける。
- **`smoke:live` が費用 $0 のまま全シナリオ失敗（RSS「1本も読めません」・job_failed）なら、まず dev サーバーの再起動。** 2026-09-05、23時間動いていた `next dev` だけが外向きの fetch に失敗し、シェルからの curl は通っていた。再起動で全件成功した。
