...
test("アカウント設定が未保存でも参考ソースを登録でき、作るボタンが理由つきで出る", async ({
  ...
  // 参考ソースの欄がアカウント設定より**上**に出る（入口だから）。
  // 見出し（h2）の並びで見る。タブの「アカウント設定」を拾わないよう見出しに限定する。
  const order = await page.evaluate(() =>
    [...document.querySelectorAll("h2")]
      .map((h) => (h.textContent ?? "").trim())
      .filter((t) =>
        ["参考ソースからアカウント設定を作る", "参考ソースを追加", "アカウント設定"].includes(t),
      ),
  );
  expect(order).toEqual(["参考ソースからアカウント設定を作る", "参考ソースを追加", "アカウント設定"]);

  // 材料が無いので押せない。**理由が画面に出る**（無効化だけにしない・T-M8-37）。
  const apply = page.getByRole("button", { name: "アカウント設定を作る" });
  await expect(apply).toBeDisabled();
  await expect(page.getByText("先に参考アカウントか参考投稿を追加してください。")).toBeVisible();

  // 参考アカウントの欄はこの時点で使える（アカウント設定の保存を待たない）。
  await expect(page.getByRole("textbox", { name: "参考アカウント" })).toBeEnabled();
});
