...
配布物は**特定のプロジェクトに依存しない文言**にする（運営者の指示 2026-09-05・T-M8-436）。本リポジトリの名前・アプリ名・作業ID・固有コマンド・`src/` のパスを含めない。`npm run dev-kit` が出力の .md／.json を機械検査し、含まれていれば止まる（禁止文言の一覧は `scripts/dev-kit.mjs` の `FORBIDDEN`。固有名を足したらここへ足す）。
...
| `kit/skills/<名前>/`（12本） | **配布用スキル。`.claude/skills/<名前>/` を汎用化した派生**（手順・表・停止条件は同じ。固有名・作業ID・固有コマンド・パス・実測値だけを除く）。`.claude/skills/` のうち `scripts/dev-kit.mjs` の `EXCLUDED_SKILLS`（現在 `blog-write`・`blog-publish`＝本リポジトリのブログ運用専用）以外の全部に派生がある。出力時に本文の `/add-task` → `/docdd:add-task` 書き換え ＋ 冒頭に前提ブロック |
...
**`.claude/skills/` にスキルを足したとき**も止まる。派生 `kit/skills/<名前>/` が無ければ `npm run dev-kit` は「派生がありません（汎用化して作るか、EXCLUDED_SKILLS に理由付きで足す）」で止まる（配布物が黙って欠けるのを防ぐ）。配布しないスキル（本リポジトリだけの仕組み）なら、`scripts/dev-kit.mjs` の `EXCLUDED_SKILLS`（現在 `blog-write`・`blog-publish`）へ理由をコメントで添えて足す。逆に除外対象のスキルが `.claude/skills/` から消えても止まる（`EXCLUDED_SKILLS` を見直す）。配布用スキルの本文は除外したスキルを `/<名前>` で参照できない（出力の自己検査で止まる）。
