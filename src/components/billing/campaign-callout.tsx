 ...
 * 「カード登録が必要」（無料の条件）と「期間中に解約すれば無料」の2点はこの帯が常時表示の置き場所。
 * 料金の見出し下にあった1文「はじめての方は7日間無料（カード登録が必要）です。」は 2026-09-05 に
 * 削除した（T-M8-425・D-54→D-56）。CTAより上の開示はこの帯と特商法ページが担い、新LP（`/`）では
 * CTA直下の `TRIAL_NOTE`（`src/components/lp-new/tokens.ts`）が同じ内容を添える。
 * **「初回のみ」は 2026-08-26 に帯から外した**（T-M8-321・運営者の最終レビュー）——初回限りの開示は
 * LPのFAQ・特商法ページ・利用規約が担う。詳細な法定事項もフッタの特商法ページ・利用規約が担う（運営者の決定）。
 */
    ...
    >
      ...
      {trialAvailable ? (
        <p className={cn("text-body text-ink-2", RELEASE_CAMPAIGN.active && "mt-1")}>
          ...
          {/* 括弧の中は文節ごとに折る（390px で「ん）。」だけが最終行に落ちていた・T-M8-424 のレビュー）。 */}
          <span className="inline-block">（カード登録が必要です。</span>
          <span className="inline-block">期間中に解約すれば料金はかかりません）。</span>
        </p>
      ) : null}
