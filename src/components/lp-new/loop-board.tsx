...
const NODES: {
  ...
  // 時刻はノード4（投稿）とヒーロー板Bに任せ、ここは時間だけ（2と4が両方「予約した時刻に」だと、同じ時刻に
  // 作って・確認して・投稿する、と読めた・レビュー 2026-09-05）。語はヒーロー板Aと同じ「60〜90秒で下書きに」。
  { label: "作る", note: ["60〜90秒で", "下書きに"], list: ["60〜90秒で下書きに"], who: "auto" },
  // 「確認」は一番右のノード。1行だと 640〜1024px で板の右端を越える（実測 最大28px）ので3行。
  // 既定は下書きまでなので、飛ばせるのは自動投稿ONのときだけ（無条件に読めた・2周目）。
  // 一番右のノードなので1行を短く（「自動投稿ONなら不要」の2行だと 640px で板の右端を越える）。
  // 1行目は「下書きを見る」（誰がやるかは白ノードの色＝凡例が示す。縦リストの「あなた、」の座りも直る・2026-09-05）。
  { label: "確認", note: ["下書きを見る", "自動投稿ONなら", "不要"], list: ["下書きを見る", "自動投稿ONなら不要"], who: "you" },
  // 自動投稿をオンにした後にだけ動く（「同意後」は仕様書の語で、初心者には「何に同意？」となる——6周目）。
  // 「予約時刻から5分ほどで」は同じ板の上段の「5分」と別の意味の5分として混ざるので書かない（2026-09-05）。
  // 語はヒーロー板Bと同じ「予約した時刻に」（要件04 §8 の「概ね5分以内」は断定しない範囲）。ページ内でこの語はここと板Bだけ。
  { label: "投稿", note: ["自動投稿ONなら", "予約した時刻に"], who: "auto" },
    ...
    >
      ...
      <div className="mt-[clamp(28px,4vw,40px)] grid grid-cols-1 items-start gap-10 border-t border-hairline pt-[clamp(28px,4vw,40px)] min-[960px]:grid-cols-[minmax(0,4fr)_minmax(0,8fr)] min-[960px]:items-center">
        <div>
          ...
          {/* 既定はオフ（下書きまで）なので、オフの間に誰が投稿するかも言う（「オフの間は下書きまで」・3周目→2026-09-05 で短縮）。 */}
          {/* 分析の起点（ボタン1つ）と1日1回は同じ板のリング図と h3 が言うので、ここでは繰り返さない（108→56字・2026-09-05）。 */}
          {/* 「分析の結果も、勝手には反映しません」も同じ板のノード7「いい案だけ あなたが選ぶ」が言う。ページ内で同じ但し書きが6回出ていたので2文で止める（レビュー 2026-09-05）。 */}
          <p className={cn("mt-4 text-ink-2", BODY)}>
            自動投稿は、あなたがオンにしてから。いつでも止められ、オフの間は下書きまで。
          </p>
        </div>
