...
interface Stop {
  ...
  title: string;
  who: Who[];
  /** 本文。文節折りで語の途中で切れる句は inline-block の span で括る（停止04）。 */
...
const STOP_SOURCE: Omit<Stop, "no">[] = [
  {
    ...
    // 「届く」で待たずに来る感じ（ヒーロー板Cの「10分おきに新着」と対・2026-09-05）。
    // 分野名はページ内でここだけ文字で出す（「6分野」だけでは自分の分野があるか分からない。スクショの分野は
    // 739px 幅でも読めない・レビュー 2026-09-05）。語の出典は facts.ts（OPERATED_THEME_OPTIONS）で直書きしない。
    body: `${THEME_LABELS.join("・")}の新着が、重要度つきで届く。気になった記事は「すぐに投稿作成」。`,
  ...
  },
  {
    ...
    who: ["auto", "you"],
    // 「予約すれば、その時刻に」で予約の存在を知らない初見にも因果が1文で読める（2026-09-05）。
    body: "型を選ぶだけで60〜90秒。予約すれば、その時刻に自動で。プロンプトはその場で書き換えOK。",
    // 画像生成の提供元はアプリ画面と同じ表示名（OpenAI／Gemini）。「ChatGPT」と書かない。
    // BYOK の開示は主語（プラン全体）を残す（「画像だけBYOK」に読めた・2周目）。何のキーかは FAQ「始めるのに何が必要ですか？」が担う。
    note: `画像も各ポストに1枚（OpenAI／Gemini）。${PLANS.standard.displayName}はご自身のAPIキーで。`,
  ...
  },
  {
    title: "投稿",
    who: ["you", "auto"],
    // 「枠」はスケジュール枠の内部用語で初心者に伝わらない（6周目）。体言止め2文（2026-09-05）。
    // 「選ぶだけ」は直前の停止「型を選ぶだけで」と「だけ」が続くので使わない。
    body: "予約ごとに「下書きまで」か「そのまま投稿」。次に動く時刻もひと目で。",
  ...
  ...(HOME_SHOT_READY ? [HOME_STOP] : []),
  {
    ...
    who: ["ai", "you"],
    // 「アカウント.md」はページ内でここが初出（「複数のプロンプトを管理」より前）。読者向けの語は「発信方針」で、
    // 括弧の補足はここ1回だけ（以後の節は「発信方針」で通す。言い換えが「発信方針のメモ」「＝アカウント設定」の
    // 2通りあり、読者がつなぐことになっていた・レビュー 2026-09-05）。
    // 改善案は表示専用（全文＋コピー）で、貼るかは利用者が決める（PRD K-2）。「自動で賢くなる」とは書かない。
    // 「（1日1回）」は上限で読者の得ではなく、リング図の分析ノードが言う。押す→貼る→効くの流れを切らない（2026-09-05）。
    // 「発信方針（アカウント.md）と」は inline-block: auto-phrase は「）」の直後で折るので、390px で助詞「と」が行頭に来た。
    body: (
      <>
        「分析を開始」を押すと、AIが改善案を出す。気に入った案だけ、
        <span className="inline-block">発信方針（アカウント.md）と</span>
        プロンプトに貼る。次の投稿から効く。
      </>
    ),
    // 写真はプロンプト編集画面で、「分析を開始」のボタンは写っていない（探して見つからない・2周目）。
    note: "画面は、改善案を貼る先（プロンプト編集）。",
