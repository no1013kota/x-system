...
const CARDS: {
...
}[] = [
  {
    ...
    title: "投稿の型を、何個でも",
    // 上限は既定パターンを含めて数える（post-patterns-store.ts）ので「から始めて、最大N件まで」。
    // 画面名「パターン」の括弧書きは図版の「パターンを追加」が見せているので置かない（2026-09-05）。
    body: `標準の5種類から始めて、最大${PATTERN_MAX_COUNT}件まで。型ごとにプロンプトを直せます。`,
  ...
  },
  {
    ...
    title: "プロンプトを無限に管理",
    // アカウント.md の読者向けの語は「発信方針」（画面ツアー停止04が初出で括弧を付け、以後はこの語で通す・レビュー 2026-09-05）。
    // 図版の行名「アカウント設定 vN」は実画面どおり。惹き語「自由」は隣の固定見出し「AIモデルも自由に決定」に譲る（1画面に1つ）。
    body: `発信方針と画像の指示は、保存するたびに本棚に並ぶ（${SHELF_MAX}件まで）。切り替えも、前の版へ戻すのもいつでも。`,
    figure: <ShelfFigure />,
  },
  {
    id: "models",
    icon: "tune",
    title: "AIモデルも自由に決定",
    // 「選べます」は見出し「AIモデルも自由に決定」と列挙が言う（101→52字・2026-09-05）。
    body: "文章は Claude／GPT／Gemini、画像は GPT Image／Nano Banana。迷ったら最初の設定のままで。",
    figure: <ModelsFigure />,
  },
];
