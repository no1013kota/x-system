...
describe("SC-01 LP: デザイン制約", () => {
  ...
  it("安全性の説明がFAQに残っている（独立セクションを持たないため）", () => {
    ...
    // 2026-09-05 のブラッシュアップ（答えを結論→条件の順に書き直し）でも、この2語と Q6 の
    // 「はじめてのお申し込みに限り」は据え置き。文は自由に磨いてよいが、語が消えたらここで落ちる。
 ...
 * 2026-09-05 の簡潔化（運営者の依頼「説明しすぎ・冗長」）では固定文言は据え置き、本文だけを短くした。
 * そのとき画面から外した仕様書の語（表示専用・定刻・従量課金）と「省略OK」が戻らないことも同じ形で見る。
 * 同日のレビューで外した重複（「勝手には反映しません」の2回目・アカウント.md の言い換え「発信方針のメモ」・
 * 招待カードの「紹介した方」・旧 metadata の「反映するかは」）も同じ形で見る。
 */
describe("SC-01 LP: 運営者指定の文言（T-M8-421）", () => {
  it("見出し・CTAの文言が指定どおり", () => {
    ...
    // 削除した文言が戻っていない（コメントは除く。仕様書の語は説明として残してよい）。
    const rendered = NEW_LP_SOURCES.replace(/\/\*[\s\S]*?\*\/|\/\/[^\n]*/g, "");
    for (const removed of [
      "繰り返す",
      "実際の管理画面です",
      "棒の長さ＝",
      "7日間、実物で",
      // 2026-09-05 簡潔化: 仕様書の語を画面に出さない（初心者の言葉に開く）。
      "表示専用",
      "定刻",
      "従量課金",
      "省略OK",
      // 2026-09-05 レビュー: 同じ但し書きの重複と語の揺れ。主導権はリング図7「いい案だけ あなたが選ぶ」と
      // 「使うほど」段2が言い、アカウント.md の読者向けの語は「発信方針（アカウント.md）」（初出のツアー04だけ括弧）。
      "勝手には反映しません",
      "発信方針のメモ",
      "紹介した方",
      "反映するかは",
    ]) {
      expect(rendered, `「${removed}」が戻っている`).not.toContain(removed);
    }
  });
