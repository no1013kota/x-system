...
const COLLECTED = [
  ...
  },
  {
    term: "設定・発信定義書",
    description:
      "ペルソナ、発信テーマ、トーン＆マナー、NG設定、これらから生成される発信定義書、通知設定、ニュースの表示条件。",
  },
  {
...
const PURPOSES = [
  ...
  "プロンプト集（公開ページ）の提供。利用者が作成したプロンプト・アカウント.mdを、識別情報を付けない匿名の形で掲載します（掲載停止はお問い合わせ窓口で受け付けます）",
  "お問い合わせへの対応",
  "法令または行政機関の要請に基づく対応",
];
...
const RETENTION = [
  ...
  },
  {
    term: "ニュースと通知",
    description:
      "取得または作成から40日を経過したものを自動的に削除します（未読の通知は既読より後に削除します）。",
  },
  {
    term: "外部APIの利用量の明細",
  ...
  {
    // 要決定D-19（2026-08-20 解決）。Sentryの現行プランの保持期間は90日固定で短縮できないため、
    // **実際の値を書く**（書けない期間を書くと虚偽になるので、確認できるまで空欄にしていた）。
    term: "エラーの記録（不具合の調査用）",
    description:
      "委託先（Sentry）の設定により90日を経過したものが自動的に削除されます。記録には認証情報・APIキー・投稿前の入力内容を含めません。",
  },
];
    ...
    >
      ...
      <LegalArticle n={9} title="開示・訂正・削除等のご請求">
        ...
        <p>
          ...
          ご本人であることを確認したうえで、遅滞なく回答します。
          手数料はいただきません。なお、法令により応じられない場合は、その理由をお伝えします。
        </p>
        <p>
          設定画面からご自身で変更できる項目（アカウント設定、通知設定、APIキー、Xアカウントの連携）は、
