...
import { applyUpdateBaseMdManual } from "./base-md";
import { closePool, getPool, withTransaction } from "./db/pool";
import { AppError } from "./observability/errors";

/**
 * DB integration tests for base_md manual edit (M-1, 要件05 §8, 要件02 §3.3).
 * 変更履歴とロールバックは T-M8-362 で廃止した（戻したいときは本棚で別の本文を選ぶ）。
...
describe("base_md manual edit (db)", () => {
  let available = false;
  beforeAll(async () => {
    try {
  ...
  it("md plan: updates base_md and version atomically", async () => {
