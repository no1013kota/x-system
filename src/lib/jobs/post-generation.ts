...
/**
 * `<user_input>` ブロックの本文。**未指定の項目は行を出さない**（「（未指定）」という文字列を
...
async function loadJob(db: Queryable, jobId: string): Promise<JobRow | null> {
  const { rows } = await db.query<JobRow>(
    `select gj.pattern_id, gj.pattern_spec, gj.trigger, gj.input, gj.x_account_id, gj.attempt,
            xa.user_id, xa.base_md, xa.settings, xa.x_premium, xa.writing_checkpoints, p.plan
...
async function existingDraftId(
  db: Queryable,
  jobId: string,
): Promise<string | null> {
  ...
  // GEN-FIX・JSON修復・出典再生成は同一jobの内部callで追加reserveしない（開始時1回のみ）。
  {
    try {
      ...
      if (error instanceof AppError && error.code === "usage_paused") {
        ...
        throw new PostGenerationTerminalError(
          "usage_paused",
          "usage paused (concealed limit reached)",
        );
      }
  ...
  try {
    ...
    if (prereqError) {
      await persistFailure(
        ...
        failCtx,
        {
          code: prereqError.code,
          message: "実行前提が不足しています。設定をご確認ください。",
          stage: "validating",
        },
        EMPTY_USAGE,
      );
    ...
    const baseMdOverride =
      typeof job.input?.base_md_override === "string" &&
      job.input.base_md_override.trim() !== ""
        ? job.input.base_md_override
        : null;
    const system = buildGenSystem(
      baseMdOverride ?? job.base_md,
      normalizeWritingCheckpointIds(job.writing_checkpoints),
    );
    // この生成にだけ使うプロンプト（T-M8-92）。指定があれば通常の解決を飛ばす。
    // 保存はしない——恒久化は投稿作成画面の「保存して以後も使う」かAI設定＞プロンプトが担う。
    const promptOverride =
      typeof job.input?.prompt_override === "string" &&
      job.input.prompt_override.trim() !== ""
    ...
    const webSearch = webSearchForSpec(
      spec,
      hasInputUrl,
      job.attempt,
  ...
  */
    ...
    const shorten = async (text: string, limit: number): Promise<string> => {
      ...
      const out = await fixProvider.textGen.generate({
        system: [],
        user: PT_FIX.replaceAll("{{limit}}", String(limit)).replaceAll(
          "{{post}}",
          text,
        ),
      ...
      if (error instanceof InvalidProviderOutputError) {
        ...
        await persistFailure(
          ...
          failCtx,
          {
            ...
            stage: "writing",
            providerRawError: formatFailureRawError(
              error,
              providerRawOutputOf(error),
            ),
          },
          error.usage,
        );
    ...
    const usage: GenerationUsage = {
      calls: usageCalls,
      estimated_cost_usd_total: usageCalls.reduce(
    ...
    */
      await ensureAutoPostPublishJob(db, {
        xAccountId: job.x_account_id,
        draftId,
      });
    } else {
      await createDraftCreatedNotification(db, {
        userId: job.user_id,
        draftId,
      });
    }
