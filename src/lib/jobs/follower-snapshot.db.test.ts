...
/**
 * DB integration for follower snapshot (K-3, 要件02 §3.11, T-M5-14→T-M8-255→T-M8-257):
 * 入口は2つ。「分析を開始」からの単一アカウント記録（JST当日のupsert・同日再実行は上書き・
 * token/読取不能は理由つきで書かない）と、毎時cronのバッチ（当日分が無い・**契約が有効な**
 * アカウントの選定・重複防止・失敗skip）。X読取は mock を注入（原価台帳は経由しない）。
 */
