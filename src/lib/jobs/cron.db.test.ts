      ...
      await withTransaction(async (c) => {
        ...
        const lateSched = (
          await c.query<{ id: string }>(
            `insert into generation_jobs (x_account_id, kind, trigger, pattern, status, scheduled_for)
             values ($1,'post_generation','schedule','p1','queued', now() + interval '2 min') returning id`,
            [xid],
          )
        ).rows[0].id;
        const earlySched = (
          await c.query<{ id: string }>(
            `insert into generation_jobs (x_account_id, kind, trigger, pattern, status, scheduled_for)
             values ($1,'post_generation','schedule','p1','queued', now() + interval '1 min') returning id`,
            [xid],
          )
        ).rows[0].id;
        const noSched = (
        ...
        const staleId = (
          await c.query<{ id: string }>(
            `insert into generation_jobs (x_account_id, kind, trigger, pattern, status, attempt, locked_at, locked_by, started_at)
             values ($1,'post_generation','manual','p1','running',1, now() - interval '15 min','w', now()) returning id`,
            [xid],
          )
        ).rows[0].id;
