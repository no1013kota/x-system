...
export const LEGAL_ENTITY = {
  ...
  representative: "松本洸太",
  /**
   * 所在地の表示（運営者の指示 2026-08-25「住所は公開しないでください」）。
   *
   * **省略できる。ただし条件付き**——消費者庁「通信販売広告Q&A」Q17は、
   * 「消費者からの請求によって、広告表示事項を記載した書面又は電子メール等を**遅滞なく**
   * 提供することを広告に表示し、かつ、実際に請求があった場合に**遅滞なく**提供できるような
   * 措置を講じている場合には、事業者の住所及び電話番号の表示を省略することも可能」としている
   * （2026-08-25 に消費者庁のページで確認）。
   *
   * **条件は「書く」ことではなく「実際に応じられること」。** 請求が来たら遅滞なく
   * 住所を返す必要があり、返せないと省略が認められない。問い合わせ窓口（`email`）で
   * 受け付ける前提でこの表示にしている。**窓口を止めるなら住所の表示へ戻すこと。**
   *
   * バーチャルオフィスの住所を表示する道もある（Q18）が、
   * 「現に活動している住所」であることと運営事業者との合意が要る。
   */
  address: "請求があった場合には遅滞なく開示します",
  /**
   * 問い合わせ窓口（T-M8-343・運営者の指示 2026-08-27。運営者個人のGmailから変更）。
   *
   * **アプリ内の問い合わせ導線もこの値を使う**（`SUPPORT_EMAIL` が未設定でも
   * 法務上の窓口が消えないように、こちらを既定とする）。
   * **運営者向けアラートの宛先（`SUPPORT_EMAIL`）とは役割が別**——ここは利用者へ見せる
   * 窓口、あちらは異常を受け取る先。Gmailを送信に使うと自分宛のメールが受信トレイに
   * 入らないため、同じ値にすると異常に気付けない（`check:mail-sender` が警告する）。
   */
  email: "support@exosai.net",
  /**
