      ...
      const auto = (
        await c.query<{ id: string }>(
          `insert into schedule_slots (x_account_id, pattern_id, weekdays, time_jst, mode, theme, enabled)
           values ($1,(select id from post_patterns where x_account_id = $1 and seed_key = 'p1'),'{1,2,3}','09:00','auto','other',true) returning id`,
          [xid],
        )
      ).rows[0].id;
      const draft = (
        await c.query<{ id: string }>(
          `insert into schedule_slots (x_account_id, pattern_id, weekdays, time_jst, mode, theme, enabled)
           values ($1,(select id from post_patterns where x_account_id = $1 and seed_key = 'p1'),'{4,5}','10:00','draft','other',true) returning id`,
          [xid],
        )
      ).rows[0].id;
