 ...
 *
 * 対応する設定（要件03 §2.2）:
 * - `subscription_update.proration_behavior = "always_invoice"` → 値上げは**切り替え即時**。
 *   差額（旧プランの未使用分を引き、新プランの残り期間分を足す・秒単位の日割り）を
 *   **その場で決済**し、Stripe の確認画面に内訳（Prorated credit / Prorated charge / Amount due today）が
 *   出る（運営者の指示 2026-08-23・T-M8-267。以前の `create_prorations` は次回請求へ合算で、
 *   確認画面には何も出なかった——実測）
...
export function planChangeEffects(input: PlanChangeEffectInput): PlanChangeEffects {
  ...
  */
  ...
  return {
    upgrade: {
      headline: "すぐに切り替わり、差額は日割りでその場で決済されます",
      detail: "旧プランの未使用分を差し引き、新プランの残り期間分をお支払いいただきます（内訳は確認画面に出ます）",
    },
    downgrade: {
      headline: `${end}に切り替わります`,
