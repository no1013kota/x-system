...
describe("capPostCount", () => {
  ...
  it("未知パターンは既定の上限を使う（既存の挙動を壊さない）", () => {
    ...
    expect(capPostCount(maxPostsFor("unknown"), posts(8)).dropped).toBe(0);
  });
