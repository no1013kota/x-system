...
- 44個のアイコンで足りなくなったら `scripts/generate-icons.mjs` の一覧へ足して再生成する（手で `icon-paths.ts` を編集しない）。
- 共通部品へ寄せたことで、1つの変更が複数画面へ同時に効く。**変更時は使用箇所を `rg` で確認する**（2026-08-04時点で `Badge` は18ファイル・8画面、`Notice` は8ファイル、`CardTitle` は19ファイル、`PatternRadioGroup` は2画面が使う）。**この数は増える前提で、正は `rg` の結果**——ここに書いた数字は「小さいと思って触ると広く影響する」ことの目安にすぎない（T-M8-44 でチップを寄せた結果 `Badge` が5画面→8画面へ増え、この行の更新が漏れていた）。
