...
- 共通部品へ寄せたことで、1つの変更が複数画面へ同時に効く。**変更時は使用箇所を `rg` で確認する**（`Badge` は5画面、`PatternRadioGroup` は2画面が使う）。
