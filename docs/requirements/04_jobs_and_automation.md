...
- `scheduler_tick`のcleanupは40日を過ぎた`news`通知を先に削除し、その後に参照されない40日超の`news_items`と、**新着順で500件（`NEWS_MAX_STORED_ITEMS`）を超えた`news_items`**（T-M8-188・DB肥大防止。draft・通知payloadから参照される行は残す）を各500件まで削除する。`external_api_usage_events`の**400日**超の明細（T-M8-373で40→400日）と、400日超の`kpi_daily`も削除する。期限切れ削除の失敗は投稿系jobを失敗させず、Sentryへ記録して次回へ繰り越す。
...
| v1.55 | 2026-08-23 | scheduler_tick の日次処理に「契約期間の補完」を追加（T-M8-258・レビュー反映） |
...
### 事業KPIの日次スナップショット（T-M8-373）

`scheduler_tick` に相乗りし、**1日1回**（`cron_runs` の `(job_name='kpi_snapshot', window_key=JSTの日付)`。日付が変わって最初のtickが担う）、`kpi_daily`（要件02 §3.31）へ指標を書き出す（`src/lib/ops/kpi.ts`）。

- **状態指標**（契約者数・MRR・登録者累計）は「前日の終わり」として前日の日付で1回だけ書く（過去の状態は復元できない）
- **出来事指標**（登録・確認・X連携・トライアル開始・生成消費・原価・解約）は直近3日を毎回計算し直す（遅れて届くStripeイベント・原価を拾う）
