...
| `signIn` | email, password, captcha_token, next(optional) | session/redirect | Turnstile token必須。generic error。`email_not_confirmed`のみ再送状態。契約未選択は`/plans`、他はsafeな相対`next`または`/app`。認証成功後はprofileを1回読み、欠損時だけ作成・再読込する（通常ログインで存在確認upsertを行わない）。profile確認に失敗したsessionは破棄する |
