...
- **トライアルは最初の有料期間と1つの枠を共有する**（運営者の指示 2026-08-23・要決定D-36 解決）。Stripe はトライアルを独立した期間（`current_period_end = trial_end`）として扱い、有料化の日に `current_period_start = trial_end` の新しい期間を始める（`trial_end` は有料化後も残る。テストクロックで実測）。そのまま期間キーにするとトライアル7日で満額を使い有料化の日にもう一度満額へ戻るため、「トライアル中」または「トライアル終了日から始まった期間」のあいだは期間の開始を `trial_used_at`（トライアル開始）に読み替える（`usagePeriodStartExpr`）。2回目以降の有料期間は通常どおり。**トライアル中はリセット日を出さない**（リセットは最初の有料期間の終わりで、その日時は有料化まで Stripe から分からない。画面は「次回の更新日」と書く）。
...
| v1.62 | 2026-08-23 | レビュー反映: Portal の実挙動（別Productでも期間末予約・予約付きでも変更可）、schedule を読めない回は予約を上書きしない、return 経路も schedule を読む、トライアルは独立した期間、既存契約者の補完（T-M8-258/260） |
| v1.63 | 2026-08-23 | レビュー反映: Portal の実挙動（別Productでも期間末予約・予約付きでも変更可）、schedule を読めない回は予約を上書きしない、return 経路も schedule を読む、トライアルは独立した期間、既存契約者の補完（T-M8-258/260） |
| v1.64 | 2026-08-23 | トライアルは最初の有料期間と利用枠を共有（有料化で満額へ戻さない・運営者の指示・D-36 解決・T-M8-258） |
| v1.65 | 2026-08-23 | レビュー反映: Portal の実挙動（別Productでも期間末予約・予約付きでも変更可）、schedule を読めない回は予約を上書きしない、return 経路も schedule を読む、トライアルは独立した期間、既存契約者の補完（T-M8-258/260） |
...
| v1.74 | 2026-08-23 | レビュー反映: Portal の実挙動（別Productでも期間末予約・予約付きでも変更可）、schedule を読めない回は予約を上書きしない、return 経路も schedule を読む、トライアルは独立した期間、既存契約者の補完（T-M8-258/260） |
| v1.75 | 2026-08-23 | レビュー反映: Portal の実挙動（別Productでも期間末予約・予約付きでも変更可）、schedule を読めない回は予約を上書きしない、return 経路も schedule を読む、トライアルは独立した期間、既存契約者の補完（T-M8-258/260） |
| v1.76 | 2026-08-23 | トライアルは最初の有料期間と利用枠を共有（有料化で満額へ戻さない・運営者の指示・D-36 解決・T-M8-258） |
| v1.77 | 2026-08-23 | 値上げの日割り差額をその場で決済（always_invoice）へ。Stripe確認画面に内訳が出る（T-M8-267） |
| v1.78 | 2026-08-23 | プラン変更の差額は次回請求へ合算（create_prorations）を維持し、説明は商品説明（PLAN_CHANGE_NOTE）で行う（T-M8-267） |
| v1.79 | 2026-08-23 | Stripe確認画面に独自文が書けないことと、Portal帰還時に日割り差額を出す方針を明記（T-M8-267） |
| v1.80 | 2026-08-23 | レビュー反映: Portal の実挙動（別Productでも期間末予約・予約付きでも変更可）、schedule を読めない回は予約を上書きしない、return 経路も schedule を読む、トライアルは独立した期間、既存契約者の補完（T-M8-258/260） |
| v1.81 | 2026-08-23 | レビュー反映: Portal の実挙動（別Productでも期間末予約・予約付きでも変更可）、schedule を読めない回は予約を上書きしない、return 経路も schedule を読む、トライアルは独立した期間、既存契約者の補完（T-M8-258/260） |
| v1.82 | 2026-08-23 | トライアルは最初の有料期間と利用枠を共有（有料化で満額へ戻さない・運営者の指示・D-36 解決・T-M8-258） |
| v1.83 | 2026-08-23 | 値上げの日割り差額をその場で決済（always_invoice）へ。Stripe確認画面に内訳が出る（T-M8-270） |
| v1.84 | 2026-08-23 | プラン変更の差額は次回請求へ合算（create_prorations）を維持し、説明は商品説明（PLAN_CHANGE_NOTE）で行う（T-M8-270） |
| v1.85 | 2026-08-23 | Stripe確認画面に独自文が書けないことと、Portal帰還時に日割り差額を出す方針を明記（T-M8-270） |
| v1.86 | 2026-08-23 | 解約前の引き止めクーポン（retention）と、解約予定のアプリ内取り消しを追加（T-M8-271/272） |
