...
会員登録時は現行の利用規約versionへの明示同意とプライバシーポリシー確認を必須にし、versionと時刻をprofileへ保存する。重大改定で再同意が必要な場合は、既存データ閲覧を許可したまま生成・投稿前に`legal_consent_required`（`details.missing=terms_consent|privacy_acknowledgement`、`settingsPath=/app/consent`）を返して同意画面を表示する。 登録画面が `?src=<slug>`（LPの追跡URLから引き継いだ流入元・T-M8-423）を持っていれば、hidden で action へ渡し、登録直後に `profiles.signup_source` へ1回だけ書く（形式外・未登録は `''`。失敗しても登録は止めない）。
...
（T-M8-295・運営者の指示 2026-08-25）。タブも出さず、`AppLockedPage` を1枚出す——
触れるものが1つも無いのにタブだけ並べても、押した先で同じ断り文句を読むだけになる。
登録の入口はロック画面の「プランを登録する」（`/plans`）に1本化する。
**解約済み・支払い滞りはこれに含めない**。課金・プランのタブに「プランを再開」（T-M8-264）と
お支払い情報の更新（T-M8-273）があり、ここを塞ぐと**戻る手段ごと消える**。
判定は「ロックされている」かつ「`plan` が無い」の両方。
...
Portal Configurationは`subscription_update.proration_behavior=always_invoice`により、値上げを**即時に切り替え、差額を日割り計算してその場で決済**する（旧プランの未使用分をマイナス、新プランの残り期間分をプラスで秒単位。運営者の指示 2026-08-23・T-M8-275）。Stripeの確認画面に内訳と本日の支払額が出る（その見出し「本日が期日の金額」はStripe組み込みで差し替えられない）。`create_prorations`にすると差額は次回更新日の請求書へ合算され確認画面には何も出ない——**アプリ側の説明と食い違うため、doctorが設定のずれを検出する**。**Stripeの確認画面・Checkoutの文言はこちらで差し替えられない**（ポータルで設定できる文字は見出し・利用規約リンク・戻り先リンク・ビジネス名のみ。セッションの自由文は完了後に出る `after_completion.custom_message` だけ。2026-08-23にAPI定義と実画面で確認）。**下位変更のときだけ**Stripeが「現在の請求期間の最終日 (◯月◯日) に更新されます」と自動で説明し、**即時に切り替わる上位変更では日割りの説明が出ない**。そのためプラン変更の説明はアプリ側が担う——課金タブの効果説明（押す前）と、**Portalから戻った直後の日割り差額の案内**（`lib/stripe/proration-preview.ts`。要件06 §1.1）。**この案内は「即時決済された差額」を先に見る**（`loadRecentProrationCharge`・T-M8-296）。`always_invoice` では差額が即時に請求・決済されるため**次回請求の下見には1行も出ず**、未請求分だけを見ていた実装では説明がどの経路でも出なくなっていた（2026-08-25、テストクロックで実測: 変更時に `subscription_update` の請求書が即時 paid になり、次回プレビューは月額1行だけ）。直近15分の `subscription_update` の請求書に限って「ただいまお支払いいただきました」と出し、**未請求のまま残っている差額**（`always_invoice` にする前に発生したもの）は従来の「次回のご請求に加算されます」で出し分ける。商品説明（プラン選択画面でプラン名の直下・Checkoutにも出る）は**運営者がStripeダッシュボードで整えた文面をコードの正本へ取り込み、3環境で同一にする**。doctor「プラン管理（Stripe）」が `proration_behavior` のずれも検出する。`schedule_at_period_end.conditions=[decreasing_item_amount]`に該当する値下げだけを期間末予約へ切り替える。**ただしトライアル中の下位変更はその場で切り替える**（T-M8-299・運営者の指示 2026-08-25）。期間末予約は「払った期間ぶんは使えるべき」という理由なので、**1円も払っていないトライアルには当てはまらない**（下げたのに枠が上位のまま待たされる）。Portalの`conditions`は契約の状態で分岐できないため、**戻ってきたときにアプリ側で畳む**——`applyTrialDowngradeNow`が予約を`release`してから`proration_behavior=none`で価格を差し替える（トライアル中の価格変更は即時・請求は発生しない。2026-08-25 実測）。**利用枠もリセットする**——Stripeはトライアル中の価格変更で`current_period_start`を動かさない（同実測）ので自動では戻らない。`profiles.usage_epoch`を1つ進めて期間キーを変える（`usage_events`は消さないので原価の台帳は残る）。判定は**変更前後のプランの比較**で行い（Portalが予約を作らずその場で下げる経路も拾う）、上位への変更では戻さない。**期間末予約は契約本体のPriceを変えず subscription schedule を付ける**ため、同期は `subscription.schedule` があれば schedule を読み、次フェーズのPriceが現在と違い既知のプランなら `profiles.scheduled_plan` / `scheduled_plan_at` へ保存する（T-M8-260。schedule取得の失敗は記録して**保存済みの予約を上書きせず**契約本体だけ反映する——失敗の空と正常の空を同じ null にしない。Portal からの戻り `/api/stripe/return` も同じく schedule を読む——読まないと予約を null で書き、後続の本物の webhook が stale 扱いになって予約が消える）。**同じ理由で戻りは discount も読む**（T-M8-293）。`subscriptions.retrieve` に `expand: ["discounts"]` を付け、割引率はクーポン側にあるので `discounts[0].source.coupon`（**IDの文字列**。`discount.coupon` は返らない・2026-08-24 実データで確認）からクーポンを1回引く。読まずに戻ると、解約導線で受け取った引き止めクーポンが**戻った瞬間に null で消され**、`created` が現在時刻になるため後続の webhook も stale になり、「半額適用中」の表示が一度も出ない（2026-08-24、運営者の実アカウントで発生）。**Portal の実挙動（2026-08-23 テストモードの Portal を実ブラウザで操作して確認）**: Price が別 Product でも値下げは期間末予約の schedule（phases=[現在の期間, 新Priceが1秒]・`end_behavior=release`。切替後すぐ解除されて契約から外れる）になる。予約付きの契約でも Portal は別プランへの変更を受け付け、その場合は予約が置き換わる（上位なら即時）。**「今のプランのまま続ける（予約だけ取り消す）」は Portal に無い**ので、取り消しは Server Action `cancelScheduledPlanChangeAction`（要件05）が本人の契約の schedule を `release` して予約列を空にする。webhookが届く前でも画面から消えるように、解除直後にアプリ側でも空へ戻す。課金タブは予約中も「プランを変更」を出したまま、その隣に取り消しを置く。`subscription_cancel.mode=at_period_end`（**有料契約のみ**。トライアルはアプリ内で即時終了・T-M8-278）、`proration_behavior=none`、`trial_update_behavior=continue_trial`を固定し、請求履歴と支払方法更新も有効化する。**引き止めクーポンは利用回数の上限（`max_redemptions`）で尽きると`valid=false`になり、以後は提示されない**——doctorがそれを検出する（2026-08-23、テスト環境のクーポンが上限2回で尽きていた。上限は作成後に変更できないため、上限なしのクーポンを作り直して差し替えた）。**提示は顧客ごとに1回だけ**で、2回目は割引が切れた後でもStripeがセッション作成を400で断る（"is still usable by the customer"・2026-08-24 実測。API から直接付けるのは何回でもできるので、これはPortalの提示側の制限）。そのまま失敗させると**解約画面そのものが開けなくなる**ので、**提示なしで開き直す**（T-M8-280）。**解約前の引き止めクーポンは`flow_data.subscription_cancel.retention`で明示する**（T-M8-272）——ダッシュボードの「顧客維持クーポン」設定は**Portalのトップから解約したときだけ**効き、`flow_data`で解約画面へ直接入るアプリの導線には効かない（2026-08-23、運営者が「クーポンが提示されずに解約された」と報告。テストモードで有無を実測）。クーポンIDは環境ごとに`STRIPE_RETENTION_COUPON_ID`で持ち、未設定なら提示しない。doctor「プラン管理（Stripe）」が未設定（提示されない）と無効（Stripeに無い）を区別して知らせる。

**解約は確認とアンケートを挟む**（T-M8-277・運営者の指示 2026-08-23）。「解約する」を押したら、(1)何が止まり何が残るかを示す確認（`cancellationEffects`。煽らず事実だけ。データが残ることも同時に伝える。**ただし「閲覧できます」とは書かない**——解約後は課金・プラン以外のタブがロックされる〔T-M8-269〕ので、データは消えないが**開いて見ることはできない**。「消えません（解約中は開けませんが、再開すればそのまま使えます）」と、消えないことと見るには再開が要ることを1行で言い切る・運営者の指摘 2026-08-24）→ (2)理由のアンケート（**1つ以上・複数選択可**・自由記述は任意。`cancellation_surveys`へ保存。T-M8-294で複数選択にした——理由は1つに絞れないことが多く、絞らせると集計が「仕方なく選んだ1つ」に寄って何を直すべきかの判断を誤らせる）→ (3)Stripeの解約画面（引き止めクーポンはここで出る）。**アンケートの保存に失敗しても(3)へ進む**。**無料トライアル中の解約はその場で終了させる**（T-M8-278・運営者の指示 2026-08-23）。払っていない期間を「終了日まで使える」とするのは説明しにくいため、Portalへ送らずアプリ内で`subscriptions.cancel`する（`cancel-now.ts`・Server Action `cancelTrialNowAction`。有料契約はこの経路を使わない）。**残りのトライアル期間は捨てない**——`profiles.trial_ends_at`を残し、期限内に「無料トライアルを再開」を押したら`POST /api/stripe/resume`が**同じ`trial_end`で契約を作り直す**（Stripeは終了した契約を再開できないため必ず新しい契約になる。テストクロックで実物確認）。期限切れなら通常の有料再開。

**残りのトライアルは元のプランに限らない**（T-M8-298・運営者の指示 2026-08-25）。期限内なら**どのプランでも**その日まで無料で始められる。判定は`lib/billing/remaining-trial.ts`に集約する（`/plans`・設定＞課金・Checkout・再開APIが違う答えを出すと「画面には無料と書いてあるのに請求された」が起きる。2026-08-25、解約後に`/plans`から選んで即座に満額課金された）。
- **Checkout**は`trial_used_at`があっても`trial_ends_at`が未来なら`subscription_data.trial_end`で引き継ぐ（`trial_period_days`は付けない）。
- **`/plans`のCTAはStripeを挟まない**——カードが保存済みなら`POST /api/stripe/resume`に`{plan}`を渡してその場で作る。**402（カード未保存）のときだけ**Checkoutへ倒す。それ以外の失敗でCheckoutへ倒すと「無料のはず」が有料の申し込みに化けるのでやらない。
- 再開APIの`plan`は既知のプランIDだけ受ける（不正値は元のプランへ倒さず`validation_error`）。

**再開の冪等キーは時間で区切らない**（T-M8-297）。Stripeの冪等リプレイは24時間有効なので、時間バケット（旧実装は10分）だと**同じバケット内で「再開→解約→再開」をすると2回目が1回目の応答＝解約済みの契約を返し**、`LIVE_SUBSCRIPTION_STATUSES`に入らないため「再開を確定できませんでした」になる（2026-08-25、運営者の報告「何回か押すと再開できます」を実物のStripeで再現）。**置き換える契約のID**（`subscriptions.list`の先頭）とプランを鍵に含め、置き換える契約が無いときだけ時間で区切る。
...
| **AIクレジット** | 100,000 | 文章・画像のAI実行の**実費**（**1クレジット=0.01円相当**・T-M8-109→T-M8-325） |

**AIクレジット（T-M8-109）**: 生成・画像の2枠を金額制の1本へ統合した。内部的に**1クレジット=0.01円相当**（円換算1ドル=160円・PRD §6.1。**UIに円換算は出さない**）。
**2026-08-27に粒度を100倍にした**（T-M8-325・運営者の指示）。金額としては従来と同じ（プレミアム＝1,000円ぶん）だが、
丸めが1円単位だったころは $0.001 の実行（0.16円）も1クレジット＝1円として引かれ、**細かい実行ほど割高**だった。
単位変更にあたり、旧単位の消費済みカウントが残らないよう**世代を1つ進めて全員0から数え直した**
（migration `20260827000001`。案2・運営者の決定。`usage_events` の台帳は残るので原価の追跡はできる）。消費は回数ではなく実費ベース:
1. **開始前の確認**: **残量が尽きていないか**だけを見る（`reserveUsage`）。ここでは**1クレジットも書かない**
2. **実費の記録（成功確定時）**: 実費（原価台帳と同じ推定原価×160円×100・切り上げ・最低1）を1回だけ書く（冪等key `job:{id}:{type}:charge`）。**上限チェックはしない**——既に発生した実費は拒否できない
3. **返還は無い**: 押さえていないので返すものが無い

**予約（reserve）は2026-08-27に廃止した**（T-M8-324・運営者の指示）。以前は開始時に見積もりを押さえ、
成功時に実費で精算し、失敗時に返還していた。そのため**完了のたびに使用量が下がって見え**、
利用者から「何もしていないのに減る」と問い合わせが来た。いまは**実費が確定したときだけ書く**。

**上限の当たり方**（要決定D-48・案A・運営者の決定 2026-08-27）:
- 残量が尽きていれば**新しい生成を始めさせない**
- **走り出した生成は最後まで通す**ので、使用量は上限を超えうる（残量はマイナスで表示する。
  0で丸めると、超過した事実も繰り越される事実も画面から消える）
- **超過分は次の期間へ繰り越して差し引く**（`carryOverUsage`。上限100,000に対し105,000使ったら次は5,000から）。
  無かったことにすると、期間の変わり目に毎回チャラになり上限が実質的に効かなくなる

モデル別の見積もり（カタログ`estimateCredits`）は**画面表示専用**になった（AIモデル設定の「約N クレジット」）。
...
**失敗しても枠は引かれない**（T-M8-324）。実費を書くのは成功確定時だけなので、retryable失敗でqueuedへ差し戻される間も何も保持しない。`failJob`／`finalizeFailedJob` の `refundUsage` 呼び出しは残してあるが、**変更をデプロイした時点で走行中だったjob**が持つ旧方式のreserveを返すための経過措置で、通常は no-op（`jobs/terminal.ts` に明記）。
...
- **トライアルは独立した期間**として数え、**有料化の日に枠が満額へ戻る**（運営者の指示 2026-08-23・要決定D-38 再決定）。Stripe はトライアルを独立した期間（`current_period_end = trial_end`）として扱い、有料化の日に `current_period_start = trial_end` の新しい期間を始めるので、その区切りをそのまま利用枠の区切りにする（テストクロックで実測）。「試して気に入ったら払う」人が払った初日から満額を使えるほうが素直、という判断（費用は最大でトライアル1周ぶん増える）。トライアル中のリセット日表示は**トライアル終了日**。
...
| v1.55 | 2026-08-23 | 上位変更の差額はその場で決済（always_invoice・T-M8-275）、トライアルは独立した枠（D-38 再決定）、解約前の確認とアンケート（T-M8-277） |
| v1.56 | 2026-08-23 | トライアル解約は即時終了＋残り期間で再開（T-M8-278）、適用中の割引を同期（T-M8-279） |
| v1.57 | 2026-08-24 | 引き止めクーポンは顧客ごとに1回。2回目は提示なしで解約画面を開き直す（T-M8-280） |
| v1.58 | 2026-08-24 | 解約確認の「残ります（閲覧できます）」を「消えません（解約中は開けませんが、再開すればそのまま使えます）」へ修正（T-M8-285） |
...
| v1.62 | 2026-08-25 | 再開の冪等キーを対象契約IDへ（T-M8-297）。残りトライアルはどのプランでも無料・カード再入力なし（T-M8-298） |
| v1.62 | 2026-08-25 | ログイン失敗を原因ごとに言い分ける方針へ（§1.1・T-M8-295・運営者の指示）。パスワード再設定の非開示は維持 |
| v1.63 | 2026-08-25 | トライアル中の下位変更は即時＋利用枠リセット（T-M8-299） |
