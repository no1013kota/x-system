...
**解約は確認とアンケートを挟む**（T-M8-277・運営者の指示 2026-08-23）。「解約する」を押したら、(1)何が止まり何が残るかを示す確認（`cancellationEffects`。煽らず事実だけ。データが残ることも同時に伝える）→ (2)理由のアンケート（選択必須・自由記述は任意。`cancellation_surveys`へ保存）→ (3)Stripeの解約画面（引き止めクーポンはここで出る）。**アンケートの保存に失敗しても(3)へ進む**。トライアル中に解約しても即時停止ではなく、Stripeは`cancel_at = trial_end`を立てるだけで**トライアル終了日まで使える**（2026-08-23 テストクロックで実測）。取り消せば**残り日数のままトライアルが続く**。
