...
| # | 項目 | 結果 | 根拠 |
|---|---|---|---|
| 1 | リリース判定ゲート `npm run release:check`（typecheck→lint→依存監査→全テスト→build）がローカルで全成功 | ✅ 済 | T-M6-20（exit 0 確認・2026-07-25） |
...
| 9 | 依存脆弱性（`sharp`/`next`/`postcss` の high）の breaking upgrade と再検証（要決定 D-7） | 開発 | リリース前の保守枠 |
