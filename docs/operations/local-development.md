...
| `npm run blog:diagram -- <in.svg> <out.png>` | 記事の図（`blog/diagrams/<slug>-<name>.svg`）を2倍密度・パレット化の PNG にして `public/blog-images/<out.png>` に書き（出力先はこの配下に限る）、本文へ貼る Markdown を表示する。**文字の下限を機械的に見る**（プレビューを開き忘れても止まる・原則3）: SVG の `font-size` 属性の最小値をスマホ幅（375px）へ換算し、**12px 未満なら生成せず終了コード1**（幅1200の図なら最小40px以上。`font-size` が1つも無い SVG も止まる。`transform` で拡大縮小した文字は拾えないので図では使わない）。あわせて **375px 幅に縮めたプレビュー PNG を一時ディレクトリ（`os.tmpdir()`・`<out>-phone375.png`）へ書き**、パスを表示するので開いて読めるか確かめる。図は幅1200で描く（それ未満は警告） |
| `npm run dev-kit` | **配布用の Claude Code 開発キット（プラグイン `docdd`）を作る**（T-M8-434。T-M8-435 で zip 配布を廃止、T-M8-436 で固有文言を除去）。正本は `kit/`（利用者向け README・公開リポジトリ用 README・VERSION・雛形 `templates/`・**配布用スキル `skills/`（`.claude/skills/` を汎用化した派生。固有名・作業ID・固有コマンド・パスを含まない）**・プラグイン専用スキル `plugin-skills/`）と、**ルートの `.mcp.json`・`.claude/settings.json`（permissions だけ）・`scripts/check-doc-dates.mjs`・`scripts/check-doc-refs.mjs`・`scripts/audit-check.mjs`（写しを持たず、このリポジトリのものをそのまま配る）**。出力は (a) マーケットプレイス一式 `dist/docdd/`（gitignore 済み。公開リポジトリ no1013kota/claude-docdd-dev-kit の直下へ中身を置く。スキル本文の `/add-task` 等を `/docdd:add-task` へ書き換え、各 SKILL.md 冒頭に前提ブロックを差し込み、雛形の docs・tasks も同じ書き換え。出力の .md／.json に固有文言が残っていれば止まる。`claude` コマンドがあれば `claude plugin validate --strict` まで通す）、(b) `kit/BUILD.json`（版・中身のハッシュ・日付・**元スキル `.claude/skills/` のハッシュ**。コミット対象）。**中身が変わったのに `kit/VERSION` が同じなら止まる**（`-- --same-version` で明示的に許す）。`-- --check` は生成せず突き合わせだけ（`src/lib/ops/dev-kit.test.ts` が呼ぶ。元スキルが変わって派生が未確認でも赤になる）。公開の手順は `kit/PUBLISHING.md` |
