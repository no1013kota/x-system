...
4. **対象テスト**（変更種別で選ぶ）: migration/RLS=schema/seed/RLS/別ユーザー拒否/制約/**ロール権限(GRANT)**。Action/API=認証/入力検証/所有権/Origin/署名/DB反映。job/Cron=lease/状態遷移/冪等/再試行/stale回収/deadline/同時実行。課金・利用量=Webhook重複/イベント順序/契約状態/枠の原子的更新。X・AI=モックで契約と記帳（liveはしない）。`rg` で対応する `*.db.test.ts`・route/actionテストを探し、無ければ追加する。
