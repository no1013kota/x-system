...
//
// **この手順は人の記憶に預けられていた**（CLAUDE.md 原則3）。実際に複数の文書で
// 更新日が置き去りになっており、
...
/*
  **冒頭ヘッダの version と末尾の変更履歴が食い違っていないか**。

  実際に1本の文書が冒頭と履歴で8版分ずれ、しかも
...
/*
  **変更履歴を求めるのは仕様の正本だけ**（PRD・requirements/ と、このリポジトリで正本と決めた文書。
  ...
  `requirements/README.md` は分け方の案内（本文に `| バージョン | v1.0 |` の例を含む）なので対象外。
  一致する文書が無ければ単に対象が無いだけ（0件なら下の検査で止まる）。
*/
...
for (const file of files) {
  ...
  /*
    **表ヘッダも見る。** 行だけ残ってヘッダが消えると Markdown の表として描画されず、
    履歴が読めない孤立した行になる（実際にこの状態の文書があった）。
    行があることだけを見ていると気付けない。
  */
